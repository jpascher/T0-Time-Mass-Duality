...
\title{T0 Geometrische Kosmologie En}
...
```
	...
	The central discovery of the simulation is that the sum of these tiny "detours" causes the \textbf{effective total path length, $\Leff$, to be systematically longer} than the direct Euclidean distance $d$ between the source and the observer.
...
```
	...
	The simulation proves that the T0-Theory, in a static, flat universe, can explain cosmological redshift as a purely geometric effect.
		...
		- \textbf{No Expansion:} The universe is not expanding.
		- \textbf{No Dark Energy:} The concept becomes obsolete.
		- \textbf{The Hubble Constant Reinterpreted:} $\Hubble$ is not an expansion rate but a fundamental constant describing the interaction of light with the geometry of the T0 vacuum.
	...
	\begin{lstlisting}[language=Python, caption={Conceptual Python code for the FEM simulation of geometric redshift.}, label={lst:fem_code}]
		...
		z_per_Mpc = redshift_z / euclidean_distance \textit{ (3.26e6 } GRID_SIZE) # Scale to Mpc
