...
Erweiterte Python-Anwendung mit harmonischer Intelligenz
...
class XiFFTApp:
    ...
    def analyze_generated(self):
        ...
        def analyze_thread():
            try:
                ...
                use_exact = getattr(self, 'use_exact_ratios', tk.BooleanVar(value=True)).get()
                
                self.xi_fft = XiFFT(44100, threshold, resolution, use_exact)
                ...
                # Harmonische Intelligenz nutzen?
                use_harmonic = getattr(self, 'use_harmonic_intelligence', tk.BooleanVar(value=True)).get()
                f0_hint = f0 if use_harmonic else None
                
                analysis = self.xi_fft.analyze(self.current_signal, freq_range, progress_callback, min_amp, max_amp, f0_hint)
    ...
    def plot_spectrum(self, analysis, signal, sample_rate):
        ...
        self.ax_spectrum.set_title('Frequenzspektrum - ξ-FFT mit harmonischer Intelligenz')
        ...
        frequencies = [p['frequency'] for p in peaks[:25]]  # Top 25
        magnitudes = [p['magnitude'] for p in peaks[:25]]
        
        # Farben basierend auf harmonischen Matches
        colors = []
        for peak in peaks[:25]:
            if peak.get('is_harmonic_match'):
                category = peak.get('interval_category', '')
                if category == 'Perfect':
                    colors.append('red')  # Perfekte Konsonanzen
                elif category == 'Consonant':
                    colors.append('orange')  # Konsonanten
                elif category == 'Complex':
                    colors.append('purple')  # Komplexe Harmonien
                else:
                    colors.append('green')  # Andere harmonische Matches
            else:
                colors.append('steelblue')  # Standard
        
        # Balkendiagramm
        bars = self.ax_spectrum.bar(frequencies, magnitudes, width=2, alpha=0.7, color=colors)
        
        # Labels für wichtigste Peaks
        for i, (freq, mag, peak) in enumerate(zip(frequencies[:8], magnitudes[:8], peaks[:8])):
            label_text = f'{freq:.1f}Hz'
            if peak.get('is_harmonic_match'):
                interval_name = peak.get('interval_name', '')
                if len(interval_name) < 15:  # Nur kurze Namen anzeigen
                    label_text += f'\n[{interval_name}]'
            
            self.ax_spectrum.text(freq, mag + max(magnitudes)*0.05, label_text, 
        ...
        # Legende für Farben
        legend_elements = [
            plt.Rectangle((0,0),1,1, facecolor='red', alpha=0.7, label='Perfekte Konsonanz'),
            plt.Rectangle((0,0),1,1, facecolor='orange', alpha=0.7, label='Konsonanz'),
            plt.Rectangle((0,0),1,1, facecolor='green', alpha=0.7, label='Harmonisch'),
            plt.Rectangle((0,0),1,1, facecolor='steelblue', alpha=0.7, label='Standard')
        ]
        self.ax_spectrum.legend(handles=legend_elements, loc='upper right', fontsize=8)
    ...
    def plot_peaks_and_xi(self, analysis):
        ...
        self.ax_peaks.clear()
        self.ax_peaks.set_title('Top Peaks & ξ-Verhältnisse (Exakte Brüche)')
        
        peaks = analysis['peaks'][:12]  # Top 12
        ...
        # Peaks als Scatter-Plot mit Farb-Kodierung
        ...
        for peak in peaks:
            if peak.get('is_harmonic_match'):
                if peak.get('interval_category') == 'Perfect':
                    colors.append('red')
                    sizes.append(150)
                elif peak.get('interval_category') == 'Consonant':
        ...
        scatter = self.ax_peaks.scatter(frequencies, magnitudes, c=colors, s=sizes, alpha=0.7)
        
        # Beschriftung der wichtigsten Peaks
        for i, (freq, mag, peak) in enumerate(zip(frequencies[:6], magnitudes[:6], peaks[:6])):
            label = f'{freq:.1f}Hz'
            if peak.get('is_harmonic_match'):
                interval = peak.get('interval_name', '')
                if len(interval) < 12:
                    label += f'\n{interval}'
            
            self.ax_peaks.annotate(label, (freq, mag), 
                                 xytext=(5, 5), textcoords='offset points', fontsize=7)
        
        # ξ-Verhältnisse als Verbindungslinien (nur exakte)
        exact_ratios = [r for r in xi_ratios if r.get('isExactMatch')]
        colors_xi = plt.cm.Set3(np.linspace(0, 1, len(exact_ratios)))
        
        for i, ratio in enumerate(exact_ratios[:6]):  # Nur Top 6 für Übersichtlichkeit
            f_low = ratio['freqLow']
            ...
            if low_mag > 0 and high_mag > 0:
                self.ax_peaks.plot([f_low, f_high], [low_mag, high_mag], 
                                 color=colors_xi[i], linewidth=2, alpha=0.6,
                                 label=f'ξ={ratio["xiRatio"]:.3f} [{ratio["intervalName"]}]')
        ...
        if len(exact_ratios) > 0:
            self.ax_peaks.legend(loc='upper right', fontsize=7)
    ...
    def display_analysis_text(self, analysis, analysis_type):
        ...
        result_text = "=== ξ-FFT ANALYSE ERGEBNISSE (MIT HARMONISCHER INTELLIGENZ) ===\n\n"
        ...
        result_text += f"Exakte Verhältnisse: {'✅' if analysis.get('useExactRatios', False) else '❌'}\n"
        ...
        # Musikalische Analyse (wenn exakte Verhältnisse verwendet)
        if analysis.get('useExactRatios') and 'musicalAnalysis' in analysis:
            musical = analysis['musicalAnalysis']
            result_text += f"🎼 Muster: {musical.get('pattern', 'Unbekannt')}\n"
            result_text += f"🎵 Harmonizität: {musical.get('harmonicity', 'Unbekannt')}\n"
            if musical.get('consonanceRatio', 0) > 0:
                result_text += f"🎶 Konsonanz: {musical['consonanceRatio']:.1%}\n"
        ...
        if len(peaks) > 0:
            ...
            for i, peak in enumerate(peaks[:15], 1):
                ...
                # Zusätzliche Markierungen für harmonische Matches
                harmonic_marker = ""
                if peak.get('is_harmonic_match'):
                    interval_name = peak.get('interval_name', 'Harmonic')
                    category = peak.get('interval_category', '')
                    if category == 'Perfect':
                        harmonic_marker = f" 🔴[{interval_name}]"
                    elif category == 'Consonant':
                        harmonic_marker = f" 🟠[{interval_name}]"
                    else:
                        harmonic_marker = f" 🟢[{interval_name}]"
                
                result_text += f"{i:2}. {peak['frequency']:7.2f} Hz  →  {peak['magnitude']:8.4f}  ({peak['magnitudeDB']:+6.1f} dB){marker}{harmonic_marker}\n"
            
            result_text += "\n⚡ TOP ξ-VERHÄLTNISSE (EXAKTE BRÜCHE):\n"
            result_text += "-" * 80 + "\n"
            for i, ratio in enumerate(xi_ratios[:15], 1):
                result_text += f"{i:2}. ξ({ratio['freqHigh']:7.2f}/{ratio['freqLow']:7.2f}) = {ratio['xiRatio']:8.4f}"
                
                # Erweiterte Informationen für exakte Verhältnisse
                if ratio.get('isExactMatch'):
                    interval_name = ratio['intervalName']
                    result_text += f"  📐[{interval_name}]"
                    if ratio.get('isConsonant'):
                        result_text += " ♪"  # Konsonanz-Symbol
                    
                    # Zeige exakten Bruch
                    if ratio.get('xiRatioExact'):
                        exact_frac = ratio['xiRatioExact']
                        result_text += f" = {exact_frac.numerator}/{exact_frac.denominator}"
                else:
                    # Spezielle Verhältnisse auch für Float-Werte markieren
            ...
            # Suche nach Schwebungsmustern (sowohl exakte als auch Näherungen)
            beating_pairs = []
            for ratio in xi_ratios[:15]:
                xi_val = ratio['xiRatio']
                if 1.005 < xi_val < 1.3:  # Erweiteter Bereich für Schwebungen
                    beat_freq = ratio['freqHigh'] - ratio['freqLow']
                    interval_name = ratio.get('intervalName', 'Unbenannt')
                    beating_pairs.append((ratio['freqLow'], ratio['freqHigh'], beat_freq, interval_name, ratio.get('isExactMatch', False)))
            
            if beating_pairs:
                result_text += "Erkannte Schwebungen:\n"
                for low_f, high_f, beat_f, name, is_exact in beating_pairs[:8]:
                    exactness_marker = " 📐[Exakt]" if is_exact else ""
                    result_text += f"   {high_f:.2f} Hz - {low_f:.2f} Hz = {beat_f:.2f} Hz ({name}){exactness_marker}\n"
                ...
                # Zusätzliche harmonische Analyse
                exact_beating_count = sum(1 for _, _, _, _, is_exact in beating_pairs if is_exact)
                if exact_beating_count > 0:
                    result_text += f"\n   🎼 {exact_beating_count} exakte harmonische Schwebungen erkannt\n"
            else:
                result_text += "Keine klaren Schwebungsmuster erkannt.\n"
                
            # Harmonizitäts-Zusammenfassung (für exakte Verhältnisse)
            if analysis.get('useExactRatios') and 'musicalAnalysis' in analysis:
                musical = analysis['musicalAnalysis']
                result_text += f"\n🎼 HARMONISCHE ZUSAMMENFASSUNG:\n"
                result_text += "-" * 80 + "\n"
                
                if 'categories' in musical:
                    for category, count in musical['categories'].items():
                        emoji = {'Perfect': '🔴', 'Consonant': '🟠', 'Dissonant': '🟡', 'Complex': '🟣', 'Microtone': '🔵'}.get(category, '⚪')
                        result_text += f"   {emoji} {category}: {count} Intervalle\n"
                
                consonant_count = musical.get('consonantRatioCount', 0)
                total_exact = musical.get('exactRatioCount', 0)
                
                if total_exact > 0:
                    result_text += f"\n   📐 Exakte Verhältnisse: {total_exact}\n"
                    result_text += f"   ♪ Konsonante Intervalle: {consonant_count}\n"
                    result_text += f"   🎵 Harmonizität: {musical.get('harmonicity', 'Unbekannt')}\n"
                    result_text += f"   🎼 Erkanntes Muster: {musical.get('pattern', 'Unbekannt')}\n"
...
def main():
    ...
    print("🚀 ξ-FFT Schwebungs-Analyse - Windows GUI mit harmonischer Intelligenz")
    print("=" * 70)
    print("Erweiterte Python-Version mit exakten Verhältnissen und")
    print("musikalischer Intelligenz für optimale Frequenz-Analyse")
    print("=" * 70)
    ...
    try:
        ...
        import matplotlib
        from fractions import Fraction
        print("✅ Alle Dependencies verfügbar (inklusive Fraction)")
    ...
    try:
        ...
        print("✅ GUI bereit - Starte erweiterte Anwendung...")
        print("\nNeue Features:")
        print("• 🎼 Harmonische Intelligenz: 10-100x schneller für Musik-Signale")
        print("• 📐 Exakte Verhältnisse: Keine Gleitkomma-Rundungsfehler")
        print("• 🎵 Musikalische Analyse: Erkennt Akkorde, Intervalle, Tonleitern")
        print("• ⚡ Adaptive Algorithmen: Nutzt musikalische Wahrscheinlichkeiten")
        print("• 🎶 65+ exakte Intervalle: Von Unison bis Mikrotonale Kommas")
    ...
    def analyze_file(self):
        ...
        def analyze_thread():
            try:
                ...
                use_exact = self.use_exact_ratios.get()
                
                self.xi_fft = XiFFT(self.loaded_sample_rate, threshold, resolution, use_exact)
