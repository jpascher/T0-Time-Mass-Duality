...
| Version | DOI | Focus |
|---------|-----|-------|
| v1.2.3 | *(assigned on Zenodo upload)* | The reversibility test: **fundamental ⇔ reversible**; information is the OUTPUT of a comparator, not a base unit; the bit as the ultimate (irreducible, irreversible) reduction, Landauer kT ln2 as that irreversibility (Doc. 301). Three levels of information (bit / description / absolute identity). Reflexive close in two registers: temporal and topological boundarylessness booked as consequences (recursion has no reachable edge, ∂T⁴=∅, time = D(k), no beginning / no outside); realist posit and the “Word reflex” diagnosis marked as beyond the mathematics (P35). FFGFT explicitly Level 2 (description) |
