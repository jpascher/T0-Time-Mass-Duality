...
def replace_urls(file_path, mapping):
  ...
  updated = False
  # Allgemeines Muster für URLs mit Wildcards
  url_pattern = re.compile(r'\\href\{(https://github\.com/jpascher/T0-Time-Mass-Duality/tree/main/2/pdf/[Deutsch|English]/.*?)\}')

  for old_name, new_name in mapping.items():
    lang_dir = "English" if old_name.endswith("_en.tex") else "Deutsch"
    old_name_pdf = old_name.replace(".tex", ".pdf")
    # Flexibles Muster mit Wildcards für den Dateinamen
    old_url_pattern = re.compile(
      rf"https://github\.com/jpascher/T0-Time-Mass-Duality/tree/main/2/pdf/{lang_dir}/{re.escape(old_name_pdf).replace(' ', '.*?')}.*?\.pdf"
    )
    new_url = f"https://github.com/jpascher/T0-Time-Mass-Duality/tree/main/2/pdf/{lang_dir}/{new_name}.pdf"

    # URLs ersetzen
    matches = old_url_pattern.findall(content)
    for match in matches:
      if match in content:
        content = content.replace(match, new_url)
        changes.append(f"In {file_path}: '{match}' -> '{new_url}'")
        updated = True
