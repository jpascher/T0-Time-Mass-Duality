...
- Masse: Aus \(E = M c^2\), mit \(c = 1\), folgt \(M = E\). Überprüfung mit \(G\): \([G] = [L^3 M^{-1} T^{-2}] = [(E^{-1})^3 (E)^{-1} (E^{-1})^{-2}] = [E^4]\). Da \(G = 1\), ist dies konsistent.
...
- **Feinstrukturkonstante (\(\alpha_{EM} = 1\))**  
  ...
  - **Herleitung**: \(\alpha_{EM} = e^2 / (4\pi \varepsilon_0 \hbar c)\). Mit \(\hbar = c = 1\), \(\varepsilon_0 = e = 1\) (Ebene 2.5), folgt \(\alpha_{EM} = 1^2 / (4\pi \cdot 1 \cdot 1) = 1\).
...
- **Compton-Wellenlänge des Elektrons (\(\lambda_{C,e} = 1/m_e\))**  
  - **Herleitung**: SI: \(\lambda_{C,e} = h / (m_e c) = 2\pi \hbar / (m_e c)\). T0: \(\hbar = c = 1\):
    \[
...
- **Josephson-Konstante (\(K_J = 1/\pi\))**  
  - **Herleitung**: SI: \(K_J = 2e / h = e / (\pi \hbar)\). T0: \(\hbar = e = 1\):
    \[
    K_J = \frac{1}{\pi}.
    \]
...
- **von-Klitzing-Konstante (\(R_K = 2\pi\))**  
  - **Herleitung**: SI: \(R_K = h / e^2 = 2\pi \hbar / e^2\). T0: \(\hbar = e = 1\):
    \[
    R_K = 2\pi.
    \]
...
- **Schwinger-Grenze (\(E_S = m_e^2\))**  
  - **Herleitung**: SI: \(E_S = m_e^2 c^3 / (e \hbar)\). T0: \(\hbar = c = e = 1\):
    \[
    E_S = m_e^2.
    \]
...
- **Stefan-Boltzmann-Konstante (\(\sigma = \pi^2/60\))**  
  - **Herleitung**: SI: \(\sigma = \pi^2 k_B^4 / (60 \hbar^3 c^2)\). T0: \(\hbar = c = k_B = 1\):
    \[
    \sigma = \frac{\pi^2}{60}.
    \]
...
- **Hawking-Temperatur (\(T_H = 1/(8\pi M)\))**  
  - **Herleitung**: SI: \(T_H = \hbar c^3 / (8\pi G M k_B)\). T0: \(\hbar = c = G = k_B = 1\):
    \[
    T_H = \frac{1}{8\pi M}.
    \]
...
- **Bekenstein-Hawking-Entropie (\(S_{BH} = 4\pi M^2\))**  
  - **Herleitung**: SI: \(S_{BH} = k_B c^3 A / (4 \hbar G)\), \(A = 16\pi G^2 M^2 / c^4\). T0: \(\hbar = c = G = k_B = 1\):
    \[
...
- **Planck-Länge (\(l_P = 1\))**: \(l_P = \sqrt{\hbar G / c^3} = 1\). Grundeinheit.
- **T0-Länge (\(r_0\))**: \(r_0 / l_P = \xi \approx 7519\), \(r_0 \approx 1,33 \times 10^{-4} l_P\). \(\xi \cdot l_P = \lambda_h / (32\pi^3)\).
...
1. **Higgs-Länge (\(\lambda_{C,h} \approx 1,6 \times 10^{-20} l_P\))**:
   - \(\lambda_{C,h} = 1 / m_h\), \(m_h \approx 125 \, \text{GeV}\), \(m_P \approx 1,22 \times 10^{19} \, \text{GeV}\):
     \[
...
2. **Compton-Länge (\(\lambda_{C,e} \approx 2,1 \times 10^{-23} l_P\))**:
   - \(\lambda_{C,e} = 1 / m_e\), \(m_e \approx 0,511 \, \text{MeV}\):
     \[
...
3. **Bohr-Radius (\(a_0 \approx 4,2 \times 10^{-23} l_P\))**:
   - SI: \(a_0 = \hbar^2 / (m_e e^2)\). T0: \(a_0 \approx \lambda_{C,e} / \alpha_{EM,SI} \approx 137 \cdot \lambda_{C,e}\). Modellkonvention: \(a_0 \approx \lambda_{C,e}\).

4. **Horizont (\(d_H \approx 5,4 \times 10^{61} l_P\))**:
   - \(d_H = 1 / H_0\), \(H_0 \approx 2,3 \times 10^{-18} \, \text{s}^{-1}\):
     \[
...
5. **Korrelationslänge (\(L_T \approx 3,9 \times 10^{62} l_P\))**:
   - \(L_T = \beta_T^{-1/4} \cdot \xi^{-1/2} \cdot l_P\), \(\beta_{T,SI} \approx 0,008\), \(\xi \approx 7519\):
     \[
...
- **Elektronenradius**: \(r_e = \alpha_{EM,SI} / (2\pi) \cdot \lambda_{C,e} \approx 2,4 \times 10^{-23} l_P\).
