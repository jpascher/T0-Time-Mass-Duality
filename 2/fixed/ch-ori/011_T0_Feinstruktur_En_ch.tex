\section*{Abstract}
	...
	where $c_e$ and $c_\mu$ are coefficients. These coefficients are derived directly from the geometric structure of the T0 Theory and are not free parameters. They arise from the integration over fractal paths in spacetime, based on spherical geometry and time-mass duality. Specifically, $c_e$ is derived from the volume integration of the unit sphere in the effective fractal dimension $\Dfrak^{\,\text{eff}} \approx 2.973$ (cumulative over the RG running from Planck to EW scale), while $c_\mu$ follows from the surface integration.
	...
	With $k_e / k_\mu = \sqrt{3}/2$ (from the harmonic fifth in the tetrahedral symmetry) and $\Dfrak^{\,\text{eff}} \approx 2.973$, this approximates to:
	\begin{equation}
	...
	\textbf{Example of the Necessity:} In the T0 Theory, the exponent $5/2$ for $m_e$ corresponds to the volume integration in 2.5 effective dimensions (fractal correction via $\Dfrak^{\,\text{eff}} \approx 2.973$), while $2$ for $m_\mu$ follows from the surface integration in 2D symmetry (tetrahedral projection). Simplifying to $\alpha = K$ (without $\xipar$) would erase these geometric origins and make the theory unable to correctly predict, e.g., the mass ratio $m_e/m_\mu \propto \xipar^{1/2}$. Instead, it would introduce an arbitrary constant that destroys the predictive power of the T0 Theory – similar to ignoring $\pi$ in circle geometry making area calculation impossible.
	...
	\begin{longtable}{lcc}
		...
		$\Dfrak^{\text{eff}}$ & cumulative over RG & $2.973$ \\
		$\Kfrak$ & $0.986$ & $0.986$ \\
...
\normalsize
	...
	\begin{enumerate}
		...
		\item Effective fractal dimension: $\Dfrak^{\text{eff}} = 2.973$ (local: $D_f^{\text{space}} = 3 - \xi \approx 2.9999$)
	...
	\section{Connection to Leptonic Precision Tests}
	...
	\begin{description}
		...
		\item[$\Dfrak$ ($D_f$)]: Fractal dimension of spacetime in the T0 Theory, with local value $D_f^{\text{space}} = 3 - \xi \approx 2.9999$ and effective cumulative value $D_f^{\text{eff}} \approx 2.973$.
