\section*{Abstract}
	...
	$D_f^{\,\text{eff}} \approx 2.973$ is \textbf{not identical} to $D_f^{\,\text{space}} = 2.9999$. It describes the cumulative effect of the fractal structure on mass dynamics over the entire RG running.
	...
	The factor~100 describes the amplification of the tiny local deviation~$\xi$ through the RG running over 17~orders of magnitude.
	...
	\begin{tcolorbox}[colback=green!5!white,colframe=green!75!black,title={Two Types of Corrections in T0 Calculations}]
		...
		\begin{itemize}
			\item Cumulative effect of fractal spacetime structure ($D_f^{\,\text{space}} = 2.9999$) over the RG running, described by $D_f^{\,\text{eff}} \approx 2.973$
	...
	\begin{description}
		...
		\item[$D_f^{\text{eff}}$] Effective fractal dimension (cumulative effect over RG running), $D_f^{\text{eff}} \approx 2.973$
