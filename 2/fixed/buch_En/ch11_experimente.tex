...
\section{Lepton Lifetimes and the Strong Coupling Constant}

A further quantitative prediction concerns the strong coupling constant, which can be derived purely geometrically for the first time:
\begin{equation}
\alpha_s(m_\tau) = D \cdot \xsi^{1/4} = 0.3224
\end{equation}
where $D = 3$ is the spatial dimension. This value deviates only $2.3\%$ from the experimental result. In combination with the torus geometry, a prediction of the lifetime ratio $\tau_\mu/\tau_\tau$ with $0.19\%$ accuracy follows --- entirely from T0 parameters, without experimental input beyond the lepton masses (\href{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/2/pdf/160_T0_Lepton-Lebensdauer-Verh_En.pdf}{Doc.~160}).
