...
\begin{align}
	...
	[L] &= [E^{-1}] \quad \text{(from } \lambda = \hbar c/p\text{)} \\
...
The gravitational constant follows from $\xi$ and the electron mass (a 
...
\begin{equation}
	\boxed{G_{\text{SI}} = \frac{\xi^2}{4 m_e} \times C_{\text{conv}}}
	\label{eq:G_complete}
\end{equation}
...
\begin{itemize}
	...
	\item $m_e = 0.511$ MeV (electron mass, already fractally corrected via $v = 246\,\text{GeV}$)
...
\begin{remark}[Historical factor $K_{\text{frak}}$]
	...
	absorbed in the measured Higgs VEV $v = 246\,\text{GeV}$ and thus in $m_e$. The mass 
...
\begin{equation}
	G_{\text{SI}} = 6.674 \times 10^{-11}\,\text{m}^3\,\text{kg}^{-1}\,\text{s}^{-2}
	\label{eq:G_result}
\end{equation}
...
\begin{equation}
	k_B = 1.380649 \times 10^{-23}\,\text{J}\,\text{K}^{-1} \quad \text{(exact)}
...
This hierarchy emerges entirely from $\xi$ and the fractal structure 
of spacetime.
...
\begin{enumerate}
	...
	\item Gravitational constant: $G = \frac{\xi^2}{4m_e} \times C_{\text{conv}}$ (fractal correction absorbed in $m_e$)
