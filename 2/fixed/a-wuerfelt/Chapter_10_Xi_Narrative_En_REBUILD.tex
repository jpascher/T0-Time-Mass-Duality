...
with the Hubble energy (natural units, $\hbar = 1$):

\begin{equation}
	E_H = E_0 \cdot \xi^{41/4} = 1.41 \times 10^{-33}\,\text{eV}
	\quad\Longrightarrow\quad
	H_0^{\text{T0}} = \frac{E_H}{\hbar} = 66.2\,\text{km/s/Mpc}
	\label{eq:hubble_constant_t0}
\end{equation}

\textbf{Note:} The T0 prediction deviates by $-1.9\%$ from the Planck 
value ($67.4\,$km/s/Mpc). The exponent $41/4$ requires physical 
justification from $\xi$ field theory.
