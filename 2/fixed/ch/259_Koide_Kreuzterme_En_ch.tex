...
$Q(\xi)$ is monotonically increasing in $\xi$ (larger $\xi$ raises $\tau$-terms
for $p<1$, $N$ grows faster than $Z^2$, so $Q=N/Z^2$ rises).
To lower $Q$ from 0.6690 to 0.66667, one would need smaller $\xi$.

But the document gives $\xi_{\text{Koide}}\approx1.372\times10^{-4}$ --- which is
\emph{larger} than $\xi=1.333\times10^{-4}$. This appears contradictory.
Either there is an error in the monotonicity analysis, or the document's
$\xi_{\text{Koide}}$ refers to the value giving $Q=2/3$ for other reasons
(with additional fractal corrections).

\begin{mdframed}[linecolor=blue!50,linewidth=1.5pt,roundcorner=4pt,backgroundcolor=blue!5]
\textbf{Subsequent correction --- Python verification (J.\,Pascher):}

Numerical verification shows that the initial monotonicity assumption was
\emph{wrong}. $Q(\xi)$ is \textbf{monotonically decreasing} in this range:
...
no contradiction. Bisection search ($R-2K=0$) yields:
\[
