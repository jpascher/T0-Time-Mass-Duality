...
\begin{align}
	...
	\xi_{\text{num}} &= 1{,}0 \times 10^{-5} \quad 
	\text{(Simulationsparameter;}\\
	& \quad \text{physikalisch: $\xi = 4/30000$, Dok.~009)}
\end{align}
		...
		\textbf{Hinweis zu $\xi$-Werten:} Der physikalische T0-Parameter ist $\xi_{\text{phys}} = 4/30000 \approx 1{,}333 \times 10^{-4}$ (Dok.~009). Die QFT-Herleitung über den Higgs-Sektor (Dok.~097) ergibt $\xi = \lambda_h^2 v^2/(16\pi^3 m_h^2) \approx 1{,}30 \times 10^{-4}$ (2{,}4\% Abweichung). Dieses Dokument verwendet den \textbf{numerischen Simulationsparameter} $\xi_{\text{num}} = 1{,}0 \times 10^{-5}$, der die numerische Stabilität im Zahlenraum des Algorithmus optimiert. Alle konkreten Werte unten (Gate-Amplituden, ppm-Verschiebungen) sind mit $\xi_{\text{num}}$ berechnet.
	\end{tcolorbox}
	...
	\textbf{T0-Ansatz}: Periodenfindung durch Energiefeld-Resonanz mit minimalen systematischen Fehlern
	...
	\textbf{Messbare Frequenzverschiebung}: 10 ppm (berechnet mit $\xi = 1,0 \times 10^{-5}$)
	...
	\begin{itemize}
		...
		\item \textbf{T0}: 10 ppm systematische Verschiebungen in Gatter-Operationen
		\item \textbf{Erkennungsschwelle}: Erfordert Präzision besser als 1 ppm
	\end{itemize}
	
	\section{Schlussfolgerung}
	
	\subsection{Bedeutung des $\xi$-Parameters}
	
	\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=Zusammenfassung der T0-Interpretation]
		Die T0-Energiefeld-Formulierung mit Higgs-abgeleitetem $\xi$-Parameter repräsentiert einen alternative Interpretation der Quantenmechanik und Quantencomputing:
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{lcc}
			...
			QFT-Berechnung (Dok.~097, $16\pi^3$) & $1{,}301 \times 10^{-4}$ & s. Hinweis \\
