\section*{Abstract}
	...
	A subtle but crucial point: In T0 theory, \emph{two different} fractal dimensions appear that must not be confused.
	
	\subsection{The Spacetime Dimension $D_f^{\,\text{space}} = 3 - \xi$}
	...
	\subsection{The Effective Dimension $D_f^{\,\text{eff}} \approx 2.973$ and the RG Running}
	
	The physically relevant correction does not arise from the local dimension alone, but from the \emph{cumulative effect} of this minimal deviation over the entire energy range from the Planck scale to the electroweak scale. The logarithmic distance between the scales is:
	\begin{equation}
		\ln\!\left(\frac{\ell_{\text{EW}}}{\ell_P}\right) \approx \ln(10^{17}) \approx 39
	\end{equation}
	
	From this running and the geometric averaging over relevant scales emerges an effective amplification factor of~100. The cumulative fractal correction is therefore not~$\xi$ but~$100\xi \approx 0.0133$, i.e., approximately~$1.3\%$.
	
	The equivalent power formula reads:
	\begin{equation}
		K_{\text{frak}} = \left(\frac{D_f^{\,\text{eff}}}{3}\right)^{\!D_f^{\,\text{eff}}/2} \qquad \text{with } D_f^{\,\text{eff}} \approx 2.973
	\end{equation}
	
	$D_f^{\,\text{eff}} \approx 2.973$ is \textbf{not identical} to $D_f^{\,\text{space}} = 2.9999$. It describes the cumulative effect of the fractal structure on mass dynamics over the entire scale flow.
	...
	This derivation shows: $K_{\text{frak}}$ is not an adjusted correction but a necessary consequence of consistency between fractal integration and direct geometric derivation. The effective fractal dimension $D_f^{\,\text{eff}} \approx 2.973$ is the ONLY one that makes both paths compatible.
	...
	\subsection{Perspective 1: Power Formula with Effective Dimension}
	
	The equivalent power form reads:
	\begin{equation}
		K_{\text{frak}} = \left(\frac{D_f^{\,\text{eff}}}{3}\right)^{D_f^{\,\text{eff}}/2} \approx 0.9867 \qquad \text{with } D_f^{\,\text{eff}} \approx 2.973
		\label{eq:exact_kfrak_en}
	\end{equation}
	
	\textbf{Important:} The local spacetime dimension $D_f^{\,\text{space}} = 2.9999$ may \emph{not} be substituted here --- this gives $(2.9999/3)^{1.5} \approx 0.99993$, not~$0.9867$. The effective dimension $D_f^{\,\text{eff}} \approx 2.973$ describes the cumulative effect of the RG running over 17 orders of magnitude.
	...
	\begin{equation}
		...
		\label{eq:linear_kfrak_en}
	\end{equation}
	
	This simplification is justified because:
	\begin{itemize}
		\item $\xi \ll 1$, so higher orders are negligible
	...
	\begin{align}
		...
		K_{\text{frak}}^{\text{power}} &= \left(\frac{2.973}{3}\right)^{2.973/2} = 0.98666...
	\end{align}
	...
	\subsection{Relationship between $D_f^{\,\text{space}}$, $D_f^{\,\text{eff}}$, and $K_{\text{frak}}$}
	
	The two fractal dimensions and the correction factor are connected by:
	\begin{align}
		D_f^{\,\text{space}} &= 3 - \xi \approx 2.9999 \quad \text{(local)} \\
		K_{\text{frak}} &= 1 - 100\xi \approx 0.9867 \quad \text{(cumulative)} \\
		D_f^{\,\text{eff}} &\approx 2.973 \quad \text{such that } (D_f^{\text{eff}}/3)^{D_f^{\text{eff}}/2} = K_{\text{frak}}
	\end{align}
	
	The factor~100 describes the amplification of the tiny local deviation~$\xi$ through the scale flow over 17~orders of magnitude.
	...
	\section{Distinction: Fractal Corrections vs. Rounding Errors}
	
	\begin{tcolorbox}[colback=green!5!white,colframe=green!75!black,title={Two Types of Corrections in T0 Calculations}]
		\textbf{1. Physical Fractal Correction} ($\sim 1.3\%$):
		\begin{itemize}
			\item Cumulative effect of fractal spacetime structure ($D_f^{\,\text{space}} = 2.9999$) over the scale flow, described by $D_f^{\,\text{eff}} \approx 2.973$
	...
	\begin{tcolorbox}[colback=cyan!5!white,colframe=cyan!75!black,title={Exponential Forms in T0}]
		\textbf{Reference:} Document 008\_T0\_xi-und-e
	...
	\begin{tcolorbox}[colback=magenta!5!white,colframe=magenta!75!black,title={Geometric Derivation of $\xi$}]
		\textbf{Reference:} Document 009\_T0\_xi\_ursprung
	...
	\begin{description}
		...
		\item[$D_f^{\text{space}}$] Local fractal dimension of spacetime, $D_f^{\text{space}} = 3 - \xi \approx 2.9999$
		\item[$D_f^{\text{eff}}$] Effective fractal dimension (cumulative effect over scale flow), $D_f^{\text{eff}} \approx 2.973$
