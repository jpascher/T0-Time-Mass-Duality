\section*{Abstract}
	...
	\begin{tcolorbox}[colback=blue!10!white, colframe=blue!75!black, title=References to Complementary T0 Formulations]
		...
		\begin{itemize}
			...
			Shows how the same parameter $\xi$ explains cosmological redshift in a static universe (see Document~026)
		\end{itemize}
...
\begin{equation}
  \resizebox{\textwidth}{!}{$\displaystyle \begin{aligned}
			D_{\text{fract}}^{\,\text{eff}} &\approx 2.973 \quad \text{(effective fractal dimension, cumulative over RG running)} \\
...
\end{equation}
		...
		\begin{itemize}
			...
			\item \textbf{Note:} This model ($K_{\text{fract}} = 1 - (D_{\text{fract}}-2)/68$) is a simplified representation. Doc.~133 uses the equivalent model $K_{\text{frak}} = 1 - 100\xi \approx 0.9867$ (full derivation). Both models are consistent for approximate calculations; Doc.~133 applies for precision predictions.
