...
\begin{tabular}{lcccc}
	...
	\begin{tcolorbox}[colback=red!5,colframe=red!50,title=Correction (May 2026)]
		The following section is from a working note (Nov.~2025) and assumes a value $S \approx 2.8275 \pm 0.0002$ as "identified 2025 data" from a 73-qubit Bell test. This value is \emph{not substantiated} in available records and should not be read as a real measurement. The actual hardware measurement (Doc.~147, IBM Heron r2, May 2026) gives $S = 2.74$ ($96.9\%$ of the Tsirelson bound), where the deviation from $2\sqrt{2}$ is NISQ decoherence, not the $\xi$-effect. The $\xi$-fit to $2.8275$ below is therefore moot; the theoretical $\ln N$ scaling formula remains valid as a structure.
	\end{tcolorbox}
