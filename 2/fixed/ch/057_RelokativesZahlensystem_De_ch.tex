...
\scriptsize
	...
	\begin{lstlisting}[language=Python, breaklines=true, frame=none, basicstyle=\scriptsize\ttfamily]
...
\end{lstlisting}
	...
	\begin{lstlisting}[language=Python, breaklines=true, frame=none, basicstyle=\scriptsize\ttfamily]
def solve_energy_field(self,
x: np.ndarray, t: np.ndarray) -> np.ndarray:
# T0-Framework: c² = 1 + xi (relationale Koppelung)
c_squared = 1.0 + abs(self.xi)  # NICHT nur xi!

for i in range(2, len(t)):
for j in range(1, len(x)-1):
spatial_laplacian = (E[j+1,i-1] - 2*E[j,i-1] + 
E[j-1,i-1]) / (dx**2)
# Wellengleichung mit relationaler Geschwindigkeit
E[j,i] = 2*E[j,i-1] - E[j,i-2] + c_squared * (dt**2) 
* spatial_laplacian
\end{lstlisting}
	
	\subsubsection{Relationale Quantengates}
	\begin{lstlisting}[language=Python, breaklines=true, frame=none, basicstyle=\scriptsize\ttfamily]
def hadamard_t0(self, E_field_0: float,
E_field_1: float) -> Tuple[float, float]:
...
# Relationale Energiefeld-Korrelation 
# statt absoluter Berechnung
E_corr = self.xi * (E1 * E2) / (r**2)
base_resonance = math.exp(-((omega - math.pi)**2) /
(4 * abs(self.xi)))
