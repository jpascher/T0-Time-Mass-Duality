\section*{Abstract}
	...
	\section*{7. Empirische Vorhersagen (aus $\xi$ abgeleitet)}
	\addcontentsline{toc}{section}{7. Empirische Vorhersagen (aus $\xi$ abgeleitet)}
	
	\textbf{Neutrinomassen:}
	\[
	m_\nu = \frac{\xi^2}{2} \cdot m_e \approx 4{,}54 \, \text{meV} \quad \Rightarrow \quad \Delta m_{21}^2 \sim 10^{-5} \, \text{eV}^2
	\]
	\textit{Hinweis: Die Formel $m_\nu = \xi^2/2 \cdot m_e$ liefert eine Neutrinomasse, die mit kosmologischen Obergrenzen konsistent ist. Die Massendifferenzen aus Neutrinooszillationsdaten ($\Delta m^2_{21} \approx 7{,}5 \times 10^{-5}$ eV²) werden damit dem Zahlenwert nach reproduziert, jedoch sind die Masseneigenzustände in der T0-Ableitung noch nicht vollständig hergeleitet.}
