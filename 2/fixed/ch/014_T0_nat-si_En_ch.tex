\section*{Abstract}
	...
	\begin{align}
		...
		\textbf{2. Experimental:} \quad & m_e^{\mathrm{SI}} = 9.109 \times 10^{-31}~\mathrm{kg} \\
		\textbf{3. T0 prediction:} \quad & S_{T0} = m_e^{\mathrm{SI}}/m_e^{\mathrm{T0}} = 1.7827\!\times\!10^{-30} \\
		\textbf{4. Empirical fact:} \quad & 1~\mathrm{MeV}/c^2 = 1.7827 \times 10^{-30}~\mathrm{kg} \\
		\textbf{5. Conclusion:} \quad & \text{T0 theory \textbf{predicts} the MeV mass scale}
	\end{align}
	...
	The slight deviation from the experimental value $1/\alpha = 137.036$ is due to higher-order fractal corrections that are accounted for in the complete correction procedure.
	...
	\begin{itemize}
		...
		\begin{equation}
			m_e^{\mathrm{T0}} = 0.511 \quad \text{(geometric number, without correction)}
		\end{equation}
