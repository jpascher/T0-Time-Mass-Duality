\section*{Abstract}
	...
	Es wäre \textbf{unehrlich}, für g-2 perfekte Übereinstimmung zu behaupten, wenn Massen bereits ~1\% abweichen!
	...
	\begin{table}[h]
		...
		\begin{tabular}{lcccc}
			...
			\midrule
			Elektron & $1{,}184 \times 10^{-3}$ & $1{,}160 \times 10^{-3}$ & 2{,}03\% & ✓ \\
			Myon & $1{,}188 \times 10^{-3}$ & $1{,}166 \times 10^{-3}$ & 1{,}89\% & ✓ \\
			Tau & $1{,}269 \times 10^{-3}$ & (nicht gemessen) & -- & Vorhersage \\
			\bottomrule
		\end{tabular}
	...
	\begin{itemize}
		...
		\item ✓ ~2\% Abweichung konsistent mit Massenpräzision (~1\%)
	...
	\begin{itemize}
		\item T0-Vorhersage: $a_\tau = 1{,}269 \times 10^{-3}$
		\item Aus Verhältnis: $a_\tau = 1{,}280 \times 10^{-3}$
		\item Unterschied: ~1\%
	\end{itemize}
	...
	\begin{equation}
		k_{\text{geom}}^{\text{(rekonstruiert)}} = \frac{S_3/f}{a_e^{\text{(exp)}}} = \frac{2\pi^2 / 7500}{1{,}160 \times 10^{-3}} = 2{,}269
	\end{equation}
	
	\textbf{Vergleich:}
	\begin{itemize}
		\item Geometrisch hergeleitet: $k_{\text{geom}} = (2/\sqrt{\varphi}) \times \sqrt{2} = 2{,}224$
		\item Aus Experiment rekonstruiert: $k_{\text{geom}}^{\text{(rek)}} = 2{,}269$
		\item Differenz: 2{,}0\% (genau im Bereich der erwarteten Unsicherheit!)
	\end{itemize}
	
	\subsection{Verwendung des rekonstruierten Korrekturwerts}
	
	Wenn wir den rekonstruierten Wert $k_{\text{geom}}^{\text{(rek)}} = 2{,}269$ verwenden:
	
	\begin{table}[h]
		\centering
		\begin{tabular}{lcccc}
			\toprule
			\textbf{Lepton} & \textbf{Mit $k=2{,}224$} & \textbf{Mit $k=2{,}269$} & \textbf{Experiment} & \textbf{Abw.} \\
			\midrule
			Elektron & $1{,}184 \times 10^{-3}$ & $1{,}160 \times 10^{-3}$ & $1{,}160 \times 10^{-3}$ & \textbf{0\%} ✓ \\
			Myon & $1{,}188 \times 10^{-3}$ & $1{,}164 \times 10^{-3}$ & $1{,}166 \times 10^{-3}$ & \textbf{0{,}2\%} ✓ \\
			Tau & $1{,}269 \times 10^{-3}$ & $1{,}246 \times 10^{-3}$ & (nicht gemessen) & Vorhersage \\
			\bottomrule
		\end{tabular}
	...
	\begin{important}{Entscheidender Punkt}
		Mit dem rekonstruierten Korrekturwert $k_{\text{geom}}^{\text{(rek)}} = 2{,}269$ verschwinden die Abweichungen:
		\begin{itemize}
	...
	\begin{table}[h]
		...
		\begin{tabular}{lcc}
			...
			\midrule
			Rein geometrisch & $1{,}269 \times 10^{-3}$ & $k_{\text{geom}} = 2{,}224$ (geometrisch) \\
			Mit rek. $k_{\text{geom}}$ & $1{,}246 \times 10^{-3}$ & $k_{\text{geom}} = 2{,}269$ (aus $a_e$) \\
			Aus Verhältnis & $1{,}280 \times 10^{-3}$ & Nur $f$ (exakt) \\
			\midrule
	...
	\begin{enumerate}
		...
		\begin{itemize}
			\item ✓ Bestätigt rekonstruierten $k_{\text{geom}} = 2{,}269$
	...
	\begin{keypoint}[Ehrlichkeit und Konsistenz]
		Die T0-Theorie erklärt g-2 aus denselben geometrischen Prinzipien wie Massen, fundamentale Konstanten ($G$, $\alpha$, $v$) und Generationenstruktur. Die ~2\% Abweichung bei Absolutwerten ist konsistent mit der Präzision aller T0-Vorhersagen und ehrlich dargestellt. Verhältnis-Vorhersagen wie $\Delta a(\tau - \mu) / \Delta a(\mu - e) = 18{,}57$ sind parameterfrei und präzise -- analog zur Koide-Formel für Massen. Dies ermöglicht klare experimentelle Tests bei Belle~II.
	\end{keypoint}
