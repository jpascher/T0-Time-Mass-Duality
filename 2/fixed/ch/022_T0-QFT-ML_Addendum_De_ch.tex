\section*{Abstract}
	...
	\textbf{Validierung}: Reduziert $\Delta$ von 12.3\% auf $<0.1\%$ bei $5\pi/4$; theoretischer Vorhersagewert CHSH$^{\text{T0}} \approx 2.8275$ (vs.~QM 2.8284), $\Delta=0.04\%$ (Hardware-Messung liefert $S=2.74$, dekohärenzbegrenzt; siehe Korrektur unten).
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{lccc}
			...
			\midrule
			CHSH (N=73, theoret.) & 2.8276 & 2.8275 & +75 \\
	...
	\textbf{Kritische Analyse}: Ist $\xi = 1{,}340\times10^{-4}$ (gegenüber Basis $4/30000$) eine Parameter-Anpassung oder geometrische Emergenz?
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{lcc}
			...
			Präzision & $\sim$1{,}2\,\% globales $\Delta$ & $\sim$0{,}89\,\% globales $\Delta$ \\
			Parameter & 0 (reine $\phi$-Skalierung) & 1 (kalibriertes $\xi$) \\
			Falsifizierbarkeit & Hoch (feste Vorhersage) & Mittel (an Daten angepasst) \\
	...
	\textbf{Auflösung}: Die Anpassung ist \textit{nicht} äquivalent zur fraktalen Korrektur --- sie ist eine \textit{Manifestation}:
	\begin{itemize}
		\item \textbf{Fraktale Korrektur}: $\exp(-\xi n^2/D_f)$ ist parameterfrei (emergent aus $D_f=3-\xi$)
		\item \textbf{$\xi$-Anpassung}: Justiert $\xi$ um $\mathcal{O}(\xi)$ = 0{,}5\,\% um QFT-Fluktuationen zu berücksichtigen ($\delta E \sim \xi^2$)
		\item \textbf{Analogie}: Wie das Running der Feinstrukturkonstante --- $\alpha(\mu)$ ist ``angepasst'', aber QED sagt das Running voraus
	\end{itemize}
	
	\textbf{Urteil}: Angepasstes $\xi$ ist \textit{selbstkonsistent} (sagt DUNE, Rydberg mit demselben Wert voraus), reduziert aber die Parameterfreiheit von 0 auf 0{,}005 (effektiv). Testbar durch unabhängige Experimente, die zu $\xi \approx 1{,}34\times10^{-4}$ konvergieren.
	...
	\end{equation}
	\textbf{Einwand}: Verletzt $\text{CHSH}^{\text{T0}}=2{,}8275$ die Bell-Schranke (2)?
	...
	\end{itemize}
	\textbf{Kritischer Punkt}: Wäre $\xi=0$ exakt, würde T0 zu lokalem Realismus mit $S\leq2$ reduzieren. Nicht-verschwindendes $\xi$ ist der ``Preis'' für die QM-Vorhersagen --- aber weiterhin lokal (kein FTL).
...
\section{Synthese: Das T0-ML-vereinheitlichte Bild}
	...
	\end{equation}
	mit $i \in \{\text{Bell}, \text{Neutrino}, \text{Rydberg}\}$, Gewichte $w_{\text{Bell}}=0{,}5$, $w_{\nu}=0{,}3$, $w_{\text{Ryd}}=0{,}2$.
	...
	\textbf{Unsicherheit}: Bootstrap-Resampling (1000 Läufe): $\sigma_\xi = 0{,}003\times10^{-4}$ ($\pm0{,}2\,\%$).
	...
	\begin{longtable}{p{2.4cm}p{4.0cm}p{4.0cm}}
		...
		Freie Parameter & 0 (reine Geometrie) & 1 (angepasstes $\xi$, aber selbstkonsistent) \\
