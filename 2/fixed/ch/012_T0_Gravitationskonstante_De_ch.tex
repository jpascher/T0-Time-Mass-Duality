\section*{Abstract}
	...
	\subsection{T0-Perspektive: G in zwei Darstellungen}
	
	\begin{keyresult}
		\textbf{G aus T0-Sicht:}
		
		\textbf{1. In T0-natürlichen Einheiten:}
		\begin{equation}
			c = \hbar = \alpha = G = 1
		\end{equation}
		Alle Konstanten werden auf 1 gesetzt (maximale Vereinfachung).
		
		\textbf{2. Rekonstruktion des SI-Wertes:}
		
		\textbf{a) Aus $\xi$ (dieses Dokument):}
		\begin{equation}
			G_{\text{SI}} = \frac{\xi^2}{4m_e} \times C_{\text{dim}} \times C_{\text{conv}} \times K_{\text{frak}}
		\end{equation}
		Explizite Herleitung aller Umrechnungsfaktoren (Sections 3-8).
		
		\textbf{b) Alternative Planck-Formulierung (Dokument 127):}
		\begin{equation}
			G = \frac{\ell_P^2 \times c^3}{\hbar}
		\end{equation}
		Zeigt direkte Verbindung zur Quantengravitation-Skala.
		
		\textbf{Beide Darstellungen sind äquivalent und zeigen: G ist emergent, nicht fundamental!}
		
		Einziger freier Parameter in T0: $\xi = \frac{4}{3} \times 10^{-4}$
	\end{keyresult}
	
	\subsection{Dokument-Struktur}
	
	Dieses Dokument konzentriert sich auf die technisch-mathematische Herleitung der $\xi$-basierten Formulierung mit vollständiger Begründung aller Umrechnungsfaktoren. Für die philosophische Perspektive ("Warum ist G emergent?") und praktische Konsequenzen siehe Dokument 127.
	...
	\subsection{Verbindung zur Planck-Skala}
	
	Die fundamentale T0-Beziehung $G = \frac{\xi^2}{4m_e}$ (in natürlichen Einheiten) ist äquivalent zur Planck-basierten Formulierung:
	...
	\begin{itemize}
		\item \textbf{Planck-Perspektive:} G emergiert an der Quantengravitation-Skala $\ell_P \approx 1.616 \times 10^{-35}$ m
		\item \textbf{T0-Perspektive:} G folgt aus geometrischem Parameter $\xi$ und Elektronmasse
		\item \textbf{Äquivalenz:} Beide zeigen, dass G nicht fundamental ist
	\end{itemize}
	
	Die Planck-Formulierung ist konzeptuell elegant und zeigt die direkte Verbindung zur Quantengravitation. Die $\xi$-basierte T0-Formulierung (die wir im Folgenden verwenden) macht alle Umrechnungsschritte von natürlichen Einheiten zu SI explizit und zeigt die geometrische Herkunft.
	
	\textbf{Für detaillierte Diskussion der Planck-Perspektive, philosophische Implikationen und praktische Konsequenzen siehe Dokument 127.}
	...
	\begin{itemize}
		...
		\item $K_{\text{renorm}} = 0.986$: Fraktale Korrektur (konsistent mit $K_{\text{frak}}$)
	\end{itemize}
	...
	\subsection{Stufe 6: Fraktale Korrektur}
	
	Die endgültige Korrektur berücksichtigt die effektive fraktale Dimension $D_f^{\,\text{eff}} \approx 2{,}973$ (kumulativ über den RG-Lauf) mit der konsistenten Formel:
	\begin{equation}
	...
	Anwendung der fraktalen Korrektur:
	\begin{equation}
	...
	\begin{itemize}
		...
		\item Gleiche effektive fraktale Dimension: $D_f^{\,\text{eff}} \approx 2{,}973$
	\end{itemize}
	...
	\begin{derivation}
		...
		\begin{align}
			D_f^{\,\text{eff}} &\approx 2{,}973 \quad \text{(effektive fraktale Dimension über RG-Lauf)} \\
			K_{\text{frak}} &= 1 - \frac{D_f - 2}{68} = 1 - \frac{0.94}{68} = 0.986
		\end{align}
		
		\textbf{Geometrische Bedeutung:} 
		Der Faktor 68 entspricht der tetraedischen Symmetrie der T0-Raumstruktur. Die effektive fraktale Dimension $D_f^{\,\text{eff}} \approx 2{,}973$ beschreibt die kumulative Wirkung der Raumzeit-Porosität ($D_f^{\,\text{Raum}} = 3 - \xi \approx 2{,}9999$) über den RG-Lauf.
	...
	\begin{derivation}
		...
		Der Wert $D_f^{\,\text{eff}} \approx 2{,}973$ wird nicht willkürlich gewählt, sondern ergibt sich zwingend aus der Konsistenzbedingung zweier unabhängiger Herleitungswege des Massenverhältnisses $m_e/m_\mu$ (vgl.\ Dok.~133).
		...
		\begin{itemize}
			...
			\item Dies ist \textbf{nur möglich} mit $D_f^{\,\text{eff}} \approx 2{,}973$
		\end{itemize}
		...
		Die Lösung dieser Gleichung ergibt zwingend $D_f^{\,\text{eff}} \approx 2{,}973$. Jeder andere Wert würde zu inkonsistenten Vorhersagen für $\alpha$ führen.
		
		\textbf{Physikalische Bedeutung:}
		Die effektive fraktale Dimension $D_f^{\,\text{eff}} \approx 2{,}973$ stellt sicher, dass:
		\begin{itemize}
	...
	\begin{keyresult}
		\textbf{Konsistenz der fraktalen Korrektur:}
	...
	\begin{verification}
		...
		\begin{enumerate}
			...
			\item Die \textbf{effektive fraktale Dimension} $D_f^{\,\text{eff}} \approx 2{,}973$ ist universell für alle Skalierungsphänomene
		\end{enumerate}
	...
	\begin{enumerate}
		\item \textbf{Dunkle Materie:} Kann durch $\xi_0$-Feldeffekte erklärt werden
