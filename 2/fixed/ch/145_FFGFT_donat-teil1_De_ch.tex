\section{Einleitung}
	...
	In einem Raum mit fraktaler Dimension \(D_f\) lautet das allgemeine Coulomb-Gesetz:
	\begin{equation}
		F_{\text{Coulomb}} \propto \frac{1}{r^{D_f - 1}}.
		\label{eq:fractal_coulomb_general}
	\end{equation}
	Für das heutige Universum mit \(D_f = 3-\xi\) ergibt sich die winzige, aber prinzipiell messbare Modifikation:
	\begin{equation}
		F_{\text{Coulomb}} \propto \frac{1}{r^{2 - \xi}}.
		\label{eq:fractal_coulomb}
	\end{equation}
	\textbf{Sonderfall frühes Universum \((t \to t_{\min},\; D_f \to 2)\):}
	Am Beginn des Universums, wenn die dritte Raumdimension noch nicht vollständig emergiert ist, gilt \(D_f = 2\). Dann vereinfacht sich das Coulomb-Gesetz zu einem linearen Potential:
	\begin{equation}
		F_{\text{Coulomb}}\big|_{D_f=2} \propto \frac{1}{r^{1}} = \frac{1}{r}.
		\label{eq:coulomb_df2}
	\end{equation}
	Dies entspricht dem aus der 2D-Physik bekannten logarithmischen Einschlusspotential und hat direkte Konsequenzen für die Struktur von Ladungsverteilungen in der Planck-Ära.
	...
	Bei echten Fraktalen gilt: Wenn man die Auflösung um Faktor \( r \) erhöht, steigt die Anzahl der sichtbaren Strukturen um \( r^D \). Allgemein:
	\[
	N(r) \propto r^{D_f}
	\]
	Heutiges Universum (\( D_f = 3 - \xi \)):
	\[
	N(r) \propto r^{(3-\xi)}
	\]
	\textbf{Sonderfall frühes Universum \((t \to t_{\min},\; D_f = 2)\):}
	\[
	N(r)\big|_{D_f=2} \propto r^{2}
	\]
	Die Raumstruktur ist flächenartig – die Anzahl sichtbarer Strukturen wächst nur quadratisch mit der Auflösung.
	...
	\[
	\rho_{\text{vac}} \sim \int_0^{\omega_P} \omega^{D_f} \, d\omega \sim \omega_P^{D_f+1}
	\]
	Für das heutige Universum \((D_f = 3-\xi \approx 3)\):
	\[
	\rho_{\text{vac}}\big|_{D_f \approx 3} \sim \omega_P^4 \sim \left(\frac{c}{\ell_P}\right)^4
	\]
	\textbf{Sonderfall frühes Universum \((t \to t_{\min},\; D_f = 2)\):}
	\[
	\rho_{\text{vac}}\big|_{D_f=2} \sim \omega_P^3 \sim \left(\frac{c}{\ell_P}\right)^3
	\]
	...
	\textbf{Sonderfall frühes Universum \((t \to t_{\min},\; D_f = 2)\):}
	Für $D_f = 2$ entfällt die dritte Raumdimension als Stabilisierungsreservoir. Der minimale Torus-Radius kollabiert auf die Planck-Skala selbst:
	\[
	r_{\text{min}}\big|_{D_f=2} \sim \ell_{\text{Planck}}
	\]
	Dies ist die absolute untere Grenze – der Torus kann nicht kleiner als ein Planck-Längen-Radius werden. Das früheste Universum hatte damit eine maximal kompakte Torus-Topologie bei $r \sim \ell_P$.
	...
	\begin{itemize}
		...
		Primärquelle: 025\_T0\_Kosmologie\_De.pdf
		...
		Primärquellen: 025\_T0\_Kosmologie\_De.pdf, FFGFT\_Narrative\_Master\_En.pdf
		...
		Primärquellen: 039\_Zwei-Dipole-CMB\_En.pdf, 025\_T0\_Kosmologie\_De.pdf
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Die Hubble-Spannung (Diskrepanz zwischen lokaler und CMB-basierter $H_0$) wird in Dokument~026 aufgelöst.
	...
	\begin{itemize}
		\item 025\_T0\_Kosmologie\_De.pdf
		\item FFGFT\_Narrative\_Master\_En.pdf
	...
	\[
	H_0 \approx 66{,}2\,\text{km/s/Mpc} \quad \text{(siehe Dokument~026)}
	\]
	...
	In einem \(D_f\)-dimensionalen Raum lautet die allgemeine Coulomb-Kraft:
	\[
	F \propto \frac{1}{r^{D_f - 1}}
	\]
	Standardphysik (\( D = 3 \)):
	\[
	F \propto \frac{1}{r^2}
	\]
	Heutiges Universum, fraktaler Raum (\( D_f = 3 - \xi \)):
	\[
	F \propto \frac{1}{r^{2 - \xi}}
	\]
	Für \( \xi = \frac{4}{3} \times 10^{-4} \):
	\[
	F \propto \frac{1}{r^{1{,}9998667\ldots}}
	\]
	Auf großen Skalen führt dies zu einer winzigen Modifikation, die ''Dunkle Energie''-Effekte erklärt!

	\textbf{Sonderfall frühes Universum \((t \to t_{\min},\; D_f = 2)\):}
	\[
	F\big|_{D_f=2} \propto \frac{1}{r^{1}} = \frac{1}{r}
	\]
	Am Anfang des Universums gab es kein $1/r^2$-Gesetz, sondern ein lineares Einschlusspotential. Dies ist physikalisch äquivalent zum 2D-Coulomb-Potential und hatte fundamentale Auswirkungen auf die Strukturbildung in der Planck-Ära.
	...
	In einem $D_f$-dimensionalen Raum skaliert der Casimir-Druck allgemein als:
	\[
	P_{\text{Casimir}} \propto -\frac{\hbar c}{d^{D_f+1}}
	\]
	Heutiges Universum (\( D_f \approx 3 \)):
	\[
	...
	\textbf{Sonderfall frühes Universum \((t \to t_{\min},\; D_f = 2)\):}
	\[
	P_{\text{Casimir}}\big|_{D_f=2} \propto -\frac{\hbar c}{d^3}
	\]
	Der schwächere Abstandsexponent ($d^{-3}$ statt $d^{-4}$) bedeutet, dass der Casimir-Druck in der Planck-Ära auf großen Abständen stärker und auf kleinen Abständen schwächer wirkte als heute – mit direkten Folgen für die Torus-Stabilität am Anfang des Universums.
	...
	In einem allgemeinen \(D_f\)-dimensionalen Raum gilt für das Energiespektrum der Turbulenz:
	\[
	E(k) \propto k^{-(D_f + 2)/3}
	\]
	Kolmogorov-Spektrum (\( D = 3 \)):
	\[
	E(k) \propto k^{-5/3}
	\]
	Heutiges Universum, fraktales Spektrum (\( D_f = 3 - \xi \)):
	\[
	...
	\textbf{Sonderfall frühes Universum \((t \to t_{\min},\; D_f = 2)\):}
	\[
	E(k)\big|_{D_f=2} \propto k^{-4/3}
	\]
	Der flachere Exponent \(-4/3\) bedeutet, dass in der Planck-Ära Energie weniger effizient auf kleine Skalen kaskadierte – ein fundamentaler Unterschied zur heutigen kosmischen Turbulenz.
