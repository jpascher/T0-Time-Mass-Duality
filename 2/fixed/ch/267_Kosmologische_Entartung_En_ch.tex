...
\textbf{The factor~3 counts the three observable spatial dimensions.}
The peaks $3, 18, 42, 78$ are not arbitrary -- they are exactly
$3 \cdot \{1, 6, 14, 26\}$. The factor~3 arises because the observer
measures in 3D space (plus time-unfolding): a T$^4$ mode
$\vec{n} = (n_1, n_2, n_3, n_4)$ is perceived not through its full
torus radius $|n|^2$ but through the spatial projection
\[
  k_\mathrm{obs}^2 = \frac{n_1^2 + n_2^2 + n_3^2}{L_\mathrm{tor}^2},
  \qquad
  \ell \propto k_\mathrm{obs}.
\]
For the most symmetric ground mode $(1,1,1,0)$ -- all three spatial
directions equal, $n_4 = 0$ -- one has $n_1^2+n_2^2+n_3^2 = 3$. This
shifts the T$^4$ ground peaks $|n|^2 = 1, 6, 14, 26$ to the observable
series $3, 18, 42, 78$. The fractal spatial dimension $D_f = 3 - \xi
\approx 3$ is \emph{consistent} with this (the factor is the spatial
dimension, slightly modified by $\xi$, correction $\xi/3 \approx
4.4\times10^{-5}$), but the origin of the factor is the 3D projection,
not a dispersion rescaling.
The detailed derivation and the honest caveat (the rigorous selection
mechanism via the spherical $C_\ell$ projection remains open, P30) are
in Doc.~268, Steps~8--9.

\textbf{Two filters produce the peak structure.} The observed discrete,
sharp CMB peaks follow from two properties of the same complete T$^4$
acting together -- seen from two reference points:

\begin{itemize}
  \item \textbf{Filter~1 -- compactness (the T$^4$ as a whole, from outside):}
        A compact (finite) space gives a \emph{discrete} mode spectrum;
        an infinite-open space gives a continuous one -- no sharp peaks.
        This filter forces the \emph{existence} of discrete peaks.
  \item \textbf{Filter~2 -- fractal structure (from inside, decompactified):}
        The fractal granulation ($D_f = 3-\xi$, Jacobi degeneracy of the
        four-square sum and Bose-Einstein weighting) weights the discrete
        modes unequally. Only this produces the \emph{dominant} peaks at
        $|n|^2 = 3, 6, 10, 14, \ldots$ rather than a uniform lattice.
        This filter forces the \emph{location} of the peaks.
\end{itemize}

The two filters are not two different objects but two reference points
on the same complete T$^4$: compact-finite from outside (Filter~1,
discrete modes), fractally granulated inside down to $L_0 = \xi\cdot\ell_P$
(Filter~2). The fractal non-closure is therefore not a \emph{second}
property contradicting compactness, but the inside view of the same
closed torus -- a matter of reference point (inside/outside, P20). Only
both filters together give the observed CMB series; the discrete peaks
thus argue \emph{for} the compact T$^4$ and \emph{against} a
purely-infinite-open universe.

\textbf{Honest caveat:} This conclusion is FFGFT-internal. The peaks do
\emph{not} force spatial compactness -- $\Lambda$CDM produces the same
discrete peaks from a \emph{temporal} bound (the sound horizon at
recombination) in a spatially infinite universe. This is the cosmological
degeneracy: the peaks alone do not distinguish spatial compactness (FFGFT)
from a temporal bound ($\Lambda$CDM).
