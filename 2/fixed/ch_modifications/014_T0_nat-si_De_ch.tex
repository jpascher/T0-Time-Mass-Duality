...
\begin{align}
	...
	\textbf{2. Experimenteller Input:} \quad & m_e^{\mathrm{SI}} = 9.1093837 \times 10^{-31}~\mathrm{kg} \quad \text{(gem.)} \\
...
\end{align}
\textbf{5. Tiefgreifende Schlussfolgerung:} Die T0-Theorie \textbf{vorhersagt} die MeV-Massenskala
...
\begin{tabular}{p{6cm}p{6cm}}
	...
	\begin{itemize}
		...
		\item Fraktale Korrekturverfahren für Präzisionsanpassung
	\end{itemize}
	...
	Die leichte Abweichung vom experimentellen Wert $1/\alpha = 137.036$ ist auf fraktale Korrekturen höherer Ordnung zurückzuführen, die im vollständigen Korrekturverfahren berücksichtigt werden.
	
	\section{Integration der fraktalen Korrektur in natürliche Einheiten}
	
	Die Formeln in der T0-Theorie passen in natürlichen Einheiten ohne explizite fraktale Korrektur, da diese Einheiten die geometrische Essenz der Theorie isolieren. Für exakte Umrechnungen in SI-Einheiten ist die fraktale Korrektur jedoch essenziell, um selbstähnliche Korrekturen der Vakuumgeometrie einzubeziehen.
	
	\subsection{Warum passen die Formeln in natürlichen Einheiten ohne fraktale Korrektur?}
	...
	\begin{itemize}
		\item \textbf{Der $\xi$-Parameter ist intrinsisch dimensionslos}: $\xi$ repräsentiert die reine Geometrie des Vakuumfelds und wirkt wie ein üniverseller Skalierungsfaktor.''
		
		\item \textbf{Approximative Gültigkeit für grobe Berechnungen}: Viele T0-Formeln sind exakt in der geometrischen Idealform, ohne Korrektur.
		
		\item \textbf{Beispiel: Elektronenmasse in natürlichen Einheiten}:
		\begin{equation}
			m_e^{\mathrm{T0}} = 0.511 \quad \text{(geometrische Zahl, ohne Korrektur)}
		\end{equation}
		Dies ``passt'' sofort, weil $\xi$ die geometrische Skala setzt.
	\end{itemize}
	
	\subsection{Warum ist fraktale Korrektur für exakte SI-Umrechnungen notwendig?}
	
	SI-Einheiten sind menschliche Konventionen, die die geometrische Reinheit der T0-Theorie ``verunreinigen''. Um exakte Übereinstimmung mit Experimenten zu erreichen, muss die fraktale Korrektur \textbf{explizit angewendet} werden, weil:
	...
	\subsection{Mathematische Spezifikation der fraktalen Korrektur}
	
	Die fraktale Korrektur wird explizit definiert als:
	\begin{equation}
...
\begin{table}[h]
	\centering
	\caption{Vergleich der geometrischen Idealisierung in T0-Einheiten und physikalischen Exaktheit mit fraktaler Korrektur.}
	\label{tab:approximation-exaktheit}
	\adjustbox{max width=\textwidth}{%
\begin{tabular}{p{3.5cm}p{6cm}p{6cm}}
			\toprule
			\textbf{Aspekt} & \textbf{Ohne fraktale Korrektur (T0-Einheiten)} & \textbf{Mit fraktaler Korrektur (für SI-Umrechnung)} \\
			\midrule
...
\end{table}
	...
	Die Formeln ``passen'' in T0-Einheiten ohne Korrektur, weil diese Einheiten die \textbf{geometrische Essenz} der Physik erfassen. Für die Umrechnung in messbare SI-Einheiten wird fraktale Korrektur \textbf{explizit notwendig}, um die \textbf{selbstähnlichen Korrekturen} der fraktalen Vakuumgeometrie einzubeziehen.
...
\begin{tabular}{p{3cm}p{10cm}}
			...
			$R_f$ & Fraktaler Korrekturfaktor \\
			$f_{\text{fraktal}}$ & Fraktale Korrekturfunktion \\
			...
			$\delta_n$ & Fraktale Korrekturkoeffizienten; dimensionslos \\
			\bottomrule
		\end{tabular}%
}
