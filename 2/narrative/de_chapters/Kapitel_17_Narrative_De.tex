\chapter{Alternative zu GR + $\Lambda$CDM in der fraktalen T0-Geometrie}
	...
	Diese Gleichungen beschreiben im Standardmodell die Expansion des Universums in Abhängigkeit von Materie, Strahlung, Krümmung und einer kosmologischen Konstante. In der FFGFT wird diese \enquote{Expansion} als fraktale Skalenänderung reinterpretiert. Das Modell benötigt jedoch typischerweise sechs oder mehr freie Parameter und zusätzliche Annahmen wie Inflation und Dunkle-Materie-Partikel.
...
\begin{equation}
	\begin{aligned}
		S &= \int \sqrt{-g} \, \Biggl[ \frac{R}{16\pi G} 
		+ \xi \cdot \rho_0^2 \Biggl( (\partial_\mu \ln a)^2 \\
		&\quad + \sum_{k=1}^\infty \xi^k (\nabla^k \ln a)^2 \Biggr) 
		+ \mathcal{L}_m \Biggr] d^4x.
	\end{aligned}
\end{equation}
	...
	wobei \(l_0 = l_P \cdot \xi \approx \SI{2.15e-39}{\meter}\) die fundamentale Korrelationslänge ist.
	...
	\begin{equation}
		\begin{aligned}
			H^2(a) &\approx H_0^2 \Biggl[ \Omega_b a^{-3} \\
			&\quad + \xi^2 \left(1 + \xi^{1/2} \frac{\langle \delta^2 \rangle}{a^3} \right) \Biggr],
		\end{aligned}
	\end{equation}
