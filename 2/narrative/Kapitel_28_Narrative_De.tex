...
Dieses Kapitel baut auf den vorangegangenen Erkenntnissen auf. Wir haben in den ersten 27 Kapiteln die fundamentalen Prinzipien der FFGFT kennengelernt: die Time-Mass-Dualität, die fraktale Geometrie mit Parameter $\xi = \frac{4}{3} \times 10^{-4}$, die Emergenz des Raums, und zahlreiche Anwendungen dieser Prinzipien.
...
Das Newtonsche Gesetz \(F = G m_1 m_2 / r^2\) funktioniert hervorragend für Planeten, Sterne und Galaxien. Aber gilt es für ein einzelnes Proton, das ein anderes Proton anzieht? Die Antwort lautet: Nein, nicht fundamental.
	...
	\begin{tcolorbox}[title={\textbf{Wichtige Symbole und ihre Einheiten}}, colback=blue!5!white, colframe=blue!75!black]
		\begin{tabular}{p{0.3\textwidth}p{0.3\textwidth}p{0.35\textwidth}}
			...
			\(c^2\) & Lichtgeschwindigkeit quadriert & \si{\meter^2\per\second^2} \\
			\(\alpha, \beta\) & Superpositionskoeffizienten & dimensionslos \\
			\(\phi_1, \phi_2\) & Superpositionszustände & dimensionslos \\
			\(\Re\) & Realteil & -- \\
			\(m_p\) & Protonmasse & \si{\kilo\gram} \\
			\(\psi(x)\) & Wellenfunktion & dimensionslos \\
		\end{tabular}
	\end{tcolorbox}
	
	\textbf{Einheitenprüfung (Newtonsches Gesetz):}
	\begin{align*}
	...
	\begin{align*}
		[\delta \rho(x)] &= \si{\meter\cubed\per\kilo\gram\per\second\squared} / \si{\meter\squared\per\second\squared} \cdot \si{\joule\per\meter^3} \cdot \text{dimensionslos} = \si{\kilo\gram\per\meter^3}
	\end{align*}
	...
	\textbf{Einheitenprüfung:}
	\begin{align*}
		[E_{\text{self}}] &= \si{\meter\cubed\per\kilo\gram\per\second\squared} \cdot \si{\kilo\gram^2} / \si{\meter\squared\per\second\squared} \cdot \si{\per\meter^6} \cdot \si{\meter^6} \cdot \text{dimensionslos} = \si{\joule}
	\end{align*}
	...
	\textbf{Einheitenprüfung:}
	\begin{align*}
		[F_g] &= \text{dimensionslos} \cdot \si{\meter\cubed\per\kilo\gram\per\second\squared} \cdot \si{\kilo\gram^2} / \si{\meter\squared} = \si{\newton}
	\end{align*}
...
Dieses Kapitel hat unsere Reise durch die FFGFT um wichtige Aspekte erweitert. Die hier entwickelten Konzepte bauen direkt auf den Erkenntnissen der Kapitel 1-27 auf und bereiten den Boden für die folgenden Untersuchungen.
