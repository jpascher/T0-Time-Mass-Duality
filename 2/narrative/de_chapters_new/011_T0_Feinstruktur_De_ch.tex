...
\section*{Abstract}
	...
	Die Fundamentale Fraktalgeometrische Feldtheorie (FFGFT) bietet erstmals eine geometrische Herleitung der Feinstrukturkonstante. Statt sie als freien Parameter zu betrachten, folgt $\alpha$ aus der fraktalen Struktur der Raumzeit und der Zeit-Masse-Dualität.
	...
	\subsection{Die Essenz der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT)}
	
	\begin{keyresult}
		\textbf{Die Fundamentale Fraktalgeometrische Feldtheorie (FFGFT) kann auf eine einzige Formel reduziert werden:}
	...
	\begin{foundation}
		...
		Die Wahl der ersten Variante erfolgt, weil sie die höchste Präzision bietet und die geometrische Einheit der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT) bewahrt, ohne zirkuläre Anpassungen an experimentelle Daten.
	\end{foundation}	
	...
	Die Fundamentale Fraktalgeometrische Feldtheorie (FFGFT) postuliert zwei äquivalente Darstellungen für die Leptonenmassen:
	\begin{align*}
	...
	In der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT) kommt es zu scheinbar zirkulären Verhältnissen, die jedoch Ausdruck der tiefen Verwobenheit der fundamentalen Konstanten sind:
	\begin{align*}
		\alpha &= f(\xipar) \\
		\xipar &= g(\alpha)
	\end{align*}
	...
	Diese Methode unterstreicht die Stärke der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT): Jede Formel muss nicht nur numerisch passen, sondern dimensions- und geometrisch konsistent sein.	
	...
	Die Fundamentale Fraktalgeometrische Feldtheorie (FFGFT) zeigt, dass $\frac{4}{3}$ eine universelle geometrische Konstante ist, die die gesamte Physik durchzieht. Von der Feinstrukturkonstante bis zu Teilchenmassen taucht dieses Verhältnis immer wieder auf.
	...
	\begin{description}
		\item[$\xipar$ ($\xi_0$)]: Fundamentaler geometrischer Parameter der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT), der die Skalierung der fraktalen Raumzeit-Struktur beschreibt. Er ist dimensionslos und leitet sich aus geometrischen Prinzipien ab (Wert: $\frac{4}{3} \times 10^{-4}$).
		\item[$\Kfrak$ ($K_{\text{frak}}$)]: Fraktale Korrekturkonstante, die renormalisierende Effekte in der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT) berücksichtigt. Sie korrigiert bare Werte zu experimentellen Messwerten (Wert: 0.986).
		...
		\item[$\alphaem$ ($\alpha$)]: Feinstrukturkonstante, eine dimensionslose Kopplungskonstante der Quantenelektrodynamik (QED), die die Stärke der elektromagnetischen Wechselwirkung quantifiziert (Wert: $\approx 7.297 \times 10^{-3}$ oder $1/137.04$ in der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT)).
		\item[$\Dfrak$ ($D_f$)]: Fraktale Dimension der Raumzeit in der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT), die eine Abweichung von der klassischen Dimension 3 andeutet (Wert: 2.94).
