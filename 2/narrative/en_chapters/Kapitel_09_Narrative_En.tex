...
\textbf{Important clarification:} In natural units $\alpha = 1$ and $G = 1$ -- the couplings are not free parameters but unit normalisations (see Dok~043). The following SI representations show how the \textit{apparently small} values arise through conversion factors from natural to SI units. The powers of $\xi$ encode the physical hierarchy; the remaining quantities are scale adaptation factors of the unit transformation.
...
where $m_e$ is the electron mass, $C_{\text{conv}}$ is the unit conversion factor, and $K_{\text{frak}} \approx 0.987$ is the fractal correction. This gives:
\begin{equation}
...
\begin{equation}
E_{GUT} \approx \frac{1}{\xi^2} \approx 10^{16} \text{ GeV}
\end{equation}
...
\item The gravitational constant emerges from $\xi$, $m_e$, and $K_{\text{frak}}$
\item The hierarchy problem is solved geometrically
\item Running of couplings is determined by fractal structure
\item Grand unification occurs at $E_{GUT} \sim 1/\xi^2$
