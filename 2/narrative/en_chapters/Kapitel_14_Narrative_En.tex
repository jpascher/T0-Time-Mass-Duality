...
This is exactly what FFGFT describes for the emergence of the universe. The "Big Bang" is not an explosion into pre-existing space, but this activation front -- a fractal amplitude front that transforms the vacuum from a near-minimal state ($\rho \approx \rho_{\min}$) to a stable state ($\rho = \rho_0$). $\rho(\vec{x},t)$ is the vacuum amplitude density -- a quantity that measures the strength of vacuum fluctuations, comparable to neural activity in a brain. $\rho_0$ is the equilibrium density at which the vacuum becomes stable.
...
$S(t_0) \approx 1 + \xi \ln(10^4) \approx 1.001$ is the stretching factor (dimensionless), $t_0$ the age of the universe ($\SI{4.35e17}{\second}$).

With $v_b \approx 1.000013\,c$ and $S \approx 1.001$, the formula yields $R \approx c\,t_0 \cdot 1.001 \approx \SI{13.8}{\gigalightyear}$. The observed value $\SI{46.5}{\gigalightyear}$ requires an additional stretching factor $\approx 3.4$, whose derivation from the full time-dependent integration is the subject of ongoing research.
