\chapter{Cosmology Without Inflation  The Fractal Alternative to Inflation  Narrative Version of FFGFT}
...
The ``horizon problem'' assumes that causally disconnected regions cannot have the same properties. But in fractal spacetime, the vacuum field $\Phi = \rho\, e^{i\theta}$ possesses \textbf{fractal phase coherence}: its phase $\theta$ is correlated across all scales -- not because signals travel faster than light, but because the field was \textit{never uncorrelated in the first place}.

The mechanism is analogous to a crystal: all atoms in a perfect crystal are phase-coherent, not because they communicate with each other, but because the crystal order as a \textit{boundary condition} fixes the phase globally. Similarly, the fractal topology of the vacuum locks the phase $\theta$ on all scales. The CMB is homogeneous because the vacuum has existed as a global structure from the outset -- there were never separate regions that needed to ``synchronise''.

The fractal structure extends beyond the apparent horizon.
...
\textbf{Note on the apparent paradox:} The reader may wonder how ``flatness'' is compatible with the brain metaphor of fractal ``convolutions''. The answer lies in the distinction between two scales: \textit{cosmological flatness} ($\Omega \approx 1$) means the large-scale curvature of the universe is vanishingly small -- the universe as a whole is Euclidean. \textit{Fractal convolutions} concern the internal structure at small scales -- the micro-geometry of spacetime. A brain illustrates precisely this duality: the cerebral cortex is a two-dimensional surface that is geometrically ``flat'' -- one could unfold it and lay it on a table. But its \textit{folding} into three dimensions generates enormous complexity at constant volume. Likewise, the universe is flat on large scales ($\Omega = 1$), while its internal fractal depth ($D_f = 3 - \xi$) generates structure on all scales.
