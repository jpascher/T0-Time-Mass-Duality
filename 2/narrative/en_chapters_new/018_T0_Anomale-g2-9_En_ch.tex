\thispagestyle{fancy}
	...
	\begin{table}[htbp]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{p{5cm} p{10cm}}
				...
				$K_{\text{frac}}$ & Fractal correction, $K_{\text{frac}} = 1 - 100 \xi \approx 0.9867$ \\
				...
				$g_{T0}$ & Mass-independent T0 coupling, $g_{T0} = \sqrt{\alpha K_{\text{frac}}} \approx 0.0849$ \\
				...
				$\lambda$ & September prototype calibration parameter, $\lambda \approx 2.725 \times 10^{-3}$ MeV (from muon discrepancy) \\
			\end{tabular}%
		}
	\end{table}
	\section{Introduction and Clarification of Consistency}
	In the pure FFGFT~\cite{T0_SI}, the T0 effect is the complete contribution: SM approximates geometry (QED loops as duality effects), thus $a_\ell^{T0} = a_\ell$. Fits Post-2025 data at $\sim 0.15\sigma$ (Lattice-HVP resolves tension). Hybrid view optional for compatibility.
	...
	\begin{equation}
		g_{T0} = \sqrt{\alpha} \cdot \sqrt{K_{\text{frac}}} \approx 0.0849,
	\end{equation}
	since $T_{\text{field}} = 1 / E_{\text{field}}$ and $E_{\text{field}} \propto \xi^{-1/2}$. Explicitly:
	\begin{equation}
		g_{T0}^2 = \alpha \cdot K_{\text{frac}}.
	\end{equation}
	...
	\begin{equation}
		m_T(\xi) = \frac{m_e}{\xi} \cdot \sin(\pi \xi) \cdot \pi^2 \cdot \sqrt{\frac{\alpha}{K_{\text{frac}}}} \cdot R_f(D_f),
	\end{equation}
	...
	\begin{equation}
		F_2^{T0}(0) = \frac{g_{T0}^2}{8\pi^2} \int_0^1 dx \, \frac{m_\ell^2 x (1-x)^2}{m_\ell^2 x^2 + m_T^2 (1-x)} \cdot K_{\text{frac}}.
	\end{equation}
	For $m_T \gg m_\ell$, it approximates to:
	\begin{equation}
		F_2^{T0}(0) \approx \frac{g_{T0}^2 m_\ell^2}{48 \pi^2 m_T^2} \cdot K_{\text{frac}} = \frac{\alpha K_{\text{frac}}^2 m_\ell^2}{48 \pi^2 m_T^2}.
	\end{equation}
	...
	Substitution yields:
	\begin{equation}
		a_\ell^{T0} = \frac{\alpha(\xi) K_{\text{frac}}^2(\xi) m_\ell^2}{48 \pi^2 m_T^2(\xi)} \cdot \frac{1}{1 + \left( \frac{\xi E_0}{m_T} \right)^{-2/3}} = 153 \times 10^{-11} \times \left( \frac{m_\ell}{m_\mu} \right)^2.
	\end{equation}
	...
	\begin{enumerate}[label=\textbf{Step \arabic*:}]
		\item $\frac{\alpha(\xi)}{2\pi} K_{\text{frac}}^2 \approx 1.146 \times 10^{-3}$.
		\item $\times m_\mu^2 / m_T^2 \approx 1.146 \times 10^{-3} \times 4.098 \times 10^{-4} \approx 4.70 \times 10^{-7}$ (exact: SymPy-ratio).
		\item Full loop integral (SymPy): $F_2^{T0} \approx 6.141 \times 10^{-9}$ (incl. $K_{\text{frac}}^2$ and exact integration).
	...
	\begin{equation}
		\Delta a_\mu^{\text{T0}} = \frac{\alpha K_{\text{frac}}^2 m_\mu^2}{48 \pi^2 m_T^2} \cdot F_{dual} = 153 \times 10^{-11},
	\end{equation}
	...
	\begin{table}[htbp]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{p{3.5cm} p{4.5cm} p{5cm}}
				...
				\hline
				\textbf{Core Idea} & Duality $T \cdot m = 1$; fractal spacetime ($D_f = 3 - \xi$); time field $\Delta m(x,t)$ extends Lagrangian. & Points as vibrating strings in 10/11 Dim.; extra Dim. compactified (Calabi-Yau). \\
				\hline
				...
				\hline
				\textbf{Fractal/Quantum Foam} & Fractal damping $K_{\text{frac}} = 1 - 100\xi$; approximates QCD/HVP. & Quantum foam from string interactions; fractal-like in Loop-Quantum-Gravity hybrids. \\
				\hline
		...
		\caption{Comparison between FFGFT and String Theory (updated 2025, Rev. 9)}
	...
	\end{result}
	\appendix
	\section{Appendix: Comprehensive Analysis of Lepton Anomalous Magnetic Moments in FFGFT (Rev. 9 -- Revised)}
	...
	This appendix synthesizes the iterative discussion on resolving lepton g-2 anomalies in FFGFT. Key queries addressed:
	\begin{itemize}
		...
		\item Differences to September-2025 prototype (calibration vs. parameter-free; integrated from original doc).
	\end{itemize}
	...
	\begin{table}[htbp]
		...
		\caption{Extended Table: T0 Formula in Hybrid and Pure Perspectives (2025 Update, Rev. 9)}
	...
	\begin{table}[htbp]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{p{1.5cm} p{1.8cm} p{0.9cm} p{2.8cm} p{3.8cm} p{1.5cm} p{3.5cm}}
				\toprule
				Lepton & Perspective & T0 Value ($ \times 10^{-11}$) & SM Pre-2025 ($ \times 10^{-11}$) & Total (SM + T0) / Exp. Pre-2025 ($ \times 10^{-11}$) & Dev. ($\sigma$) & Explanation (Pre-2025) \\
				\midrule
		...
		\caption{Hybrid vs. Pure T0: Hybrid Perspective -- Pre-2025 Data ($ \times 10^{-11}$; Tau bound scaled)} \label{tab:hybrid_pure_hybrid}
	\end{table}
	
	% Table 2: Pure T0 Perspective
	\begin{table}[htbp]
		\centering
		\footnotesize
		\resizebox{\textwidth}{!}{%
		...
		\caption{Hybrid vs. Pure T0: Pure T0 Perspective -- Pre-2025 Data ($ \times 10^{-11}$; Tau bound scaled)} \label{tab:hybrid_pure_pure}
	\end{table}
	\textbf{Notes (Rev. 9):} Muon Exp.: $116592059(22) \times 10^{-11}$; SM: $116591810(43) \times 10^{-11}$ (tension-enhancing HVP). Summary: Pre-2025 hybrid superior (fills 4.2$\sigma$ muon); pure predictive (fits bounds, embeds SM). T0 static -- no ``movement'' with updates.
	\subsection{Uncertainties: Why Does SM Have Ranges, T0 Exact?}
	\begin{table}[htbp]
		\centering
		\small
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{p{2cm} p{3cm} p{3cm} p{6cm}}
				...
				\makecell[l]{Uncertainty\\notation} & $\pm 43 \times 10^{-11}$ (1$\sigma$; syst.+stat.) & $\pm 0.1\%$ (from $\delta\xi \approx 10^{-6}$) & SM: model-uncertain (HVP sims); T0: parameter-free. \\
	...
	\begin{table}[htbp]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{p{1.5cm} p{1.5cm} p{1.5cm} p{2.5cm} p{2.5cm} p{2cm} p{2.5cm}}
				...
				Muon ($\mu$) & Pure T0 & 153 (core) & Embed SM $\to$ $\sim 116591963 \times 10^{-11}$ (scaled) & $116592059 \times 10^{-11}$ & $0.009$ \% & Core to discrepancy; fully embedded -- fits, but ``hides'' Pre-2025. \\
				Electron (e) & Hybrid (SM + T0) & 0.0036 & SM $115965218.073 + 0.0036 = 115965218.076 \times 10^{-11}$ & $115965218.073 \times 10^{-11}$ & $2.6 \times 10^{-12}$ \% & Perfect; T0 negligible -- no issue. \\
	...
	\begin{table}[htbp]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{p{3cm} p{3cm} p{3cm} p{3cm}}
				...
				QED Embedding & Not detailed (SM-dom.) & Standard series with $\alpha(\xi) \cdot K_{\text{frac}} \approx 1159652180 \times 10^{-12}$ & QED from duality; no extra factors. \\
	...
	\begin{table}[htbp]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{p{2cm} p{3cm} p{3cm} p{3cm}}
				...
				Formula & $\frac{5\xi^4}{96\pi^2 \lambda^2} \cdot m_\ell^2$ ($K=2.246\times10^{-13}$; $\lambda$ calib. in MeV) & $\frac{\alpha K_{\text{frac}}^2 m_\ell^2}{48 \pi^2 m_T^2} \cdot F_{dual}$ (no calib.; $m_T=\SI{5.22}{\giga\electronvolt}$) & Simpler vs. detailed; muon value adjusted (153 ppb). \\
				...
				2025 Update Expl. & Loop suppression in QCD (0.6$\sigma$) & Fractal damping $K_{\text{frac}}$ ($\sim 0.15\sigma$) & QCD vs. geometry; both reduce discrepancy. \\
