% Chapter file: 012_T0_Gravitationskonstante_En_ch.tex
% Source: 012_T0_Gravitationskonstante_En.tex

% Original: \chapter{\textbf{FFGFT: The Gravitational Constant}
\chapter{FFGFT: The Gravitational Constant}

\hfuzz=200pt
\allowdisplaybreaks

\section*{Abstract}
		This document presents the systematic derivation of the gravitational constant $G$ from the fundamental principles of FFGFT. The complete formula $G_{\text{SI}} = \frac{\xi_0^2}{4 m_e} \times C_{\text{conv}} \times K_{\text{frak}}$ explicitly shows all required conversion factors and achieves complete agreement with experimental values (< 0.01\% deviation). Special attention is given to the physical justification of the conversion factors that establish the connection between geometric theory and measurable quantities.
	
	
	\section{Introduction: Gravitation in FFGFT}
	...
	\begin{derivation}
		...
		FFGFT postulates a fundamental geometric relation between the characteristic length parameter $\xi$ and the gravitational constant:
		...
		This equation describes how the characteristic length scale $\xi$ (defined by the tetrahedral space structure) determines the strength of gravitational coupling. The factor 2 corresponds to the dual nature of mass and space in FFGFT.
	...
	FFGFT uses the electron mass as the characteristic scale:
	\begin{equation}
	...
	\subsection{Unit System of FFGFT}
	
	\begin{dimensional}
		\textbf{Dimensional Analysis in Natural Units:}
		
		FFGFT works in natural units with $\hbar = c = 1$:
		\begin{align}
	...
	\begin{derivation}
		...
		where $E_{\text{char}}$ is a characteristic energy scale of FFGFT.
	...
	The characteristic energy scale $E_{\text{char}} = 28.4\,\text{MeV}$ arises from the fundamental fractal structure of FFGFT:
	...
	From the fine-structure constant derivation in FFGFT, the fundamental reference energy is known:
	\begin{equation}
		E_0 = 7.400\,\text{MeV}
	\end{equation}
	...
	FFGFT postulates a fundamental fractal scaling ratio:
	\begin{equation}
		R_f = \frac{4}{3}
	\end{equation}
	This ratio corresponds to the tetrahedral packing density in three-dimensional space and appears in all scaling relations of FFGFT.
	...
	\begin{derivation}
		...
		FFGFT accounts for the fractal structure of spacetime at Planck scales:
	...
	\begin{derivation}
		...
		The value $D_f = 2.94$ (with $\delta = 0.06$) is not chosen arbitrarily but follows necessarily from the consistent derivation of the fine-structure constant $\alpha$ in FFGFT.
	...
	\begin{verification}
		...
		FFGFT makes the following testable predictions:
	...
	\begin{center}
		\begin{tabular}{lcc}
			\toprule
			\textbf{Aspect} & \textbf{Einstein} & \textbf{FFGFT} \\
			\midrule
	...
	\begin{warning}
		...
		FFGFT predicts deviations from Newton's law of gravitation at characteristic length scales:
