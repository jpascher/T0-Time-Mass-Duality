Die Reaktor-Antineutrino-Anomalie (RAA) beschreibt ein historisch beobachtetes Defizit von etwa 6\% in der Rate gemessener Elektron-Antineutrinos im Vergleich zu den Vorhersagen älterer Flussmodelle (z.~B. Huber-Mueller-Modell) in kurzen Basislinien-Reaktor-Experimenten (Daya Bay, Double Chooz, RENO u.~a.). Diese Anomalie wurde erstmals 2011 prominent und führte zu Spekulationen über sterile Neutrinos.
	...
	Mit typischen Störungen \(\delta \rho / \rho_0 \approx 10^{-6}\) (dimensionless) prognostiziert die fraktale FFGFT ein \(\Delta P \approx 0.06\) (dimensionless), was numerisch mit dem historischen Defizit übereinstimmt – unabhängig von der mainstream-Auflösung durch Flussmodelle.
	...
	wobei gilt:
	\begin{itemize}
		\item \(R\): Ratio aus beobachtetem zu vorhergesagtem Antineutrino-Fluss (dimensionless),
	...
	wobei gilt:
	\begin{itemize}
		\item \(\nu\): Neutrino-Zustand (komplexe Phase, dimensionless),
		\item \(\theta_\nu\): Vakuumphase (in Radiant, dimensionless),
		\item \(\xi = \frac{4}{3} \times 10^{-4}\): Fraktaler Skalenparameter (dimensionless).
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \(\Delta \theta\): Phasendifferenz (dimensionless).
	\end{itemize}
	...
	wobei gilt:
	\begin{itemize}
		\item \(\theta_0\): Ungestörte Phase (dimensionless),
		\item \(\delta \rho / \rho_0\): Relative Dichtestörung (dimensionless),
	...
	wobei gilt:
	\begin{itemize}
		\item \(U_{\text{PMNS}}\): Standard-PMNS-Mischungsmatrix (dimensionless),
		\item Der Exponentialterm: Phasenkorrektur (dimensionless).
	\end{itemize}
	...
	wobei gilt:
	\begin{itemize}
		\item \(P\): Überlebenswahrscheinlichkeit (dimensionless, 0 bis 1),
		\item \(\theta_{13}\): Mischungswinkel (dimensionless),
		\item \(L\): Basislinie (in \si{m}),
		\item \(E\): Neutrino-Energie (in \si{MeV}),
		\item 1.27: Konversionsfaktor für Einheiten (dimensionless in dieser Form).
	\end{itemize}
	...
	\end{equation}
	wobei \(\Delta P\): Änderung der Wahrscheinlichkeit (dimensionless).
	
	Validierung: Numerische Übereinstimmung mit historischem Defizit von 6\%.
	
	\subsection{Energieabhängigkeit}
	
	Der Effekt maximiert bei 4–6\,\si{MeV} durch Resonanz mit fraktaler Skala \(l_0 \cdot \xi^{-1}\), wobei \(l_0\): Referenzlänge (in \si{m}), \(\xi^{-1}\): Skalenerweiterung (dimensionless), passend zum historischen „Bump“.
	
	\subsection{Comparison with Sterile-Neutrino-Hypothese}
