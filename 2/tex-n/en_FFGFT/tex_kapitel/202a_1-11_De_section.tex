\section*{Zusammenfassung}
	...
	In der Fundamentale Fraktalgeometrische Feldtheorie (FFGFT, früher T0-Theorie) gibt es genau \textbf{einen einzigen fundamentalen Parameter}: den geometrischen Skalenparameter \(\xi = \frac{4}{3} \times 10^{-4}\). Alle anderen Größen – einschließlich der fraktalen Dimension \(D_f\), der Feinstrukturkonstante \(\alpha\), des Planckschen Wirkungsquantums \(\hbar\) (sowie \(h = 2\pi \hbar\)), der Speed of light \(c\), der Gravitational constant \(G\) und aller charakteristischen Skalen (Planck length, -Zeit, -Masse etc.) – werden \textbf{zwangsläufig und parameterfrei aus \(\xi\) abgeleitet}. Insbesondere gilt:
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item \(G\): Gravitational constant (Einheit: m$^{3}$\,kg$^{-1}$\,s$^{-2}$),
	...
	Die Variation führt zu Standardbeiträgen aus \(R\) sowie zusätzlichen Termen aus \(\xi \cdot \mathcal{L}_{\text{fractal}}\), die auf makroskopischen Skalen (\(r \gg 10^{-15}\)~m) verschwinden. Die effektive Metrik lautet \(g_{\mu\nu}^{\text{eff}} = g_{\mu\nu} + \xi h_{\mu\nu}(\mathcal{F})\) mit Skalenfunktion \(\mathcal{F}(r) = \ln(1 + r/r_\xi)\) (dimensionless, r: Abstand in m, \(r_\xi\): Fraktale Kernskala \(\approx 10^{-15}\)~m). Der fraktale Term erklärt Dunkle Materie als geometrischen Effekt und sorgt für UV-Finitheit ohne Renormierung.
	...
	\begin{itemize}
		...
		\item \(N(\epsilon)\): Anzahl selbstähnlicher Einheiten bei Auflösung \(\epsilon\) (dimensionless),
		\item \(\epsilon\): Skalenfaktor (dimensionless).
	\end{itemize}
	...
	\begin{equation}
		T(x,t) \cdot m(x,t) = 1
	\end{equation}
	folgt aus der fraktalen Selbstähnlichkeit: Skalentransformationen \(\xi^k\) verknüpfen Zeitintervalle mit Massenskalen, sodass das Produkt invariant bleibt (T: Zeitdichte in s/m$^{3}$, m: Massendichte in kg/m$^{3}$, Produkt dimensionless = 1). Vakuumstabilität erzwingt diese Konstanz.
	...
	\begin{itemize}
		...
		\item \(k\): Hierarchiestufe (ganzzahlig, dimensionless).
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \(\gamma\): Lorentz-Faktor (dimensionless).
	\end{itemize}
	...
	\begin{itemize}
		\item \(\mu\): Interpolationsfunktion (dimensionless),
	...
	\begin{itemize}
		...
		\item \(M\): Masse (kg),
		\item \(\Theta\): Heaviside-Schrittfunktion (dimensionless).
	\end{itemize}
	...
	\subsection{Comparison with Loop Quantum Gravity und Stringtheorie}
	...
	\section{Testable Predictions und Beobachtungen}
	...
	\begin{itemize}
		...
		\item \(D\): Entfernung (m),
		\item \(\kappa\): Korrekturkonstante (dimensionless),
