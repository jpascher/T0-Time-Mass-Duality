Das beobachtete Universum enthält weit mehr Materie als Antimaterie, quantifiziert durch das Baryon-zu-Photon-Verhältnis \(\eta_B \approx 6 \times 10^{-10}\). Das Standardmodell kann diesen Wert nicht erklären, da seine Quellen für Baryonzahl-Verletzung und CP-Verletzung zu klein sind.
	...
	\begin{tcolorbox}[title={\textbf{Important Symbols and their Units}}, colback=blue!5!white, colframe=blue!75!black]
		\begin{tabular}{p{0.3\textwidth}p{0.3\textwidth}p{0.35\textwidth}}
			...
			\(\eta_B\) & Baryon-zu-Photon-Verhältnis & dimensionless \\
			...
			\(B\) & Baryonzahl & dimensionless \\
			\(N_w\) & Windungszahl & dimensionless \\
			...
			\(\epsilon\) & Netto-Asymmetrie pro Windung & dimensionless \\
			\(\Delta \theta_{\text{CP}}\) & CP-verletzende Phasenverschiebung & dimensionless (radiant) \\
			\(\phi_0\) & Fundamentale Bias-Phase & dimensionless (radiant) \\
			\(\Delta k\) & Fraktale Skalenabweichung & dimensionless \\
			\(\dot{\rho} / \rho\) & Relative Amplitudenänderung & \si{\per\second} \\
			\(H(t)\) & Hubble-Parameter & \si{\per\second} \\
			\(n_B / s\) & Baryondichte pro Entropie & dimensionless \\
			\(g_*\) & Effektive Freiheitsgrade & dimensionless \\
			\(n_\gamma\) & Photondichte & \si{\per\meter\cubed} \\
			\(U\) & Fraktale Matrixdarstellung & dimensionless \\
	...
	\textbf{Unit Check (Baryonzahl-Verletzung):}
	\begin{align*}
		[B] &= \text{dimensionless} \\
	...
	\end{align*}
	Mit Integration über Volumen dimensionless.
	...
	\textbf{Unit Check:}
	\begin{align*}
		[\Gamma_w] &= \text{dimensionless} \cdot \text{dimensionless} = \si{\per\second} \quad (\text{skaliert durch Energien})
	\end{align*}
	...
	\subsection{Comparison with anderen Modellen}
