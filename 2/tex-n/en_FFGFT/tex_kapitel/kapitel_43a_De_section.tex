\newpage
	...
	wobei gilt:
	\begin{itemize}
		\item \(\Phi(x)\): Vakuumfeld (dimensionless, normiert),
		\item \(\rho(x)\): Amplitudenfeld (Einheit: kg$^{1/2}$\,m$^{-3/2}$, repräsentiert Inertie und Gravitation),
		\item \(\theta(x)\): Phasenfeld (dimensionless, repräsentiert Zeitfluss und Quantenkohärenz),
	...
	\end{equation}
	wobei \(k\): Hierarchiestufe (dimensionless, ganzzahlig).
	...
	\end{equation}
	wobei \(\lambda\): Skalierungsfaktor (dimensionless). Dies impliziert ein Packungsdefizit von \(\xi\).
