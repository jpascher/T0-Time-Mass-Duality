\newpage
	...
	wobei gilt:
	\begin{itemize}
		\item \(|\psi\rangle\): Qubit-Zustand (dimensionless, als Vektor im Hilbert-Raum),
		\item \(\alpha, \beta\): Komplexe Amplituden (dimensionless, mit Normalisierungsbedingung),
		\item \(|0\rangle, |1\rangle\): Basiszustände (dimensionless).
	\end{itemize}
	...
	wobei gilt:
	\begin{itemize}
		\item \(\theta_{\text{qubit}}\): Phasenkonfiguration für das Qubit (dimensionless),
		\item \(\theta_0\): Globale Vakuumphase (dimensionless),
		\item \(\phi_0, \phi_1\): Fraktal skalierte Phasenwinkel (dimensionless),
	...
	wobei gilt:
	\begin{itemize}
		\item \(\vartheta\): Polarwinkel (dimensionless, \(\propto \xi \cdot \Delta \rho\)),
		\item \(\varphi\): Azimutalwinkel (dimensionless, \(\propto \Delta \theta\)).
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \(\Delta \theta\): Phasenverschiebung (dimensionless).
	\end{itemize}
	...
	\begin{itemize}
		...
		\item \(\Phi\): Vakuumfeld (dimensionless),
		\item \(B\): Phasensteifigkeit (Einheit: kg\,m$^{-1}$\,s$^{-2}$),
		\item \(\nabla \theta\): Phasengradient (dimensionless pro m),
	...
	wobei gilt:
	\begin{itemize}
		\item \(\psi\): Wellenfunktion (dimensionless),
	...
	wobei gilt:
	\begin{itemize}
		\item \(\gamma^\mu\): Dirac-Matrizen (dimensionless),
	...
	Die Herleitung basiert auf der Time-Mass-Dualität \(T \cdot m = 1\), mit \(T\): Zeitfeld (Einheit: s/m$^{3}$), und fraktaler Geometrie \(\beta = 2Gm/r\) (dimensionless), \(\xi = 2\sqrt{G} \cdot m\) (dimensionless).
	...
	\begin{itemize}
		...
		\item \(f_{\text{spin}}(x,t)\): Spin-Strukturfunktion (dimensionless, \(f_{\text{spin}} = A \cdot e^{i(\vec{k} \cdot \vec{x} - \omega t + \phi_{\text{spin}})}\)).
	\end{itemize}
	...
	\subsection{Comparison with Standard-Interpretationen}
