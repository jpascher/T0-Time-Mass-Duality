...
% Standardized preamble - 041_parameter_derivation_En.tex
...
\begin{document}
	...
	\begin{abstract}
		This documentation presents the complete, non-circular derivation of all parameters of the T0 theory. The systematic presentation shows how the fine-structure constant $\alpha = 1/137$ follows purely from geometric principles, without presupposing it. All derivation steps are explicitly documented to definitively refute charges of circularity.
	\end{abstract}
	
	\section{Introduction}
	
	The T0 theory represents a revolutionary approach that demonstrates that fundamental physical constants are not arbitrary but follow from the geometric structure of three-dimensional space. The central claim is that the fine-structure constant $\alpha = 1/137.036$ is not an empirical input but a compelling consequence of space geometry.
	
	To eliminate any suspicion of circularity, this document presents the complete derivation of all parameters in logical order, starting from purely geometric principles and without using experimental values except for fundamental natural constants.
	\tableofcontents
	...
	The factor 4/3 is not coincidental but represents the \textbf{pure fourth}, one of the fundamental harmonic intervals:
	
	\begin{equation}
		\frac{4}{3} = \text{Frequency ratio of the pure fourth}
	\end{equation}
	
	Just as musical intervals are universal:
	\begin{itemize}
		\item \textbf{Octave:} 2:1 (always, whether string, air column, membrane)
	...
	\begin{itemize}
		\item If divided into 4 equal ``vibration zones''
		\item Compared to 3 zones
		\item Yields the ratio 4:3
	\end{itemize}
	
	This is \textbf{pure geometry}, independent of the material!
	...
	This shows the complete harmonic structure of space:
	\begin{itemize}
	...
	The T0 theory thus shows: Space is musically/harmonically structured, and 4/3 (the fourth) is its basic signature!
	
	\textbf{The factor $10^{-4}$:}
	...
	It is the T0-calculated Higgs parameter factor $0.0647 \approx 6.5 \times 10^{-2}$, which reduces the loop suppression by a factor of 20:
	...
	This exponent determines the non-linear mass scaling in the T0 theory.
	
	\section{Lepton Masses from Quantum Numbers}
	
	The masses of the leptons follow from the fundamental mass formula:
	
	\begin{equation}
		m_x = \frac{\hbar c}{\xi^2} \times f(n, l, j)
	\end{equation}
	
	where $f(n, l, j)$ is a function of the quantum numbers:
	
	\begin{align}
		f(n, l, j) = \sqrt{n(n+l)} \times \left[j + \frac{1}{2}\right]^{1/2}
	\end{align}
	
	For the three leptons, this yields:
	...
	These masses are not empirical inputs but follow from $\xi$ and the quantum numbers.
	
	\section{The Characteristic Energy $E_0$}
	
	The characteristic energy $E_0$ follows from the gravitational length scale and the Yukawa coupling:
	
	\begin{equation}
		E_0^2 = \beta_T \cdot \frac{yv}{r_g^2}
	\end{equation}
	
	With $\beta_T = 1$ in natural units and $r_g = 2Gm_\mu$ as the gravitational length scale:
	...
	A remarkable alternative derivation of $E_0$ arises directly from the geometric mean of the electron and muon masses:
	...
	\subsection{Comparison with the Gravitational Derivation}
	
	The value from the geometric mean (7.35 MeV) agrees remarkably well with the value from the gravitational derivation (7.398 MeV). The difference is less than 1\%:
	...
	The fact that $E_0$ corresponds to the geometric mean of the fundamental lepton energies has deep physical significance:
	
	\begin{itemize}
		\item $E_0$ represents a natural electromagnetic energy scale between electron and muon
		\item The relation is purely geometric and requires no knowledge of $\alpha$
		\item The mass ratio $m_\mu/m_e = 206.77$ is itself determined by the quantum numbers
	\end{itemize}
	...
	With further higher-order quantum corrections, the value converges to 7.398 MeV.
	
	\subsection{Verification of the Fine-Structure Constant}
	...
	\subsection{Overview of the Two Geometric Derivations}
	
	The T0 theory offers two independent, purely geometric paths to determine $E_0$, both without knowledge of the fine-structure constant:
	...
	This path uses:
	\begin{itemize}
		\item The geometric parameter $\xi$ from tetrahedron packing
		\item The gravitational length scales $r_g = 2Gm$
	...
	This path uses:
	\begin{itemize}
		\item The geometrically determined masses from quantum numbers
		\item The principle of the geometric mean
		\item The intrinsic structure of the lepton hierarchy
	\end{itemize}
	
	\subsection{Mathematical Consistency Check}
	
	To show that both paths are consistent, set them equal:
	...
	After conversion to MeV, this yields $m_e \approx 0.511$ MeV, confirming the consistency.
	
	\subsection{Geometric Interpretation of the Duality}
	
	The existence of two independent geometric paths to $E_0$ is no coincidence but reflects the deep geometric structure of the T0 theory:
	
	\textbf{Structural duality:}
	\begin{itemize}
		\item \textbf{Microscopic:} The geometric mean represents the local structure between adjacent lepton generations
		\item \textbf{Macroscopic:} The gravitational-geometric formula represents the global structure across all scales
	\end{itemize}
	
	\textbf{Scale relations:}
	
	The two approaches are connected by the fundamental relation:
	\begin{equation}
		\frac{E_0^{\text{grav}}}{E_0^{\text{geom}}} = \sqrt{\frac{4\sqrt{2} m_\mu}{\xi^4 m_e m_\mu}} = \sqrt{\frac{4\sqrt{2}}{\xi^4 m_e}}
	\end{equation}
	
	This relation shows that both paths are linked through the geometric parameter $\xi$ and the mass hierarchy.
	
	\subsection{Physical Significance of the Duality}
	...
	The agreement within 0.65\% confirms the geometric consistency of the T0 theory.
	
	\section{The T0 Coupling Parameter $\varepsilon$}
	
	The T0 coupling parameter arises as:
	...
	As an independent confirmation, $\alpha$ can also be derived via fractal renormalization:
	...
	In the T0 theory literature, two physically different parameters are denoted by the symbol $\kappa$, which can lead to confusion. These must be clearly distinguished:
	...
	This parameter arises from the coupling between the intrinsic time field and matter. The T0 Lagrangian density is:
	...
	\begin{itemize}
		\item Explains the observed flat rotation curves of galaxies
		\item Eliminates the need for dark matter
		\item Arises naturally from the time field-matter coupling
	\end{itemize}
	
	\subsection{Summary of the $\kappa$ Parameters}
	...
	\begin{center}
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{|l|c|c|l|}
				...
				Gravitational field & $\kappa_{\text{grav}}$ & $4.8 \times 10^{-11}$ m/s$^2$ & Modification of the potential \\
				\hline
		\end{tabular}}
	\end{center}
	
	The clear distinction between these two parameters is essential for understanding the T0 theory.
	\section{Complete Mapping: Standard Model Parameters to T0 Equivalents}
	...
	The table is organized such that each parameter is defined before it is used in subsequent formulas.
	
	% Table 1: Levels 0-3 (Core parameters)
	\begin{table}[htbp]
		\centering
		\resizebox{\textwidth}{!}{%
\begin{adjustbox}{max width=\textwidth}
			\begin{tabular}{@{}p{0.30\textwidth}@{}}p{3cm}p{2.7cm}p{2.7cm}}
				\toprule
				...
				\midrule
				\multicolumn{4}{l}{\textbf{LEVEL 2: ENERGY SCALES (from $\xi$ and Planck scale)}} \\
				\midrule
...
\end{adjustbox}
		...
		\caption{Standard Model parameters in hierarchical order of their T0 derivation (Part 1: Levels 0--3)}
		\label{tab:sm-params-1}
	\end{table}
	
	% Table 2a: Levels 4-5 (Fermion and Neutrino masses)
	\begin{table}[htbp]
		\centering
		\resizebox{\textwidth}{!}{%
\begin{adjustbox}{max width=\textwidth}
...
\end{adjustbox}
		...
		\caption{Standard Model parameters in hierarchical order of their T0 derivation (Part 2a: Levels 4--5)}
		\label{tab:sm-params-2a}
	\end{table}
	
	% Table 2b: Levels 6-7 (Mixing matrices and derived parameters)
	\begin{table}[htbp]
		\centering
		\resizebox{\textwidth}{!}{%
\begin{adjustbox}{max width=\textwidth}
			\begin{tabular}{@{}p{0.30\textwidth}@{}}p{3cm}p{2.7cm}p{2.7cm}}
				...
				$\theta_{23}$ (Atmospheric) & $49.2^\circ$ & $\theta_{23} = \arcsin\sqrt{m_{\nu_2}/m_{\nu_3}}$ & $49^\circ$ \\
				$\theta_{13}$ (Reactor) & $8.57^\circ$ & $\theta_{13} = \arcsin(\xi^{1/3})$ & $8.6^\circ$ \\
...
\end{adjustbox}
		...
		\caption{Standard Model parameters in hierarchical order of their T0 derivation (Part 2b: Levels 6--7)}
...
\end{adjustbox}
	...
	\begin{enumerate}
		\item \textbf{Level 0}: Only $\xi$ as the fundamental constant
	...
	Each level uses only parameters defined in previous levels.
	...
	\textbf{Unit system:}
	All T0 values apply in natural units with $\hbar = c = 1$. For experimental comparisons, transformation to SI units is required.
	...
	\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=Warning: Fundamental Differences]
		The T0 system postulates a \textbf{static, eternal universe} without Big Bang, while standard cosmology is based on an \textbf{expanding universe} with Big Bang. The parameters are therefore often not directly comparable but represent different physical concepts.
	\end{tcolorbox}
	...
	\begin{longtable}{p{4cm}p{3cm}p{4cm}p{3cm}}
		\caption{Hierarchically ordered cosmological parameters} \\
		\toprule
		\textbf{Parameter} & \textbf{$\Lambda$CDM Value} & \textbf{T0 Formula} & \textbf{T0 Interpretation} \\
		\midrule
		\endfirsthead
		
		\multicolumn{4}{c}{{\bfseries Continuation of Table}} \\
		\toprule
		\textbf{Parameter} & \textbf{ΛCDM Value} & \textbf{T0 Formula} & \textbf{T0 Interpretation} \\
		\midrule
		\endhead
		...
		Entropy & Grows monotonically & $S_{total} = const$ & Cyclic \\
		& (heat death) & Regeneration & conserved \\[0.3em]
		
	\end{longtable}
	
	\subsection{Critical Differences and Test Opportunities}
...
\end{adjustbox}
	...
	\subsection{Summary: From 6+ to 0 Parameters}
...
\end{adjustbox}
	...
	\subsection{Critical Notes on Testability}
	\label{subsec:testability_notes}
	
	\textbf{(*) On wavelength-dependent redshift:}
	
	The detection of wavelength-dependent redshift is currently \textbf{at the absolute limit} of what is technically feasible:
	
	\begin{itemize}
		\item \textbf{Required precision}: $\Delta z/z \sim 10^{-6}$ for radio vs. optical
		\item \textbf{Current best spectroscopy}: $\Delta z/z \sim 10^{-5}$ to $10^{-6}$
		\item \textbf{Systematic errors}: Often larger than the sought signal
		\item \textbf{Atmospheric effects}: Additional complications
	\end{itemize}
	
	\textbf{Future possibilities}:
	\begin{itemize}
		\item \textbf{ELT (Extremely Large Telescope)}: Could achieve required precision
		\item \textbf{SKA (Square Kilometre Array)}: Precise radio measurements
		\item \textbf{Space telescopes}: Eliminate atmospheric disturbances
		\item \textbf{Combined observations}: Statistics over many objects
	\end{itemize}
	
	The test is thus in principle possible but requires the next generation of instruments or very refined statistical methods with current technology.
	
	\textbf{(**) On mass-energy equivalence:}
	
	The formula $E = mc^2$ holds identically in both systems. The difference lies in the \textbf{interpretation}:
	
	\begin{itemize}
		\item \textbf{$\Lambda$CDM}: Mass is a fundamental property of particles
		\item \textbf{T0 system}: Mass arises from resonances in the $\xi$-field (see Yukawa parameter derivation)
	\end{itemize}
	
	The formula itself remains unchanged, but in the T0 system, $m$ is not a constant but $m = m(\xi, E_{field})$ - a function of field geometry. Practically, this makes no measurable difference for $E = mc^2$.
	\appendix
	
	\section{Appendix: Purely Theoretical Derivation of the Higgs VEV from Quantum Numbers}
	
	\subsection{Summary}
	
	This appendix shows a completely theoretical derivation of the Higgs vacuum expectation value $v \approx 246$ GeV from the fundamental geometric properties of the T0 theory. The method uses exclusively theoretical quantum numbers and geometric factors, without using empirical data as input. Experimental values serve only for verification of predictions.
	
	\subsection{Fundamental Theoretical Foundations}
	
	\subsubsection{Quantum Numbers of Leptons in the T0 Theory}
	
	The T0 theory assigns quantum numbers $(n, l, j)$ to each particle, arising from the solution of the three-dimensional wave equation in the energy field:
	...
	The T0 theory provides two equivalent formulations for particle masses:
	...
	The geometric factors arise from the analytical solution of the three-dimensional wave equation. For the fundamental leptons:
	...
	\subsubsection{Verification of the Factors}
	...
	With the geometric factors, the direct method follows:
	
	\begin{align}
		\frac{m_\mu}{m_e} &= \frac{\xi_e}{\xi_\mu} = \frac{f_e}{f_\mu} = \frac{1}{\frac{16}{5}} = \frac{5}{16}
	\end{align}
	
	\textbf{Attention:} This is the inverse ratio! Since $\xi \propto 1/m$, we obtain:
	
	\begin{align}
		\frac{m_\mu}{m_e} &= \frac{f_\mu}{f_e} = \frac{\frac{16}{5}}{1} = \frac{16}{5} = 3.2
	\end{align}
	
	\subsubsection{Correction via Yukawa Couplings}
	...
	\subsubsection{Calculation of the Corrected Ratio}
	...
	\subsubsection{Connection of the Two Methods}
	
	Since both methods describe the same masses:
	...
	\subsubsection{Elimination of the Masses}
	...
	\subsubsection{Solution for the Characteristic Mass Scale}
	...
	In natural units, the conversion factor to Planck energy corresponds:
	...
	The Higgs VEV is typically at a fraction of this characteristic scale:
	...
	\subsubsection{Determination of the Geometric Factor}
	
	From consistency with the electron mass follows:
	
	\begin{align}
		\alpha_{\text{geo}} &= \frac{v}{E_\xi} = \frac{245.1}{7500} = 0.0327
	\end{align}
	
	This factor can be expressed as a geometric relation:
	...
	The purely theoretical derivation of the Higgs VEV is:
	...
	\subsection{Improvement via Quantum Corrections}
	
	\subsubsection{Accounting for Loop Corrections}
	...
	\subsubsection{Determination of the Quantum Correction Factor}
	...
	The derivation shows that the Higgs VEV is a direct geometric consequence of the three-dimensional space structure:
	
	\begin{equation}
		v \propto \xi_0^{-1/2} \propto \left(\frac{\text{Characteristic length}}{\text{Planck length}}\right)^{1/2}
	\end{equation}
	
	\subsubsection{Quantum Field Theoretical Significance}
	...
	The T0 theory enables:
	
	\begin{enumerate}
		\item Predicting the Higgs VEV from pure geometry
		\item Calculating all lepton masses from quantum numbers
		\item Theoretically understanding mass ratios
		\item Geometrically interpreting the role of the Higgs mechanism
	\end{enumerate}
	
	\subsection{Validation of the T0 Methodology}
	
	\subsubsection{Response to Methodological Criticism}
	
	The T0 derivation might superficially appear circular or inconsistent, as it combines different mathematical approaches. A careful analysis, however, shows the robustness of the method:
	...
	\begin{itemize}
		\item Higgs VEV determined experimentally
		\item Yukawa couplings adjusted to masses
	...
	This derivation supports the central thesis of the T0 theory that all fundamental parameters are derivable from the geometry of three-dimensional space. The Higgs mechanism thus becomes a necessary consequence of space geometry rather than an ad-hoc introduced concept.
	
	\subsubsection{Experimental Tests}
	
	The predictions can be tested by more precise measurements:
	
	\begin{itemize}
		\item Improved determination of the Higgs VEV
		\item Precision lepton mass measurements
		\item Tests of predicted mass ratios
		\item Search for deviations at higher energies
	\end{itemize}
	
	The T0 theory shows the potential to provide a truly fundamental and unified description of all known phenomena in particle physics, based exclusively on geometric principles.
	...
	\begin{enumerate}
		...
		\item There exist multiple independent paths to the same result
		\item Specifically for $E_0$, there are two geometric derivations that are consistent
		\item The theory is free of circularity
		\item The distinction between $\kappa_{\text{mass}}$ and $\kappa_{\text{grav}}$
	\end{enumerate}
	
	The T0 theory thus demonstrates that the fundamental constants of nature are not arbitrary numbers but compelling consequences of the geometric structure of the universe.
	% ========================================
	% ENGLISH VERSION
	% ========================================
	
	\appendix
	\section{List of Used Formula Symbols}
	...
	\begin{longtable}{lll}
		...
		\textbf{Symbol} & \textbf{Meaning} & \textbf{Value/Formula} \\
		\midrule
		$H_0$ & Hubble constant & $67.4$ km/s/Mpc (ΛCDM) \\
		$T_{CMB}$ & CMB temperature & $2.725$ K \\
		$z$ & Redshift & dimensionless \\
		$\Omega_\Lambda$ & Dark energy density & $0.6847$ (ΛCDM), $0$ (T0) \\
		$\Omega_{DM}$ & Dark matter density & $0.2607$ (ΛCDM), $0$ (T0) \\
		$\Omega_b$ & Baryon density & $0.0492$ (ΛCDM), $1$ (T0) \\
	...
	\begin{longtable}{lll}
		...
		$\rho_{tet}$ & Tetrahedron packing density & $0.68$ \\
