...
\input{T0_preamble_standalone_En} % Korrigierte englische Preamble

\title{\textbf{Ontological Hierarchy of Energy Reduction}\\[0.5cm]
	 The Levels of Fundamental Reality in Natural Units\\[0.3cm]
	\normalsize From Time-Mass Duality to Universal Energy Field}
\author{Ontological Systematics}
\date{January 2025}
...
\begin{document}
	...
	\begin{keyresult}[The Most Fundamental Reality]
		...
		\begin{center}
			 A Universal Energy Field $E_{\text{Field}}(x,t)$
		\end{center}
	...
	\begin{table}[H]
		...
		\begin{tabular}{clp{5cm}l}
			\toprule
			\textbf{Level} & \textbf{Description} & \textbf{What exists} & \textbf{Status} \\
			\midrule
			0 & Energy Field & $E_{\text{Field}}(x,t)$ & Absolutely fundamental \\
			1 & Time-Mass Duality & $T \cdot m = 1$ & First emergence \\
			2 & Geometry & $\xi$, 4D Torsion & Structural reality \\
			3 & Effective Theory & Modified Laws & Phenomenological \\
			4 & SI Physics & $c, \hbar, G$ separate & Conventional \\
			\bottomrule
		\end{tabular}
		\caption{The five ontological levels}
	\end{table}
	
	\subsection{Causal Relationships}
	
	\begin{equation}
		\text{Level 0} \Rightarrow \text{Level 1} \Rightarrow \text{Level 2} \Rightarrow \text{Level 3} \Rightarrow \text{Level 4}
	\end{equation}
	
	where $\Rightarrow$ means: "determines" or "allows to emerge"
	
	\subsection{Reduction to Energy}
	
	\begin{center}
		\textbf{At all levels holds in natural units:}
		...
		\normalsize for some $n \in \mathbb{Z}$
	\end{center}
	
	\textbf{Everything is energy!}
	...
	\begin{keyresult}[Main Result]
		T0 theory possesses a clear **five-level ontological hierarchy**:
		
		\vspace{0.5cm}
		
		\begin{center}
			\begin{tikzpicture}[scale=0.8]
				\draw[ultra thick, gold, ->] (0,5) -- (0,0);
				
				\node[left, font=\footnotesize] at (0,5) {\textbf{Level 0:} Energy Field};
				\node[right, font=\footnotesize] at (0,5) {\textcolor{gold}{ABSOLUTELY FUNDAMENTAL}};
				
				\node[left, font=\footnotesize] at (0,4) {\textbf{Level 1:} Time-Mass Duality};
				\node[right, font=\footnotesize] at (0,4) {\textcolor{red!80!black}{First Emergence}};
				
				\node[left, font=\footnotesize] at (0,3) {\textbf{Level 2:} Geometry};
				\node[right, font=\footnotesize] at (0,3) {\textcolor{orange!80!black}{Structural}};
				
				\node[left, font=\footnotesize] at (0,2) {\textbf{Level 3:} Effective};
				\node[right, font=\footnotesize] at (0,2) {\textcolor{yellow!80!black}{Phenomenological}};
				
				\node[left, font=\footnotesize] at (0,1) {\textbf{Level 4:} SI Physics};
				\node[right, font=\footnotesize] at (0,1) {\textcolor{green!50!black}{Conventional}};
				
				\node[below, font=\small] at (0,0) {\textbf{Observable}};
			\end{tikzpicture}
		\end{center}
		
		\vspace{0.5cm}
		
		\textbf{Through natural units, everything is reduced to energy.}
		
		\textbf{The 4D geometry is Level 2 -- emergent from duality (Level 1).}
		
		\textbf{The universal energy field (Level 0) is the absolute foundation.}
	\end{keyresult}
	
	\subsection{The Ultimate Reduction}
	
	\begin{revolutionary}[The Truth of Physics]
		...
		Space, time, mass, charge, forces, particles -- all these are only different **manifestations of a single universal energy field**.
		
		\vspace{0.3cm}
		
		In natural units this becomes mathematically explicit:
		
		\begin{equation}
			[X] = [E]^n \quad \text{for every physical quantity } X
		\end{equation}
		
		\vspace{0.3cm}
		
		The time-mass duality $T \cdot m = 1$ is the key to this insight.
		
		\vspace{0.3cm}
		
		The 4D torsion crystal is the geometric manifestation of this fundamental truth.
	\end{revolutionary}
