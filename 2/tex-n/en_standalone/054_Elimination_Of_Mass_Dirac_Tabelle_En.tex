...
% Define dummy chapter counter for chngcntr in shared ebook preamble
\newcounter{chapter}

% Standardized preamble - 054_Elimination_Of_Mass_Dirac_Table_En.pdf
...
\begin{document}
	...
	\begin{landscape}
		\footnotesize
		\begin{longtable}{p{5.5cm}p{1.8cm}p{3.5cm}p{2.5cm}p{3.5cm}p{1.8cm}p{1cm}}
			...
			\midrule
			Lepton g-2 (T0, qualitative) & -- & conceptual contribution pattern driven by $\xi$ & -- & see \texttt{018\_T0\_Anomalous-g2-10\_En.pdf} & -- & -- \\
			
			Muon g-2 anomaly (Ref.) & 1 & $\Delta a_{\mu}$ (experimental) & $\mathbf{2.51 \times 10^{-9}}$ & $2.51 \times 10^{-9}$ (Fermilab) & $\mathbf{100.0\%}$ & $\checkmark$ \\
			
			T0 share of muon anomaly & \% & $\frac{a_{\mu}^{(T0)}}{\Delta a_{\mu}} \times 100\%$ & $\mathbf{9.2\%}$ & Calculated (2.31/25.1) & $\mathbf{100.0\%}$ & $\checkmark$ \\
		...
		\begin{longtable}{p{4cm}p{1.8cm}p{3.8cm}p{3.2cm}p{3.2cm}p{1.8cm}p{1cm}}
			...
			\multicolumn{7}{l}{\textbf{NOTE: Complex formulas show 99.98-100.04\% agreement (rounding errors)}} \\
		...
		\begin{longtable}{p{3.5cm}p{2cm}p{2.5cm}p{4cm}p{3cm}p{1.8cm}p{1cm}}
			...
			Length & $E = 1/L$ & Energy = 1/Length & $3.862 \times 10^{-13}$ m & Inverse relationship & $\mathbf{100\%}$ & $\checkmark$ \\
			
			Time & $E = 1/T$ & Energy = 1/Time & $1.288 \times 10^{-21}$ s & Inverse relationship & $\mathbf{100\%}$ & $\checkmark$ \\
			...
			Electromagnetic geometry factor & Ratio & $\sqrt{4\pi/9}$ & $1.18270$ & Mathematically exact & $\mathbf{100\%}$ & $\star$ \\
			
			% COMPLETE SI UNITS ENERGY COVERAGE
			\multicolumn{7}{l}{\textbf{COMPLETE SI UNITS ENERGY COVERAGE - ALL 7/7 UNITS}} \\
			\midrule
			
			Electric current & $I = E/T$ & Energy flow rate & $[E]$ dimension & Direct energy relationship & $\mathbf{100\%}$ & $\checkmark$ \\
			
			Amount of substance (Mole) & $[E^2]$ dimension & Energy density ratio & Dimensional structure & SI-defined $N_A$ & $\mathbf{Def.}$ & $\star$ \\
			
			Luminous intensity (Candela) & $[E^3]$ dimension & Energy flow perception & Dimensional structure & SI-defined 683 lm/W & $\mathbf{Def.}$ & $\star$ \\
			
			% NOTE ON PERFECT AGREEMENT
			\multicolumn{7}{l}{\textbf{NOTE: Simple energy relationships show 100\% agreement (no errors)}} \\
