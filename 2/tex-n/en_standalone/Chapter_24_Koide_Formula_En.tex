...
This phenomenological approach (phase eigenmodes) complements the fundamental T0 approach:
\begin{itemize}
\item \textbf{116\_T0\_koide-formel-3\_En.tex}: Fundamental T0-Yukawa derivation with $m = r \cdot \xi^p \cdot v$, exact mass ratios from $\xi$ exponents $(r,p)$ for each lepton
\item \textbf{046\_Teilchenmassen\_En.tex}: Complete T0 particle mass systematics
\item \textbf{006\_T0\_Teilchenmassen\_En.tex}: T0 mass derivation from time-mass duality
\end{itemize}
Both approaches show $Q = 2/3$ with no free parameters from $\xi = 4/3 \times 10^{-4}$ and are fully consistent.
\end{tcolorbox}
