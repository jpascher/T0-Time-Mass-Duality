...
\title{The T0 Energy Field Model:\\[0.3cm]
...
\begin{document}
	...
	This document uses the standard convention of particle physics:
	...
	In this system:
	\begin{itemize}
		\item Fine structure constant: $\alpha \approx \frac{1}{137.036} = 7.297 \times 10^{-3}$
		\item Energy = Mass: $E = m$
		\item Length = Time = Energy$^{-1}$: $[L] = [T] = [E^{-1}]$
	\end{itemize}
	
	\textbf{Note:} Alternative Heaviside-Lorentz units ($4\pi\epsilon_0 = 1$, then $\alpha = e^2 = 1$) lead to the same physical results, only with different mathematical form.
	...
	\subsection{T0 derivation}
	
	The fine structure constant follows from $\xi$ and $E_0$ (derivation in Document 011):
