...
\newpage	
	...
	The T0 theory establishes an unambiguous derivation chain from the geometric parameter $\xi$:
	
	\begin{enumerate}
		\item \textbf{Starting Point -- Geometry}: The tetrahedral packing deficit
		\[
		\xi = 1 - \frac{V_{\text{Tetrahedron}}}{V_{\text{Sphere}}} = \frac{4}{3} \times 10^{-4}
		\]
		on the Planck scale is the only fundamental parameter.
		
		\item \textbf{Harmonic Hierarchy}: From $\xi$ follows the characteristic energy scale
		\[
		E_0 = e^{\kappa/2} \approx e^{7/2} \approx 33.12
		\]
		with the harmonic number $\kappa = 7$.
		
		\item \textbf{Fine-Structure Constant}: Directly derivable as
		\[
		...
		\item \textbf{Mass Ratios}: Derivable from fractal hierarchy and $\xi$-couplings (see separate T0 documents).
	\end{enumerate}
	
	This hierarchy shows: Only $\xi$ is fundamental, everything else follows mathematically or is convention in unit choice.
