...
\title{\textbf{T0-Theory: The Fractal Correction $K_{\text{frak}}$}\\[0.5cm]
...
\begin{document}
	...
	\subsection{The Proof via Two Equivalent α Definitions}
	
	\textbf{The decisive proof:} The fractal correction $K_{\text{frak}}$ is not arbitrarily chosen but follows necessarily from the requirement that two different derivations of $\alpha$ must yield the same value!
	
	\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title={Unique Determination of $K_{\text{frak}}$ and $D_f$}]
		\textbf{Two equivalent formulas for $\alpha$:}
		
		\textbf{Formula 1 (from $\xi$ and $E_0$):}
		\begin{equation}
			\alpha = \xi \cdot \left(\frac{E_0}{1 \text{ MeV}}\right)^2
		\end{equation}
		
		\textbf{Formula 2 (directly from masses):}
		\begin{equation}
		...
		\textbf{But:} Experimentally, $\alpha = m_e \cdot m_\mu / 7380$, not 7500!
		
		The difference $7500/7380 = 1.0163 \approx 1/K_{\text{frak}}$ uniquely determines:
		\begin{equation}
			K_{\text{frak}} = \frac{7380}{7500} = 0.984
		\end{equation}
		
		With $K_{\text{frak}} = 1 - 100\xi$ it follows:
		\begin{equation}
			D_f = 3 - \xi = 2.9998667
		\end{equation}
	...
	This derivation shows: $K_{\text{frak}}$ is not an adjusted correction but a necessary consequence of consistency between two independent α formulas. The fractal dimension $D_f = 2.94$ follows directly from this.
