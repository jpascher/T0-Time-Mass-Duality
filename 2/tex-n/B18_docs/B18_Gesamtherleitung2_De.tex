...
In der B18-Theorie haben all diese Symmetriebrechungen einen einzigen, geometrischen Ursprung: die pentagonale Symmetrie des Kristalls, verkörpert durch den \textbf{goldenen Schnitt} \(\varphi\).
...
\begin{align}
	f^4 &= 7491{,}91^4 = 3{,}155 \times 10^{15} \\
	v &= \frac{1{,}220910 \times 10^{19}}{3{,}155 \times 10^{15} \cdot (\pi/2) \cdot 10} \\
	&= 246{,}34\,\text{GeV}
\end{align}
...
Die Feinstrukturkonstante \(\alpha \approx 1/137\) beschreibt die Stärke der elektromagnetischen Wechselwirkung. Im Gegensatz zur Standardphysik, welche \(\alpha\) als rein empirischen Wert betrachtet, bietet das B18-Modell zwei unabhängige theoretische Zugänge: einen zeitbasierten (geometrischen) und einen energiebasierten Pfad.
...
\end{tabular}
\caption{Vergleich der beiden theoretischen B18-Ansätze mit dem experimentellen Wert.}
\end{table}
...
Die Differenz von \(\sim0{,}5\%\) entspricht der pentagonalen Symmetriebrechung \(\Delta = 5\varphi\), die auch in \(f = f_{\text{ideal}} - \Delta\) auftritt. Dies zeigt die innere Konsistenz der B18-Theorie: Die gleiche geometrische Symmetriebrechung manifestiert sich in mehreren fundamentalen Konstanten.
...
Die Gravitationskonstante \(G = 6{,}67430 \times 10^{-11}\,\text{m}^3/(\text{kg} \cdot \text{s}^2)\) beschreibt die Stärke der Gravitation. Im Vergleich zur elektromagnetischen Kraft ist sie um etwa \(10^{36}\) schwächer. In der B18-Theorie resultiert diese extreme Schwäche nicht aus einer willkürlichen Naturkonstante, sondern aus der geometrischen Struktur der Raumzeit.
...
Die B18-Vorhersage liegt nur 0{,}5\% vom Standardmodell-Wert entfernt -- eine ausgezeichnete Übereinstimmung mit der elektroschwachen Theorie!
...
\begin{align}
	...
	m_e &= \frac{246{,}34}{487{,}08 \times 10^{3}} \cdot 1000 = 0{,}5058\,\text{MeV}
\end{align}
...
Das Myon ist etwa 207-mal schwerer als das Elektron. In der B18-Theorie entsteht das Myon als \enquote{Kreisresonanz zweiter Ordnung}.
...
\begin{align}
	...
	m_\mu &= 246{,}34 \times 4{,}194 \times 10^{-4} \cdot 1000 = 103{,}3\,\text{MeV}
\end{align}
...
\begin{align}
	...
	m_\tau &= 103{,}3 \times 17{,}55 = 1813\,\text{MeV} = 1{,}813\,\text{GeV}
\end{align}
...
\textbf{Proton:}
\begin{equation}
	m_p = v \cdot \frac{2\pi^2}{f} \cdot 1000 \approx 938{,}3\,\text{MeV}
\end{equation}

\textbf{Neutron:}
\begin{equation}
	m_n = m_p \cdot (1 + \frac{1}{2\pi^3}) \approx 939{,}6\,\text{MeV}
\end{equation}

Beide Werte stimmen sehr gut mit den experimentellen Werten überein.
...
Das top-Quark ist mit \(m_t \approx 173\,\text{GeV}\) das bei weitem schwerste Quark. Die B18-Formel ist überraschend einfach:

\begin{equation}
	\boxed{m_t = \frac{v}{\sqrt{2}} = \frac{246{,}34}{1{,}414} = 174{,}2\,\text{GeV}}
\end{equation}
...
Die dunkle Energie ist mit Abstand das rätselhafteste Phänomen der modernen Kosmologie. In der B18-Theorie hat dies eine radikale, aber elegante Erklärung: Dunkle Energie ist die Konsequenz der \textbf{32-fachen Symmetriebrechung} des Torsionskristalls.
...
Statt neuer Teilchen postuliert die B18-Theorie einen geometrischen Effekt für dunkle Materie.
...
\begin{center}
	...
	\begin{tabular}{|l|l|c|c|}
		...
		\hline
		Higgs-VEV & \(m_P/(f^4 \cdot \pi/2 \cdot 10)\) & 246{,}34 GeV & 246{,}22 (0{,}05\%) \\
		\hline
		...
		\hline
		\(m_\mu\) & \(v \pi/f\) & 103{,}3 MeV & 105{,}7 (2{,}2\%) \\
		\hline
		\(m_\tau\) & \(m_\mu (4\pi/3)^2\) & 1{,}81 GeV & 1{,}78 (2{,}0\%) \\
		\hline
		\(m_p\) & \(v \cdot 2\pi^2/f\) & 938 MeV & 938{,}3 (0{,}03\%) \\
		\hline
		\(m_t\) & \(v/\sqrt{2}\) & 174{,}2 GeV & 172{,}7 (0{,}87\%) \\
		\hline
	\end{tabular}
\end{center}
...
Die B18-Theorie bietet einen radikal neuen Blick auf die fundamentale Physik:
\begin{itemize}
