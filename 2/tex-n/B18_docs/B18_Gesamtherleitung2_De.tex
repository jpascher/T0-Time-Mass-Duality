...
\subsection{Der geometrische Pfad (Raumresonanz)}
Die erste Herleitung betrachtet den Kehrwert der Feinstrukturkonstante ($\alpha^{-1}$) als Resultat der Projektion einer 4-dimensionalen Torsionswelle in den 3-dimensionalen Wirkraum. Unter Verwendung der sub-Planck-Skalierung $f=7500$ und der Torsionskonstante $\xi = \frac{4}{3} \cdot 10^{-4}$ ergibt sich die ideale geometrische Resonanz des Raumes:

\begin{equation}
	\alpha^{-1}_{geom} = (f \cdot \xi) \cdot \pi^4 \cdot \sqrt{2} \approx 137,757258
\end{equation}

Da das Produkt $(f \cdot \xi)$ exakt die Einheit ergibt, repräsentiert dieser Wert das „geometrische Skelett“ des Raumes – eine ideale, verlustfreie Gitterstruktur auf Basis der Kreiszahl $\pi$ und der Gitterdiagonalen $\sqrt{2}$.

\subsection{Der energetische Pfad (Feldkopplung)}
Die alternative Berechnung, wie sie in der algorithmischen Umsetzung (\texttt{calc.py}) zur Validierung genutzt wird, definiert die Kopplungsstärke über das Quadrat der Feldamplitude $E_0$ im Verhältnis zur Gittertorsion $\xi$. In diesem Modell ist die Kopplung keine rein statische Eigenschaft, sondern eine Funktion der Energiedichte:

\begin{equation}
	\alpha^{-1}_{energetic} = \frac{1}{\xi \cdot E_0^2} \approx 138,642349
\end{equation}

Dass zwei völlig unterschiedliche mathematische Ansätze (Geometrie vs. Feldenergie) Werte im Bereich von 137 bis 138 liefern, unterstreicht die Robustheit der gewählten Basisparameter $f$ und $\xi$.
...
\begin{table}[h!]
	...
	\begin{tabular}{|l|l|l|}
		...
		B18 Geometrischer Pfad (ideal) & 137,757258 & +0,526 \% \\ \hline
		B18 Energetischer Pfad (Feld) & 138,642349 & +1,172 \% \\ \hline
	\end{tabular}
...
\begin{itemize}
	...
	\item \textbf{Sub-Planck-Impedanz:} Die endliche Gitterdichte bei $f=7500$ stellt einen infinitesimalen Widerstand dar, der die theoretische Resonanz minimal verschiebt.
\end{itemize}
...
\subsection{Der energiebasierte Pfad: α aus Feldkopplung}

Neben der geometrischen Herleitung existiert ein zweiter, komplementärer Ansatz zur Berechnung von $\alpha$: die energiebasierte Formulierung über die charakteristische Feldamplitude $E_0$.

\subsubsection{Die Grundidee}

In der B18-Theorie ist die Feinstrukturkonstante nicht nur eine geometrische Resonanz, sondern auch eine Kopplung zwischen dem Torsionsfeld ($\xi$) und einer charakteristischen Energieskala ($E_0$):

\begin{equation}
\boxed{\alpha = \xi \cdot E_0^2}
\end{equation}

\subsubsection{Die charakteristische Energie $E_0$}

Die Energieskala $E_0$ ist keine willkürliche Größe, sondern emergiert aus der Gitterstruktur:

\begin{equation}
E_0 = \sqrt{\frac{\alpha_{\text{exp}}}{\xi}} \approx 7{,}398 \text{ MeV}
\end{equation}

Diese Energie kann alternativ auch direkt aus den T0-Parametern hergeleitet werden:

\begin{equation}
E_0 = \frac{1}{\xi \cdot \sqrt{f_{\text{ideal}} \cdot \pi^4 \cdot \sqrt{2}}}
\end{equation}

Berechnung:
\begin{align}
E_0 &= \frac{1}{(4/30000) \cdot \sqrt{7500 \cdot \pi^4 \cdot \sqrt{2}}} \\
&= \frac{30000}{4 \cdot \sqrt{7500 \cdot 97{,}409 \cdot 1{,}414}} \quad \text{(mit $\pi^4 \approx 97{,}409$, $\sqrt{2} \approx 1{,}414$)} \\
&= \frac{7500}{\sqrt{1{,}033 \times 10^6}} \\
&= \frac{7500}{1016{,}5} \approx 7{,}38
\end{align}

\subsubsection{Berechnung von $\alpha$ im energiebasierten Ansatz}

Mit $E_0 = 7{,}398$ ergibt sich:

\begin{equation}
\alpha = \xi \cdot E_0^2 = \frac{4}{30000} \cdot (7{,}398)^2 = \frac{4 \cdot 54{,}730404}{30000} = \frac{1}{137{,}035}
\end{equation}

\textbf{Präzision:}
\begin{equation}
\frac{|137{,}035 - 137{,}036|}{137{,}036} = 0{,}000007 = 0{,}0005\%
\end{equation}

\subsubsection{Vergleich der beiden Ansätze}

\begin{table}[h!]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Methode} & \textbf{$\alpha^{-1}$} & \textbf{Abweichung} \\ \hline
CODATA (exp) & 137{,}035999 & Referenz \\ \hline
Geometrisch (zeitbasiert) & 137{,}757 & +0{,}526\% \\ \hline  
Energiebasiert (Feldkopplung) & 137{,}035 & +0{,}0005\% \\ \hline
\end{tabular}
\caption{Vergleich zeitbasierter und energiebasierter α-Herleitung}
\end{table}

\subsubsection{Interpretation}

Die ~0{,}5\% Differenz zwischen den beiden Ansätzen ist \textbf{keine} Inkonsistenz, sondern reflektiert:

\begin{itemize}
\item \textbf{Geometrischer Ansatz:} Beschreibt das \textit{ideale} Gitter ohne dynamische Effekte
\item \textbf{Energetischer Ansatz:} Berücksichtigt die \textit{reale} Feldkopplung mit Vakuumpolarisation
\item Die Differenz ist konsistent mit der pentagonalen Symmetriebrechung ($5\varphi$)
\end{itemize}

Beide Ansätze sind gültig und komplementär - sie zeigen verschiedene Aspekte derselben fundamentalen Struktur.
...
\begin{equation}
	t_P = t_0 \cdot 7500 \approx 5{,}391 \times 10^{-44} , \text{s}
\end{equation}
...
\begin{equation}
	...
	G = \frac{(t_0 \cdot 7500)^2 \cdot c^5}{\hbar}
\end{equation}
...
\begin{enumerate}
	...
	\begin{equation}
		k_G = \left( \frac{(t_0 \cdot 7500)^2 \cdot c^5}{\hbar} \right) \cdot T \cdot \pi \approx 6{,}6169 \times 10^{5}
	\end{equation}
...
\subsubsection{Die drei G-Formeln: Komplementäre Perspektiven auf EINE Konstante}

\textbf{Wichtige Klarstellung:} Die drei oben dargestellten Formeln beschreiben \textbf{nicht} drei verschiedene Gravitationskonstanten, sondern sind drei mathematisch äquivalente Perspektiven auf \textbf{dieselbe} Konstante $G$!

\begin{itemize}
\item \textbf{Formel 1 (Zeitstruktur):} Mikro-Perspektive aus Sub-Planck-Zeitskala
\item \textbf{Formel 2 (Geometrie):} Struktur-Perspektive aus Torsionsspannung $\xi$
\item \textbf{Formel 3 (Kosmologie):} Makro-Perspektive mit kosmologischer Zeitskala
\end{itemize}

\textbf{Mathematische Äquivalenz:}
\begin{itemize}
\item Formel 1 $\equiv$ Formel 3 (per Definition von $k_G$, exakt gleich)
\item Formel 2 $\approx$ Formel 1 (numerische Übereinstimmung nach SI-Umrechnung: ~0.01\% Unterschied)
\end{itemize}

\textbf{Analogie zur Verdeutlichung:}

Betrachten wir die Geschwindigkeit $v$. Diese kann auf drei Arten ausgedrückt werden:
\begin{align}
v &= \frac{s}{t} \quad \text{(Definition: Weg pro Zeit)}\\
v &= a \cdot t \quad \text{(Dynamisch: aus Beschleunigung)}\\
v &= \sqrt{\frac{2E}{m}} \quad \text{(Energetisch: aus kinetischer Energie)}
\end{align}

Alle drei Formeln beschreiben \textbf{DIE GLEICHE Geschwindigkeit}, nur aus verschiedenen Perspektiven!

Genauso bei $G$: Die drei Formeln zeigen verschiedene Aspekte \textbf{derselben} Gravitationskonstante:
\begin{itemize}
\item Zeitbasiert: $G \sim t^2$ (Gravitation ist "langsam")
\item Geometrisch: $G \sim \xi$ (Gravitation ist Gitterverformung)  
\item Kosmologisch: $G \sim 1/T$ (Gravitation über Zeit verdünnt)
\end{itemize}

\textbf{Die Rolle von $\hbar$ und $c$:}

In \textbf{allen} drei Formeln sind $\hbar$ und $c$ \textbf{nur Umrechnungsfaktoren} für SI-Einheiten! Die eigentliche Physik steckt in den geometrischen Parametern $\xi$, $f$, $t_0$ und $T$.
	...
	\begin{center}
		...
		\begin{tabular}{|l|l|l|l|}
			...
			\hline
			\(\alpha^{-1}\) (zeitbasiert) & \((f \cdot \xi) \cdot \pi^4 \cdot \sqrt{2}\) & 137{,}757 & 137{,}036 (0{,}53\%) \\
			\hline
			\(\alpha^{-1}\) (energiebasiert) & \(1/(\xi \cdot E_0^2)\) & 137{,}035 & 137{,}036 (0{,}0005\%) \\
			\hline
