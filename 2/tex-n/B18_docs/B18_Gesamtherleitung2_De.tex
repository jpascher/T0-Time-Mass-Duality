...
Die T0-Formeln enthalten oft geometrische Kalibrierungsfaktoren (wie \(k\)-Faktoren), deren Herleitung mit Unsicherheiten behaftet ist. Wenn wir jedoch \textbf{Verhältnisse} zwischen Messgrößen bilden, kürzen sich diese Faktoren heraus!

\textbf{Beispiel: Leptonmassen}

Die Massendifferenzen zwischen den Generationen folgen geometrischen Mustern:
\begin{equation}
	\frac{m_\tau - m_\mu}{m_\mu - m_e} \approx \text{geometrischer Faktor abhängig von } f
\end{equation}

Da der Kalibrierungsfaktor in Zähler und Nenner erscheint, kürzt er sich heraus. Das Verhältnis ist dann eine \textbf{reine Funktion von \(f\)} und damit mathematisch exakt bestimmbar!

\textbf{Beispiel: Anomale magnetische Momente (g-2)}

Das gleiche Prinzip gilt für die anomalen magnetischen Momente der Leptonen. Die T0-Theorie sagt voraus:
\begin{align}
	a_e &= \frac{S_3/f}{k_{\text{geom}}} \\
	\Delta a_{\mu-e} &= \frac{4\pi}{f^{5/3}} \cdot \frac{1}{k_{\text{geom}}} \\
	\Delta a_{\tau-\mu} &= \frac{4\pi}{f^{4/3}} \cdot \frac{1}{k_{\text{geom}}}
\end{align}

Das Verhältnis der Differenzen:
\begin{equation}
	\boxed{\frac{\Delta a_{\tau-\mu}}{\Delta a_{\mu-e}} = \frac{f^{5/3}}{f^{4/3}} = f^{1/3} = 19{,}57}
\end{equation}

Der unsichere Faktor \(k_{\text{geom}}\) \textbf{kürzt sich vollständig heraus}!

\textbf{Rekonstruktion von Kalibrierungsfaktoren}

Umgekehrt können wir aus präzisen experimentellen Daten die Kalibrierungsfaktoren bestimmen:
\begin{equation}
	k_{\text{geom}}^{\text{rek}} = \frac{S_3/f}{a_e^{\exp}}
\end{equation}

Wenn wir dies tun, verbessert sich die Präzision für abhängige Vorhersagen dramatisch. Dies zeigt, dass Abweichungen von einigen Prozent nicht aus der fundamentalen Theorie stammen, sondern aus Unsicherheiten in den Projektionsfaktoren.
