...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{@{} l >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X @{}}
		\toprule
		\textbf{Teilchen} & \textbf{T0-Vorhers. (GeV)} & \textbf{SI (kg)} & \textbf{Exper. (GeV)} & \textbf{Exp. SI (kg)} & \textbf{Abw.} \\
		\midrule
...
\begin{table}[h]
	\centering
	\small  % behalten für kompakteres Aussehen
...
\end{table}
	...
	\begin{equation}
		...
		\begin{cases} 
			...
			D_{\text{quark}} = |Q| \cdot D_f \cdot (\xi^{\text{gen}}) \cdot (1 + \alpha_s \pi n_{\text{eff}}) \cdot \frac{1}{\text{gen}^{1.2}} & \text{(Quar.)}
		\end{cases}
	\end{equation}
	\begin{itemize}
...
\end{table}
	...
	\begin{equation}
		P(\nu_\mu \to \nu_e) = 4 |U_{\mu 3} U_{e 3}^*|^2 \sin^2 \left( \frac{\Delta m_{31}^2 L}{4E} \right) + \text{CP-Term} +  \text{Interfer}.
		\label{eq:oszprob}
	\end{equation}
...
\subsection*{Finale Vorhersagen vs. Experimentelle Werte (GeV, post-hoc $K_{\text{corr}}$)}
...
\begin{table}[h]
	...
	\begin{tabularx}{\textwidth}{@{} l >{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X @{}}
		...
		\bottomrule
	\end{tabularx}
	\caption{Finale Vorhersagen vs. experimentelle Werte (GeV, nach Anwendung von post-hoc $K_{\text{corr}}$)}
...
\begin{equation}
	D =
	\begin{cases}
		1 + (gen - 1) \cdot \alpha_{\text{em}} \pi & \text{(Leptonen)} \\[0.5em]
		|Q| \cdot D_f \cdot \xi^{\text{gen}} \cdot (1 + \alpha_s \pi n_{\text{eff}}) / \text{gen}^{1.2} & \text{(Quarks)} \\[0.5em]
		N_c (1 + \alpha_s) \cdot e^{-(\xi/4) N_c} \cdot 0.5 \Lambda_{\text{QCD}} & \text{(Baryonen)} \\[0.5em]
		...
		m_{q1} + m_{q2} + \Lambda_{\text{QCD}} \cdot K_{\text{frak}}^{n_{\text{eff}}} & \text{(Mesonen)} \\[0.5em]
		m_t \cdot \phi \cdot (1 + \xi D_f) & \text{(Hig./Bos.)}
	\end{cases}
\end{equation}
