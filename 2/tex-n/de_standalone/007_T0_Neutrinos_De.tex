...
% Standardisierter Präambel - 007\_T0\_Neutrinos\_De.pdf
\input{T0_preamble_standalone_De}

\title{\textbf{T0-Theorie: Neutrinos}\\[0.5cm]
	\large Die Photonen-Analogie, geometrische Oszillationen und Koide-Erweiterung\\[0.3cm]
...
\begin{document}
	...
	\begin{koidebox}
		...
		Geometrische Phasen (von $f(n,\ell,j)$) dominieren, ergänzt durch kleine $\Delta m^2_{ij} \approx (0,1-0,2) \times 10^{-4}$ eV$^2$ von $\delta$. Dies vereinbart T0 mit Daten ohne vollständige Hierarchie.
		
		\textbf{WARNUNG:} Hochgradig spekulativ; testbar durch zukünftige $\Sigma m_\nu$-Messungen (z.B. Euclid 2026+).
	\end{koidebox}
	...
	\begin{keyresult}
		...
		\begin{enumerate}
			...
			\item \textbf{Einheitliche Masse (Basis):} Alle Flavour-Zustände haben $m_\nu \approx 4,54$ meV (quasi-entartet)
	...
	\begin{warning}
		...
		\begin{itemize}
			...
			\item Spezifische, testbare Vorhersagen (z.B. $\Sigma m_\nu$, $Q_\nu$)
		\end{itemize}
		...
		\begin{itemize}
			...
			\item \textbf{Experimentell ununterscheidbar:} Ähnlich zum Standardmodell
			\item \textbf{Hochgradig spekulative Basis:} Photonen-Analogie und Koide-Erweiterung unbewiesen
		\end{itemize}
		
		\textbf{Gesamtbewertung: Interessante Hypothese, aber höchst spekulativ und unbestätigt}
	\end{warning}
	...
	\begin{table}[h]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tabular}{lcccc}
				...
				\midrule
				Feinstrukturkonstante & $\alpha^{-1} = 137,036$ & $137,036$ & $< 0,001\%$ & \checkmarkx Etabliert \\
				Gravitationskonstante & $G = 6,674 \times 10^{-11}$ & $6,674 \times 10^{-11}$ & $< 0,001\%$ & \checkmarkx Etabliert \\
				Geladene Leptonen & $99,0\%$ Genauigkeit & Präzise bekannt & $\sim 1\%$ & \checkmarkx Etabliert \\
				Quarkmassen & $98,8\%$ Genauigkeit & Präzise bekannt & $\sim 2\%$ & \checkmarkx Etabliert \\
				\midrule
				\textbf{Neutrinomassen (Koide-Erw.)} & $m_{\nu_i} \approx 4-5$ meV & $< 100$ meV & Unbekannt ($\Delta Q_\nu <1\%$) & \warningx Spekulativ \\
				\textbf{Neutrinooszillationen} & Geometrische Phasen + $\delta$ & $\Delta m^2 \neq 0$ & Teilweise kompatibel & \warningx Problematisch \\
				\bottomrule
		\end{tabular}}
		\caption{T0-Neutrinos im Vergleich zu etablierten T0-Erfolgen (Aktualisiert mit Koide)}
	\end{table}
	...
	\begin{enumerate}
		...
		\item \textbf{Oszillationstheorie:} Strenge Herleitung der Hybrid-Effekte
