...
% Standardized preamble - 026_T0_Geometrische_Kosmologie_De.pdf
...
\begin{document}
	...
	\end{equation}
	Die parameterfreie Vorhersage der T0-Theorie ist $H_0^{\text{T0}} = \xi \cdot c$, ohne zusätzlichen geometrischen Faktor.
	...
	\end{equation}
	Mit dem universellen Parameter $\xi = \frac{4}{3} \times 10^{-4}$ ergibt sich:
	\begin{align*}
		H_0^{\text{T0}} &= \xi \cdot c = \frac{4}{3} \times 10^{-4} \times 299\,792\,\text{km/s} \\
		&\approx 40{,}0 \, \frac{\text{km}}{\text{s} \cdot \text{Mpc}}
	\end{align*}
	Der experimentell extrahierte Standardwert $H_0^{\text{exp}} \approx 70\,$km/s/Mpc (Faktor $\approx 7/4$) ist modellabhängig und setzt ein expandierendes Friedmann-Universum voraus. Der Faktor $7/4$ ist eine offene Forschungsfrage. 026 verwendet keinen freien Geometriefaktor mehr \cite{riess2019} und bietet eine natürliche Erklärung für die ``Hubble-Spannung'', da leichte Variationen der Gittergeometrie in verschiedenen Himmelsrichtungen zu unterschiedlichen Messwerten führen können.
	...
	\begin{lstlisting}[language=Python, caption={Konzeptioneller Python-Code für die FEM-Simulation der geometrischen Rotverschiebung.}, label={lst:fem_code}]
		...
		H0_formula = XI * C_SPEED / 1e3 * 3.0857e22 / 3.0857e22  # H0 = xi * c in km/s/Mpc
		H0_formula = XI * C_SPEED / 1e3  # simplified: xi * c in km/s -> /Mpc implicit
