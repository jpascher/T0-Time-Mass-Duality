...
% Standardized preamble - 009_T0_xi_ursprung_De.tex
...
\end{adjustbox}
	...
	\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title={Die Einheit der physikalischen Konstanten}]
		Alle fundamentalen Konstanten der Physik lassen sich direkt aus dem Parameter $\xi$ ($\xi = \frac{4}{3} \times 10^{-4}$) ableiten, der die geometrische Signatur der Raumzeit repräsentiert. Die Beziehungen lauten:
	...
	\begin{equation}
		\frac{V_{\text{tetra}}}{V_{\text{sphäre}}} \approx 0.513 \quad \Rightarrow \quad \xi = \frac{4}{3} \times 10^{-4}
	\end{equation}
	
	Diese geometrische Wurzel von $\xi$ manifestiert sich in allen physikalischen Konstanten:
	\begin{itemize}
		\item $c$: Die Ausbreitungsgeschwindigkeit von Kausalität in fraktaler Raumzeit
		\item $h$: Die fundamentale Skala der Quantisierung
		\item $G$: Die Kopplung zwischen Geometrie und Materie
	\end{itemize}
	
	\subsection{Ausblick auf dimensionenlose Konstanten}
	
	\begin{tcolorbox}[colback=green!5!white,colframe=green!75!black,title={Erweiterung zur Feinstrukturkonstante}]
		Neben der Beschreibung dimensionaler Konstanten wie $c$, $h$ und $G$ bietet die T0-Theorie eine konsistente Grundlage zur Untersuchung dimensionenloser Konstanten wie der Feinstrukturkonstanten $\alpha$. 
		
		Die Hierarchie der Massenverhältnisse, die durch $\xi$ und den Exponenten $\kappa = 7$ festgelegt wird, deutet auf eine tiefere Struktur hin, die auch die elektromagnetische Kopplung bestimmt:
		
		\begin{equation}
			\alpha \approx \frac{1}{137.036} \quad \text{(zu untersuchen: Verbindung zu } \xi \text{ und } e\text{)}
		\end{equation}
		
		Zukünftige Arbeiten könnten vertiefen, wie $\xi$ und die fraktale Raumzeit die Hierarchie weiterer Konstanten beeinflussen. Die Verbindung zwischen der geometrischen Struktur ($\xi$, $D_f$) und dimensionenlosen Kopplungen ($\alpha$, Yukawa-Kopplungen) bleibt ein aktives Forschungsgebiet innerhalb der T0-Theorie.
	\end{tcolorbox}
	...
	Die Ableitung aller Konstanten aus $\xi$ zeigt, dass das Universum nicht durch separate, unabhängige Parameter beschrieben wird, sondern durch eine grundlegende Geometrie und harmonische Struktur vereint ist.
	
	Dies stellt die numerische Willkür in gängigen Modellen infrage und eröffnet den Weg zur Reduktion aller Konstanten auf eine einzige fundamentale Größe:
	...
	\begin{enumerate}
		...
		\item \textbf{Vorhersagekraft:} Präzise Beziehungen zwischen scheinbar unabhängigen Größen
		\item \textbf{Geometrische Wurzel:} Alle Physik emergiert aus Raumzeit-Geometrie
		\item \textbf{Testbarkeit:} Spezifische Vorhersagen für Abweichungen bei hohen Energien
	\end{enumerate}
	...
	\begin{table}[htbp]
		\centering
\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{@{}p{0.15\textwidth}@{\hspace{2mm}}p{0.50\textwidth}@{\hspace{2mm}}p{0.28\textwidth}@{}}
			\toprule
...
\end{adjustbox}
	...
	\begin{table}[htbp]
		\centering
\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{@{}p{0.30\textwidth}@{}p{9cm}@{}}
			\toprule
...
\end{adjustbox}
	...
	\begin{table}[htbp]
		\centering
\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{@{}p{0.30\textwidth}@{}p{8cm}@{}}
			\toprule
