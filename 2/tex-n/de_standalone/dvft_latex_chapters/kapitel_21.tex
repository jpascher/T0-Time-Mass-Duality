...
L \supset m \nabla\theta \cdot ẋ.
Since \nabla\$theta = g$ is constant near Earth's surface,
...
Δφ = (m/\hbar) \int [\nabla\theta \cdot (ẋ$_1$ - ẋ$_2$)] dt.
Let \nabla\$theta = g$ ẑ (constant). Then:
Δφ = (mg/\hbar) \int (ż$_1$ - ż$_2$) dt.
...
Δz(t) ∝ t$^2$ \to ż(t) ∝ t.
Thus:
Δφ = (m/\hbar) \int g ż(t) dt ∝ \int g t dt ∝ g T$^2$.
