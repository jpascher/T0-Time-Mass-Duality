...
S = \int d^4x \sqrt−g [ (1/(16\piG)) R + 𝓛_\theta(X, I_1, I_2) + 𝓛_m(g_{\mu\nu},ψ_m) ].
Here:
• R is the Ricci scalar (geometry),
• 𝓛_m is matter Lagrangian,
• 𝓛_\theta encodes vacuum microphysics:
𝓛_\theta = −Λ_v + (\rho_0/2)X − (\eta/(3a_0^2)) X^{3/2} + \alpha_1 I_1 + \alpha_2 I_2,
with invariants:
I_1 = V_{\mu\nu} V^{\mu\nu},
...
where:
𝓛_X = \partial𝓛_\theta/\partialX = \rho_0/2 − (\eta/(2a_0^2)) X^{1/2}.
...
T^{(\theta,DVFT)}_{\mu\nu} = 2\alpha_1 \partialI_1/\partialg^{\mu\nu} + 2\alpha_2 \partialI_2/\partialg^{\mu\nu} − g_{\mu\nu}(\alpha_1 I_1 + \alpha_2 I_2).
