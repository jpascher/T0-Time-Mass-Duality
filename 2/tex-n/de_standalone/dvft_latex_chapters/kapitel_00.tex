...
dynamic vacuum field described by a complex scalar \varphi(x)=\rho(x)e
i\theta(x) where \varphi(x) is dynamic vacuum field,
\rho(x) is vacuum amplitude and \theta(x) is vacuum phase. The vacuum possesses an intrinsic field with its
...
vacuum field \varphi(x)=\rho(x)e
i\theta(x)
, where \rho(x) represents the vacuum amplitude (inertial density) and \theta(x)
