...
1. A Lorentzian spacetime (M, g_{\mu\nu}).
2. Lorentz and diffeomorphism invariance.
3. A global U(1) symmetry \theta \to \theta + const.
...
2
𝑔
\mu𝜈 \partial\mu\rho \partial𝜈\rho − 𝑉(\rho) + 𝐹(𝑋),
...
\rho
2𝑔
\mu𝜈 \partial\mu\theta \partial𝜈\theta.
The potential is:
𝑉(\rho) = 𝜆(\rho
2 − \rho0
2
...
is:
𝑇\mu𝜈
(\Phi) = \partial\mu\Phi∗ \partial𝜈\Phi + \partial\mu\Phi\partial𝜈\Phi∗ − 𝑔\mu𝜈[𝑔
𝛼𝛽 \partial𝛼\Phi∗ \partial𝛽\Phi + 𝑉(|\Phi|
2
)].
For the nonlinear phase:
𝑇\mu𝜈
(\theta) = 𝐹𝑋 \partial\mu\theta \partial𝜈\theta − 𝑔\mu𝜈𝐹(𝑋),
where 𝐹𝑋 = \partial𝐹/ \partial𝑋. Curvature arises because 𝑇\mu𝜈
(\Phi)
...
2𝐹𝑋\nabla
\mu\theta) = 0.
Step-by-step: From ℒ\Phi, \partialℒ/ \partial(\partial\mu\theta) = −\rho
2𝐹𝑋\nabla
...
large:
|\partial_r \theta| \to \infty as r \to r_H
