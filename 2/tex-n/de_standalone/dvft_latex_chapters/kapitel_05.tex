...
Phase gradients \delta\theta propagate at light speed, modifying T\mu\nu(\varphi). Einstein's GR equation then becomes:
G\mu\$nu = 8$\piG ( T\mu\nu (m) + T\mu\nu(\varphi))
...
GR states that curvature is determined by the Einstein equation G_{\mu\nu} = 8\piGT_{\mu\nu}, but it does not
...
saturation in its potential V(|\varphi|$^2$). As a collapsing object approaches the classical singularity, the vacuum
...
dark energy to residual dynamic vacuum field energy, \varepsilon_vac = \rho$_0$$^2$\thetȧ$^2$ + V(\rho$_0$), providing a natural physical
