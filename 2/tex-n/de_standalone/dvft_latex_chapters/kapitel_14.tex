...
has no geometry, no time, and no light-speed. When the universe ignited, \rho transitioned from 0 \to \rho_0,
...
(A v_b^2 − B) ½ f'^2 + U(f) = C.
Assuming U(0) = U(\rho_0) = 0 (degenerate vacua) and front connecting \rho_0 \to 0, boundary conditions require
C = 0, so:
(A v_b^2 − B) ½ f'^2 + U(f) = 0.
...
• ΔU = U(0) − U(\rho_0) is the vacuum-energy difference driving expansion.
...
3. Energy conservation limits how fast \rho can rise from 0 \to \rho_0.
