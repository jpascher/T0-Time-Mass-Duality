...
% Standardized preamble - 018_T0_Anomale-g2-9_De.tex
...
\begin{document}
	...
	\begin{itemize}
		\item \textbf{$\xi$:} Universeller geometrischer Parameter, $\xi = \frac{4}{30000} \approx 1.33333 \times 10^{-4}$
		\item \textbf{$a_\ell$:} Totales anomalen Moment, $a_\ell = (g_\ell - 2)/2$ (reine T0)
		\item \textbf{$E_0$:} Universelle Energiekonstante, $E_0 = 1/\xi \approx \SI{7500}{\giga\electronvolt}$
		\item \textbf{$K_{\text{frak}}$:} Fraktale Korrektur, $K_{\text{frak}} = 1 - 100 \xi \approx 0.9867$
		\item \textbf{$\alpha(\xi)$:} Feinstrukturkonstante aus $\xi$, $\alpha \approx 7.297 \times 10^{-3}$
		\item \textbf{$N_{\text{loop}}$:} Schleifen-Normalisierung, $N_{\text{loop}} \approx 173.21$
		\item \textbf{$m_\ell$:} Leptonenmasse (CODATA 2025)
		\item \textbf{$T_{\text{field}}$:} Intrinsisches Zeitfeld
		\item \textbf{$E_{\text{field}}$:} Energiefeld, mit $T \cdot E = 1$
		\item \textbf{$\Lambda_{T0}$:} Geometrische Cutoff-Skala, $\Lambda_{T0} = \sqrt{1/\xi} \approx \SI{86.6025}{\giga\electronvolt}$
		\item \textbf{$g_{T0}$:} Massenunabhängige T0-Kopplung, $g_{T0} = \sqrt{\alpha K_{\text{frak}}} \approx 0.0849$
		\item \textbf{$\phi_T$:} Zeitfeld-Phasenfaktor, $\phi_T = \pi \xi \approx 4.189 \times 10^{-4}$ rad
		\item \textbf{$D_f$:} Fraktale Dimension, $D_f = 3 - \xi \approx 2.999867$
		\item \textbf{$m_T$:} Torsions-Mediator-Masse, $m_T \approx \SI{5.22}{\giga\electronvolt}$ (geometrisch, SymPy-validiert)
		\item \textbf{$R_f(D_f)$:} Fraktaler Resonanzfaktor, $R_f \approx 3830.6$ (aus $\Gamma(D_f)/\Gamma(3) \cdot \sqrt{E_0/m_e}$)
		\item \textbf{$p$:} RG-Dualitäts-Exponent, $p = -2/3$ (aus $\sigma^{\mu\nu}$-Dimension in fraktalem Raum)
		\item \textbf{$\lambda$:} Sept.-Prototyp-Kalibrierungsparameter, $\lambda \approx 2.725 \times 10^{-3}$ MeV (aus Myon-Diskrepanz)
	\end{itemize}
	...
	\begin{align*}
		...
		&= 0.01584 \cdot 0.0860 \cdot 3830.6 \\
		&\approx 5.22\ \text{GeV}.
	\end{align*}
	...
	\textbf{Vereinheitlichte T0-Berechnung aus $\xi$ (2025-Werte):}
	\begin{itemize}
		\item \textbf{Elektron} ($n=-12$):
		\begin{itemize}
			\item $m_\ell / m_\mu = 0.00484$
			\item $(m_\ell / m_\mu)^2 = 2.34 \times 10^{-5}$
			\item $a_\ell$ aus $\xi$: $0.0036 \times 10^{-12}$
			\item Experiment: $1159652180.46(18) \times 10^{-12}$
		\end{itemize}
		\item \textbf{Myon} ($n=-11$):
		\begin{itemize}
			\item $m_\ell / m_\mu = 1$
			\item $(m_\ell / m_\mu)^2 = 1$
			\item $a_\ell$ aus $\xi$: $153 \times 10^{-11}$
			\item Experiment: $116592070(148) \times 10^{-11}$
		\end{itemize}
		\item \textbf{Tau} ($n=-7$):
		\begin{itemize}
			\item $m_\ell / m_\mu = 16.82$
			\item $(m_\ell / m_\mu)^2 = 282.8$
			\item $a_\ell$ aus $\xi$: $43300 \times 10^{-7}$
			\item Experiment: $< 9.5 \times 10^{3} \times 10^{-7}$
		\end{itemize}
	\end{itemize}
	\emph{Voll geometrisch; korrigiert für $a_e$.}
	\label{tab:results}
	...
	\textbf{Vergleich zwischen T0-Theorie und String-Theorie (aktualisiert 2025, Rev. 9):}
	
	\begin{itemize}
		\item \textbf{Kernidee}: 
		\begin{itemize}
			\item T0-Theorie: Dualität $T \cdot m = 1$; fraktale Raumzeit ($D_f = 3 - \xi$); Zeitfeld $\Delta m(x,t)$ erweitert Lagrangedichte.
			\item String-Theorie: Punkte als vibrierende Strings in 10/11 Dim.; extra Dim. kompaktifiziert (Calabi-Yau).
		\end{itemize}
		
		\item \textbf{Vereinheitlichung}:
		\begin{itemize}
			\item T0-Theorie: Integriert SM (QED/HVP aus $\xi$, Dualität); erklärt Massenhierarchie via $m_\ell^2$-Skalierung.
			\item String-Theorie: Vereinheitlicht alle Kräfte via String-Vibrationen; Gravitation emergent.
		\end{itemize}
		
		\item \textbf{g-2-Anomalie}:
		\begin{itemize}
			\item T0-Theorie: Kern $\Delta a_\mu^{\text{T0}} = 153 \times 10^{-11}$ aus Ein-Schleife + Inbettung; passt Pre/Post-2025 ($\sim 0.15 \sigma$).
			\item String-Theorie: Strings prognostizieren BSM-Beiträge (z.\,B. via KK-Moden), aber unspezifisch ($\pm 10\%$ Unsicherheit).
		\end{itemize}
		
		\item \textbf{Fraktal/Quantum Foam}:
		\begin{itemize}
			\item T0-Theorie: Fraktale Dämpfung $K_{\text{frak}} = 1 - 100\xi$; approximiert QCD/HVP.
			\item String-Theorie: Quantum Foam aus String-Interaktionen; fraktal-ähnlich in Loop-Quantum-Gravity-Hybriden.
		\end{itemize}
		
		\item \textbf{Testbarkeit}:
		\begin{itemize}
			\item T0-Theorie: Prognosen: Tau g-2 ($4.33 \times 10^{-7}$); Elektron-Konsistenz via Inbettung. Keine LHC-Signale, aber Resonanz bei 5.22 GeV.
			\item String-Theorie: Hohe Energien (Planck-Skala); indirekt (z.\,B. Schwarzes-Loch-Entropie). Wenige Low-Energy-Tests.
		\end{itemize}
		
		\item \textbf{Schwächen}:
		\begin{itemize}
			\item T0-Theorie: Noch jung (2025); Inbettung neu (November); mehr QCD-Details benötigt.
			\item String-Theorie: Moduli-Stabilisierung ungelöst; keine vereinheitlichte Theorie; Landscape-Problem.
		\end{itemize}
		
		\item \textbf{Ähnlichkeiten}:
		\begin{itemize}
			\item T0-Theorie: Geometrie als Basis (fraktal vs. extra Dim.); BSM für Anomalien; Dualitäten (T-m vs. T-/S-Dualität).
			\item String-Theorie: Potenzial: T0 als ``4D-String-Approx.''? Hybrids könnten g-2 verbinden.
		\end{itemize}
	\end{itemize}
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{@{}p{0.12\textwidth} p{0.18\textwidth} p{0.12\textwidth} p{0.20\textwidth} p{0.20\textwidth} p{0.18\textwidth}@{}}
				\toprule
			...
			Elektron (e) & Reine T0 (voll, kein SM) (Post-2025) & 0.0036 & Nicht addiert (integriert QED aus $\xi$) & 1159652180.46 (full embed) $\approx$ Exp. 1159652180.46(18) $\times 10^{-12}$ & 0 $\sigma$ \\
			Myon ($\mu$) & Hybrid (additiv zu SM) (Pre-2025) & 153 & 116591810(43) (inkl. alter HVP $\sim$6920) & 116591963 $\approx$ Exp. 116592059(22) & $\sim$0.02 $\sigma$ \\
			Myon ($\mu$) & Reine T0 (voll, kein SM) (Post-2025) & 153 & Nicht addiert (SM $\approx$ Geometrie aus $\xi$) & 116592070 (embed + core) $\approx$ Exp. 116592070(148) & $\sim 0.15 \sigma$ \\
			Tau ($\tau$) & Hybrid (additiv zu SM) (Pre-2025) & 43300 & $<$ $9.5 \times 10^{8}$ (Grenze, SM $\sim$0) & $<$ $9.5 \times 10^{8}$ $\approx$ Grenze $<$ $9.5 \times 10^{8}$ & Konsistent \\
			Tau ($\tau$) & Reine T0 (voll, kein SM) (Post-2025) & 43300 & Nicht addiert (SM $\approx$ Geometrie aus $\xi$) & 43300 (progn.; integriert ew/HVP) $<$ Grenze $9.5 \times 10^{8}$ & 0 $\sigma$ (Grenze) \\
				\bottomrule
		\end{tabular}
		\end{adjustbox}
	...
	\begin{table}[htbp]
		\centering
		\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{@{}p{0.12\textwidth} p{0.18\textwidth} p{0.18\textwidth} p{0.12\textwidth} p{0.12\textwidth} p{0.14\textwidth} p{0.14\textwidth}@{}}
				\toprule
	...
	% Tabelle 1: Hybrid-Perspektive
	\begin{table}[htbp]
		\centering
		\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{@{}p{0.12\textwidth} p{0.14\textwidth} p{0.10\textwidth} p{0.18\textwidth} p{0.20\textwidth} p{0.12\textwidth} p{0.14\textwidth}@{}}
				\toprule
				Lepton & Perspektive & T0-Wert ($ \times 10^{-11}$) & SM Pre-2025 ($ \times 10^{-11}$) & Total (SM + T0) / Exp. Pre-2025 ($ \times 10^{-11}$) & Abw. ($\sigma$) & Erklärung (Pre-2025) \\
				\midrule
			...
			\end{tabular}
		\end{adjustbox}
		\caption{Hybrid vs. Reine T0: Hybrid-Perspektive -- Pre-2025-Daten ($ \times 10^{-11}$; Tau-Grenze skaliert)} \label{tab:hybrid_pure_hybrid}
	\end{table}
	
	% Tabelle 2: Reine T0-Perspektive
	\begin{table}[htbp]
		\centering
		\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{@{}p{0.12\textwidth} p{0.14\textwidth} p{0.10\textwidth} p{0.18\textwidth} p{0.20\textwidth} p{0.12\textwidth} p{0.14\textwidth}@{}}
			...
			\end{tabular}
		\end{adjustbox}
		\caption{Hybrid vs. Reine T0: Reine T0-Perspektive -- Pre-2025-Daten ($ \times 10^{-11}$; Tau-Grenze skaliert)} \label{tab:hybrid_pure_pure}
	\end{table}
	...
	\begin{table}[htbp]
		\centering
		\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{@{}p{0.15\textwidth} p{0.20\textwidth} p{0.20\textwidth} p{0.42\textwidth}@{}}
				\toprule
				...
				\makecell[l]{Unsicherheits\\notation} & $\pm 43 \times 10^{-11}$ (1$\sigma$; syst.+stat.) & $\pm 0.1\%$ (aus $\delta\xi \approx 10^{-6}$) & SM: modell-unsicher (HVP-Sims); T0: parameterfrei. \\
	...
	\begin{table}[htbp]
		...
		\small
		\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{p{1.5cm} p{1.5cm} p{1.5cm} p{2.5cm} p{2.5cm} p{2cm} p{2.5cm}}
				\toprule
	...
	\begin{table}[htbp]
		\centering
		\begin{adjustbox}{max width=\textwidth}
		\begin{tabular}{@{}p{0.16\textwidth} p{0.25\textwidth} p{0.25\textwidth} p{0.30\textwidth}@{}}
				\toprule
