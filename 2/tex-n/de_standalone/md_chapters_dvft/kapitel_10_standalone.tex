...
𝓛_vac = (A_\rho/2)(\partial_t \rho)² - (B_\rho/2)|\nabla\rho|² + (A_\theta/2)\rho²(\partial_t \theta)² - (B_\theta/2)\rho²|\nabla\theta|² - U(\rho).
For a homogeneous FRW universe (\rho(t), \theta(t), \nabla\rho = \nabla\theta = 0):
𝓛_hom = (A_\rho/2)\rhȯ² + (A_\theta/2)\rho² \thetȧ² - U(\rho).
...
K = (A_\rho/2)\rhȯ² + (A_\theta/2)\rho² \thetȧ².
...
A_\rho(\rhö+ 3H\rhȯ) - A_\theta\rho \thetȧ² + dU/d\rho = 0,
where:
...
• a stable minimum at \rho₀ (preferred vacuum amplitude),
• positive curvature at \rho₀ (vacuum stiffness),
...
Where:
• \Lambda₀ = microphysical residual vacuum energy density,
• \kappa = vacuum amplitude compressibility,
• \lambda = higher-order stabilization.
Near the minimum:
U(\rho) \approx \Lambda₀ + (1/2)m_\rho² (\rho - \rho₀)²,
with m_\rho² = \kappa/A_\rho.
...
1. The vacuum amplitude \rho has a preferred value \rho₀ (microphysical equilibrium).
2. The local vacuum energy density U(\rho₀) = \Lambda₀ is \textit{not zero}.
3. On large scales, \rho(t) approaches \rho₀ and remains nearly constant due to strong Hubble damping.
4. Therefore, the vacuum behaves like a nearly constant energy density with w \approx -1.
The measured value:
\rho_\Lambda \approx 7 \times 10⁻²⁷ kg/m³
\Omega_\Lambda \approx 0.70–0.75
matches DVFT if:
\Lambda₀ = U(\rho₀) \approx 0.7 \rho_crit.
...
g_vac(r) \approx (8\piG/3) \rho_\Lambda r.
...
• Cosmological acceleration: homogeneous U(\rho₀) dominates.
8. Numerical Comparison with Observations
Given:
• H₀ \approx 67–70 km/s/Mpc,
• \rho_crit = 3H₀²/(8\piG),
• \Omega_\Lambda \approx 0.7,
DVFT requires:
U(\rho₀) = \Lambda₀ \approx 0.7 \rho_crit.
...
• On cosmic scales, \rho \approx \rho₀ ⇒ w \approx -1, matching dark-energy observations.
• On solar/galactic scales, U(\rho) is negligible; \nabla\rho dominates gravity.
• DVFT dark energy matches measured values \Omega_\Lambda \approx 0.7 and w \approx -1 with no additional fields.
