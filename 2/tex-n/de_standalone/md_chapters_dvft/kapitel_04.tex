...
$\bullet$ Metric g_{\muν}, determining curvature.
$\bullet$ Phase field \theta, governing vacuum convergence.
The kinetic invariant is:
X ≡ -g^{\muν} $\nabla$_\mu\theta $\nabla$_ν\theta.
The Dynamic vacuum field Curvature Tensor (DVFT) is defined as:
V_{\muν} ≡ $\nabla$_\mu$\nabla$_ν\theta − (1/4) g_{\muν} □\theta,
with □\theta = g^{\alpha\beta} $\nabla$_\alpha$\nabla$_\beta\theta.
3. DVFT Action (Pure Gravity + Vacuum + Matter)
The full DVFT action is:
S = $\int$ d⁴x $\sqrt$−g [ (1/(16\piG)) R + 𝓛_\theta(X, I$_1$, I$_2$) + 𝓛_m(g_{\muν},ψ_m) ].
Here:
$\bullet$ R is the Ricci scalar (geometry),
$\bullet$ 𝓛_m is matter Lagrangian,
$\bullet$ 𝓛_\theta encodes vacuum microphysics:
𝓛_\theta = −Λ_v + (\rho$_0$/2)X − (\eta/(3a$_0$\textsuperscript{2})) X^{3/2} + \alpha$_1$ I$_1$ + \alpha$_2$ I$_2$,
with invariants:
I$_1$ = V_{\muν} V^{\muν},
I$_2$ = V_{\mu}^{ \alpha} V_{\alpha}^{ \beta} V_{\beta}^{ \mu}.
4. \theta Field Equation (Dynamics)
Varying S with respect to \theta gives the DVFT vacuum equation:
$\nabla$_\mu ( 𝓛_X $\nabla$^\mu\theta ) + \alpha$_1$ 𝓔^{(1)}[\theta,g] + \alpha$_2$ 𝓔^{(2)}[\theta,g] = 0,
where:
𝓛_X = $\partial$𝓛_\theta/$\partial$X = \rho$_0$/2 − (\eta/(2a$_0$\textsuperscript{2})) X^{1/2}.
...
1. k-essence (from X):
T^{(\theta,kess)}_{\muν} = 2 𝓛_X $\nabla$_\mu\theta $\nabla$_ν\theta − g_{\muν} 𝓛_\theta(kess).
2. DVFT curvature-like part:
T^{(\theta,DVFT)}_{\muν} = 2\alpha$_1$ $\partial$I$_1$/$\partial$g^{\muν} + 2\alpha$_2$ $\partial$I$_2$/$\partial$g^{\muν} − g_{\muν}(\alpha$_1$ I$_1$ + \alpha$_2$ I$_2$).
...
$\bullet$ X is large → 𝓛_X  $\approx$  constant.
$\bullet$ DVFT invariants I$_1$, I$_2$ are suppressed.
$\bullet$ T^{(\theta)}_{\muν}  $\approx$  −Λ_eff g_{\muν}.
...
In galaxies (g ~ a$_0$ or below):
$\bullet$ Nonlinear term X^{3/2} dominates,
$\bullet$ DVFT invariants contribute significantly,
$\bullet$ \theta-field deviates strongly from GR predictions.
