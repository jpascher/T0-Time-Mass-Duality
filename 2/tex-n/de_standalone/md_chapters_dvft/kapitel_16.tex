...
• H_0 inferred from early-universe data (CMB, Planck), and
• H_0 measured from the late universe (Cepheids and SN Ia).
...
U(\rho) = ½ \sigma (\rho – \rho_0)^2 + …
Vacuum energy density:
\rho_vac = ½ A \rhȯ^2 + U(\rho)
...
H^2 = (1 / 3M_pl^2) [\rho_m + \rho_vac(\rho, \rhȯ)]
with:
\rho_vac = ½ A \rhȯ^2 + U(\rho)
\rho(t) satisfies:
...
4. Early Universe Prediction (CMB Value of H_0)
At recombination:
...
5. Late Universe Prediction (Local Value of H_0)
After structure formation:
• S_backreact \neq 0
• Overdensities and voids perturb \rho(x,t)
• Coarse-grained local amplitude: \rhō_local \neq \rho*
• \rhȯ_local may be nonzero
Thus:
\rho_vac(local) = ½ A \rhȯ_local^2 + U(\rhō_local)
and:
...
Thus different cosmic epochs naturally exhibit different effective H_0 values.
...
It arises because:
• Early universe = coherent vacuum amplitude \rightarrow gives H_CMB
• Late universe = structure-backreacted vacuum amplitude \rightarrow gives H_local
