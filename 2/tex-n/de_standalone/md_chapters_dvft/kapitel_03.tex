...
empty but contains a complex scalar field Φ(x), whose amplitude \rho and phase \theta encode the internal state
...
fields \psi coupled to Φ.
The vacuum Lagrangian is:
ℒΦ = −
1
2
𝑔
𝜇𝜈 \partial𝜇\rho \partial𝜈\rho − 𝑉(\rho) + 𝐹(𝑋),
...
is:
𝑇𝜇𝜈
(Φ) = \partial𝜇Φ∗ \partial𝜈Φ + \partial𝜇Φ\partial𝜈Φ∗ − 𝑔𝜇𝜈[𝑔
𝛼𝛽 \partial𝛼Φ∗ \partial𝛽Φ + 𝑉(|Φ|
2
)].
...
2𝐹𝑋\nabla
𝜇𝜃) = 0.
Step-by-step: From ℒΦ, \partialℒ/ \partial(\partial𝜇𝜃) = −\rho
2𝐹𝑋\nabla
...
\nabla ⋅ (𝐹𝑋\nabla\Phi) = 4𝜋𝐺\rho𝑚.
Define Newtonian potential Φ_N = - (\mu / \rho_0) \varphi (scaling for units).
