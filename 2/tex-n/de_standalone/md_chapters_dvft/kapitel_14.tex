...
has no geometry, no time, and no light-speed. When the universe ignited, \rho transitioned from 0 → \rho₀,
...
A v_b² f''(\xi) − B f''(\xi) + U'(f(\xi)) = 0.
Multiply by f'(\xi) and integrate:
(A v_b² − B) ½ f'^2 + U(f) = C.
Assuming U(0) = U(\rho₀) = 0 (degenerate vacua) and front connecting \rho₀ → 0, boundary conditions require
C = 0, so:
(A v_b² − B) ½ f'^2 + U(f) = 0.
...
or:
v_b < sqrt(B/A) ≡ c_\rho.
Thus \textbf{DVFT predicts a finite upper bound on space-creation speed}:
v_b(t) \leq c_\rho,
...
A(\partialₜ²\rho + 3H\partialₜ\rho) − B(\partialᵣ²\rho + 2\partialᵣ\rho/r) + U'(\rho) = 0,
where H = ȧ/a.
...
• \DeltaU = U(0) − U(\rho₀) is the vacuum-energy difference driving expansion.
...
3. Energy conservation limits how fast \rho can rise from 0 → \rho₀.
...
speed c) only applies \textit{inside} the \rho > 0 activated domain.
...
This does \textbf{not} mean the boundary moved at 3.36 c.
...
speed’ greater than c \textit{without violating relativity}, since no signals propagate faster than c within space.
...
Thus the theory \textit{can be fitted to observational results} by constraining:
\DeltaU/\sigma, B/A, and the shape of U(\rho).
