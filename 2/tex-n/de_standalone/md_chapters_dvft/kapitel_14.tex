...
has no geometry, no time, and no light-speed. When the universe ignited, \rho transitioned from 0 → \rho$_0$,
...
𝓛_\rho = ½ A ($\partial$ₜ\rho)\textsuperscript{2} - ½ B ($\nabla$\rho)\textsuperscript{2} - U(\rho),
leading to the Euler--Lagrange equation:
A $\partial$ₜ\textsuperscript{2}\rho - B $\nabla$\textsuperscript{2}\rho + U'(\rho) = 0.
...
In DVFT:
$\bullet$ Space exists where \rho(x,t) > 0.
$\bullet$ Pre-space (non-space) exists where \rho(x,t) = 0.
...
A v_b\textsuperscript{2} f''(\xi) - B f''(\xi) + U'(f(\xi)) = 0.
Multiply by f'(\xi) and integrate:
(A v_b\textsuperscript{2} - B) ½ f'^2 + U(f) = C.
Assuming U(0) = U(\rho$_0$) = 0 (degenerate vacua) and front connecting \rho$_0$ → 0, boundary conditions require
C = 0, so:
(A v_b\textsuperscript{2} - B) ½ f'^2 + U(f) = 0.
Since U(f)  $\geq$  0, a nontrivial front requires:
A v_b\textsuperscript{2} - B < 0,
or:
v_b < sqrt(B/A) ≡ c_\rho.
...
A($\partial$ₜ\textsuperscript{2}\rho + 3H$\partial$ₜ\rho) - B($\partial$ᵣ\textsuperscript{2}\rho + 2$\partial$ᵣ\rho/r) + U'(\rho) = 0,
where H = ȧ/a.
In a thin-front approximation \rho(r,t)  $\approx$  f(r - R(t)), the evolution of R(t) obeys:
\sigma R¨ + 3H \sigma R˙ + (2\sigma / R) = ΔU,
where:
$\bullet$ \sigma is surface tension of the amplitude front,
$\bullet$ ΔU = U(0) - U(\rho$_0$) is the vacuum-energy difference driving expansion.
...
3. Energy conservation limits how fast \rho can rise from 0 → \rho$_0$.
...
$\bullet$ The front moves at v_b(t)  $\leq$  c_\rho ~ c.
$\bullet$ The interior region expands with scale factor a(t).
...
DVFT predicts:
$\bullet$ A finite space-creation speed v_b(t), controlled by vacuum micro-constants A, B and potential
shape U(\rho).
$\bullet$ The cosmic horizon size (~46.5 Gly) arises from the combined effect of v_b(t)  $\leq$  c_\rho and
...
Conclusion
$\bullet$ Space creation corresponds to the outward propagation of the vacuum amplitude \rho.
$\bullet$ The boundary speed v_b(t) is finite because the amplitude field obeys a hyperbolic PDE.
$\bullet$ The maximal speed is the vacuum amplitude signal speed c_\rho = $\sqrt$(B/A).
$\bullet$ Cosmological expansion amplifies R(t) → ~46.5 Gly today.
$\bullet$ The observed effective 3.36c ratio is not a physical propagation speed but a cumulative result of
