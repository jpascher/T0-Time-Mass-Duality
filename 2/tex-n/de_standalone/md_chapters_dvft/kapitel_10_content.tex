...
w_DVFT = (K - U) / (K + U).
Important limits:
• K ≪ U \rightarrow w \rightarrow -1 (dark-energy–like)
• K ~ U \rightarrow -1 < w < -1/3 (dynamical dark energy)
• K ≫ U \rightarrow w \rightarrow +1 (stiff fluid; irrelevant today)
...
• a stable minimum at \rho_0 (preferred vacuum amplitude),
• positive curvature at \rho_0 (vacuum stiffness),
• anharmonic corrections stabilizing deviations.
Thus the coarse-grained continuum potential becomes:
U(\rho) = Λ_0 + (κ/2)(\rho - \rho_0)^2 + (\lambda/4)(\rho - \rho_0)⁴ + …
Where:
• Λ_0 = microphysical residual vacuum energy density,
• κ = vacuum amplitude compressibility,
• \lambda = higher-order stabilization.
Near the minimum:
U(\rho) \approx Λ_0 + (1/2)m_\rho^2 (\rho - \rho_0)^2,
...
1. The vacuum amplitude \rho has a preferred value \rho_0 (microphysical equilibrium).
2. The local vacuum energy density U(\rho_0) = Λ_0 is *not zero*.
3. On large scales, \rho(t) approaches \rho_0 and remains nearly constant due to strong Hubble damping.
...
matches DVFT if:
Λ_0 = U(\rho_0) \approx 0.7 \rho_crit.
...
• Cosmological acceleration: homogeneous U(\rho_0) dominates.
8. Numerical Comparison with Observations
Given:
• H_0 \approx 67–70 km/s/Mpc,
• \rho_crit = 3H_0^2/(8\piG),
• \Omega_Λ \approx 0.7,
DVFT requires:
U(\rho_0) = Λ_0 \approx 0.7 \rho_crit.
...
• On cosmic scales, \rho \approx \rho_0 ⇒ w \approx -1, matching dark-energy observations.
