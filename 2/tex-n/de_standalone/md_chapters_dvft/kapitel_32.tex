...
\rho(x) = \rho₀ + \Delta\rho,
where |\Delta\rho/\rho₀| \approx 10⁻⁶ near dense nuclear activity.
Such amplitude fluctuations modify the neutrino phase propagation equation:
\partial²_t\theta - v²_\theta \nabla²\theta + \alpha(\rho)\theta = 0,
where \alpha(\rho) changes slightly due to \Delta\rho.
...
P_survival \approx 1 - \gamma \Delta\rho,
with \gamma representing neutrino sensitivity to local vacuum shifts.
For \Delta\rho/\rho₀ \approx 10⁻⁶ and \gamma \approx 10³–10⁴:
\DeltaP \approx 5–7\%.
...
identifies the correct environmental variable: \Delta\rho—the modification of vacuum amplitude due to nuclear
processes.
7. DVFT Predictions for Experimental Verification
If DVFT is correct, then:
• The deficit should increase with reactor power.
• Different isotopic mixtures (U-235 vs Pu-239) should produce different \Delta\rho and thus different
deficits.
