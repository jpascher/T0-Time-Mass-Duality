...
with X = -g^{\muν} $\partial$_\mu\theta $\partial$_ν\theta.
...
ds\textsuperscript{2} = -e^{2\Phi(r)}dt\textsuperscript{2} + dr\textsuperscript{2}/(1 - 2Gm(r)/r) + r\textsuperscript{2} d\Omega\textsuperscript{2}.
...
X = -(1 - 2Gm(r)/r) \theta'(r)\textsuperscript{2}.
From the k-essence stress-energy tensor:
T_{\muν} = 2 L_X $\partial$_\mu\theta $\partial$_ν\theta - g_{\muν} L_\theta
3. Stress-Energy Components
Define:
L_\theta = -Λ_v + (\rho_0/2)X - (\eta/(3 a_0\textsuperscript{2})) X^{3/2},
L_X = $\partial$L_\theta/$\partial$X = \rho_0/2 - (\eta/(2a_0\textsuperscript{2})) X^{1/2}.
...
L_X(X_0) = 0.
Setting L_X=0 gives:
X_0^{1/2} = (\rho_0 a_0\textsuperscript{2})/\eta.
Thus, the vacuum phase reaches a 'saturation' point X_0, limiting further compression. The core energy
density becomes finite:
\rho_core = -Λ_v + (\rho_0\textsuperscript{3} a_0⁴)/(6 \eta\textsuperscript{2}).
5. Core Geometry
With \rho = \rho_core = constant, the Einstein equation gives a de Sitter--like interior:
m(r) = (4\pi/3)\rho_core r\textsuperscript{3},
1 - 2Gm(r)/r = 1 - (8\piG/3)\rho_core r\textsuperscript{2}.
Thus, the interior metric is:
ds\textsuperscript{2}_core  $\approx$  -[1 - (Λ_eff r\textsuperscript{2})/3] dt\textsuperscript{2} + dr\textsuperscript{2}/[1 - (Λ_eff r\textsuperscript{2})/3] + r\textsuperscript{2} d\Omega\textsuperscript{2},
...
ds\textsuperscript{2}  $\approx$  Schwarzschild metric.
...
$\bullet$ GR predicts infinite collapse. DVFT prevents this by saturating the vacuum phase.
$\bullet$ The black hole interior becomes a finite-size 'quantum core.'
$\bullet$ As mass falls in, both the horizon and the core radius increase.
$\bullet$ No singularity exists. Space cannot compress indefinitely.
$\bullet$ The final object is a quantum vacuum condensate, not a point of infinite density.
...
$\bullet$ GR-like exterior geometry,
$\bullet$ A finite-density quantum core replacing the singularity,
$\bullet$ A mechanism for black hole growth and evolution,
$\bullet$ A plausible resolution of the information paradox.
