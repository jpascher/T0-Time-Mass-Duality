...
\end{enumerate}
FFGFT satisfies all three using the single T0-Vakuum field \Phi = ? e^{i$\theta$}, without introducing extra fields, new
...
\end{enumerate}
The early universe in FFGFT undergoes a transition from a highly coherent T0-Vakuum phase (large ?, uniform
...
\end{enumerate}
To calculate the observed ratio ?_B $\approx$ 6 $\times$ 10???, FFGFT requires:
\begin{itemize}
...
FFGFT uniquely provides a physical meaning to \DeltaE_CP as the energy bias between opposite-winding
