...
has no geometry, no time, and no light-speed. When the universe ignited, ρ transitioned from 0 → ρ_0,
...
(A v_b² − B) ½ f'^2 + U(f) = C.
Assuming U(0) = U(ρ_0) = 0 (degenerate vacua) and front connecting ρ_0 → 0, boundary conditions require
C = 0, so:
(A v_b² − B) ½ f'^2 + U(f) = 0.
...
3. Energy conservation limits how fast ρ can rise from 0 → ρ_0.
...
shape U(ρ).
\textbullet{} The cosmic horizon size (~46.5 Gly) arises from the combined effect of v_b(t) ≤ c_ρ and
...
\textbullet{} The maximal speed is the vacuum amplitude ($\\rho_0 = 1/\\xi^2$ from T0) signal speed c_ρ = √(B/A).
\textbullet{} Cosmological expansion amplifies R(t) → ~46.5 Gly today.
