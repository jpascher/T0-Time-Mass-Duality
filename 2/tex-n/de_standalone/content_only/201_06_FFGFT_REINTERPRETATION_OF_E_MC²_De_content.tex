...
Theory (FFGFT), without using Einstein?s field equations. The FFGFT provides physical explanation of
...
FFGFT treats spacetime as a physical quantum medium described by the phase field $\theta$(x,t). Particles appear
...
\begin{enumerate}
  \item The FFGFT T0-Vakuum Field
\end{enumerate}
The T0-Vakuum is represented by the complex order parameter:
\Phi(x) = ?(x) e^{i$\theta$(x)},
with ? the T0-Vakuum density and $\theta$ the T0-Vakuum phase.
In flat spacetime, the FFGFT kinetic invariant is:
X = (1/c?)($\partial$_t$\theta$)? ? ($\nabla$$\theta$)?.
A simplified FFGFT Lagrangian\footnote{Siehe auch T0-Dokument: \texttt{027_T0_lagrndian_De.pdf}} for deriving particle-like excitations is:
...
\begin{enumerate}
  \item Quadratic Expansion of the FFGFT Action
\end{enumerate}
...
\begin{enumerate}
  \item Dispersion Relation of FFGFT T0-Vakuum Excitations
\end{enumerate}
...
m = ? m_$\theta$ / c.
Thus, the FFGFT T0-Vakuum excitations obey:
E? = p?c? + m? c?.
In the rest frame of the T0-Vakuum excitation (p = 0), the dispersion relation reduces to:
E? = m? c?.
Taking the positive-energy branch:
E = mc?.
This is derived entirely from the FFGFT T0-Vakuum field Lagrangian and its excitations---no Einstein field
...
\end{enumerate}
From the FFGFT Hamiltonian density:
...
\begin{enumerate}
  \item Physical Meaning of E = mc? in FFGFT
\end{enumerate}
FFGFT gives a more satisfying interpretation of E = mc?:
\begin{enumerate}
...
\end{enumerate}
Conclusion
E = mc? emerges naturally from FFGFT as the rest-energy relation for quantized T0-Vakuum-phase excitations.
The result is fully derivable from the FFGFT Lagrangian using:
\begin{itemize}
...
\end{itemize}
Mass--energy equivalence arises fundamentally from the microstructure of the T0-Vakuum in FFGFT.
