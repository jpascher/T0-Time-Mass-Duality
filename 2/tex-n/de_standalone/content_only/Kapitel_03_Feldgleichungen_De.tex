...
Φ(x) = ρ(x) e^{iθ(x)}
where ρ(x) ≥ 0 is the vacuum amplitude ($\\rho_0 = 1/\\xi^2$ from T0) and θ(x) is the vacuum phase. This decomposition reflects the
...
𝑅
16\piG + ℒΦ + ℒm(𝜓, Φ, 𝑔)],
where R is the Ricci scalar, G is Newton's constant, ℒΦ is the vacuum Lagrangian, and ℒm is for matter
...
T\mu\nu
(Φ) = ∂\muΦ∗ ∂\nuΦ + ∂\muΦ∂\nuΦ∗ − 𝑔\mu\nu[𝑔
\alpha\beta ∂\alphaΦ∗ ∂\betaΦ + 𝑉(|Φ|
2
)].
For the nonlinear phase:
T\mu\nu
(\theta) = FX ∂\mu\theta ∂\nu\theta − 𝑔\mu\nuF(X),
where FX = ∂F/ ∂X. Curvature arises because T\mu\nu
(Φ)
...
2FX∇
\mu\theta) = 0.
Step-by-step: From ℒΦ, ∂ℒ/ ∂(∂\mu\theta) = −\rho
2FX∇
...
\textbullet{} ∂θ creates spacetime curvature
