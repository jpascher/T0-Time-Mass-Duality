...
\textbullet{} e^{iα} = +1 → symmetric state → bosons
\textbullet{} e^{iα} = –1 → antisymmetric state → fermions
...
\textbullet{} The vacuum amplitude ($\\rho_0 = 1/\\xi^2$ from T0) ρ develops extreme gradients (large |∇ρ|² term).
\textbullet{} The vacuum phase θ becomes singular or multi-valued (large ρ²|∇θ|² term).
...
\textbullet{} Only α = 0 or π are allowed → bosons or fermions.
