...
DVFT defines the T0-Vakuum field as \Phi = ? e^{i$\theta$}. In the weak-field, low-acceleration outer regions of
...
These equations are derived from the basic DVFT equation \Phi = ? e^{i$\theta$} and the T0-Vakuum Lagrangian.
Complete derivation of this equation has been given below.
\begin{enumerate}
  \item DVFT T0-Vakuum Lagrangian and \Phi = ? e^{i$\theta$}
\end{enumerate}
...
\end{itemize}
DVFT identifies gravitational potential ? through phase evolution:
...
Starting from the fundamental DVFT field \Phi = ? e^{i$\theta$}, we derived:
\begin{itemize}
