...
Q = (m_e + m_μ + m_τ) / ( (\sqrtm_e + \sqrtm_μ + \sqrtm_τ)^2 ),
experimentally:
Q = 2/3 \pm 10⁻^5.
...
m_i \propto \sqrt(U''(ρ_i)) · | e^{iθ_i} - 1 |.
Using:
|e^{iθ} - 1|^2 = 2(1 - cosθ),
the mass becomes:
m_i = K · (1 - cosθ_i),
...
Then:
m_e = K(1 - cosθ_0)
m_μ = K(1 - cos(θ_0 + 2π/3))
m_τ = K(1 - cos(θ_0 + 4π/3)).
...
U(ρ) = κ(ρ - ρ_0)^2 + λ(ρ - ρ_0)^4 + …
supports a limited number of stable localized minima.
Nonlinear dynamic media naturally produce:
\textbullet{} three stable modes,
\textbullet{} 120° phase spacing,
\textbullet{} triplet standing waves.
Thus DVFT predicts:
\textbullet{} three charged leptons,
\textbullet{} with masses tied to phase geometry,
\textbullet{} not arbitrary Yukawa couplings.
The Koide relation therefore reflects vacuum structure, not coincidence.
6. Full DVFT Derivation Summary
DVFT \rightarrow m_i \propto (1 - cosθ_i)
Three equally spaced phase eigenmodes \rightarrow θ_i = θ_0 + 2πi/3
This produces: (\sqrtm_e, \sqrtm_μ, \sqrtm_τ) lying at 120° in mass space.
This enforces the identity:
Q = (m_e + m_μ + m_τ) / ( (\sqrtm_e + \sqrtm_μ + \sqrtm_τ)^2 ) = 2/3.
Thus Koide arises from:
\textbullet{} phase structure of vacuum,
\textbullet{} amplitude–phase coupling in Φ = ρ e^{iθ},
\textbullet{} geometric symmetry of vacuum eigenmodes.
