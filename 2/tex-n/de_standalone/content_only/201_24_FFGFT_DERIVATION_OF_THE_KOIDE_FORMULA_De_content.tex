...
Q = (m_e + m_$\mu$ + m_?) / ( ($\sqrt$m_e + $\sqrt$m_$\mu$ + $\sqrt$m_?)^2 ),
experimentally:
Q = 2/3 $\pm$ 10??.
...
\end{enumerate}
In FFGFT, the mass of a stable excitation arises from local curvature of the T0-Vakuum potential U(?) and
from the phase shift $\theta$ of the oscillation mode:
m_i ? $\sqrt$(U''(?_i)) ? | e^{i$\theta$_i} ? 1 |.
Using:
...
\end{enumerate}
Define:
a = $\sqrt$m_e, b = $\sqrt$m_$\mu$, c = $\sqrt$m_?.
...
\end{enumerate}
FFGFT $\rightarrow$ m_i ? (1 ? cos$\theta$_i)
Three equally spaced phase eigenmodes $\rightarrow$ $\theta$_i = $\theta$? + 2?i/3
This produces: ($\sqrt$m_e, $\sqrt$m_$\mu$, $\sqrt$m_?) lying at 120? in mass space.
This enforces the identity:
Q = (m_e + m_$\mu$ + m_?) / ( ($\sqrt$m_e + $\sqrt$m_$\mu$ + $\sqrt$m_?)^2 ) = 2/3.
Thus Koide arises from:
\begin{itemize}
  \item phase structure of T0-Vakuum,
  \item amplitude--phase coupling in \Phi = ? e^{i$\theta$},
