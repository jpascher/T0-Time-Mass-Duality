...
This video \href{https://www.youtube.com/watch?v=OywWThFmEII}{OywWThFmEII} is nothing short of \textbf{sensational} for the T0 theory, as it describes precisely the cosmological conundrum for which T0 offers an elegant solution. The contradictions presented in the video are catastrophic for standard cosmology, yet are \textbf{expected and predicted} for T0. Recent reviews and studies from 2025 underscore the ongoing crisis in cosmology and confirm the relevance of these anomalies \cite{sarkar2025, landstry2025, bengaly2025}.
...
\begin{itemize}
	\item \textbf{CMB Dipole}: Points towards Leo, 370 km/s
	\item \textbf{Quasar Dipole}: Points towards the Galactic Center, $\sim$1700 km/s \cite{mittal2024}
...
In my project documents (\texttt{redshift\_deflection\_En.pdf}, \texttt{063\_cosmic\_En.pdf}) it is described in detail:

\textbf{CMB in the T0 Model:}
\begin{itemize}
	\item The CMB temperature is given by: $T_{\text{CMB}} = \frac{16}{9} \xi^2 \times E_\xi \approx 2.725$ K
	\item The CMB dipole is \textbf{not a Doppler motion}, but an \textbf{intrinsic anisotropy} of the $\xi$-field
	\item The $\xi$-field ($\xi = \frac{4}{3} \times 10^{-4}$) is the fundamental vacuum field from which the CMB arises as equilibrium radiation
\end{itemize}
...
\textbf{Crucially:} The redshift depends on wavelength!
...
\begin{enumerate}
	...
	\item \textbf{Combination} of a local $\xi$-field gradient and actual motion
\end{enumerate}

\subsection{3. The 90° Orthogonality: A Hint at Field Geometry}
...
\begin{itemize}
	...
	\item The CMB dipole indicates the \textbf{$\xi$-field anisotropy} (vacuum field)
...
This duality could imply geometric orthogonalities between matter and radiation components. Recent 2025 analyses reinforce this tension with hints of superhorizon fluctuations and residual dipoles \cite{sarkar2025, bengaly2025}.
...
Different $H_0$ measurements use different wavelengths $\rightarrow$ different apparent "Hubble constants"! Recent 2025 investigations into dipole tensions support the need for alternative models \cite{landstry2025, bengaly2025}.

\section{Alternative Explanatory Pathways Without Redshift}

\subsection{The Fundamental Paradigm Shift}

If it turns out that cosmological redshift does not exist or has been fundamentally misinterpreted, the T0 model offers alternative explanations that work completely without expansion.

\subsection{Accounting for Cosmic Distances and Minimal Effects}

A crucial physical aspect is accounting for the extremely large scales of cosmological observations:

\begin{itemize}
	\item \textbf{Typical observation distances:} $1 - 10^4$ Megaparsec ($3 \times 10^{22} - 3 \times 10^{26}$ meters)
	\item \textbf{Cumulative effects:} Even minimal percentage changes accumulate over these scales to become measurable quantities
\end{itemize}
...
The required rates of change are extremely small ($10^{-15} - 10^{-25}$ per unit) and lie below current laboratory detection limits, but become measurable over cosmological scales.

\subsection{Consequences for the Observed Phenomena}
...
\begin{table}[htbp]
	...
	\end{tabular}}
	\caption{T0 vs. Standard Cosmology: Comparison of Predictions}
	\label{tab:t0-comparison}
\end{table}

The video concludes with: \textit{``Whichever way you turn, something in cosmology doesn't add up.''}

\textbf{T0 Answer:} It adds up perfectly -- if one stops interpreting the CMB anisotropy as motion and instead acknowledges the wavelength-dependent redshift in the fundamental $\xi$-field.

The \textbf{1.3 million quasars} of the Quaia catalog are not the problem -- they are the \textbf{proof} that our interpretation of the CMB was wrong. T0 had already predicted these consequences before these observations were made. Current developments from 2025, such as isotropy tests with quasars, reinforce this confirmation \cite{sarkar2025}.

\textbf{Next step:} The data described in the video should be analyzed specifically for wavelength-dependent effects. The T0 predictions are so specific that they might already be testable with existing multi-wavelength catalogs.
