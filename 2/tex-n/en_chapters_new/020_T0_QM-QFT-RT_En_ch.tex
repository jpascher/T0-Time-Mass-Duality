...
% Original: \chapter{\Huge\textbf{T0-Theory: Network Representation and Dimensional Analysis}}
...
Where:
\begin{itemize}
	\item $V$ represents the set of nodes in the spacetime continuum, encompassing not only spatial positions but also temporal components to reflect the 3+1-dimensionality of physical space;
	...
	\item $T(v)$ represents the value of the time field at node $v$, integrating the absolute time $t_0$ as a fundamental scale;
	\item $E(v)$ represents the value of the energy field at node $v$, linked to the mass duality.
\end{itemize}

The fundamental time-energy duality relation $T(v) \cdot E(v) = 1$ is maintained at each node, ensuring consistent invariance across the entire network. This definition is fully compatible with the Lagrangian extensions in the T0 theory, as described in \cite{T0_tm_erweiterung}, and allows for discrete discretization of continuous fields.
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\textwidth}{@{} c c c X @{}}
		...
		3  & $4/3 \approx 1.333$ & 1.00   & Standard physical space (T0 core) \\
		4  & $8/4 = 2$          & 1.50   & Kaluza-Klein-like extensions \\
		5  & $16/5 = 3.2$       & 2.40   & Fractal scalings in CMB \\
		6  & $32/6 \approx 5.333$ & 4.00 & Hexagonal networks in quantum computing \\
...
\label{subsec:number_space_relation}
...
\begin{table}[htbp]
	...
	\begin{tabular}{cccc}
		\toprule
		\textbf{Number} & \textbf{Predicted} & \textbf{Predicted} & \textbf{Validation} \\
		\textbf{Size} & \textbf{Optimal $\xipar_{\text{res}}$} & \textbf{Success Rate} & \textbf{Metric} \\
		\midrule
