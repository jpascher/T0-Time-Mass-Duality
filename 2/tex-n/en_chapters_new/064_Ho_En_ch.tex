\chapter{The T0-Model: The Hubble Parameter in a Static Universe \\}

\section*{Abstract}
		The T0-model reinterprets the Hubble parameter $H_0$ within a static universe framework where observed redshift arises from photon energy loss during propagation through the omnipresent $\xi$-field rather than spatial expansion. Using the universal geometric constant $\xi = \frac{4}{3} \times 10^{-4}$ and energy field dynamics, we derive the Hubble parameter as $H_0^{\text{T0}} = \xi \cdot c \approx 40\,$km/s/Mpc as a parameter-free prediction. The discrepancy with the model-dependent standard value $H_0^{\text{exp}} \approx 70\,$km/s/Mpc (factor $\approx 7/4$) arises from different data interpretation. This approach eliminates dark energy, resolves the Hubble tension naturally, and provides a unified description based on three-dimensional space geometry in natural units where $\hbar = c = k_B = 1$.
	...
	\begin{longtable}{|c|l|l|}
		\hline
		\textbf{Symbol} & \textbf{Meaning} & \textbf{Natural Units} \\
		\hline
...
\end{equation}
	...
	For theoretical simplicity and to establish the basic mechanism, we consider the constant coupling approximation where the coupling function is unity:
	
	\begin{equation}
		f\left(\frac{E}{E_\xi}\right) = 1
	\end{equation}
	...
	The linear dependence on energy reflects the geometric nature of the $\xi$-field interaction: each unit of propagation distance causes the same fractional energy loss, independent of photon energy.
	
	\subsection{Solution for Cosmological Distances}
	
	For cosmological observations where the energy loss remains small compared to the initial photon energy ($\xi x \ll 1$), we can solve the differential equation perturbatively. The resulting energy as a function of distance becomes:
	...
	\begin{equation}
		z = \frac{E_0 - E(x)}{E(x)} \approx \frac{E_0 - E(x)}{E_0} = \xi \cdot x
	\end{equation}
	
	This fundamental relationship shows that redshift is proportional to distance alone, independent of photon energy, providing a natural explanation for the observed Hubble law without requiring spatial expansion.
	...
	\begin{formula}
		...
		\begin{equation}
			H_0^{\text{T0}} = \xi \cdot c
		\end{equation}
	\end{formula}
	
	This remarkable result shows that the Hubble parameter emerges directly from the single geometric constant $\xi$, with no dependence on photon energy or any other free parameter.
	
	\subsection{Energy Independence}
	
	A key feature of the corrected T0 energy loss mechanism is that the redshift depends only on distance, not on photon energy. This is consistent with observations: all spectral lines in a distant galaxy show the same redshift, regardless of wavelength. No additional energy scale parameter is needed.
	...
	\begin{align}
		H_0^{\text{T0}} &= \xi \cdot c \\
		&= \frac{4}{3} \times 10^{-4} \times 299{,}792\,\text{km/s} \\
		&\approx 40.0\,\text{km/s/Mpc}
	\end{align}
	...
	\begin{align}
		[H_0] &= [T^{-1}] \text{ (in SI units)} \\
		[\xi \cdot c] &= [1] \times [LT^{-1}] = [T^{-1}] \quad \checkmark
	\end{align}
...
\begin{tabular}{lccc}
			...
			Hubble parameter & $H_0 = \xi \cdot c$ & $[T^{-1}]$ & \checkmark \\
...
\begin{tabular}{lccc}
			...
			\midrule
			\rowcolor{blue!20}
			\textbf{T0 Prediction} & \textbf{40.0} & \textbf{Parameter-free} & \textbf{$H_0 = \xi \cdot c$} \\
	...
	The T0 prediction $H_0^{\text{T0}} = 40\,$km/s/Mpc is a parameter-free result from the same $\xi$ that determines particle masses. The ratio $H_0^{\text{exp}}/H_0^{\text{T0}} \approx 7/4$ (accurate to $0.07\%$) may have geometric significance in the torsion crystal formalism. The discrepancy arises because the ``experimental\'\' $H_0$ is model-dependent, extracted assuming Friedmann expansion.
	
	The Hubble tension between early-universe and local measurements ($67$--$73\,$km/s/Mpc) may reflect different model assumptions in data extraction rather than different physics. In T0, there is a single value $H_0^{\text{T0}} = \xi \cdot c = 40\,$km/s/Mpc.
	
	\begin{experimental}
		The T0-model predicts a single Hubble parameter $H_0^{\text{T0}} = \xi \cdot c \approx 40\,$km/s/Mpc. The apparent Hubble tension in standard cosmology dissolves because $H_0^{\text{exp}}$ is model-dependent.
	\end{experimental}
	...
	The T0-model reinterprets the Hubble parameter as a geometric energy loss rate $H_0^{\text{T0}} = \xi \cdot c \approx 40\,$km/s/Mpc rather than an expansion rate, providing a parameter-free prediction from the same constant that determines particle masses.
