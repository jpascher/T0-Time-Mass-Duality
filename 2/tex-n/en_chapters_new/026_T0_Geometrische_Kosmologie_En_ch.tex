% Original: \chapter{\textbf{T0-Kosmologie: Rotverschiebung als geometrischer Pfad-Effekt in einem statischen Universum}
	...
	\end{equation}
	The parameter-free T0 prediction is $H_0^{\text{T0}} = \xi \cdot c$, without any additional geometric factor.
	...
	\end{align*}
	The experimentally extracted standard value $H_0^{\text{exp}} \approx 70\,$km/s/Mpc (factor $\approx 7/4$) is model-dependent and assumes an expanding Friedmann universe. The factor $7/4$ is an open research question \cite{riess2019} and provides a natural explanation for the "Hubble tension," as slight variations in grid geometry in different directions of the sky could lead to differing measured values.
