...
Φ = ρ e^{iθ},
where:
• ρ(x,t) = vacuum amplitude → inertia & gravity
• θ(x,t) = vacuum phase → quantum behavior
...
• governed by vacuum phase θ, not classical mass density.
...
A proton creates a small amplitude bump δρ(x):
ρ(x) = ρ₀ + δρ(x).
...
• DVFT defines gravity as vacuum amplitude deformation ρ(x), valid for both localized and
superposed states.
• A proton in superposition does NOT produce two fields — it produces one unified field ∝ |ψ|².
