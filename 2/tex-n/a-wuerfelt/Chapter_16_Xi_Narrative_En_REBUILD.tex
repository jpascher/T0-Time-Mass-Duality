...
\begin{remark}[Discrepancy with the standard value $H_0 \approx 70\,$km/s/Mpc]
	The ratio $H_0^{\text{exp}}/H_0^{\text{T0}} \approx 70/40 = 7/4$ 
	(accurate to $0.07\%$) is non-trivial. The ``experimental'' $H_0$ is 
	extracted from raw data under the assumption of an expanding Friedmann 
	universe and is therefore model-dependent. In particular, the luminosity 
	distance $d_L = d \cdot (1+z)$ presupposes a cosmic scale factor that 
	does not exist in T0. Whether the factor $7/4$ has geometric significance 
	in the torsion crystal formalism is an open research question (see also 
	Chapter~9).
\end{remark}
