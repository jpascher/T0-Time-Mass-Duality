\chapter{\textbf{Ontologische Hierarchie der Energie-Reduktion}\\[0.5cm]
 Die Ebenen der fundamentalen Realität in natürlichen Einheiten\\[0.3cm]
...
\node[level, fill=gold!30] (L0) {\textbf{EBENE 0: ABSOLUTES FUNDAMENT}\\Universelles Energiefeld $E_{\text{Feld}}(x,t)$};
...
\node[right of=L0, xshift=3.5cm, text width=3cm, font=\scriptsize] {\textcolor{gold!80!black}{\textbf{Rein ontologisch}}};
...
\begin{center}
 Ein universelles Energiefeld $E_{\text{Feld}}(x,t)$
\end{center}
...
\section{Zusammenfassung der Hierarchie}
...
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{clp{3cm}p{3cm}}
\toprule
\textbf{Ebene} & \textbf{Beschreibung} & \textbf{Was existiert} & \textbf{Status} \\
\midrule
0 & Energiefeld & $E_{\text{Feld}}(x,t)$ & Absolut fundamental \\
1 & Zeit-Masse-Dualität & $T \cdot m = 1$ & Erste Emergenz \\
2 & Geometrie & $\xi$, 4D-Torsion & Strukturelle Realität \\
3 & Effektive Theorie & Modifizierte Gesetze & Phänomenologisch \\
4 & SI-Physik & $c, \hbar, G$ separat & Konventionell \\
\bottomrule
\end{tabular}}
\caption{Die fünf ontologischen Ebenen}
\end{table}

\subsection{Kausale Beziehungen}

\begin{equation}
\text{Ebene 0} \Rightarrow \text{Ebene 1} \Rightarrow \text{Ebene 2} \Rightarrow \text{Ebene 3} \Rightarrow \text{Ebene 4}
\end{equation}

wobei $\Rightarrow$ bedeutet: \enquote{determiniert} oder \enquote{lässt emergieren}

\subsection{Reduktion auf Energie}

\begin{center}
\textbf{Auf allen Ebenen gilt in natürlichen Einheiten:}
...
\normalsize für irgendein $n \in \mathbb{Z}$
\end{center}
...
$\Downarrow$ \textit{Tiefere Analyse}
...
$\Downarrow$ \textit{Ultimative Reduktion}
...
$\Downarrow$ \textit{Fundamentale Wahrheit}
...
\node[right, font=\footnotesize] at (0,5) {\textcolor{gold}{ABSOLUT FUNDAMENTAL}};
