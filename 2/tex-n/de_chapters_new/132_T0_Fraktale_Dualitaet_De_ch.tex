...
% Tabelle 1: Zeit-Dualität und Massen (FIXED for KDP - removed resizebox, using small font)
{\small % 9pt font - readable and above KDP 7pt minimum
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
	\toprule
	\textbf{Aspekt} & \textbf{T0-Theorie} & \textbf{DoT-Theorie} & \textbf{Parallele} \\
	\midrule
	\textbf{Zeit-Dualität} & Modulus via \( \xi \approx 10^{-4} \); fraktaler Bruch: \( f(\xi, T_0) = \prod (1 + \xi^n / T_0) \) & Hyperbolischer Modulus: \( \| t_c \| = \sqrt{t_r^2 - t_i^2} \); bei \( t_r = t_i \): Euklidisch & Beide nutzen Wurzel-Moduli für Dualität; Skalen-Bruch bei Iteration \\
	\midrule
	\textbf{Massen} & \( m = \frac{\hbar}{T_0 c^2} \cdot \sum_k \xi^k \phi_k \); Hierarchien als \( \xi^n \) & \( m = \gamma m_0 \) via Disentanglement; \( m_0 \) aus Knoten-Zeit & Masse als inverse Zeit-Fluktuation; 98\%+ Genauigkeit ohne freie Parameter \\
	\midrule
	\textbf{Energie} & \( E = m c^2 \) emergent; \( E \propto \xi^{-1/2} T_0 \); \( \| m \| = \text{const} \) & \( E_c = m_0 c^2 + j \gamma m_0 v c \); \( \| E_c \| = m_0 c^2 \) & Parameterfreie Derivation durch Modulus-Konservierung \\
	\bottomrule
\end{tabularx}
}
...
% Tabelle 2: Fraktale Iteration (FIXED for KDP - removed resizebox, using small font)
{\small % 9pt font - readable and above KDP 7pt minimum
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
	\toprule
	\textbf{Aspekt} & \textbf{T0-Theorie} & \textbf{DoT-Theorie} & \textbf{Parallele} \\
	\midrule
	\textbf{Fraktale Iteration} & \( d_H = \log(1/\xi) / \log(T_0) \approx 2.37 \); QM/GR-Iteration (\( \alpha = \xi^2/4\pi \)) & Fraktale Dimension als Zeit-Ratio; dritte Quantisierung via Levels & Beide iterieren Zeit fraktal; vereinigt QM/GR \\
		\midrule
