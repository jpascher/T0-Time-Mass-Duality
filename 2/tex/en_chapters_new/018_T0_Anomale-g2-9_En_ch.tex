...
\chapter{018 T0 Anomale-g2-9 En}
...
\thispagestyle{fancy}
	...
	In the pure T0 Theory~\cite{018_T0_SI}, the T0 effect is the complete contribution: SM approximates geometry (QED loops as duality effects), so $a_\ell^{T0} = a_\ell$. Fits post-2025 data at $\sim 0.15\sigma$ (lattice HVP resolves tension). Hybrid view optional for compatibility.
	...
	\begin{explanation}{Scaling Explanation}
		The formula $m_\ell = E_0 \cdot \sin(\pi \xi)$ connects masses directly to geometry, as detailed in~\cite{018_T0_gravitational_constant} for the gravitational constant $G$.
	\end{explanation}
	...
	\end{equation}
	yielding $\alpha \approx 7.297 \times 10^{-3}$ (calibrated to CODATA 2025; detailed in~\cite{018_T0_fine_structure}).
	...
	This term generates a one-loop diagram with two T0 vertices (quadratic enhancement $\propto g_{T0}^2$), now without vanishing trace due to the $\gamma^\mu$-structure~\cite{018_bell_muon}.
	
	\begin{derivation}{Coupling Derivation}
		The coupling $g_{T0}$ follows from the torsion extension in~\cite{018_QFT_T0}, where the time field interaction solves the hierarchy problem and induces the vectorial mediator.
	\end{derivation}
...
\begin{longtable}{@{}p{1.2cm}p{1.6cm}p{1.2cm}p{2.4cm}p{2.4cm}p{1.2cm}p{2cm}@{}}
		\caption{Extended Table: T0 Formula in Hybrid and Pure Perspectives (2025 Update, Rev. 9)} \label{018_tab:extended_comparison}\\
		\toprule
...
\begin{longtable}{@{}p{1cm}p{1.6cm}p{0.9cm}p{2.5cm}p{2.2cm}p{1.4cm}p{2.2cm}@{}}
		\caption{Hybrid vs. Pure T0: Pre-2025 Data ($ \times 10^{-11}$; Tau Bound Scaled)} \label{018_tab:hybrid_pure}\\
		\toprule
