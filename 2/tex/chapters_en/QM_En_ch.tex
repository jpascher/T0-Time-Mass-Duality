\chapter[Qm (QM)]{Qm (QM)}
	...
	\begin{table}[ht]
		...
		\begin{tabular}{lcccc}
			\toprule
			Aspect & Fractal Correction & $\xi$-Fit (Calibration) & Combined Effect & $\Delta$-Red.\ (\%) \\
			\midrule
			QM (n=6, Rydberg) & Stabilizes div.\ (44\%$\to$1\%) & Fits MPD ($\Delta$=0.16\%) & $<$0.15\% global & +85 \\
			Bell (CHSH, N=73) & Damps non-loc.\ ($\xi \ln N$) & Min.\ to obs (0.04\%$\to<$0.01\%) & Locality establ. & +75 \\
			Neutrino ($\Delta m^2_{21}$) & $\xi^2$-Suppr.\ (Hierarchy) & Adapt.\ NuFit (0.5\%$\to$0.4\%) & PMNS-consistent & +20 \\
			QFT (Higgs-$\lambda$) & Conv.\ loops (O($\xi$)) & Stable $\mu$=100 GeV & No blow-up & +50 \\
