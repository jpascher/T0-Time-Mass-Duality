...
\begin{document}
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabularx}{\textwidth}{XXXX}
			...
			\midrule
			\textbf{Time Duality \& Modulus} & Dimensionless modulus via \( \xi = \frac{4}{3\pi} \int r^2 dr \approx 10^{-4} \); scales with \( T_0 \neq 1 \) to fractal break: \( f(\xi, T_0) = \prod (1 + \xi^n / T_0) \). & Hyperbolic modulus: \( \| t_c \| = \sqrt{t_r^2 - t_i^2} = \tau \) (Eq. 1, p. 29); at \( t_r = t_i \): Euclidean space \( (c, c) \). & \textbf{Strong Parallel}: Both use ``broken'' root moduli for duality (stable \( T_0 / t_r \) vs. flowing \( \xi / t_i \)); generates scale break upon iteration (\( T_0 > 1 \) ≈ inner levels), parameter-free to \( c \approx 3 \times 10^8 \, \text{m/s} \). \\
			\midrule
			\textbf{Mass Derivation from Time} & \( m = \frac{\hbar}{T_0 c^2} \cdot \sum_k \xi^k \phi_k \) (fractal series); at \( T_0 \neq 1 \): Divergence to hierarchies (e.g., lepton masses as \( \xi^n \)). & Mass from time delay: \( m = \gamma m_0 \) via disentanglement (p. 55); \( m_0 \) from minimal node time (two inner levels). & \textbf{Direct Parallel}: Mass as inverse time fluctuation (T0's \( 1/T_0 \) ≈ DoT's \( t_i / t_r \)); fractal iterative – DoT's nodes ≈ T0's \( \xi \)-powers; both predict 98\%+ accuracy without free parameters (e.g., electron mass from geometry). \\
			\midrule
			\textbf{Energy-Momentum Conservation} & \( E = m c^2 \) emergent from dual: \( E \propto \xi^{-1/2} T_0 \); conserved via \( \| m \| = \text{const} \) in fractal series. & Complex energy: \( E_c = m_0 c^2 + j \gamma m_0 v c \), modulus \( \| E_c \| = m_0 c^2 \) (Eq. 24, p. 60); derives \( E = \gamma m_0 c^2 \). & \textbf{Exact Parallel}: Parameter-free \( E = mc^2 \)-derivation through modulus conservation; T0's \( \xi \)-series ≈ DoT's \( j \)-component – both ``break'' at \( v \neq 0 \) to relativity, without Einstein's postulates. \\
			\midrule
			\textbf{Fractal Iteration \& Unification} & Fractal break: \( d_H = \log(1/\xi) / \log(T_0) \approx 2.37 \); iterates for QM/GR (e.g., \( \alpha = \xi^2/4\pi \)). & Fractal dimension as ratio inner/outer time (p. 61); third quantization via recurrent levels (\( N \approx 10^{117} \) for \( \Lambda \)-reduction, Eq. 4, p. 68). & \textbf{Deep Parallel}: Both iterate time scales fractally (T0's \( \xi^n \) ≈ DoT's levels); unifies QM (granular) / GR (continuous) – DoT's \( N \)-averaging ≈ T0's CMB geometry for static universe. \\
			\midrule
			\textbf{\( c \)-Derivation (Speed of Light)} & \( c = 1 / \sqrt{\xi T_0} \); corrected by 0.07\% via Planck discreteness. & \( c \) as ``Speed of Creation'' in inner time; ideal 300,000,000 m/s, measured 299,792,458 via quantum foam (p. 62). & \textbf{Parallel}: Both geometric from time duality, with small correction for discreteness; parameter-free, anti-arbitrary. \\
			\bottomrule
		\end{tabularx}
	}
