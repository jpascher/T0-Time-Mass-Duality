...
\title{Basic Principle of Natural Units}
...
\section[T0 Nat Si (T0 nat-si)]{T0 Nat Si (T0 nat-si)}
...
\section*{In SI units:}
	...
	\end{equation}
	where $S_{\text{T0}} = 1.782662 \times 10^{-30}$ is the fundamental scaling factor \textbf{derived} in Section~\ref{L-T0_nat-si-0466}, not defined.
	...
	In natural units, physics is reduced to a geometric, dimensionless basis (cf. Section~\ref{L-T0_nat-si-0465}). The fundamental constants serve only as a scale, and the core formulas hold approximately without additional corrections because:
