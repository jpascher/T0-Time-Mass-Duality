...
\usepackage{booktabs}
% Standardized preamble - EliminationOfMassEn.tex
%\input{T0_preamble_En} % Commented out since we're defining everything here
% Custom commands for T0 model
...
\newcommand{\Tfieldt}{\mathcal{T}(\vec{x},t)} % Time field with arguments
\newcommand{\xipar}{\xi} % Xi parameter
\newcommand{\Tzero}{T_0} % Characteristic time scale
\begin{document}
