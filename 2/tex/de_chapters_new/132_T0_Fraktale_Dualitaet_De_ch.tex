...
\resizebox{\textwidth}{!}{%
	\begin{tabularx}{\textwidth}{XXXX}
		...
		\midrule
		\textbf{Zeit-Dualität \& Modulus} & Dimensionsloser Modulus via \( \xi = \frac{4}{3\pi} \int r^2 dr \approx 10^{-4} \); skaliert mit \( T_0 \neq 1 \) zu fraktalem Bruch: \( f(\xi, T_0) = \prod (1 + \xi^n / T_0) \). & Hyperbolischer Modulus: \( \| t_c \| = \sqrt{t_r^2 - t_i^2} = \tau \) (Eq. 1, S. 29); bei \( t_r = t_i \): Euklidischer Raum \( (c, c) \). & \textbf{Starke Parallele}: Beide nutzen "gebrochene" Wurzel-Moduli für Dualität (stabil \( T_0 / t_r \) vs. fließend \( \xi / t_i \)); erzeugt Skalen-Bruch bei Iteration (\( T_0 > 1 \) ≈ innere Levels), parameterfrei zu \( c \approx 3 \times 10^8 \, \text{m/s} \). \\
		\midrule
		\textbf{Massen-Ableitung aus Zeit} & \( m = \frac{\hbar}{T_0 c^2} \cdot \sum_k \xi^k \phi_k \) (fraktale Serie); bei \( T_0 \neq 1 \): Divergenz zu Hierarchien (z. B. Lepton-Massen als \( \xi^n \)). & Masse aus Zeit-Delay: \( m = \gamma m_0 \) via Disentanglement (S. 55); \( m_0 \) aus minimaler Knoten-Zeit (zwei inner Levels). & \textbf{Direkte Parallele}: Masse als inverse Zeit-Fluktuation (T0's \( 1/T_0 \) ≈ DoT's \( t_i / t_r \)); fraktal iterativ – DoT's Knoten ≈ T0's \( \xi \)-Potenzen; beide vorhersagen 98\%+ Genauigkeit ohne freie Parameter (z. B. Elektron-Masse aus Geometrie). \\
		\midrule
		\textbf{Energie-Momentum-Konservierung} & \( E = m c^2 \) emergent aus Dual: \( E \propto \xi^{-1/2} T_0 \); Konserviert via \( \| m \| = \text{const} \) in fraktaler Serie. & Komplexe Energie: \( E_c = m_0 c^2 + j \gamma m_0 v c \), Modulus \( \| E_c \| = m_0 c^2 \) (Eq. 24, S. 60); leitet \( E = \gamma m_0 c^2 \) ab. & \textbf{Exakte Parallele}: Parameterfreie \( E = mc^2 \)-Derivation durch Modulus-Konservierung; T0's \( \xi \)-Serie ≈ DoT's \( j \)-Komponente – beide "brechen" bei \( v \neq 0 \) zu Relativität, ohne Einsteins Postulate. \\
		\midrule
		\textbf{Fraktale Iteration \& Unifikation} & Fraktaler Bruch: \( d_H = \log(1/\xi) / \log(T_0) \approx 2.37 \); iteriert für QM/GR (z. B. \( \alpha = \xi^2/4\pi \)). & Fraktale Dimension als Ratio inner/outer Zeit (S. 61); dritte Quantisierung via rekurrenter Levels (\( N \approx 10^{117} \) für \( \Lambda \)-Reduktion, Eq. 4, S. 68). & \textbf{Tiefe Parallele}: Beide iterieren Zeit-Skalen fraktal (T0's \( \xi^n \) ≈ DoT's Levels); vereinigt QM (granular) / GR (kontinuierlich) – DoT's \( N \)-Averaging ≈ T0's CMB-Geometrie für statisches Universum. \\
		\midrule
		\textbf{\( c \)-Ableitung (Lichtgeschwindigkeit)} & \( c = 1 / \sqrt{\xi T_0} \); korrigiert um 0.07\% via Planck-Diskretheit. & \( c \) als "Speed of Creation" in innerer Zeit; ideal 300.000.000 m/s, gemessen 299.792.458 via Quanten-Schaum (S. 62). & \textbf{Parallele}: Beide geometrisch aus Zeit-Dualität, mit kleiner Korrektur für Diskretheit; parameterfrei, anti-arbiträr. \\
		\bottomrule
	\end{tabularx}
}
