...
\chapter{Mathematische Analyse des T0-Shor Algorithmus: \\
	Theoretischer Rahmen und Berechnungskomplexität \\
	Eine rigorose Untersuchung des T0-Energiefeld-Ansatzes zur Ganzzahlfaktorisierung}
...
\begin{abstract}
	...
	\begin{align}
		\text{Zeit-Masse-Dualität}: \quad &T(x,t) \cdot m(x,t) = 1 \label{075_eq:duality}\\
		\text{Feldgleichung}: \quad &\nabla^2 T(x) = -\frac{\rho(x)}{T(x)^2} \label{075_eq:field}\\
		\text{Energieentwicklung}: \quad &\frac{\partial^2 E}{\partial t^2} = -\omega^2 E \label{075_eq:evolution}
	\end{align}
	
	Der Kopplungsparameter $\xipar$ wird theoretisch aus Higgs-Feld-Wechselwirkungen abgeleitet:
	\begin{equation}
		\xipar = g_H \cdot \frac{\langle\phi\rangle}{v_{EW}} \label{075_eq:xi_higgs}
	\end{equation}
	...
	Die Zeit-Masse-Dualität (Gl.~\ref{075_eq:duality}) definiert eine hyperbolische Metrik im $(T,m)$ Parameterraum:
	...
	\begin{table}[htbp]
		...
		\label{075_tab:method_comparison_realistic}
	\end{table}
	...
	\begin{table}[htbp]
		...
		\label{075_tab:memory_bottlenecks}
	\end{table}
