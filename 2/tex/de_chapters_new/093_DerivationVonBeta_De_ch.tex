% Chapter file: 093_DerivationVonBeta_De_ch.tex
% Source: 093_DerivationVonBeta_De.tex
% Generated from standalone document

\chapter{T0-Modell: Feldtheoretische Herleitung des $\beta$-Parameters in natürlichen Einheiten ($\hbar = c...}

\section{Einführung und Motivation}
	\label{093_sec:introduction}
	...
	\label{093_sec:natural_units}
	
	Das T0-Modell verwendet das in der modernen Quantenfeldtheorie \citep{093_peskin1995, 093_weinberg1995} etablierte System natürlicher Einheiten:
	...
	Dieses System reduziert alle physikalischen Größen auf Energiedimensionen und folgt der von Dirac \citep{093_dirac1958} etablierten Tradition.
	...
	\label{093_sec:fundamental_structure}
	
	\subsection{Zeit-Masse-Dualität}
	\label{093_subsec:time_mass_duality}
	
	Das zentrale Prinzip des T0-Modells ist die Zeit-Masse-Dualität, die besagt, dass Zeit und Masse invers miteinander verknüpft sind. Diese Beziehung unterscheidet sich fundamental von der konventionellen Behandlung in der allgemeinen Relativitätstheorie \citep{093_einstein1915, 093_misner1973}.
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{|l|c|c|c|}
			...
			\hline
			Einstein ART & $dt' = \sqrt{g_{00}} dt$ & $m_0 = \text{const}$ & \citep{093_einstein1915, 093_misner1973} \\
			Spezielle Relativität & $t' = \gamma t$ & $m_0 = \text{const}$ & \citep{093_einstein1905} \\
		...
		\label{093_tab:theory_comparison}
	\end{table}
	
	\subsection{Grundlegende Feldgleichung}
	\label{093_subsec:field_equation}
	
	Die fundamentale Feldgleichung des T0-Modells wird aus Variationsprinzipien hergeleitet, analog zum Ansatz für Skalärfeldtheorien \citep{093_weinberg1995}:
	
	\begin{equation}
		\label{093_eq:field_equation_fundamental}
		\nabla^2 m(x) = 4\pi G \rho(x) \cdot m(x)
	\end{equation}
	
	Diese Gleichung zeigt strukturelle Ähnlichkeit zur Poisson-Gleichung der Gravitation $\nabla^2 \phi = 4\pi G \rho$ \citep{093_jackson1998}, ist jedoch nichtlinear aufgrund des Faktors $m(x)$ auf der rechten Seite.
	
	Das Zeitfeld folgt direkt aus der inversen Beziehung:
	\begin{equation}
		\label{093_eq:time_field_definition}
		T(x) = \frac{1}{m(x)}
	\end{equation}
	
	\section{Geometrische Herleitung des $\beta$-Parameters}
	\label{093_sec:beta_derivation}
	
	\subsection{Sphärisch symmetrische Punktquelle}
	\label{093_subsec:spherical_solution}
	
	Für eine Punktmassenquelle verwenden wir die etablierte Methodik der Lösung von Einsteins Feldgleichungen \citep{093_schwarzschild1916, 093_misner1973}. Die Massendichte einer Punktquelle wird durch die Dirac-Deltafunktion beschrieben:
	...
	\label{093_subsec:field_solution}
	...
	Der sphärisch symmetrische Laplace-Operator \citep{093_jackson1998, 093_griffiths1999} ergibt:
	...
	\label{093_subsec:integration_constants}
	...
	\textbf{Verhalten am Ursprung}: Verwendung des Gaußschen Satzes \citep{093_griffiths1999, 093_jackson1998} für eine kleine Kugel um den Ursprung:
	\begin{equation}
	...
	\label{093_subsec:characteristic_length}
	...
	Diese Skala ist proportional zum halben Schwarzschild-Radius $r_s = 2GM/c^2 = 2Gm$ in geometrischen Einheiten \citep{093_misner1973, 093_carroll2004}.
	
	\subsection{Definition des $\beta$-Parameters}
	\label{093_subsec:beta_definition}
	...
	\label{093_sec:physical_interpretation}
	
	\subsection{Dimensionsanalyse}
	\label{093_subsec:dimensional_analysis}
	...
	\label{093_subsec:classical_connection}
	...
	\label{093_subsec:limiting_cases}
	...
	\label{093_subsec:sm_differences}
	...
	\label{093_sec:experimental_predictions}
	
	\subsection{Zeitdilatationseffekte}
	\label{093_subsec:time_dilation}
	...
	\label{093_subsec:spectroscopic_tests}
	...
	\label{093_sec:mathematical_consistency}
	
	\subsection{Erhaltungssätze}
	\label{093_subsec:conservation_laws}
	...
	\label{093_subsec:solution_stability}
