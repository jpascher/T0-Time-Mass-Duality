...
\chapter{T0-Kosmologie: Rotverschiebung als geometrischer Pfad-Effekt in einem statischen Universum Eine...}
...
\thispagestyle{fancy}
	...
	\begin{lstlisting}[language=Python, caption={Konzeptioneller Python-Code fuer die FEM-Simulation der geometrischen Rotverschiebung.}, label={lst:fem_code}]
		import numpy as np
		import heapq
		...
		# Amplitude durch XI skaliert wird. Eine echte Simulation wuerde die
		# T0-Feldgleichung numerisch loesen (z.B. mit FEniCS).
		np.random.seed(42)
		...
		# Der Weg durch einen Punkt mit hoeherer Feldenergie ist "laenger".
		return 1.0 * field_value
		
		def find_geodesic_path(t0_field, start_node, end_node):
		"""Findet den kuerzesten Pfad (Geodaete) mittels Dijkstra-Algorithmus."""
		grid_size = t0_field.shape[0]
		distances = np.full((grid_size, grid_size, grid_size), np.inf)
		distances[start_node] = 0
		pq = [(0, start_node)] # Prioritaetswarteschlange (Distanz, Knoten)
		...
		x, y, z = current_node
		# Iteriere ueber alle 26 Nachbarn im 3D-Gitter
		for dx in [-1, 0, 1]:
		for dy in [-1, 0, 1]:
		for dz in [-1, 0, 1]:
		...
		GRID_SIZE = 100 # Gittergroesse fuer die Simulation
		START_NODE = (0, 50, 50)
		END_NODE = (99, 50, 50)
		...
		print("2. Berechne geodaetischen Pfad durch das Feld...")
		...
		print(f"Effektive Pfadlaenge (Leff): {effective_path_length:.4f} Einheiten")
		...
		# z = d * C * xi => H0 = c * C * xi
		# Fuer unsere Simulation normalisieren wir d auf 1 Mpc
		...
		print("\nErgebnis: Die Simulation bestaetigt, dass die Rotverschiebung als")
