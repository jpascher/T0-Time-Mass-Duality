...
\chapter{Temperatureinheiten in nat\"urlichen Einheiten: \\
		T0-Theorie und statisches Universum \\
		($\xi$-basierte universelle Methodik)\\
		Einschlie\ss{}lich vollst\"andiger CMB-Berechnungen und kosmologischer Rotverschiebung}
...
\title{Temperatureinheiten in nat\"urlichen Einheiten: \\
	...
	Das Herz der T0-Theorie bildet eine universelle dimensionslose Konstante, die wir mit dem griechischen Buchstaben $\xi$ (Xi) bezeichnen. Diese Konstante wurde urspr\"unglich rein geometrisch aus den fundamentalen T0-Feldgleichungen abgeleitet, wie in der etablierten T0-Theorie \cite{061_T0Theory} gezeigt.
...
\end{itemize}
	...
	\begin{enumerate}
		...
		\begin{equation}
			\Tfield \cdot m(x) = 1
			\label{061_eq:time_mass_duality}
		\end{equation}
		...
		\begin{equation}
			...
			\label{061_eq:modified_metric}
		\end{equation}
	\end{enumerate}
	
	\section{Leistungsspektren-Berechnungen}
	\label{061_sec:power_spectra}
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{lcc}
			...
			\bottomrule
		\end{tabular}
		\label{061_tab:parameters}
	\end{table}
	
	\section{Aufl\"osung kosmologischer Spannungen}
	\label{061_sec:tensions}
