% Chapter file: 091_Casimir_De_ch.tex
% Source: 091_Casimir_De.tex
...
\title{Vereinheitlichung von Casimir-Effekt und kosmischer Hintergrundstrahlung: Eine fundamentale Vakuum-Theorie}
	...
	\begin{align}
		...
		L_\xi &= \text{charakteristische Vakuum-Längenskala} \approx \SI{100}{\micro\meter} \label{091_eq:Lxi_definition}
	\end{align}
	...
	Durch Einsetzen der CMB-Vakuum-Beziehung \eqref{091_eq:cmb_vacuum_relation} in die modifizierte Casimir-Formel \eqref{091_eq:modified_casimir} ergibt sich:
	...
	\begin{align}
		...
		L_\xi &= \SI{100}{\micro\meter} \quad \text{(Casimir-charakteristisch)} \label{091_eq:Lxi_value}
	\end{align}
	...
	\begin{itemize}
		...
		\item Kugel-Platte: \( \SI{1.75}{\micro\meter} \) \cite{091_xu2022}.
	...
	\begin{equation}
		L_\xi = L_\xi(\text{Geometrie}, \text{Materialien}, \omega) \label{091_eq:Lxi_dependencies}
	\end{equation}
	...
	Die Zeta-Funktion-Regularisierung führt durch analytische Fortsetzung der Spektral-Zeta-Funktion auf die regulierte Energie bei $s=-1$. Für Details siehe Anhang~\ref{091_app:zeta_full}.
	...
	Die Funktion $\xi(\gamma)$ und deren Unsicherheitsband (Monte-Carlo über $\alpha\in[0.5,2]$) ist in Abbildung~\ref{091_fig:xi_gamma_mc} dargestellt.
	...
	Für das Modell $\delta(\xi)=\beta\ln\xi$ gilt die implizite Gleichung $\xi=A_{3+\beta\ln\xi}$; numerische Lösungen sind in Abbildung~\ref{091_fig:xi_vs_beta} dargestellt.
	...
	\]
	Gleichsetzen mit \eqref{091_eq:rho_Ad}$'$ liefert
	\[
	...
	\begin{itemize}
		...
		\item Für einen Kontinuums-Impulsraum mit Modendichte \(\rho(\omega) \sim \omega^{d-1}\) kann man die Zeta-Integrale explizit auswerten; das Ergebnis besitzt dieselben Gamma-Faktoren wie in \eqref{091_eq:rho_Ad} und führt konsistent auf die Form \(\rho\propto A_d k_{\max}^{d+1}\) nach geeigneter Behandlung von Polstellen.
	\end{itemize}
