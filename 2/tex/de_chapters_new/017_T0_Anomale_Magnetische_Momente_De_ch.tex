% Chapter file: 017_T0_Anomale_Magnetische_Momente_De_ch.tex
% Source: 017_T0_Anomale_Magnetische_Momente_De.tex
% Generated from standalone document

\chapter{Erweiterte Lagrange-Dichte mit Zeitfeld zur Erklärung des Myon \(g-2\)-Anomalie - 
	Die T0-Theorie: Zeit-Masse-Dualität und anomale magnetische Momente - 
	Vollständige theoretische Ableitung ohne freie Parameter}
...
\thispagestyle{fancy}
	...
	\begin{align}
		...
		\Delta a_\mu &= 251(59) \times 10^{-11} \quad (4,2\sigma) \label{017_eq:old_discrepancy}
	\end{align}
	
	\paragraph{Aktualisierte Situation (2025):}
	Durch verbesserte Lattice-QCD-Berechnungen des hadronischen Vakuumpolarisationsbeitrags hat sich die Diskrepanz reduziert\cite{017_sm_g2_2025,017_mug2_final_2025}:
	\begin{align}
		a_\mu^{\text{exp}} &= 116\,592\,070(14) \times 10^{-11}\\
		a_\mu^{\text{SM}} &= 116\,592\,033(62) \times 10^{-11}\\
		\Delta a_\mu &= 37(64) \times 10^{-11} \quad (0,6\sigma) \label{017_eq:new_discrepancy}
	\end{align}
	...
	Die hier vorgestellte Erweiterung basiert auf der T0-Theorie\cite{017_pascher_t0_theory_2025}, die eine fundamentale Dualität zwischen Zeit und Masse postuliert:
	\begin{equation}
		T \cdot m = 1 \quad \text{(in natürlichen Einheiten)}
	\end{equation}
	
	Diese Dualität führt zu einem neuen Verständnis der Raumzeit-Struktur, wobei ein Zeitfeld $\Delta m(x,t)$ als fundamentale Feldkomponente erscheint\cite{017_pascher_lagrangian_extended_2025}.
	...
	\begin{align}
		\mathcal{L}_{\text{SM}} &= -\tfrac{1}{4} F_{\mu\nu}F^{\mu\nu} + \bar{\psi}(i\gamma^\mu D_\mu - m)\psi \label{017_eq:sm_lagrangian}\\
		F_{\mu\nu} &= \partial_\mu A_\nu - \partial_\nu A_\mu \label{017_eq:field_tensor}\\
		D_\mu &= \partial_\mu + ieA_\mu \label{017_eq:covariant_derivative}
	\end{align}
	...
	\begin{equation}
		...
		\label{017_eq:time_field_lagrangian}
	\end{equation}
	
	Hier ist $m_T$ die charakteristische Zeitfeldmasse. Die Normierung folgt aus der postulierten Zeit-Masse-Dualität und der Anforderung der Lorentz-Invarianz\cite{017_pascher_mathematical_structure_2025}.
	...
	\begin{align}
		\mathcal{L}_{\text{Wechselwirkung}} &= g_T^\ell \, \bar{\psi}_\ell \psi_\ell \, \Delta m \label{017_eq:interaction_lagrangian}\\
		g_T^\ell &= \xi \, m_\ell \label{017_eq:coupling_strength}
	\end{align}
	
	Der universelle geometrische Parameter $\xi$ ist fundamental bestimmt durch:
	\begin{equation}
		\xi = \frac{4}{3} \times 10^{-4} = 1,333 \times 10^{-4}
		\label{017_eq:xi_parameter}
	\end{equation}
	...
	\begin{align}
		...
		\label{017_eq:extended_lagrangian}
	\end{align}
	...
	Für einen skalaren Mediator mit Kopplung an Fermionen ist der allgemeine Beitrag zum anomalen magnetischen Moment gegeben durch\cite{017_peskin_schroeder_1995}:
	\begin{equation}
		\Delta a_\ell = \frac{(g_T^\ell)^2}{8\pi^2} \int_0^1 dx \frac{m_\ell^2 (1-x)(1-x^2)}{m_\ell^2 x^2 + m_T^2 (1-x)}
		\label{017_eq:one_loop_general}
	\end{equation}
	...
	\begin{align}
		\Delta a_\ell &\approx \frac{(g_T^\ell)^2}{8\pi^2 m_T^2} \int_0^1 dx \, (1-x)(1-x^2) \label{017_eq:heavy_limit}\\
	...
	Die Zeitfeldmasse wird über eine Verbindung zum Higgs-Mechanismus bestimmt\cite{017_pascher_higgs_connection_2025}:
	\begin{equation}
		m_T = \frac{\lambda}{\xi} \quad \text{mit} \quad \lambda = \frac{\lambda_h^2 v^2}{16\pi^3}
		\label{017_eq:higgs_connection}
	\end{equation}
	
	Einsetzen in Gleichung \eqref{017_eq:heavy_limit} ergibt die fundamentale T0-Formel:
	\begin{equation}
		\Delta a_\ell^{\text{T0}} = \frac{5\xi^4}{96\pi^2\lambda^2} \cdot m_\ell^2
		\label{017_eq:t0_fundamental_formula}
	\end{equation}
	...
	\begin{thebibliography}{20}
		...
		\bibitem{017_muong2_fermilab_2021}
		...
		\bibitem{017_pascher_lagrangian_extended_2025}
		Pascher, J. (2025). 
		...
		\bibitem{017_pascher_mathematical_structure_2025}
		Pascher, J. (2025). 
		...
		\bibitem{017_pascher_higgs_connection_2025}
		Pascher, J. (2025). 
		...
		\bibitem{017_peskin_schroeder_1995}
