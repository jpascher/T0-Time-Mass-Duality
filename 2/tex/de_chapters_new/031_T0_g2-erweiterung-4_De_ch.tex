...
\chapter{T0-Time-Mass-Dualitäts-Theorie: Finale Erweiterung auf Hadronen Physikalisch abgeleitete...}
...
\section*{Abstract}
	...
	\label{031_sec:parameter}
	
	\subsection{Etablierte Parameter}
	\label{031_subsec:parameter}
	
	\begin{align}
		\xi &= \frac{4}{30000} = 1.333 \times 10^{-4}, \label{031_eq:xi} \\
		D_f &= 3 - \xi = 2.999867, \label{031_eq:Df} \\
		K_{\text{frak}} &= 1 - 100\xi = 0.986667, \label{031_eq:K} \\
		E_0 &= \frac{1}{\xi} = \SI{7500}{\giga\electronvolt}, \label{031_eq:E0} \\
		m_T &= \SI{5.22}{\giga\electronvolt}, \label{031_eq:mT} \\
		F_{\text{dual}} &= \frac{1}{1 + (\xi E_0/m_T)^{-2/3}} = 0.249 \label{031_eq:F_dual}
	\end{align}
	
	\subsection{Validierte Leptonen-Formel}
	\label{031_subsec:leptonen_formel}
	
	\begin{equation}
		a_\ell^{T0} = \frac{\alpha K_{\text{frak}}^2 m_\ell^2}{48\pi^2 m_T^2} \cdot F_{\text{dual}}
		\label{031_eq:lepton_formel}
	\end{equation}
	...
	\label{031_sec:hadronen_formel}
	
	\subsection{Universeller QCD-Faktor}
	\label{031_subsec:universeller_faktor}
	...
	\label{031_subsec:finale_formel}
	
	\begin{equation}
		a_{\text{hadron}}^{T0} = a_\mu^{T0} \cdot \left(\frac{m_{\text{hadron}}}{m_\mu}\right)^2 \cdot \CQCD \cdot \Kspec
		\label{031_eq:hadron_final}
	\end{equation}
	
	\subsection{Physikalisch abgeleitete Korrekturfaktoren}
	\label{031_subsec:korrekturfaktoren}
	
	\begin{align}
		K_{\text{Proton}} &= 1.000 \quad \text{(Referenz)} \label{031_eq:K_proton} \\
		K_{\text{Neutron}} &= 1.067 \quad \text{(Spin-Struktur)} \label{031_eq:K_neutron} \\
		K_{\text{Strange}} &= 0.054 \quad \text{(Konfinement)} \label{031_eq:K_strange} \\
		K_{\text{Up}} &= 1.2 \times 10^{-4} \quad \text{(starke Dämpfung)} \label{031_eq:K_up} \\
		K_{\text{Down}} &= 5.0 \times 10^{-4} \quad \text{(starke Dämpfung)} \label{031_eq:K_down}
	\end{align}
	...
	\label{031_sec:ergebnisse}
	
	\subsection{Experimentelle Referenzdaten}
	\label{031_subsec:daten}
	...
	\begin{table}[H]
		...
		\label{031_tab:ergebnisse}
	\end{table}
	
	\subsection{Beispielrechnungen}
	\label{031_subsec:beispiele}
	...
	\label{031_sec:interpretation}
	
	\subsection{Fraktale QCD-Erweiterung}
	\label{031_subsec:fraktale_qcd}
	...
	\label{031_subsec:universalitaet}
	...
	\label{031_sec:zusammenfassung}
	
	\subsection{Erreichte Ergebnisse}
	\label{031_subsec:ergebnisse_zusammenfassung}
	...
	\label{031_subsec:schlussfolgerung}
