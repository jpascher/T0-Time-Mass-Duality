...
\begin{document}
	...
	\end{equation}
	This mesh represents the \glqq{}granular\grqq{}, fluctuating geometry of the T0 vacuum, determined by the constant $\xiT$.
	...
	The central discovery of the simulation is that the sum of these tiny \glqq{}detours\grqq{} causes the **effective total path length, $\Leff$, to be systematically longer** than the direct Euclidean distance $d$ between the source and the observer.
	...
	Since the geodesic path is a property of spacetime geometry itself, it is identical for all particles that follow it. A red and a blue photon starting at the same location will take the exact same \glqq{}detour\grqq{}. Their wavelengths are therefore stretched by the same percentage. This effortlessly explains the observed frequency independence of cosmological redshift, a point where simple \glqq{}Tired Light\grqq{} models fail.
	...
	\end{align*}
	This value is within the range of experimentally measured values \cite{riess2019} and offers a natural explanation for the \glqq{}Hubble tension,\grqq{} as slight variations in the mesh geometry in different directions could lead to different measured values.
...
\begin{lstlisting}[language=Python, caption={Conceptual Python code for the FEM simulation of geometric redshift.}, label={lst:fem_code}, basicstyle=\ttfamily\scriptsize, breaklines=true]
import numpy as np
import heapq
...
def simulate_t0_field(grid_size):
    \glqq{}\grqq{}\glqq{}Simulates a static T0 vacuum field with fluctuations.\grqq{}\glqq{}\grqq{}
    np.random.seed(42)
...
def calculate_path_cost(field_value):
    \glqq{}\grqq{}\glqq{}The cost (effective distance) to traverse a grid node.\grqq{}\glqq{}\grqq{}
    return 1.0 * field_value

def find_geodesic_path(t0_field, start_node, end_node):
    \glqq{}\grqq{}\glqq{}Finds the shortest path (geodesic) using Dijkstra's algorithm.\grqq{}\glqq{}\grqq{}
...
print(\glqq{}1. Simulating T0 vacuum field...\grqq{})
t0_vacuum = simulate_t0_field(GRID_SIZE)

print(\glqq{}2. Calculating geodesic path through the field...\grqq{})
...
print(f\glqq{}\n--- Results ---\grqq{})
print(f\glqq{}Euclidean Distance (d): {euclidean_distance:.4f} units\grqq{})
print(f\glqq{}Effective Path Length (Leff): {effective_path_length:.4f} units\grqq{})

redshift_z = (effective_path_length - euclidean_distance) / euclidean_distance
print(f\glqq{}Geometric Redshift (z): {redshift_z:.6f}\grqq{})

# Derivation of the Hubble Constant
dist_Mpc = 1.0
z_per_Mpc = redshift_z / euclidean_distance * (3.26e6 * GRID_SIZE)
H0_simulated = C_SPEED * z_per_Mpc
H0_formula = C_SPEED * GEOMETRIC_FACTOR_C * XI * 3.26e6 / (1e3)

print(\glqq{}\n--- Cosmological Prediction ---\grqq{})
print(f\glqq{}Simulated Hubble Constant (H0): {H0_simulated:.2f} km/s/Mpc\grqq{})
print(f\glqq{}Formula-based Hubble Constant (H0): {H0_formula:.2f} km/s/Mpc\grqq{})
\end{lstlisting}
