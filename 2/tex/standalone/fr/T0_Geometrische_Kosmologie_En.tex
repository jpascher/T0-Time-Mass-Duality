...
\begin{document}
	...
	\end{equation}
	This mesh represents the \og{}granular\fg{}, fluctuating geometry of the T0 vacuum, determined by the constant $\xiT$.
	...
	The central discovery of the simulation is that the sum of these tiny \og{}detours\fg{} causes the **effective total path length, $\Leff$, to be systematically longer** than the direct Euclidean distance $d$ between the source and the observer.
	...
	Since the geodesic path is a property of spacetime geometry itself, it is identical for all particles that follow it. A red and a blue photon starting at the same location will take the exact same \og{}detour\fg{}. Their wavelengths are therefore stretched by the same percentage. This effortlessly explains the observed frequency independence of cosmological redshift, a point where simple \og{}Tired Light\fg{} models fail.
	...
	\end{align*}
	This value is within the range of experimentally measured values \cite{riess2019} and offers a natural explanation for the \og{}Hubble tension,\fg{} as slight variations in the mesh geometry in different directions could lead to different measured values.
...
def simulate_t0_field(grid_size):
    \og{}\fg{}\og{}Simulates a static T0 vacuum field with fluctuations.\fg{}\og{}\fg{}
    np.random.seed(42)
...
def calculate_path_cost(field_value):
    \og{}\fg{}\og{}The cost (effective distance) to traverse a grid node.\fg{}\og{}\fg{}
    return 1.0 * field_value

def find_geodesic_path(t0_field, start_node, end_node):
    \og{}\fg{}\og{}Finds the shortest path (geodesic) using Dijkstra's algorithm.\fg{}\og{}\fg{}
...
print(\og{}1. Simulating T0 vacuum field...\fg{})
t0_vacuum = simulate_t0_field(GRID_SIZE)

print(\og{}2. Calculating geodesic path through the field...\fg{})
...
print(f\og{}\n--- Results ---\fg{})
print(f\og{}Euclidean Distance (d): {euclidean_distance:.4f} units\fg{})
print(f\og{}Effective Path Length (Leff): {effective_path_length:.4f} units\fg{})

redshift_z = (effective_path_length - euclidean_distance) / euclidean_distance
print(f\og{}Geometric Redshift (z): {redshift_z:.6f}\fg{})
...
print(\og{}\n--- Cosmological Prediction ---\fg{})
print(f\og{}Simulated Hubble Constant (H0): {H0_simulated:.2f} km/s/Mpc\fg{})
print(f\og{}Formula-based Hubble Constant (H0): {H0_formula:.2f} km/s/Mpc\fg{})
\end{lstlisting}
