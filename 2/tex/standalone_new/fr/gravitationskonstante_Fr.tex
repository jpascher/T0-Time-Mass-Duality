...
\chapter{Gravitational Constant Analysis}
	...
	Solving Equation \eqref{gravitationskonstante:eq:t0_fundamental} for $G$ yields:
	...
	Verification of Equation \eqref{gravitationskonstante:eq:g_fundamental}:
	...
	The formula \eqref{gravitationskonstante:eq:g_natural} yields $G$ in natural units (dimension $[E^{-1}]$). For experimental verification, we need $G$ in SI units with dimension $[\text{m}^3 \text{kg}^{-1} \text{s}^{-2}]$.
	...
	The T0-formula \eqref{gravitationskonstante:eq:g_complete} shows:
