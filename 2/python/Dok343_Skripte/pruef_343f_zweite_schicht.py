...
print('GF(27)* ≅ Z₂₆; Klassen als Frobenius-Orbits der Exponenten (Dok. 342: f1={g,g³,g⁹}, f2={g⁵,..}, f3={g⁷,..}, f4={g¹¹,..})\n')
...
prim=[k for k in range(26) if gcd(k,26)==1]
orbits=[]; seen=set()
for k in prim:
    if k in seen: continue
    o=sorted({k,frob(k),frob(frob(k))}); orbits.append(o); seen.update(o)
names={}
for i,o in enumerate(orbits): names[tuple(o)]=f"f{i+1}"
print("Primitive Klassen (Frobenius-Orbits in Z₂₆*, Ordnung 26):")
for o in orbits: print(f"  {names[tuple(o)]} = {o}")
# Ordnung-13-Klassen
ord13=[k for k in range(26) if k%2==0 and k%13!=0]
orb13=[]; seen=set()
for k in ord13:
    if k in seen: continue
    o=sorted({k,frob(k),frob(frob(k))}); orb13.append(o); seen.update(o)
for i,o in enumerate(orb13): names[tuple(o)]=f"g{i+1}"
...
print("""
Ablesung [B]:
  Sektorpaarung (Inversion):  f1↔f4, f2↔f3 — zwei Paare (Dok. 343 D').
  Negation:                   f_i ↔ g_j — Ordnung 26 ↔ 13 (Dok. 343 D'').
...
L=[o for o in orbits if cls(o[0]) in("f2","f3")]; S=[o for o in orbits if cls(o[0]) in("f1","f4")]
print("Schicht {f2,f3} (Leptonen):  Z₁₃-Orbits", [z13orb[z13set(o)[0]] for o in L])
print("Schicht {f1,f4} (zweite):    Z₁₃-Orbits", [z13orb[z13set(o)[0]] for o in S])
print()
...
Quantenzahlen aus der Algebra [B]:
  Schicht {f2,f3} ≙ Z₁₃-Orbits O2,O4 (zueinander invers)  → Dirac-Profil: Teilchen ≠ Antiteilchen
  Schicht {f1,f4} ≙ Z₁₃-Orbits O1,O3 (selbstinvers)       → Majorana-Profil
  Beide Schichten: Z₂-Parität 1 (massiver Sektor, Dok. 339), keine Farbe, drei Zustände je Klasse.
  Sektorpaarung k→−k: f1↔f4 (O1↔O3), f2↔f3 (O2↔O4).
Deutung [S]:
  {f2,f3} = geladene Leptonen (Dirac).  {f1,f4} = Neutrinosektor (Majorana); Dok. 340 setzt die
  aktiven Neutrinos in O3 = f4.  Offen bleibt f1 = O1 {1,3,9}: Majorana, drei Generationen,
...
Korrektur: Dok. 341 legt für Leptonen nur Ordnung 26 fest, keine Klasse; die Zuordnung
  Leptonen→{f2,f3} folgt erst hier aus dem Dirac/Majorana-Kriterium.
""")
