...
W, H = landscape(A4)
OUT = "/home/claude/SM_Teilchen_GF27_De.pdf"
...
c.drawCentredString(LBL_W+FW/2, ROW1+R+PAD+6, "SU(2)L-Dublett = Sd-Sektor [B] Dok.349")
c.drawCentredString(LBL_W+FW/2, ROW3+R+PAD+6, "SU(2)L-Dublett = Sd-Sektor [B] Dok.349")
...
c.drawString(RX, H-36, "Eichbosonen")
...
c.drawString(RX, BY1-10, "GF(3) Fixkörper [B] Dok.339")
...
c.setFont("Helvetica",7); c.setFillColorRGB(0.2,0.2,0.2)
c.drawString(RX, BY2-10, "Gal(GF(9)/GF(3))=Z2 [K] Dok.336")

# Gluonen 2×4
c.setFont("Helvetica-Bold",9); c.setFillColorRGB(0,0,0)
c.drawString(RX,H-168,"Gluonen g×8")
gsq=24; ggap=3
for gi in range(8):
...
c.setFont("Helvetica",7); c.setFillColorRGB(0.2,0.2,0.2)
c.drawString(RX,H-238,"4 Z13-Orbits×2 Z2-Par.=8 [B]")
...
c.drawString(RX,HY-19,"T~·m=1 statt Higgs (Dok.019)  Exp. gesichert [X]")
...
c.drawString(RX+4,LEG_TOP-13,"Legende")
