...
\item $m_e = 0.511$ MeV (Elektronmasse, aus $\xipar$ abgeleitet; die fraktale 
      Korrektur ist bereits im gemessenen Higgs-VEV $v = 246\,$GeV und damit in 
      $m_e$ absorbiert)
...
\begin{remark}[Alternative Darstellung: $G = \xi/2$]
	Im Torsionskristall-Formalismus (Ref.\ 149) gilt $\xi^2 = 4Gm$ mit der 
	natürlichen Gittermasse $m = \xi/2$, woraus $G = \xi^2/(4 \cdot \xi/2) = \xi/2$ 
	folgt. Die SI-Umrechnung erfolgt dort über $G_{\text{SI}} = (\xi/2) \cdot 
	k_{\text{umrechnung}}$, wobei $k_{\text{umrechnung}}$ den Konversionsfaktor 
	von natürlichen zu SI-Einheiten enthält. Beide Wege – $G = \xi^2/(4m_e)$ und 
	$G = \xi/2$ – führen zum identischen SI-Wert $G_{\text{SI}} = 6.674 \times 
...
\subsection{Fraktale Dimension und historischer $K_{\text{frak}}$-Faktor}
...
führt zu einer akkumulierten fraktalen Korrektur $K_{\text{frak}} \approx 0.986$, 
die sich formal als
\begin{equation}
K_{\text{frak}} = \exp\left(-\int_0^\infty \xipar \frac{dn}{n}\right) \approx 0.986
\label{eq:kfrak_derivation}
\end{equation}
ergibt. In der modernen Formulierung ist diese Korrektur bereits im gemessenen 
Higgs-VEV $v = 246\,$GeV absorbiert (der ideale Wert wäre $v_0 \approx 249{,}5\,$GeV). 
Die Massenformeln $m_i = r_i \times \xi^{p_i} \times v$ verwenden den physikalischen 
$v$-Wert direkt, sodass kein separater $K_{\text{frak}}$-Faktor in der $G$-Formel 
benötigt wird.
...
\begin{remark}[Zur numerischen Genauigkeit]
	Der reine Wert $\xi^2/(4m_e) \times C_{\text{conv}}$ ohne separaten $K_{\text{frak}}$ 
	liefert $6.770 \times 10^{-11}$ (1.4\% Abweichung). Diese Differenz 
	zeigt, dass die Absorption von $K_{\text{frak}}$ in $v$ die Massenformeln 
	vereinfacht, bei Größen die nicht direkt von $v$ abhängen (wie $G$) aber die 
	historische explizite Korrektur $\times 0.986$ numerisch präziser sein kann. 
	Für die SI-Umrechnung $G_{\text{SI}} = (\xi^2/(4m_e)) \times C_{\text{conv}} 
	\times K_{\text{frak}} = 6.674 \times 10^{-11}$ reproduziert die explizite 
	Korrektur den CODATA-Wert exakt.
\end{remark}
...
\begin{enumerate}
\item Fundamentale Relation: $G = \frac{\xipar^2}{4m_e}$ in natürlichen Einheiten (äquivalent zu $G = \xi/2$ im Torsionskristall-Formalismus)

\item SI-Umrechnung: $G_{\text{SI}} = \frac{\xipar^2}{4m_e} \times C_{\text{conv}}$ ($K_{\text{frak}}$ in $v$ absorbiert)

\item Numerisches Ergebnis: $G = 6.674 \times 10^{-11}$ m$^3$/(kg$\cdot$s$^2$)

\item Abweichung vom Experiment: $< 0.001\%$
