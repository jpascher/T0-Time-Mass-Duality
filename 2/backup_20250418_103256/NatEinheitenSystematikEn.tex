...
\begin{document}
	...
	\]
	\[
	\Phi(r) = -\frac{r_g}{r} + \kappa^{\text{nat}} r, \quad \kappa^{\text{nat}} = \beta_T^{\text{nat}} \cdot \frac{y v}{r_g^2}, \quad [\kappa^{\text{nat}}] = [E]
	\]
	\[
	y = \frac{1}{m_h^2}, \quad \beta_T^{\text{nat}} = 1
	\]
	\[
	...
	\begin{figure}[htbp]
		...
		\begin{tikzpicture}
			...
			\draw[thick, red, ->] (8,2) -- (11,1.5) node[midway, above] {$\beta_T^{\text{nat}} = 1$};
			\node[draw, ellipse, fill=red!10] at (6,3.9) {T0 Model: $\hbar = c = G = \alpha_{\text{EM}} = \beta_T^{\text{nat}} = 1$};
		\end{tikzpicture}
