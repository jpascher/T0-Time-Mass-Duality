...
document (algebraic coupling parameter, via $\lambda_h^2 v^2/(16\pi^3 m_h^2)$)
is \textbf{withdrawn} (R61, Doc.~190-Archive-2).

\textbf{Reason:} $\xi_\text{Higgs}$ was selected \emph{because} it hits the
measured window --- the two-spaces doctrine (geometric $\xi_0$ vs.\ algebraic
$\xi_\text{Higgs}$) is rationalisation after the selection, not an independent
justification. A data-driven parameter is not a second fundamental parameter.

\textbf{Valid:} $\xi_0 = 4/30000$ as the geometric ratio $L_0/\ell_P$.
Calculations in this document using $\xi_\text{Higgs}$ are to be classified
as preliminary work; for canonical derivations only $\xi_0$ applies.
