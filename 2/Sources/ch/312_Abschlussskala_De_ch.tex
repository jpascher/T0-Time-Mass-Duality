...
\textbf{Nachtrag: topologische Periode vs.\ Lichtumlauf
(Dok.~313).} Die Periode des Zeitzyklus ist eine
\emph{topologische} Größe, $\tau_c = L^{*}/c = 91{,}9$~Gyr. Ein
Photon bräuchte für eine Umrundung wegen der fraktalen
Wegverlängerung $1{,}35\,\%$ länger ($93{,}2$~Gyr). Für KMS,
Unruh und Gibbons--Hawking zählt die \emph{Feldperiodizität},
nicht die Photonlaufzeit: $T = \hbar H_0/(2\pi k_B)$, das Quant
...
\begin{enumerate}
  \item \textbf{Matched-Circle-Versatz.} Bei kompakter Topologie
        ...
        Richtungssignatur. \textbf{Nachtrag (Dok.~313):} Mit der
        fraktalen Wegkorrektur rückt die Streuwand auf
        $0{,}979\,\pi$ und schneidet sich nicht mehr selbst --
        dann existieren keine Matched Circles und diese Signatur
        entfällt. Sie gilt nur in der glatt gerechneten Fassung;
        als Prüffläche verbleibt die Nicht-Monotonie am Antipoden
        (Dok.~313).
...
Ruhesystem & Topologie zeichnet global ein System aus; Kandidat:
  ...
  (entfällt bei fraktaler Wegkorrektur, Dok.~313) & [K]/[S] \\
...
\textbf{Nachtrag (fraktale Wegkorrektur, Dok.~313).} $D_\text{LSS}$
ist ein \emph{Lichtweg}, $L^{*}$ eine topologische Größe -- nach
der Zuordnungsregel greift $K_\text{frak} = 1-100\xi$ hier. Damit
verschiebt sich das Verhältnis auf $L^{*}/D_\text{LSS} = 1{,}03$:
Die Streuwand liegt \emph{deutlicher} unter dem Halbzyklus. Das
ist mit den Nullresultaten der Topologiesuchen verträglicher, kostet
aber die Matched-Circle-Signatur (siehe Nachtrag im Kapitel
Relativbewegung).
