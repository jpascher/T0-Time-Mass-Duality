...
\section{Der erste Test ist überholt: worauf er sich reduziert}
Der erste Prüfversuch -- der Homologie-Check \texttt{ffgft\_hlv\_homology\_check\_v3.py} --
war ein topologischer Punktwolken-Diagnostiker. Er projizierte das Cut-and-Project in eine
dreidimensionale Punktwolke, baute darüber eine kNN-Filtration und mass die persistente
$H_1$-Homologie, HLV gegen kristalline und quasikristalline Kontrollen. Dieser Zugang ist
metrik- und $k$-abhängig, an eine endliche Wolke gebunden und in dem Sinne verlustbehaftet,
dass er die Struktur nur über einen groben topologischen Schatten sieht.

Mit der in Dok.~293 formulierten echten Hilbertraum-Variante -- auf der Linie von Dok.~230/282
-- ist dieser erste Test überholt. Dieselbe Struktur liegt dort exakt und verlustfrei vor: als
$C_3$-Fourier-Moden auf der $\mathbb{C}^3$-Faser, als $Z_3$-Zirkulant, mit der
$A_5$-Einbettung über $R_5$; und $\theta=2/9$ erscheint als exakte spektrale Invariante
$p_0=\lvert\langle v_0|R_5 v_e\rangle\rvert^2=2/9$, unmittelbar berechnet statt über eine
verrauschte Topologie geschätzt.

Damit reduziert sich der erste Test auf einen \emph{groben Quasiperiodizitäts-Detektor}. Er
kann bescheinigen, dass die HLV-Wolke quasiperiodisch und nicht kristallin ist -- und nichts
Feineres. Für die eigentlich tragende Unterscheidung ist er strukturell blind: die persistente
$H_1$ einer Punktwolke sieht die Achsen-Winkel-Struktur nicht, an der $2/9$ hängt. Der goldene
Wert $\tau=\varphi$ (exakt $2/9$) und der Kontrollwert $\tau=\sqrt2$ ($0{,}2139$) erzeugen
topologisch nahezu ununterscheidbare Wolken; der Unterschied lebt im Winkelspektrum, das die
Homologie nicht auflöst.

Zwei Folgen ergeben sich. Erstens ist der frühere Befund einer \enquote{nur schwachen Trennung
von generisch} keine Aussage über $\varphi$ oder $A_5$, sondern die erwartbare Blindheit des
groben Statistikums -- zusätzlich verschärft dadurch, dass die herangezogene
\enquote{generische} Kontrolle selbst aus dem ikosaedrischen Projektor mit $\tau=\sqrt2$ gebaut
war und deren Winkelspektrum ($61{,}87^\circ$ und $70{,}53^\circ$) strukturiert und nicht
gestreut ist. Zweitens ist das Mittel gegen diese Blindheit nicht eine sauberere Kontrolle für
den groben Test, sondern die exakte spektrale Invariante der Hilbertraum-Variante selbst. Sie
löst genau das auf, wofür die Punktwolken-Topologie blind war.

Der Befund vom geteilten Objekt ruht deshalb auf der exakten Hilbertraum-Formulierung, nicht auf
dem überholten Homologie-Diagnostiker. Die $\varphi$-Spezifität von $\theta=2/9$ ist allein in
der spektralen Variante sichtbar.
...
Der frühere Kontrast über die persistente Homologie ist dagegen erledigt, nicht offen -- er ist
durch die exakte Hilbertraum-Variante überholt und war für die $\varphi$-Frage ohnehin
strukturell blind.
