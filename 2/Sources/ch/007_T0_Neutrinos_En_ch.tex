...
\begin{tabular}{lcccc}
	...
	\textbf{Addendum (3 September 2026, Doc.~340 and Doc.~343 Theorem~D$'''$)}\\[3pt]
	The base formula $m_\nu = \frac{\xi^2}{2} \times m_e = 4.54$~meV agrees exactly
	with $m_{\nu_1}$ in Doc.~340, which derives the neutrino masses algebraically from
	the GF$(27)^*$ Galois orbits \textbf{[K]}. Doc.~007 thereby provides the physical
	motivation for the double $\xi$ suppression (neutrino $=$ almost photon $+$
	geometric decoupling) that Doc.~340 treats only algebraically.
	Neither $\xi$ nor $m_e$ are free parameters:
	$\xi$ follows from Galois group orders \textbf{[K]}\ (Doc.~338), $m_e$
	from torus quantum numbers \textbf{[K]}\ (Doc.~006); $v=246$~GeV is
	an SI scale fixation, not a free parameter.

	What Doc.~340 additionally achieves \textbf{[B]}:
	\begin{itemize}\setlength{\itemsep}{1pt}
	  \item Three different masses from orbits $O_1, O_3, O_4$:
	        $m_{\nu_1}=4.54$, $m_{\nu_2}=9.81$, $m_{\nu_3}=49.9$~meV ---
	        $\Delta m^2_{21}$ and $\Delta m^2_{31}$ reproduced to 0.4 and 1.5\,\%.
	  \item Majorana/Dirac partition: $\nu_1,\nu_2$ Majorana (orbits $O_1,O_3$
	        self-inverse); $\nu_3$ \textbf{Dirac neutrino} (orbit $O_4$,
	        Doc.~343 Theorem~D$'''$).
	  \item No Majorana phase for $\nu_3$; $m_{ee} \leq 14.4$~meV; inverted hierarchy
	        more strongly excluded.
	\end{itemize}
	The Koide extension and the n/\ell/j quantum numbers have no connection to the
	Galois orbits and remain \textbf{[S]}.
	Authoritative current state: Doc.~340 and Doc.~343.
	\end{minipage}}
	\end{center}
