...
\paragraph{Two readings of the correction.}
Doc.~352 (§\,10) takes the T0 ladder as fundamental: the $\varepsilon_i$ are
distances between bare and PDG, decreasing with generation
($\varepsilon_e>\varepsilon_\mu>\varepsilon_\tau$), and their origin is
attributed to QED extraction and SI projection -- as an attribution \SM, with the
admission that the naive QED self-energy goes the wrong way. That is Reading~A.

Doc.~367 permits a Reading~B: the Koide formula with $\theta=2/9$ is
fundamental, parameter-free from the icosahedron. The T0 ladder $r_k\,\xi^{p_k}$
is then a \emph{rational approximation} to the transcendental values $a_k^2$
with $a_k = 1+\sqrt2\cos(2/9+2\pi k/3)$. The residuals $-39\,\xi$, $-77\,\xi$,
$-116\,\xi$ emerge in Reading~B without PDG and without QED -- as the distance
between cosine and power law \KM. They differ because the three cosine values
differ: a power law $\xi^{p}$ has fixed slope $\log\xi=-8{.}92$, the Koide slopes
$2\Delta\log a/\Delta p$ are $-10{.}66$ ($e\to\mu$) and $-8{.}47$ ($\mu\to\tau$);
the ratios $r_j/r_i$ absorb the difference, the remainder is $\varepsilon$.
No deeper reason is needed: the cosine is not a power law.

Both readings reproduce the same numbers. They differ in what they declare open.
Reading~A needs the R113 bridge (QED from $T^4/\mathbb{Z}_3$). Reading~B does
not; its open question is instead: why are the rational approximation parameters
exactly the GF$(3^n)$ winding numbers, and why is the approximation so good
($0{.}5\,\%$)? That is a question for number theory, not for QED. Reading~B is
the more economical one (Register R121).
