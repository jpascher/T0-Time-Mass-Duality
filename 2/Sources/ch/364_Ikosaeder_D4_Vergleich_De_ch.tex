...
\midrule
\multicolumn{2}{l}{\textit{Graphen und Träger}} \\
$n$ & Anzahl der Ports; $n=12$ (Ikosaeder) \\
$A$ & Adjazenzmatrix des Trägers; $A_{ij}=1$ wenn Naht $(i,j)$ \\
$L=5I-A$ & Laplace-Operator; 5-regulärer Graph \\
$W=A/5$ & Mittelungsoperator (Erwartungswert Einzelnaht-Update) \\
$\mu$ & Laplace-Eigenwert; $\mu\in\{0,\,5{-}\sqrt5,\,6,\,5{+}\sqrt5\}$ \\
\midrule
\multicolumn{2}{l}{\textit{Gruppen und Darstellungen}} \\
$A_5$ & Ikosaeder-Rotationsgruppe; $|A_5|=60$ \\
$A,T_1,T_2,H$ & irreduzible $A_5$-Darstellungen; Dimensionen $1,3,3,5$ \\
$\varphi=(1{+}\sqrt5)/2$ & goldener Schnitt \\
$A_4$ & Tetraedergruppe; ganzzahlige Elemente von $A_5$; $|A_4|=12$ \\
${\rm Aut}(D_4)$ & Automorphismengruppe des $D_4$-Gitters; $|{\rm Aut}(D_4)|=1152$ \\
$W(F_4)$ & Weyl-Gruppe von $F_4$; isomorph zu ${\rm Aut}(D_4)$ \\
\midrule
\multicolumn{2}{l}{\textit{Galois und Körper}} \\
$\mathbb{Q}(\sqrt5)$ & Zahlkörper; erzeugt vom Träger-Spektrum \\
$GF(3^k)$ & endlicher Körper der Charakteristik~3 \\
$\sigma:\sqrt5\mapsto-\sqrt5$ & Galois-Automorphismus von $\mathbb{Q}(\sqrt5)$ \\
$x\mapsto x^3$ & Frobenius-Automorphismus von $GF(9)$ \\
$\zeta_n$ & primitive $n$-te Einheitswurzel \\
\midrule
\multicolumn{2}{l}{\textit{$D_4$-Gitter}} \\
$D_4$ & Wurzelgitter; $\{k\in\mathbb{Z}^4:\sum k_i\equiv0\pmod2\}$; Kusszahl~24 \\
$P_{T_1},P_{T_2},P_A,P_H$ & Spektralprojektoren von $L$ \\
\bottomrule
\end{tabular}
...
Dieses Dokument enthält keine neue Mathematik. Charaktertafel von $A_5$,
Galois-Theorie von $\mathbb{Q}(\sqrt5)$, Brauer-Grade, ${\rm Aut}(D_4)$,
kristallographische Einschränkung und Cut-and-Project sind Lehrbuchstoff
\EM. Die Marke \BM\ bezeichnet hier die exakte Nachprüfung solcher Sätze am
konkreten Träger und ihre Zusammenstellung; neu ist nur die Anwendung auf
das Verhältnis der beiden Rahmen.
...
\begin{theorem}[Spektrum, \EM/\BM]\label{thm:spektrum}
...
\begin{remark}[Dämpfungsraten, \EM/\BM]
...
\begin{theorem}[Galois vertauscht $T_1\leftrightarrow T_2$, \EM/\BM]\label{thm:galois}
...
Jede über $\mathbb{Q}$ \emph{als abstraktem Körper} definierte,
$A_5$-invariante Struktur auf dem Portraum -- ein Gitter, ein ganzzahliger
Charakter -- sieht $T_1\oplus T_2$ als \emph{einen} sechsdimensionalen
Block; $T_1\oplus T_2$ ist der einzige sechsdimensionale Untermodul mit
ganzzahligem Charakter, der ein Triplett enthält ($1\oplus H$ ist der
andere sechsdimensionale Untermodul mit ganzzahligem Charakter). Die
Unterscheidung von $T_1$ und $T_2$ ist keine Symmetrieaussage.
\end{corollary}

\begin{remark}[Die Auswahl geschieht an der archimedischen Stelle, \BM]\label{rem:arch}
Argument des Autors des externen Modells (persönliche Mitteilung,
15.~September 2026), hier exakt nachgeprüft.
Die Reparaturdynamik lebt nicht im abstrakten Körper $\mathbb{Q}$, sondern in
$\mathbb{R}$: $I-L/60$ ist eine rationale symmetrische Matrix mit reellen
Eigenwerten, und die beiden konjugierten Raten $(55\pm\sqrt5)/60$ sind die
reellen Zahlen $0{,}954$ und $0{,}879$. Beide Einbettungen
$\mathbb{Q}(\sqrt5)\hookrightarrow\mathbb{R}$ liefern dieselben zwei
Unterräume; sie unterscheiden sich nur darin, welcher den Namen $T_1$ trägt.
Iteriert man $I-L/60$ auf einem beliebigen rationalen Vektor mit Anteil im
langsamen Block, so streben die Verhältnisse der zentrierten Normen -- eine
rationale Folge -- gegen $((55+\sqrt5)/60)^2=0{,}9100$ und nicht gegen den
konjugierten Wert $0{,}7733$ (Prüfskript, Abschnitt~6). Die Auswahl des
langsamen Tripletts als Raum ist also eine Ableitung aus der Dynamik, wie
die Auswahl der positiven Wurzel von $x^2=2$ durch jede konvergente
rationale Folge; sie geschieht an der archimedischen Stelle von
$\mathbb{Q}(\sqrt5)$. Geometrisch: In den Koordinaten $(0,\pm1,\pm\varphi)$
summieren die fünf Nahtnachbarn eines Ports zu $\sqrt5\,v$, die dreißig
Zweitnachbarn (Kanten des großen Ikosaeders) zu $-\sqrt5\,v$; \enquote{$T_1$
ist Raum} heißt, dass die Reparatur nächste Nachbarn koppelt.
\end{remark}
...
Aussage als Galois-Unteilbarkeit an den \emph{endlichen} Stellen; die
archimedische Stelle trennt die Tripletts, die Stelle~3 nicht
(Satz~\ref{thm:frob}, Abschnitt~\ref{sec:phasen}). Der Vorschlag von OPH, das
...
\begin{theorem}[Restriktion auf $C_3<A_5$, \EM/\BM]\label{thm:c3}
...
\begin{theorem}[Galois wird Frobenius, \EM/\BM]\label{thm:frob}
...
\begin{theorem}[$A_5$ über $GF(9)$, \EM/\BM]\label{thm:a5gf9}
...
\begin{theorem}[Reproduktion von Dok.~314, \EM/\BM]\label{thm:aut}
...
\begin{theorem}[Der gemeinsame Kern ist $A_4$, \EM/\BM]\label{thm:a4}
...
\begin{theorem}[Ports als halbe Erstschale, \EM/\BM]
...
\begin{remark}[Was Axiom ist, \SM/\EM]
Die Herleitung des Zentrums in dieser Bemerkung stammt vom Autor des
externen Modells (persönliche Mitteilung, 15.~September 2026).
Die Zuordnung setzt zwei Axiome des externen Modells voraus: (A1) die
reversible Portantwort ist eine kompakte Lie-Algebra $\mathfrak{g}$ mit
treuer Darstellung auf dem Portmodul; (A2) $A_5$ wirkt auf $\mathfrak{g}$
durch innere Automorphismen. Daraus folgt das eindimensionale Zentrum
und ist kein drittes Axiom: Innere Automorphismen wirken trivial auf dem
Zentrum, also liegt es im $A_5$-Fixraum von $1\oplus3\oplus3'\oplus5$, der
Uniformgeraden. Zentrum null würde $\dim\mathfrak{g}=12$ halbeinfach, also
$su(2)^4$ verlangen; unter innerer Wirkung hat jeder $su(2)$-Faktor
Fixdimension $0$ oder $3$, die Fixdimension wäre ein Vielfaches von~3,
der Portmodul hat aber Fixdimension~1. Also ist das Zentrum die
Uniformgerade und $11=8+3$ die einzige Zerlegung des Rests in
Dimensionen kompakter einfacher Algebren. Das Gegenbeispiel
$u(1)^3\oplus su(2)^3$ scheitert an (A2): unter innerer Wirkung enthält
sein Modul keine~$5$; und schon ohne (A2) scheitert es an
Satz~\ref{thm:galois}, weil das Zentrum eines Torus über $\mathbb{Q}$
definiert ist und daher $3$ und $3'$ nur gleich oft enthalten kann.
Korrekt ist daher \enquote{$A_5$ plus (A1), (A2) erzwingt den Lie-Typ}.
\end{remark}
...
\begin{theorem}[Phasenkanal, \EM/\BM]\label{thm:phasen}
\begin{enumerate}[nosep]
...
Kanal fällt. In der Sprache der Stellen von $\mathbb{Q}(\sqrt5)$: Das
externe Modell wählt zwischen $T_1$ und $T_2$ an der archimedischen Stelle
(Bemerkung~\ref{rem:arch}); die FFGFT rechnet an der Stelle~3, an der
$\sqrt5$ kein Vorzeichen hat und die beiden Tripletts durch den Frobenius
vertauscht werden. Beide Auswahlen sind korrekt, an verschiedenen Stellen
-- und das ist der Grund, warum sich die beiden Rahmen genau einmal
berühren. Satz~\ref{thm:neg} ist die geometrische Seite derselben
...
% ============================================================
\section{Was projiziert wird und was nicht: Reichweite der Schlüsse}\label{sec:reichweite}
% ============================================================

Der Bezugsgegenstand dieses Dokuments ist der Torus, nicht der Ikosaeder.
Der Ikosaeder ist \emph{ein} geometrischer Körper unter vielen, die auf
$T^4/\mathbb{Z}_3$ projiziert werden können, und er ist hier nur deshalb
der Prüfkörper, weil ein externes Modell ihn als Träger vorschlägt. Jeder
andere endliche Körper -- die fünfzählige Geometrie von Dok.~285 war der
vorige Fall -- zerfällt nach demselben Muster in einen gitterverträglichen
Teil (in ${\rm Aut}(D_4)$) und einen Phasenteil (Einheitswurzeln in einer
Erweiterung $GF(3^k)$); der Ikosaeder ist die Instanz, an der das Muster
in diesem Dokument vollständig durchgerechnet ist, nicht das Muster
selbst. Keine der Abbildungen ist eine vollständige Projektion des
Ikosaeders auf den Torus; nach Satz~\ref{thm:aut} kann es eine solche
nicht geben, und das gilt für jeden Körper mit nicht-kristallographischer
Symmetrie. In der Terminologie von Dok.~330 sind die Abbildungen weder
Typ~I (verlustfreie Dekompaktifizierung) noch Typ~III (bijektive
repräsentationelle Übersetzung), sondern verlustbehaftet wie Typ~II --
oder gar keine FFGFT-Operationen. Die Schlüsse sind genau so weit
berechtigt, wie die jeweilige Abbildung auf dem treu ist, worüber
geschlossen wird. Sie gelten außerdem relativ zur Gitterwahl: $D_4$ ist
eine Setzung (Dok.~320 \SetzM), motiviert durch die dichteste Packung
in vier Dimensionen und dadurch, dass $D_4$ das einzige Gitter der
$D_n$-Familie mit $S_3$-Außensymmetrie ist, die die $\mathbb{Z}_3$ als
Trialität trägt (Dok.~314; dort auch der Casimir-Gegenbefund zugunsten
von $\mathbb{Z}^4$). Alle Aussagen dieses Dokuments über ${\rm Aut}(D_4)$
setzen diese Wahl voraus.

\paragraph{Echte Projektionen (treu auf dem Gegenstand des Schlusses).}
\begin{enumerate}[nosep]
\item $\mathbb{Z}_3$ auf den Modenorbits (Dok.~314): innere Torusstruktur,
      vollständig. Alle Trialitätsschlüsse ruhen darauf.
\item $\mathbb{Z}[\varphi]\to GF(9)$ (Satz~\ref{thm:inert}): surjektiver
      Ringhomomorphismus, Kern $(3)$. Treu für alles an der Stelle~3
      (Modenordnungen, Erweiterungsgrade, Ununterscheidbarkeit von
      $T_1,T_2$); verloren ist das Vorzeichen von $\sqrt5$ und damit alles
      Archimedische (Raten, Ordnung, Konvergenz). Kein Schluss auf Dynamik.
\item Drehung $\mapsto$ Eigenwert (Satz~\ref{thm:phasen}): treu für die
      Ordnung, mit maximalem Kern -- die Geometrie wird vergessen. Der
      Schluss \enquote{Fünfzähligkeit nur als Phase} gilt gerade deshalb.
\item $A_4\hookrightarrow W(D_4)$ (Satz~\ref{thm:a4}): treu für alles
      Tetraedrische, Fünfzähligkeit nicht enthalten.
\end{enumerate}

\paragraph{Keine Projektionen des Torus.}
\begin{enumerate}[nosep,resume]
\item Ports $=$ halbe Erstschale: nur als $\mathbb{Z}_3$-Mengen isomorph;
      die Ikosaederkanten existieren in $D_4$ nicht. Berechtigt sind
      Zählaussagen, nicht Aussagen über Nachbarschaft oder Raten.
\item Cut-and-Project $\mathbb{Z}^6\to\mathbb{R}^3$: echte Projektion eines
      \emph{anderen} Gitters ($\mathbb{Z}^6/D_6$, die zweite geschützte
      Geometrie von Dok.~285), nicht von $T^4$.
\item Port-$\mathbb{Z}_3\to$ Orbifold-$\mathbb{Z}_3$: bewiesen keine
      Projektion (Satz~\ref{thm:neg}).
\item Eichzuordnung und Raumauswahl des externen Modells: Aussagen an der
      archimedischen Stelle über dessen Antwortalgebra; nichts davon
      projiziert auf $T^4$.
\end{enumerate}

\paragraph{Welche Bestätigung das ist.} Die Projektionen bestätigen nicht,
dass der Torus die Physik ist. Sie bestätigen, dass die Torusgeometrie
konkrete, falsifizierbare Strukturvorhersagen macht und dass ein
unabhängig konstruierter Fremdkörper sie erfüllt. Vier Vorhersagen hatten
Gelegenheit zu scheitern:
\begin{itemize}[nosep]
\item \emph{Gitter}: Ordnung~5 wirkt nicht auf $D_4$ (Dok.~314). Wäre
      $A_5\subset{\rm Aut}(D_4)$, wäre Dok.~285 falsch. Bestanden.
\item \emph{Körper}: $\zeta_5$ erst in $GF(81)$, $\zeta_{13}$ erst in
      $GF(729)$, keine $\zeta_3$ (Dok.~341). Wäre $3$ in $\mathbb{Q}(\sqrt5)$
      zerlegt oder $\zeta_5\in GF(9)$, wäre die Zuordnung Verdopplung
      $\leftrightarrow$ Grad~2 falsch. Bestanden.
\item \emph{Orbifold}: $\mathbb{Z}_3$ ist Gitteroperation, keine Phase. Die
      Port-$\mathbb{Z}_3$ hätte Orbifold-$\mathbb{Z}_3$ sein können; sie ist
      es aus einem Grund nicht, der die Torusstruktur widerspiegelt. Bestanden.
\item \emph{Modenordnung}: die $G(3)/G(6)$-Schwelle trifft für $A_5$
      dieselbe Grenze wie für Ordnung~26. Schwächster Test, \SM.
\end{itemize}
Das ist Bestätigung im Sinne einer überstandenen Prüfung: mehr als innere
Konsistenz, weniger als Beweis -- und sie gilt für den Torus, nicht für
den Ikosaeder. Ein weiterer Prüfkörper (Oktaeder, Würfel, ein
$4$-dimensionales Polytop) würde dieselben vier Vorhersagen erneut
befragen, mit anderem Gitteranteil und anderem Zahlkörper; die
Tetraedergruppe und die Oktaedergruppe liegen ganz in ${\rm Aut}(D_4)$ und
hätten keinen Phasenanteil, das $600$-Zell brächte wieder $\mathbb{Q}(\sqrt5)$.
Der Ikosaeder ist in dieser Reihe nur der erste vollständig gerechnete
Fall. Nicht geprüft -- und durch keine
Projektion prüfbar -- bleibt, ob $T^4$ der physikalische Träger ist, ob
$\theta=2/9$ als Phase physikalisch real ist (weiterhin \KM, Dok.~293) und
ob Modenordnungen Teilchen sind; dafür braucht es Messung. Ebenso
unberührt bleibt die tiefere Frage, ob die FFGFT-eigenen Übersetzungen
(Typ~III, Dok.~270/330) vollständig sind: dort gilt weiterhin innere
Konsistenz ohne Aussage über Beobachtbarkeit. Wer aus diesem Dokument
mehr herausliest als die Lage des einen Berührungspunkts, überzieht.
...
3.~Aufl., Springer, 1993.
Standardreferenz für Aut$(D_4)$, Kusszahl~24 und Ordnungsverteilung.

\bibitem{Humphreys1990}
J.~E.~Humphreys,
\textit{Reflection Groups and Coxeter Groups},
Cambridge University Press, 1990.
$D_4$-Trialität, Dynkin-Diagramm und $W(F_4)$.

\bibitem{Serre1977}
J.-P.~Serre,
\textit{Linear Representations of Finite Groups},
Springer, 1977.
Charaktertafel von $A_5$, Brauer-Grade in Charakteristik~3.

\bibitem{Lidl1997}
R.~Lidl, H.~Niederreiter,
\textit{Finite Fields},
2.~Aufl., Cambridge University Press, 1997.
$GF(9)$ als Erweiterung von $GF(3)$; Frobenius-Automorphismus;
Einheitswurzeln in $GF(3^k)$.

\bibitem{Neukirch1999}
J.~Neukirch,
\textit{Algebraic Number Theory},
Springer, 1999.
Trägheit von~$3$ in $\mathbb{Q}(\sqrt5)$; Galois-Gruppe $\mathbb{Q}(\sqrt5)/\mathbb{Q}$.

\bibitem{Berger1987}
M.~Berger,
\textit{Geometry~I},
Springer, 1987.
Ikosaeder, $A_5$-Symmetrie; kanonische Ecken $(0,\pm1,\pm\varphi)$.

\bibitem{Pascher285}
J.~Pascher,
\textit{Dok.~285: FFGFT --- HLV, CP-Verletzung und Dimensionsbrücke},
FFGFT-Korpus.

\bibitem{Pascher293}
J.~Pascher,
\textit{Dok.~293: Ikosaeder, $\theta=2/9$ und C\textsubscript{3}-in-$A_5$},
FFGFT-Korpus.
...
\bibitem{Pascher341}
J.~Pascher,
\textit{Dok.~341: $GF(27)$ und GALG in der FFGFT},
FFGFT-Korpus.
