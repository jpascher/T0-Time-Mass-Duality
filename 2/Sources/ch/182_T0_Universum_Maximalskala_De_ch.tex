...
$\xipar$ ist der einzige fundamentale Parameter der T0-Theorie. Die vollständige Ableitungskette lautet: $\xipar \to \{m_e, m_\mu, \ldots\} \to G \to \lP$, mit $L_0 = \xipar \cdot \lP$ als Sub-Planck-Skala der Raumzeitgranulation. Die maximale kosmologische Skala $R_H$ wird in der Form $E_H = \Ezero \cdot \xipar^{41/4}$ geschrieben; der Exponent $41/4$ ist jedoch nicht aus $\xipar$ hergeleitet, sondern an den gemessenen $H_0$ kalibriert (Dok.~190, P35/P36). Das Verhältnis $R_H/L_0 = 6{,}49 \times 10^{64}$ ist daher die gemessene kosmische Skala in $\xipar$-Schreibweise, kein eigenständiges $\xipar$-Resultat.
...
\section{Die maximale Skala: Hubble-Radius (\texorpdfstring{$\xi$}{xi}-Form, an $H_0$ kalibriert)}

\subsection{Hubble-Energie aus \texorpdfstring{$\xi$}{xi}}

Die Hubble-Energie wird in $\xipar$-Form geschrieben (Dokument 026):
\begin{equation}
	E_H = \Ezero \cdot \xipar^{41/4}
\end{equation}
mit $\Ezero = \sqrt{m_e \cdot m_\mu} = 7{,}398\,\text{MeV}$ und $\xipar = 4/30000$. Der Exponent $41/4$ ist dabei \emph{nicht} aus $\xipar$ hergeleitet, sondern so gewählt, dass der gemessene $H_0$ getroffen wird (Dok.~190, P35/P36); $\xipar^{41/4}$ ist die gemessene kosmische Skala in $\xipar$-Schreibweise, keine Vorhersage.
...
\end{equation}
In $\xipar$-Schreibweise; der Exponent $41/4$ ist jedoch an $H_0$ kalibriert, kein eigenständiges $\xipar$-Resultat (Dok.~190, P35/P36).
\end{keyresult}
...
\caption{Skalenhierarchie in $\xipar$-Schreibweise (kosmologische Skala $R_H$ an $H_0$ kalibriert)}
\end{table}
...
\begin{align}
	...
	&\quad\Downarrow \notag \\
	&\boxed{R_H/L_0 = 6{,}49 \times 10^{64} \quad \text{(an } H_0 \text{ kalibriert)}}
\end{align}
\end{keyresult}
...
% ============================================================
% Nachtrag 7. September 2026 — P35: xi^N-Trivialität
% Quelle: Dok. 190-Archiv-2 (P35)
% Quelldokument bleibt unverändert (append-only, vgl. R50).
% ============================================================

\section*{Nachtrag P35: $\xi^N$-Trivialität (7.~September 2026)}

\textbf{Grundsatzpräzisierung (P35, korpusweit):}
Ausdrücke der Form „$X = \xi^N$" sind \emph{für sich allein aussagelos},
da sich jede positive Größe als $\xi$-Potenz schreiben lässt --- ein Exponent
allein ist keine Vorhersage.

\textbf{Was eine Aussage nicht-trivial macht:}
(i)~Die \emph{Form} ist vorwärts hergeleitet (geometrisch oder algebraisch
erzwungen), nicht nachträglich angepasst.
(ii)~Der \emph{Exponent} ist rational und aus der Struktur ablesbar
(z.\,B.\ $41/4$ aus $T^4$-Geometrie), nicht frei gewählt.
(iii)~Die Zahl (Vorfaktor, Anker) ist eine \emph{externe Kalibrierung}
und als solche deklariert (vgl.\ P20, P33, P39).

\textbf{Für dieses Dokument:} Die Exponenten der Skalenhierarchie
($R_H \sim \xi^{-41/4} \ell_P$ u.\,a.) sind geometrisch plausibel \textbf{[B]},
aber der kosmische Exponent $41/4$ ist noch nicht vorwärts hergeleitet (P20).
Aussagen dieser Art sind als \textbf{[S]} einzustufen, nicht als \textbf{[K]}.
