...
time-winding to be a log spiral with ratio $e$. Only here does HLV enter. Time appears differently
in the two frameworks (Dok.~285): HLV factorises $T^6\times S^1$, where time is an $A_5$ singlet
and decoupled as \emph{spiral time}, a separable object; FFGFT does not factorise, its spiral is
the projection of an inseparable whole. The limiting case of constant pitch ($d=0$) noted above
corresponds to the fixed point $\xi\to0$ of the recursion, in which the defect vanishes and the
winding closes --- that is HLV's decoupled singlet time; the running FFGFT winding approaches it
asymptotically.

Whether HLV's spiral time is the \emph{same} object remains a candidate --- but now sharply
testable: it would have to be a log spiral with self-similarity ratio $e$, equivalent to a $1/n$
decay of its pitch. The decisive question to HLV's formulation (sources on spiral time) is
therefore no longer \enquote{similar?} but \enquote{ratio $e$, yes or no?}. Ordered within the
three methodological layers: the derivation recursion~$\to$~log spiral is a core derivation,
obtained from $\xi$; the identification with HLV's spiral time is a bridge candidate; no physical
claim over HLV is made.

The contrast here is structural, not ontological. Both frameworks --- and likewise the two
variants within FFGFT, whether time or space is rolled up --- are mathematical models, not
reality; no factorisation and no choice of axis is thereby ontologically privileged. That one
hopes such a model is applicable to reality is the motivation, not a statement about what is.
...
(decompactified, across-scales) reading of the same time-winding. What
remains open is only whether HLV's spiral time carries the ratio $e$; that decides the
identification and is to be checked against HLV's spiral-time definition. Reproducible in
\texttt{zeit\_windung\_probe.py}, \texttt{zeit\_logspirale\_probe.py} and
