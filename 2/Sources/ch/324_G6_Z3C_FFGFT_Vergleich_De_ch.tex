...
$V$ & Vakuumoperator, $V = N_1^+N_2^+N_3^+N_1^-N_2^-N_3^-$ &
  Involution in $\mathbb{Z}_3\mathbb{C}$: $V^2 \equiv -V \pmod{3}$ \\
$P_{Nk}$ & Antiteilchen-Idempotente, $P_{Nk}^2 = P_{Nk}$ &
...
\begin{itemize}
  ...
  \item Matzkes Vakuumoperator $V = N_1^+N_2^+N_3^+N_1^-N_2^-N_3^- = -64\cdot P_{N1}P_{N2}P_{N3}$
    ist eine Involution in $\mathbb{Z}_3\mathbb{C}$: $V^2 = -64V$,
    d.h.\ $V^2 \equiv -V \pmod{3}$ \textbf{[B]}.
    Das komplementäre Pp-Produkt $P_{P1}P_{P2}P_{P3}$ ist ein Rang-1-Projektor
    mit Eigenwerten $\{0^{(7)},\, 1^{(1)}\}$, liegt jedoch auf einem anderen Strahl
    ($|\langle\text{vac}_V|\text{vac}_{Pp}\rangle| = 0$).
...
Antiteilchen-Idempotente und Vakuumoperator:
\begin{align}
...
\textbf{[B]} Matzkes Vakuumoperator $V = -64\cdot P_{N1}P_{N2}P_{N3}$
hat in der 8-dimensionalen Spinordarstellung über $\mathbb{C}$:
\begin{equation}
  \mathrm{Spur}(V) = -64, \quad \mathrm{Rang}(V) = 1, \quad V^2 = -64\,V.
\end{equation}
In der $\mathbb{Z}_3\mathbb{C}$-Arithmetik gilt $-64 \equiv -1 \pmod{3}$, also:
\begin{equation}
  V^2 \equiv -V \pmod{3} \qquad \text{(Involution — Matzkes Relation bestätigt \textbf{[B]})}.
\end{equation}
Das Pp-Produkt $P_{P1}P_{P2}P_{P3}$ ist ein Rang-1-Projektor mit
$\sigma = \{0^{(7)}, 1^{(1)}\}$, liegt aber auf einem anderen Strahl:
$|\langle\mathrm{vac}_V|\mathrm{vac}_{Pp}\rangle| = 0$ \textbf{[B]}.
Bei der ursprünglichen Analyse von Dok.~324 wurde das Pp-Produkt
irrtümlich als Matzkes $V$ behandelt; die Korrektur ist durch
\texttt{pruef\_324\_vakuum\_involution.py} belegt.
...
\begin{itemize}
  ...
  \item $V = -64\cdot P_{N1}P_{N2}P_{N3}$ ist Involution in $\mathbb{Z}_3\mathbb{C}$: $V^2 \equiv -V \pmod{3}$, $\mathrm{Rang}=1$ \textbf{[B]}
