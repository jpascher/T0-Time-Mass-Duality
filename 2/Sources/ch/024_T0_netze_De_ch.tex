...
\footnote{\textbf{Überarbeitungsvermerk (August 2026):} Die Modellierung von Faktorisierungsprozessen als $\xi$-abhängige Resonanzprobleme ist sachlich nicht begründet. $\xi$ ist ein dimensionsloser Geometrieparameter des Feldrahmens; er hat keinen operationalen Bezug zur Periodenfindung in der modularen Exponentiation. Die Verbindung zwischen dem Netzwerk-Formalismus und der Kryptographie ist zurückgezogen. Die übrigen Teile dieses Dokuments (Netzwerkdarstellung des T0-Modells, dimensionale Aspekte) sind davon unberührt (Dok.~316, August 2026).}
...
\subsection{Dimensionale Struktur im Netzwerkmodell}
\label{subsec:factorization_xi}
Der Geometrieparameter $\xipar = 4/30000$ ist eine feste Strukturgröße des
Feldrahmens. Er ist \emph{kein} einstellbarer Parameter und hat in
$\mathbb{Z}/N\mathbb{Z}$ -- dem Raum, in dem die Faktorisierung stattfindet --
keine Funktion.

\textbf{[K]} Die Resonanzstruktur der Periodenfindung beruht auf Primzahlen.
Der Bikohärenztest an 2469 Riemann-Nullstellen (Dok.~316, August 2026) zeigt
Phasenkopplung genau bei Primzahlpotenzen ($b^2 = 0{,}76$--$0{,}85$) und keine
bei zusammengesetzten Zahlen ($b^2 = 0{,}018$--$0{,}025$). Das Euler-Produkt
behandelt jede Primzahl für sich, ohne Mischterme; das duale Längenspektrum
enthält nur $k \ln p$.

\textbf{[K]} Die Periode $r$ einer modularen Exponentiation teilt
$\lambda(N) = \mathrm{lcm}(p-1, q-1)$. Sie ist eine individuelle arithmetische
Eigenschaft von $N$ -- nicht das Ergebnis eines Resonanzparameters. Die
folgende Analyse behandelt daher ausschließlich dimensionale Aspekte der
Netzwerkdarstellung.
\begin{equation}
