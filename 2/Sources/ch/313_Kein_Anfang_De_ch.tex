...
Neun Kapitel, ein Gedanke. Damit der Bogen sichtbar bleibt, hier
die Erzählung in Kurzform.

\textbf{Die Ausgangslage.} Dok.~306 sagt: Der Torus hat keinen
Rand, die Frage nach $t=0$ ist falsch gestellt. Dok.~312 macht das
konkret -- die Zeitrichtung ist ein Zyklus von $92$~Gyr. Offen
blieb die \emph{Geschichte}: Die Beobachtungen zeigen eine klare
thermische Abfolge. Wie verträgt sich ein Gradient mit einem
Kreis?

\textbf{Vorschaltung.} Alle Distanzen hier sind Lichtwege, und der
Raum ist nicht glatt. Bevor eine einzige Zahl fällt, muss geklärt
sein, wo die fraktale Wegkorrektur greift: bei Weg-durch-Struktur
ja, bei Weg-durch-Weg nein. Die Regel hat zwei Kontrolltests --
und besteht beide, einen davon (die CMB-Peaks) ohne dafür gemacht
worden zu sein.

\textbf{Der Befund.} Die gesamte beobachtbare Geschichte füllt
\emph{einen Halbzyklus}. Der Partikelhorizont sitzt auf
$0{,}14\,\%$ am Antipoden. Der \enquote{Urknall} ist damit ein
\emph{Ort} auf dem Zyklus -- die Region kleinster Massen und
langsamster Uhren --, kein Ereignis.

\textbf{Die Untergrenze.} Es gibt kein $z \to \infty$: Das
Zyklusquant $\hbar H_0$ ist eine harte Schranke, und die Massen
können nicht gegen null laufen. Die Singularität ist damit nicht
nur falsch gestellt, sondern \emph{ausgeschlossen} -- und
dieselbe Skala, die oben begrenzt ($\Lambda^{*}$), begrenzt auch
unten.

\textbf{Die Schließung und ihr Preis.} Das Massenprofil läuft mit
Steigung $\sqrt{\Omega_r} \approx 0{,}01$ in den Antipoden -- eine
gerade periodische Schließung ist auf $1\,\%$ erfüllt. Aber sie
ist eine \emph{Randbedingung}, keine Folge der Dynamik: Die
Poincaré-Diskrepanz beträgt $10^{104}$ Größenordnungen im
Exponenten. Feinkörnig ist Periodizität zulässig; grobkörnig ist
sie offen -- der harte Kern von D(ii). Dok.~295 kennt mit der
Logspirale den nicht-schließenden Fall; D(ii) ist deshalb genauer
eine \emph{Gabelung} als eine Lücke.

\textbf{Hawking.} Eine einzige KMS-Regel temperiert Unruh,
Gibbons--Hawking und Schwarze Löcher; das Membranbild aus
Kap.~04 bekommt damit sein Thermometer, und die Leiter trifft den
Kosmos bei $r_s = R_H/2$. Für die Zyklusschließung hilft das
nicht: Verdampfung ist $56$~Größenordnungen zu langsam. Hawking
passt ins Bild und \emph{verschärft} D(ii).

\textbf{Die Vorhersage.} Kehrt man die Antipoden-Bedingung um,
ist $\Omega_m$ nicht mehr frei: Die Kette
$H_0 \to T_0 \to \Omega_r \to$ Antipode liefert
$\Omega_m^{*} = 0{,}3136$ gegen Planck $0{,}315\pm0{,}007$ --
ohne angepassten Parameter, da $T_0$ aus der Atomskala stammt.
Teilchensektor und kosmischer Sektor treffen sich auf
$0{,}1\,\%$: eine Konsistenzprüfung, keine bloße Umdeutung.

\textbf{Die Prüfflächen.} Zwei: die Nicht-Monotonie am Antipoden
(C$\nu$B/GW, fern) und der Low-$\ell$-Cutoff
$\ell_\text{min} = 3{,}08$ -- er trifft die seit COBE~1992
unerklärte Quadrupol-Unterdrückung, an vorhandenen Daten.

\medskip
\noindent\textbf{In einem Satz:} Der \enquote{Anfang} ist der
Antipode des Zeitzyklus -- ein Ort, kein Ereignis; die beobachtete
Geschichte ist die eine Hälfte einer auf $1\,\%$ schließbaren
Landkarte; und die verbleibende Last liegt nicht bei den Feldern,
sondern bei der grobkörnigen Entropie.
...
\textbf{Zweiter, unabhängiger Test: die CMB-Peaks.} Die Position
des ersten akustischen Peaks ist
$\ell_1 = \pi\,D_A/r_s$ -- Winkelabstand geteilt durch
Schallhorizont, beides \emph{durchlaufene Wege}. Nach der Regel
kürzt sich $K_\text{frak}$ heraus, die Peakpositionen bleiben also
exakt unverändert. Die Gegenprobe zeigt, wie scharf dieser Test
ist: Griffe der Faktor nur auf $D_A$, verschöbe sich $\ell_1$ um
$1{,}35\,\%$, also um drei Multipole -- bei einer gemessenen
Genauigkeit von $\pm0{,}5$ wäre die Regel sofort falsifiziert.
Sie besteht diesen Test, ohne dafür konstruiert worden zu sein.
...
\section{Die Gabelung: Dok.~295 kennt die Alternative}

Die bisherige Darstellung behandelt die Periodizität als einzige
Option mit offener Entropiefrage. Das ist unvollständig: Dok.~295
(\enquote{Zeit als Logspirale}) formuliert die Nichtschließung
bereits aus, und zwar aus demselben Faktor. Eine Windung, die je
\emph{Halbdrehung} um $\pi$ vorrücken sollte, rückt gatterweise
nur um $\pi K_\text{frak}$ vor und verfehlt $\pi$ damit um
$100\pi\,\xi$ (Dok.~295). In Umlauf-Einheiten ist der
Schließungsdefekt
\begin{equation}
  d = 100\,\xi = \tfrac{1}{75}\quad\text{je Umlauf.}
\end{equation}
Damit stehen drei Fälle nebeneinander:
...
$1-d = 74/75$; die $75 = 1/(100\xi)$ fällt exakt heraus. Dass die
Schließung \emph{genau} bei 75 und nicht früher eintritt, liegt an
$\gcd(74,75) = 1$: das kleinste $n$ mit $n\cdot 74/75 \in
\mathbb{Z}$ ist $n = 75$. Fall~C
entsteht, wenn $\xi$ über die Rekursion läuft
($\xi_n \approx 1/(100(n{+}75))$): Der kumulierte Defekt wird
logarithmisch, und die Windung wird zur äquiangularen Spirale
$\rho = 75\,e^{\beta/2\pi}$ mit dem Selbstähnlichkeitsverhältnis
$e$ pro Präzessionsrunde -- nicht angesetzt, sondern Folge des
$1/n$-Zerfalls.

\textbf{Was das für D(ii) heißt.} Fall~A verlangt die
grobkörnige Schließung nach $91{,}9$~Gyr -- das ist die offene
Pflicht. Fall~B entspannt sie um den Faktor~$75$, ändert aber
nichts an der Poincaré-Diskrepanz von $10^{104}$~dex im Exponenten.
\textbf{Fall~C hebt die Entropieforderung ganz auf}, weil nichts
zurückkehren muss -- kostet dafür aber die kompakte Zeitrichtung
im Sinne von Dok.~312: $\tau_c$, das Quant $\hbar H_0$ und die
Gibbons--Hawking-Temperatur verlören ihre Grundlage.

\textbf{Die Entscheidungsfrage} ist damit benennbar und
einfacher als die Entropiefrage selbst: Ist die Zeit-Windung an
\emph{eine} Skalenstufe gebunden (dann A oder B), oder durchläuft
sie die Rekursion (dann C)? Die Rekursion in Dok.~295 ist eine
Skalen-, keine Zeitrekursion; ob die kosmologische Zeitrichtung
sie durchläuft, ist nicht entschieden. D(ii) ist deshalb genauer
als \emph{Gabelung} zu führen denn als Lücke: Zwei der drei Wege
sind gangbar, sie führen aber zu verschiedenen Theorien.
...
\section{Reichweite: Gleichgewichte an anderen Orten des Zyklus}

Aus der Ortslesart folgt eine Grenze, die getrennt zu buchen ist,
weil sie leicht überlesen wird.

\textbf{Die Rückwärtsextrapolation setzt voraus, was hier offen
ist.} Die übliche Rechnung $T(z) = T_0(1+z)$ gewinnt frühere
Temperaturen durch adiabatische Fortsetzung des heutigen
Zustands. Das setzt \emph{eine} durchgehende thermodynamische
Kette voraus -- genau die grobkörnige Verkettung, die nach dem
Vorangehenden offen ist (D(ii), Poincaré-Diskrepanz
$10^{104}$~dex).

\textbf{In der Ortslesart ist eine frühe Epoche kein früherer
Zustand desselben Systems}, sondern eine Stelle mit \emph{anderer
lokaler Struktur} -- kleinere Massen, langsamere Uhren (Kap.~B) --
und damit einem eigenen Gleichgewicht, das lokal bestimmt ist und
nicht durch Fortschreiben des heutigen. Ein Gleichgewicht an
einem anderen Ort des Zyklus ist daher aus dem hiesigen
\textbf{nicht ableitbar}; es bräuchte eine eigene strukturelle
Herleitung \emph{an jenem Ort}.

\textbf{Der Korpus zeigt das an sich selbst.} $T_0$ wird hier
nicht extrapoliert, sondern strukturell aus $\xi$ gewonnen
($k_BT_0 = (16/9)\,\xi$, Dok.~061; Kap.~F.3). Die Temperatur ist
in dieser Rechnung eine \emph{Strukturgröße}, keine Endstation
einer Abkühlungsgeschichte -- und eben deshalb ist sie
zirkelfrei.

\textbf{Was das nicht heißt.} Die Grenze entwertet keine
strukturelle Ableitung. Die $\Omega_m^{*}$-Kette (Kap.~F) läuft
über $\xi$, $H_0$ und $K_\text{frak}$, nicht über eine
adiabatische Rückrechnung, und ist nicht betroffen. Betroffen
sind allein Aussagen der Form \enquote{wie heiß war es in
Epoche~X}, sofern sie durch Fortschreiben des heutigen
Gleichgewichts gewonnen werden. Solche Aussagen sind hier
\textbf{nicht} gedeckt -- weder bestritten noch behauptet,
sondern außerhalb dessen, was diese Rechnung trägt.

\textbf{Status:} [K] als Reichweitenaussage über die eigene
Rechnung; die Frage, welches Gleichgewicht an einem gegebenen Ort
des Zyklus herrscht, ist damit als eigenständige, ungelöste
Aufgabe benannt -- sie folgt nicht aus D(ii), sondern steht neben
ihr.

\subsection*{Welche Größen bleiben dann aussagekräftig?}

Die vorstehende Grenze ist eine Absage; sie wird erst brauchbar,
wenn man sagt, was \emph{diesseits} von ihr liegt. Das Kriterium
ist dreiteilig: keine Skalenbrücke, keine Dauer, gleiche Ebene.
...
\midrule
Zählungen & $|\text{Aut}(D4)| = 1152$, Kusszahl~24,
  9 Fixpunkte & exakt; Sätze, keine Messungen \\
Verhältnisse gleicher Ebene & $\alpha$, $m_\mu/m_e$,
  Koide~$Q$ & belastbar; testbar auf
  $10^{-10}$ bis $10^{-5}$ \\
mit Skalenbrücke & $E_0$, $G$, $H_0$, $T_0$,
  absolute Massen & nur mit Anker; Verhältnisse daraus wieder
  frei (A040) \\
Zahl- und Dauergrößen & $\eta = n_B/n_\gamma$, Fensterdauern,
  Zeitspannen & nicht übertragbar \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Wo die Leichtelemente landen.} Alle drei
BBN-Observablen fallen in die unteren beiden Klassen -- auch
$Y_p$, und das ist kontraintuitiv genug, um es auszuschreiben.
$Y_p$ hängt an den dimensionslosen Größen $Q/T_f$ und
$t/\tau_n$; unter gleichförmiger Skalierung kürzen sich beide
heraus. Aber $T_f$ ist \emph{nicht gemessen}, sondern folgt aus
einer Ratengleichung mit $H$, also aus dem kosmologischen Modell,
und $t/\tau_n$ enthält eine \emph{Dauer}. $Y_p$ ist damit ein
Gemisch aus Struktur- und Modellgrößen, kein reines Verhältnis
gleicher Ebene wie $\alpha$ oder $m_\mu/m_e$. Das Kürzungsargument
ist eine \emph{Implikation} (\enquote{fällt das Modell so aus,
kürzt es sich heraus}), keine Prüffläche.

$D/H$ und ${}^7$Li stehen noch weiter außen: beide hängen an
$\eta$ und an der Fensterdauer, ${}^7$Li zusätzlich an
Sternatmosphärenmodellen bei der Messung selbst -- drei
Modellschichten übereinander.

\textbf{Folge für die ${}^7$Li-Position.} Die Korpusbuchung
(Dok.~025/063: Nukleosynthese ohne feste Zeitschranke) setzt an
der Fensterdauer an und bleibt damit konsistent. Sie ist aber
\emph{keine quantitative Vorhersage}: Aus \enquote{keine feste
Schranke} folgt kein Zahlenwert für ${}^7$Li/H. Die
Faktor-3-Diskrepanz des Standardbildes zeigt, dass irgendwo in
der Kette etwas nicht stimmt -- nicht, wo.

\textbf{Gegenprobe, damit dies keine Schutzbehauptung wird.}
Dieselbe Klassifikation trifft $\Lambda$CDM genauso: Auch dort
ist $Y_p$ kein modellfreier Test, $\eta$ kommt aus einem
Mehrparameter-Fit, und die Zuordnung gemessener Häufigkeiten zu
einer frühen Epoche läuft über dieselbe Fortsetzungsannahme.
Dok.~267 hält das bereits fest: \enquote{keiner ist zirkelfrei};
der faire Vergleich ist Sparsamkeit gegen Verankerung. Die
Klassifikation entlastet FFGFT also nicht -- sie stellt beide
Seiten unter dasselbe Kriterium.
...
\emph{einzige} verbliebene Prüffläche der Schließung.

\section{Der Low-$\ell$-Cutoff: eine zweite Prüffläche [K$|$P20]}

Es gibt eine zweite, und sie liegt näher an vorhandenen Daten. Auf
einem Torus der Kantenlänge $L^{*}$ existieren keine Moden mit
Wellenlänge $> L^{*}$; die kleinste zulässige Wellenzahl ist
$k_\text{min} = 2\pi/L^{*}$. Übersetzt in Winkelskala:
...
\noindent Unterhalb $\ell \approx 3$ fehlen Moden: Der Quadrupol
($\ell = 2$) ist stark unterdrückt, der Oktupol ($\ell = 3$) liegt
am Rand, ab $\ell \ge 4$ bleibt das Spektrum unbeeinflusst.

\textbf{Kein angepasster Parameter.} $L^{*} = 4\bar\lambda_e/
\xi^{10}$ folgt aus der Kette, $D_\text{LSS}$ aus derselben; die
Variation von $\Omega_m$ zwischen $0{,}305$ und $0{,}325$
verschiebt $\ell_\text{min}$ nur von $3{,}12$ auf $3{,}04$. Ohne
fraktale Wegkorrektur ergäbe sich $3{,}12$ -- die Aussage ist
gegen beide Freiheiten robust.

\textbf{Was beobachtet wird.} Seit COBE (1992), bestätigt durch
WMAP und Planck: Der Quadrupol liegt bei etwa einem Sechstel des
$\Lambda$CDM-Erwartungswerts, der Oktupol leicht niedrig, ab
$\ell = 4$ ist das Spektrum normal. In $\Lambda$CDM ist das seit
drei Jahrzehnten unerklärt und wird der kosmischen Varianz
zugeschrieben. Die kompakte Lesart sagt genau diese Struktur
voraus -- \emph{eine} Zahl, an der richtigen Stelle, mit intaktem
Spektrum darüber.

\textbf{Was noch fehlt.} Der genaue Unterdrückungsgrad verlangt
die Modensummation auf dem Torus statt der Integration über
$\mathbb{R}^3$ -- eine endliche Summe, kein Boltzmann-Code, aber
noch nicht ausgeführt. Erst sie entscheidet, ob der Faktor $1/6$
quantitativ getroffen wird oder nur die Richtung. Bis dahin gilt
die Übereinstimmung als \emph{Ordnungsaussage} [K$|$P20], nicht
als Präzisionstest. \emph{Prüfskript:}
\texttt{ffgft\_313\_lowl\_cutoff.py}. Praktisch außerhalb heutiger
...
\textbf{$T_0$: gebunden, und die Einheitenfrage ist geklärt.} Der
Korpus enthält eine Vorwärtsformel (Dok.~061):
$T_\text{CMB}/E_\xi = (16/9)\xi^2$ mit $E_\xi = 1/\xi$, also
$k_BT_0 = (16/9)\,\xi$ in Elektronenvolt $= 2{,}370\times10^{-4}$~eV
gegen gemessen $2{,}349\times10^{-4}$~eV, ein Treffer auf
$+0{,}92\,\%$; der Faktor $16/9 = (4/3)^2$ ist dieselbe Struktur wie
$\xi = (4/3)\times10^{-4}$. Ergänzend bindet Dok.~279 $T_0$
relational an die Casimir-Länge $L_\xi = 100{,}24~\mu$m.

\textbf{Die Einheit ist keine Nachlässigkeit, sondern die
Referenzskala des ganzen Sektors.} A266~[K] weist sie für
$\alpha = \xi E_0^2$ explizit aus: Die Gleichung ist nur korrekt als
$\alpha = \xi\,(E_0/1\,\text{MeV})^2$; die $1$~MeV im Nenner ist die
\emph{implizite Referenzskala}, \enquote{sie steht nicht explizit in
der Kurzschreibweise, ist aber unverzichtbar}, und ihre Wahl ist
\enquote{nicht frei -- sie ist der empirische SI-Anker des
FFGFT-Sektors}. Die eV-Notation in 061 ist dieselbe Konvention, nur
eine Größenordnung kleiner notiert. Das Verhältnis
$1\,\text{eV}/E_0 = 1{,}35\times10^{-7}$ ist damit der Quotient
zweier \emph{Referenzskalen}, keine physikalische Größe; von ihm
eine ganze $\xi$-Potenz zu verlangen, wäre so sinnvoll wie von
$1$~Zoll$/1$~Meter.

\textbf{Was daraus für den Status folgt.} $T_0$ ist
\emph{überbestimmt} (Abschn.~I.3: zwei unabhängige Sektoren, $0{,}1\,\%$)
und über 061 an die Atomskala gebunden. Ein eigener offener Punkt
\enquote{$T_0$} besteht nicht. Was bleibt, ist die Frage nach der
\emph{Lage des Ankers selbst} -- warum die Referenzskala dort liegt,
wo sie liegt --, und die ist identisch mit P20 und dem
Vererbungsproblem aus Dok.~309, also bereits gebucht.
...
Dok.~061/313 dagegen den \emph{CMB-Monopol} ($2{,}7255$~K) -- zwei
Größen im Abstand $3\times10^{10}$ unter demselben Symbol.
\emph{(ii)~Präzisionsangabe:} Dok.~061 nennt in der Zusammenfassung
...
Gabelung & drei Fälle (Dok.~295): $d=0$ / $75$~Umläufe / Spirale;
  C hebt die Entropieforderung auf, kostet aber $\tau_c$ & [K] \\
Entropie & feinkörnig zulässig; grobkörnige Schließung offen --
  präzisierter Kern von D(ii) & offen \\
Reichweite & Gleichgewichte an anderen Orten des Zyklus sind aus
  dem heutigen nicht ableitbar; $T_0$ strukturell aus $\xi$, nicht
  extrapoliert; $\Omega_m^{*}$-Kette unberührt & [K] \\
Belastbarkeit & Kriterium: keine Skalenbrücke, keine Dauer, gleiche
  Ebene. Zählungen und Verhältnisse gleicher Ebene ja;
  BBN-Observable ($Y_p$, D/H, ${}^7$Li) nein -- gilt für
  $\Lambda$CDM ebenso & [K] \\
...
Low-$\ell$-Cutoff & $\ell_\text{min} = 2\pi D_\text{LSS}/L^{*} =
  3{,}08$; trifft die seit COBE offene Quadrupol-Anomalie
  & [K$|$P20] \\
...
$T_0$ & \emph{überbestimmt} (zwei Sektoren, $0{,}1\,\%$); eV-Notation
  $=$ Referenzskala nach A266, kein eigener offener Punkt & [K] \\
\end{longtable}
}
