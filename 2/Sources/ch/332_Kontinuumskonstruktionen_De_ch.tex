...
\emph{Anwendungshinweis für den FFGFT-Kontext:} Im FFGFT-Korpus
(Zeit-Masse-Dualität) ist die Periode $R_m$ keine freie Setzung, sondern
durch $R_m = \hbar/(mc^2)$ physikalisch fixiert -- Zeit $\tilde{T}$ und
Masse $m$ sind dort eine einzige geometrische Dimension, nicht zwei
unabhängige Größen (Grundrelation $\tilde{T}\cdot m = 1$). Das Ausrollen
auf $\mathbb{R}_t$ ist in diesem Rahmen daher nur zulässig, wenn die
massegebundene Periode mitgeführt wird; ein Ausrollen der Zeitkoordinate
allein, ohne die fraktale Korrektur $K_\text{frak}=1-100\,\xi_0$, wäre
unvollständig. Die Pullback-Isometrie $L^2(S^1_m)\cong\mathrm{Per}_{R_m}(\mathbb{R}_t)$
(R96) gilt in diesem Rahmen exakt; die Korrektur $K_\text{frak}$ tritt
erst beim Übergang zu absoluten SI-Werten auf, nicht bei der
Darstellungsisometrie selbst.
