...
\begingroup\small\noindent
\textbf{Abstract.} A common construction makes experienced time the \emph{product} of memory. This document reverses the order: memory presupposes time rather than producing it --- the \emph{memory reflex}, counterpart of the word reflex of Doc.~301. In FFGFT time is fundamental, the cumulated defect $D(k)=\sum 100\,\xi_n\to\ln(1+k/75)$ of the unrolled winding; and the difference between a finite, forgetting kernel and an accumulating structure is already a fact about time, not first about memory. FFGFT itself does not need that split --- it carries a single, curvature-faithful winding ---; the split appears only where the time structure is projected onto a cognitive model. On the measurement side a second reflex follows: a measurement excerpt is reference-less, with no absolute place on the spiral, and the apparent uniformity of time is a \emph{linearisation artefact} with a verified error quantity $e=x-\ln(1+x)$ and the scale-invariant constant $1-\ln 2$; this linearisation sits in the \emph{projection} of the result, not in the measurement. Rolled up, the 75 parts are exactly congruent; unrolled, only asymptotically self-similar; what survives the loss of reference is the ordering alone. In the FFGFT reading it is \emph{projection-fidelity}, not reach, that discriminates --- offered as an additional, rejectable bridge candidate (restricted, Category~B), not as a replacement of any external memory definition, in a symmetric relationship between the frameworks. Inference from measurement to cause succeeds only up to a holographic blur: presence is not reconstructibility. Booking is kept separate throughout --- mathematically derived, bridge (restricted), posit and reflex diagnoses (P35).
\par\endgroup
...
(U2), and memory / non-Markovian history (U3)\footnote{The temporal sectors U1 (causal ordering / forward updating), U2 (phase and coherence) and U3 (memory, path dependence, recursive access), together with the notation $M_\tau$/$M_B$, are adopted from Krüger's Spiral-Time Memory work (2026); there $M_\tau$ denotes finite, weighted, or compressible physical memory and path dependence, $M_B$ a persistent biological-cognitive architecture (recursive access, semantic organisation, affective weighting, identity binding, persistent state carry, predictive reuse), defined operationally and functionally, not geometrically. \emph{FFGFT itself does not need this distinction}: it carries a single, curvature-faithful winding, and the sectors as well as the $M_\tau/M_B$ split serve here only as an \emph{analysis} of that framework, not as native FFGFT structure. The present document therefore does not redefine these objects; it proposes a separate, FFGFT-side bridge reading. Primary source: Krüger,~M. (2026), \emph{From Physical State Evolution to Experienced Time}, Zenodo, \texttt{doi:10.5281/zenodo.21302278}. Documented lineage: Krüger, Feeney \& Wende (2026), \emph{Medinformatics}, \texttt{doi:10.47852/bonviewMEDIN62029043}; Krüger \& Feeney (2026), \emph{Smart Wearable Technology}, \texttt{doi:10.47852/bonviewSWT62029484}.}, joined into a past--present--future
world-model. Over time the discussion narrowed to U3 alone, then to the boundary between a
finite memory kernel $M_\tau$ and an accumulative structure $M_B$, and finally to an
...
Doc.~303 requires. This identity check, like the claim-state booking (restricted, Category~B),
belongs to the governance layer that goes back to Stefaan Vossen's \emph{Dot Theory} and was
co-developed in the IPI exchange (cf.\ Doc.~272); the identity-preservation record of Doc.~303
is the FFGFT-side extension on top of it.
...
time first?}. The FFGFT-internal core of this document --- the thesis that memory presupposes
time, the linearisation reflex, and the measurement and projection epistemology --- stands on
its own; the $M_\tau/M_B$ correspondence is the illustrative, \emph{restricted} overlay from
the bridge context and carries nothing the core needs. A measurement-theoretic extension places the reference-less excerpt, the congruence/self-similarity
