...
\section{The Maximal Scale: Hubble Radius (\texorpdfstring{$\xi$}{xi}-notation, calibrated to $H_0$)}
...
\end{equation}
with $\Ezero = \sqrt{m_e \cdot m_\mu} = 7{.}398\,\text{MeV}$ and $\xipar = 4/30000$. The exponent $41/4$ here is \emph{not} derived from $\xipar$ but chosen so as to match the measured $H_0$ (Doc.~190, R35/R36); $\xipar^{41/4}$ is the measured cosmic scale in $\xipar$-notation, not a prediction.
...
\end{equation}
In $\xipar$-notation; the exponent $41/4$, however, is calibrated to $H_0$, not an independent $\xipar$ result (Doc.~190, R35/R36).
\end{keyresult}
...
\textbf{General clarification (P35, corpus-wide):}
...
(e.g.\ $41/4$ from $T^4$ geometry), not chosen freely.
...
but the cosmic exponent $41/4$ has not yet been derived forward (P20).
