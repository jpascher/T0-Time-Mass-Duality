...
\chapter*{Zeit-Emergenz und Gravitationskonstante: Zwei Präzisierungsvorschläge zu Matzkes GALG-Rahmen}
\addcontentsline{toc}{chapter}{Präzisierungsvorschläge zu Matzkes GALG-Rahmen}

\begin{abstract}
Dieses Dokument formuliert zwei Präzisierungsvorschläge zum GALG-Framework
...
\textbf{Vorschlag~2 (G):}
Die Schwarzschild-Herleitung in~\cite{Matzke2026IPI} startet vom Hyperbit
$e_i^2 = +1$ --- einer rein algebraischen, G-freien Anfangsbedingung.
Die SI-Schnittstelle wird erst durch $T_\text{Planck}$ hergestellt,
woraus $M_\text{bit} = m_P\cdot\ln2$ zwingend folgt.
Da $m_P = \sqrt{\hbar c/G}$, enthält diese Schnittstelle~$G$ ---
aber das ist strukturell unvermeidlich: jede Theorie die SI-Einheiten
produzieren will muss irgendwo eine Brücke zu gemessenen Konstanten bauen.
Der Präzisierungsvorschlag: den Anspruch \emph{„ohne freie Parameter"}
auf die algebraische Schicht $e_i^2=+1\to\text{Cl}(6)$ einschränken,
und $T_\text{Planck}$ explizit als SI-Schnittstelle benennen.
In FFGFT sind $\xi$ und $m_e$ die SI-Schnittstelle (Dok.~012) ---
ein anderer Weg, aber dieselbe strukturelle Notwendigkeit.
...
Zwei Präzisierungsvorschläge, gestützt auf
...
(2)~Die G-Ableitung in~\cite{Matzke2026IPI} ist algebraisch zwingend
ab der SI-Schnittstelle $T_\text{Planck}$ --- der Anspruch
\emph{„ohne freie Parameter"} gilt präzise für die algebraische Schicht,
nicht für die SI-Einbettung.
...
In seinem IPI-Letters-Paper 2026~\cite{Matzke2026IPI} stellt Doug die Herleitung so vor:
\begin{quote}
...
\subsection{Die SI-Schnittstelle und was sie leistet}

Dougs Anfangsbedingung ist rein algebraisch und G-frei:
\begin{equation}
  e_i^2 = +1 \quad\text{(Hyperbit, $+1$-Signatur)}
\end{equation}
Daraus folgt die Witt-Zerlegung zu Nilpotenten $\{ap, an\}$ mit $ap^2=0$,
dann Cl(6), dann der Planck-Voxel --- alles ohne~$G$.

Die SI-Schnittstelle kommt erst durch $T_\text{Planck}$:
\begin{equation}
  M_\text{bit} = \frac{k_B\,T_P\,\ln 2}{c^2} = m_P\,\ln 2
\end{equation}
Da $m_P = \sqrt{\hbar c/G}$, enthält diese Schnittstelle~$G$ ---
aber das ist strukturell \textbf{unvermeidlich}: jede Theorie die
dimensionsbehaftete SI-Größen produzieren will muss irgendwo
gemessene Konstanten einsetzen. $T_\text{Planck}$ ist die
kleinstmögliche solche Brücke.
...
Der Präzisierungsvorschlag: den Anspruch \emph{„ohne freie Parameter"}
explizit auf die algebraische Schicht $e_i^2=+1\to\text{Cl}(6)$ einschränken.
Die SI-Einbettung via $T_\text{Planck}$ ist legitim und notwendig ---
sie sollte aber als solche benannt werden.
...
SI-Schnittstelle & $T_\text{Planck}$ (legitim, unvermeidlich) & $\xi$, $m_e$ \\
Gravitation  & $G$ aus $T_\text{Planck}$ zwingend & $G = \xi^2\hbar c/(4m_e^2)\cdot K$ \\
Status       & algebraisch zwingend ab SI \SM & andere SI-Wahl \SM \\
\bottomrule
\end{tabular}
\end{center}

Beide Frameworks erhalten denselben numerischen Wert für $G$
und beide brauchen eine SI-Schnittstelle.
GALG wählt $T_\text{Planck}$ --- minimal und physikalisch gut motiviert.
FFGFT wählt $\xi$ und $m_e$ --- geometrisch aus dem Torus abgeleitet.
Beide Wege sind legitim; der Unterschied ist die Wahl der Brücke,
nicht ob eine Brücke nötig ist.
