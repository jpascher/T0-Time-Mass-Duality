...
% ============================================================
\section{Modenspezifität, Nichtabschluss und Superposition}

\subsection{Jede Mode trägt ihre eigene Periode}

Die Periode $R_m = \hbar/(mc^2)$ ist modenspezifisch: jedes Teilchen
$i$ mit Torusmode $\mathcal{T}_i=(n_i,l_i,j_i)$ trägt seine eigene
Periode. Das Ausrollen (Typ~III oder Typ~I) ist daher stets
teilchenspezifisch -- es rollt eine bestimmte Mode aus, nicht eine
Mischung. Ein gleichzeitiges Ausrollen verschiedener Perioden
$R_{m_e}$ und $R_{m_\mu}$ wäre strukturell unzulässig, weil damit
$\tilde{T}\cdot m=1$ für beide gleichzeitig verletzt würde.

\subsection{Die Nichtabschluss-Bedingung (Dok.~193)}

Dieselbe Modenspezifität liegt der Nichtabschluss-Bedingung zugrunde
(Dok.~193): Die Vorfaktoren $r_i$ sind keine Zahlen in einem Körper,
sondern Etiketten diskreter geometrischer Räume -- vollständig durch
$\mathcal{T}_i$ bestimmt. Das \textbf{multiplikative} Kombinieren
verschiedener Moden,
\[
  (r_i\cdot r_j,\; p_i+p_j) \;\notin\; \mathcal{P}
  \qquad\text{für }i\neq j,
\]
ergibt kein FFGFT-Teilchen -- das Produkt liegt außerhalb der Menge
der zulässigen Paare \textbf{[B]}. $K_{\text{frak}}$ sitzt in der
modenspezifischen Wahl von $R_m$; ein modenübergreifendes
$K_{\text{frak}}$ wäre dasselbe unzulässige Kreuzprodukt.

\subsection{Warum Superposition trotzdem erlaubt ist — und wo der Unterschied liegt}

In der Quantenmechanik sind Überlagerungen verschiedener Zustände
nicht nur erlaubt, sondern grundlegend. Ein Elektron im
sp$^3$-Hybridorbital eines Kohlenstoffatoms ist eine
Linearkombination von s- und p-Zuständen; ein angeregtes Atom
befindet sich in einer Superposition aus Grundzustand und angeregtem
Zustand. Die Operation ist \textbf{additiv}:
\[
  \ket{\psi} = c_1\ket{\psi_1} + c_2\ket{\psi_2},
  \quad |c_1|^2 + |c_2|^2 = 1.
\]
Das erzeugt kein neues $(r,p)$-Paar, keine neue Masse, keine neue
Torusmode — und widerspricht damit der Nichtabschluss-Bedingung
nicht.

\textbf{Der entscheidende Unterschied zwischen QM und FFGFT:}
In der Standard-QM ist die Zeit $t$ kein quantenmechanischer
Operator und kein Teil des Hilbertraums -- sie ist schlicht
\emph{absent} als physikalische Größe des Systems. Die
Schrödinger-Gleichung behandelt $t$ als klassischen externen
Parameter, der den Zustand von außen entwickelt, ohne selbst
zum System zu gehören. Eine Superposition
$c_1\ket{\psi_1}+c_2\ket{\psi_2}$ hat daher keine
modengebundene Zeitstruktur -- $t$ gehört nicht zur
Superposition, sondern steht außerhalb.

In FFGFT ist Zeit geometrisch an die Masse gebunden:
$\tilde{T}_i = 1/m_i$. Zwei Moden mit verschiedenen Massen
$m_1\neq m_2$ tragen verschiedene Zeitskalen
$\tilde{T}_1\neq\tilde{T}_2$. Eine Superposition solcher Moden
würde nicht nur Amplituden mischen, sondern inkompatible
Zeitskalen überlagern — das ist strukturell stärker problematisch
als in der QM, wo dieser Konflikt nicht auftritt.

Im FFGFT-Korpus ist Superposition daher als
\textbf{intermediäre Torsion innerhalb einer Mode} beschrieben
(Dok.~174): ein metastabiler Zwischenzustand zwischen stabilen
Torsionsextremen \emph{derselben} Mode, nicht eine
modenübergreifende Überlagerung. Dekohärenz drängt diesen
Zwischenzustand in eine der stabilen Konfigurationen zurück.
...
Superposition & additiv, $t$ steht außerhalb & intermediäre Torsion
  innerhalb einer Mode \\
Modenübergreifend & algebraisch erlaubt & inkompatible $\tilde{T}$,
  strukturell problematisch \\
Nichtabschluss & nicht relevant & $r_i\cdot r_j\notin\mathcal{P}$,
  multiplikativ verboten \textbf{[B]} \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Die konsistente Grenze}

Die Linearität der Quantenmechanik -- das Superpositionsprinzip --
und die Nichtabschluss-Bedingung schützen dieselbe Grenze von zwei
Seiten: Atome und gebundene Zustände erzeugen keine neuen
fundamentalen Teilchen, keine neuen Torusmoden, kein neues
$(r,p)$-Paar. Die Verbindung zwischen FFGFT und etablierter
Atomphysik läuft über die abgeleiteten Größen $\alpha$ und $m_e$
(Dok.~011, 044, 006) -- die Quantenmechanik übernimmt diese als
Eingabe und leitet daraus Bindungsenergien und Spektren ab, ohne
in die Modenstruktur einzugreifen.
