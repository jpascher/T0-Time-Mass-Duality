\section*{Abstract}
	...
	\textbf{Validation}: Reduces $\Delta$ from 12.3\% to $<0.1\%$ at $5\pi/4$; theoretical prediction $\text{CHSH}^{\text{T0}} \approx 2.8275$ (vs.~QM 2.8284), $\Delta=0.04\%$ (hardware measurement gives $S=2.74$, decoherence-limited; see correction below).
	
	\subsection{$\xi$-Fit: Data Basis}
	
	\begin{tcolorbox}[colback=red!5,colframe=red!50,title=Correction (May 2026)]
		Earlier versions of this section used a value $S \approx 2.8275 \pm 0.0002$ as "2025 data" from "IBM-like runs" and fitted $\xi$ to it. This value is \emph{not substantiated} in available hardware records. The actual hardware measurement (Doc.~147, IBM Heron r2, May 2026) gives $S = 2.74$ ($96.9\%$ of the Tsirelson bound) -- the $-3.14\%$ deviation is NISQ decoherence, not the $\xi$-effect. The $\xi$-effect ($\sim 10^{-5}$) is not resolvable with present hardware; fitting $\xi$ to hardware CHSH values is therefore moot. The formula below remains valid as a \emph{theoretical} scaling structure.
	\end{tcolorbox}
...
\textbf{Addendum (3 September 2026):} The prediction of sterile neutrinos from
the Galois algebra is withdrawn (Doc.~343 Theorem~D$'''$, R108): the Majorana
layer $\{f_1,f_2\}$ is fully occupied by $\nu_1,\nu_2$; $\nu_3$ in orbit $O_4$
is a Dirac neutrino. The $\xi^3$ suppression in the DUNE row below is therefore
not to be interpreted as a sterile signal. Authoritative: Doc.~340 and Doc.~343.
\textbf{[B]}
...
\textbf{Testability}: First DUNE runs (2026): Vorhersage $\chi^2$/DOF $<1.1$ for T0-PMNS; sterile $\xi^3$-suppression ($\Delta P<10^{-3}$).
	...
	\end{equation}
	\textbf{Objection}: Does the theoretical $\text{CHSH}^{\text{T0}}\approx2.8275$ violate Bell's bound (2)?
	...
	\begin{longtable}{p{2.4cm}p{4.0cm}p{4.0cm}}
		...
		Bell CHSH & $2 + \xi\Delta_{\text{T0}}$ (qualitative) & $2.8275$ (N=73, theoret.; HW: $2.74$) \\
