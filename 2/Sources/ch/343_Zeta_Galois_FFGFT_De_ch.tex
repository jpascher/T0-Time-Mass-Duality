...
GF$(27)^*$-Schichten: $\{f_2, f_3\}$ und $\{f_1, f_4\}$ (Zuordnung in Satz~D$'''$).
...
Welche Quantenzahlen hätte die Schicht $\{f_1, f_4\}$? Die Algebra beantwortet
...
Frobenius (Dok.~339): $f_1 \mapsto O_1=\{1,3,9\}$, $f_2 \mapsto O_2=\{2,5,6\}$,
$f_3 \mapsto O_4=\{7,8,11\}$, $f_4 \mapsto O_3=\{4,10,12\}$. Dok.~340 nimmt die
...
$f_1$ & $O_1$ & $\to f_4$ & selbstinvers & Majorana & $\{f_1,f_4\}$ \\
$f_4$ & $O_3$ & $\to f_1$ & selbstinvers & Majorana & $\{f_1,f_4\}$ \\
\bottomrule
\end{tabular}
\end{table}

Die Sektorpaarung $k \mapsto -k$ und das Majorana-Kriterium $k \mapsto k^{-1}$
zerlegen die vier Klassen deckungsgleich: \textbf{$\{f_2,f_3\}$ ist die
Dirac-Schicht, $\{f_1,f_4\}$ die Majorana-Schicht} \BM. Beide Schichten tragen
...
G$(6)$ fest, keine Klasse; die Zuschreibung ,,$\{f_2,f_3\}$ = Leptonen
(Dok.~341)`` oben in Satz~D$'$ und ,,Leptonen $\to f_1$ (Dok.~341)`` in
Dok.~342 sind beide nicht gedeckt. Die Zuordnung folgt erst hier: Geladene
Leptonen sind Dirac-Teilchen und gehören in $\{f_2,f_3\}$.

\textbf{Deutung \SM.} Die zweite Schicht ist damit nicht neu, sondern der
Neutrinosektor: Dok.~340 setzt die aktiven Neutrinos in $O_3 = f_4$. Offen
...
D$'''$ & $\{f_2,f_3\}$ Dirac-Schicht, $\{f_1,f_4\}$ Majorana-Schicht (Sektorpaarung $=$ Majorana-Kriterium) & \BM \\
D$''''$ & $f_4=O_3$ aktive Neutrinos (Dok.~340); $f_1=O_1$ steril/rechtshändig, Seesaw-Partner & \SM \\
