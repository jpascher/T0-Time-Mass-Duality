...
\textbf{(D)}~Den vier Frobenius-Konjugationsklassen primitiver Elemente
in GF$(27)^*$ (Dok.~342, Satz~A) entsprechen vier Hecke-L-Funktionen
zu den Charakteren von $\mathrm{Gal}(\mathrm{GF}(27)/\mathrm{GF}(3)) \cong \mathbb{Z}_3$;
ob diese vier Klassen verschiedene Massenschichten tragen, ist offen \SM.

Prüfskript: \texttt{pruef\_343\_zeta\_galois.py}.
\end{abstract}
...
% ============================================================
\section{Satz D: Vier Hecke-L-Funktionen der GF$(27)$-Struktur}
% ============================================================

\subsection{Galois-Gruppe und ihre Charaktere}

Die Galois-Gruppe $\mathrm{Gal}(\mathrm{GF}(27)/\mathrm{GF}(3)) \cong \mathbb{Z}_3$
hat drei Charaktere: den trivialen $\chi_\mathrm{triv}$ und zwei nicht-triviale
$\chi_\omega$, $\chi_{\omega^2}$ mit $\omega = e^{2\pi i/3}$ \BM.

Den vier Frobenius-Konjugationsklassen primitiver Elemente in GF$(27)^*$
(Dok.~342, Satz~A: Polynome $f_1, f_2, f_3, f_4$) entsprechen vier
Hecke-L-Funktionen, die das Splitting-Verhalten von Primzahlen in der
Körpererweiterung GF$(27)/$GF$(3)$ beschreiben:

\begin{table}[H]
\centering
\caption{Splitting von Primzahlen $p \neq 3$ in GF$(27)/$GF$(3)$}
\begin{tabular}{lll}
\toprule
Splitting-Typ & Bedingung & Hecke-L-Funktion \\
\midrule
vollständig zerfällt & $p \equiv 1 \pmod{3}$, $\mathrm{ord}_p(3) \mid 1$ & trivial $\chi_\mathrm{triv}$ \\
Dreier-Zyklus & $\mathrm{ord}_p(3) = 3$ (z.\,B.\ $p=13$) & $L(s,\chi_\omega)$ \\
Dreier-Zyklus & $\mathrm{ord}_p(3) = 3$ & $L(s,\chi_{\omega^2})$ \\
...
Das Resultat aus Dok.~342 (Satz~A) ordnet die vier primitiven kubischen Polynome
$f_1, \ldots, f_4$ den vier Frobenius-Konjugationsklassen in GF$(27)^*$ zu.
Ob diese vier Klassen physikalisch verschiedene Massenschichten tragen
(analog dazu, wie $f_1$ die Leptonenschicht trägt), ist eine offene Frage \SM.

Falls ja, wäre jede Schicht durch ihre eigene Hecke-L-Funktion charakterisiert.
Die Schicht zu $\chi_\mathrm{triv}$ würde vollständig-zerfallende Primzahlen
($p \equiv 1 \bmod 3$, also $p = 7, 13, 19, \ldots$) bündeln;
die Schicht zu $\chi_\omega$ jene mit Dreier-Zyklen ($p = 13$ bei $k=3$).
...
D & Vier Hecke-L-Funktionen zu Gal(GF$(27)$/GF$(3)$)$\cong\mathbb{Z}_3$ & \BM \\
D$'$ & Physik der vier Schichten $f_1,\ldots,f_4$ & \SM \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \texttt{pruef\_343\_zeta\_galois.py} ---
        ...
        Resonanztest, gewichtetes Euler-Produkt, Hecke-Diskussion.
\end{itemize}
