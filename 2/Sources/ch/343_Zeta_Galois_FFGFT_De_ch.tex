...
Vorhersage: $\nu_3$ hat keine Majorana-Phase \BM.
...
Majorana-Schicht, $\{f_3,f_4\}$ die Dirac-Schicht} \BM. \textit{Präzisierung:}
Das Majorana-Kriterium ist eine algebraische Eigenschaft des Galois-Orbits
(Selbstinversheit unter $k\mapsto k^{-1}$ mod~13). Ob dies die physikalische
Majorana-Bedingung ($\nu = \bar{\nu}$) impliziert, ist eine weitergehende
Annahme \SM; die Übersetzung ist durch Dok.~344 (Anti-Neutrino-Frage) offen. Mit der Negation
...
\textbf{Deutung \BM/\SM.} Dok.~341 legt für die Leptonen die Ordnung~26 in G$(6)$
fest; die Klasse folgt hier: Geladene Leptonen sind Dirac-Teilchen und gehören
in $\{f_3,f_4\}$ \BM. Die algebraische Majorana-Schicht $\{f_1,f_2\}$ ist
vollständig mit aktiven Neutrinos besetzt: Dok.~340 setzt $\nu_1$ in $O_1=f_1$
(orbit-algebraisch Majorana) und $\nu_2$ in $O_3=f_2$ (orbit-algebraisch
Majorana) sowie $\nu_3$ in $O_4$ (Dirac-Orbit, nicht selbstinvers) \BM.
Ob $\nu_1,\nu_2$ physikalische Majorana-Teilchen sind ($\nu=\bar\nu$), ist
durch die Galois-Algebra nicht entschieden; in der Furey/GALG-Konstruktion
(Dok.~344) ist das Anti-Neutrino Sd[0] $=$ Vss[1] $\neq$ Vss[0] $=$ Su[0],
also sind die Neutrinos dort Dirac-Teilchen \SM. Das ist kein Widerspruch, sondern eine Aussage: $\nu_3$ ist ein
\textbf{Dirac-Neutrino} \BM. Es gibt keinen leeren Platz und keine Vorhersage
steriler Neutrinos aus dieser Algebra.

\textbf{Testbare Vorhersage \BM:} $\nu_3$ hat keine Majorana-Phase;
$m_{ee}$ hängt nur von $\nu_1$ und $\nu_2$ ab:
\[
  m_{ee} = |m_1 U_{e1}^2 + m_2 U_{e2}^2|
  \leq m_{\nu_1} + m_{\nu_2} \approx 14{,}4\,\text{meV}.
\]
Dok.~340 berechnet $m_{ee}\approx 7{,}1$~meV. Die invertierte Hierarchie ist
stärker ausgeschlossen, weil der $\nu_3$-Beitrag zu $m_{ee}$ entfällt.
Testbar bei nEXO ($m_{\beta\beta}\approx5$--$20$~meV, NME-abhängig); KamLAND-Zen-800: $<28$--$122$~meV (2025).
\textbf{Verbindung zu Dok.~006 \SM.} Dok.~006 hatte denselben qualitativen
Befund bereits formuliert: Neutrinos erfahren gegenüber geladenen Leptonen
einen zusätzlichen $\xi$-Unterdrückungsfaktor (,,Doppel-$\xi$-Unterdrückung``).
Satz~D$'''$ gibt den algebraischen Grund: Majorana-Orbits ($O_1$, $O_3$) tragen
diesen doppelten Faktor, der Dirac-Orbit ($O_4$, $\nu_3$) nicht. Quantitativ
sind die beiden Dokumente jedoch nicht konsistent: Dok.~006 gibt
$m_{\nu_e}=9{,}1$, $m_{\nu_\mu}=1{,}9$, $m_{\nu_\tau}=18{,}0$~meV --- im
Vergleich zu Dok.~340 $m_{\nu_1}=4{,}54$, $m_{\nu_2}=9{,}81$,
$m_{\nu_3}=49{,}9$~meV. Abweichungen bis Faktor~9; die Reihenfolge ist
verschieden ($\nu_\mu < \nu_e < \nu_\tau$ in Dok.~006 vs.\ $\nu_1 < \nu_2 <
\nu_3$ in Dok.~340). Die $\Delta m^2$-Werte aus Dok.~340 stimmen auf 0,5--1\,\%
mit dem Experiment überein; Dok.~006 tut das nicht. Dok.~340 ist der jüngere
und algebraisch besser begründete Stand.
...
D$''''$ & $\nu_3$ Dirac-Neutrino (O4); $m_{ee}$ nur von $\nu_1,\nu_2$ ($\leq$14,4 meV); IH stärker ausgeschlossen & \BM \\
