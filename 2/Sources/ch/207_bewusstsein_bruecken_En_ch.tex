...
\subsection{Determinism and agency}\label{sec:bridgeA_determinism}

Adlam's no-go bears directly on the question of will: a purely unitary system cannot carry agency. The usual escape -- quantum randomness as the source of freedom -- is, however, not open to FFGFT. The base model is deterministic and local: the Bell correlations are geometrically real on $T^4$, not ontologically non-local (Doc~230), and the apparent randomness is a projection artefact. FFGFT is thereby \emph{more} deterministic than standard quantum mechanics, not less.

This is a sharpening, not a deficiency: a free will compatible with FFGFT cannot rest on randomness -- it would have to be compatibilist, determinism \emph{and} agency at once. Determinism here is not predictability and not fatalism: a self-referential subsystem embedded in the system cannot anticipate its own state from within, even though that state is determined. This is the load-bearing anchor -- the embedded observer, whose lived situation the decompactification \emph{is} (Doc~248/270), not constitutive openness $\xi>0$ as a whole and certainly not an open parameter of the physics.

\begin{interpretation}
FFGFT's base model is unambiguously deterministic and \emph{compatible} with a compatibilist free will -- it neither \emph{grounds} nor \emph{refutes} it. It only removes the randomness reading of freedom and relocates the question to the self-referential, embedded observer. What is explicitly \emph{not} meant is to read physical underdetermination -- such as an unfixed phase degree of freedom -- as agency: contingency is not a decision, any more than randomness is. That would be the same category error as \enquote{$X=\xi^N$, therefore derived}.
\end{interpretation}
