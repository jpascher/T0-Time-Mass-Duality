...
\medskip\noindent\textbf{Nachtrag (Dok.~372, 21.\,9.\,2026).} Der Kandidat
$1/4$ ist durch das Ikosaeder-Matrixelement $|\langle v_0|R_5|v_1\rangle|^2 = 2/9$
ersetzt: der on-shell-Weinberg-Winkel $1-M_W^2/M_Z^2$ liegt auf $0{,}4\,\%$
bei $2/9$ (Dok.~372 §3.1). Der Vergleichswert $0{,}231$ oben ist der
$\overline{\rm MS}$-Wert; für Polmassen aus $\tilde T\cdot m=1$ ist das
on-shell-Schema das natürliche. Der Skalenfluss aus $\xi$ (Dok.~323)
beschreibt das $\overline{\rm MS}$-Schema und bleibt bestehen. Die
GF(9)-Aussage dieses Abschnitts --- Grad-2-Erweiterung erklärt die zwei
Eichgruppen --- ist davon nicht berührt.

\begin{keyresult}[Status~\KM]
GF(9) liefert die algebraische Begründung für zwei Eichgruppen
(Grad-2-Galois-Erweiterung). Der Zahlenwert folgt im on-shell-Schema aus dem
$A_5$-Matrixelement $2/9$ (Dok.~372), im $\overline{\rm MS}$-Schema aus dem
$\xi$-Skalenfluss (Dok.~323).
\end{keyresult}
...
% ============================================================
\section{$\xi$ als Galois-Zahl}
\label{sec:xi_galois}

Die Massenverhältnis-Identität aus §\ref{sec:ckm} führt zu einem
neuen Ergebnis: $\xi = 4/30000$ ist keine freie Konstante, sondern
eine Galois-Zahl~\KM.

\subsection{Kernidentität}

Die FFGFT-Formel (Dok.~006) und die Galois-Gruppenordnungen
aus §\ref{sec:drei_ergebnisse} ergeben:
\begin{equation}
  \frac{(r_\mu/r_e)^2}{\xi}
  ...
  = 43200
\end{equation}
Das sind nicht zwei Rechnungen desselben Wertes, sondern eine
einzige algebraische Gleichung. Daraus folgt:
\begin{equation}
...
Die Quantenzahlen $r_g = n_{\phi,g}^2/n_{\theta,g}$ (Dok.~317) folgen
aus einer Rekursion mit Galois-Startbedingung:

\begin{center}
\begin{tabular}{lcc}
\toprule
Wicklungszahl & Wert & Galois-Herkunft \\
\midrule
$n_{\theta,1}$ & 3 & $\text{char}(\text{GF}(3^n))$ \\
$n_{\phi,1}$   & 2 & $|\text{GF}(3)^*|$ \\
$n_{\theta,2}$ & 5 & Rekursion $= 3+2$; min.\ Primteiler $|\text{GF}(81)^*|$ \\
$n_{\phi,2}$   & 4 & $\varphi(5) = $ Euler-$\varphi$ des Primteilers \\
$n_{\theta,3}$ & 9 & Rekursion $= 5+4$ \\
$n_{\phi,3}$   & 5 & min.\ Primteiler $|\text{GF}(81)^*| = 80$ \\
\bottomrule
\end{tabular}
\end{center}

Alle Faktoren in der Kernidentität sind Galois-Ordnungen:
$8 = |\text{GF}(9)^*|$, $27 = |\text{GF}(27)|$,
$5 = \text{min.Prim}(|\text{GF}(81)^*|)$.
Prüfskripte: \texttt{pruef\_330} (Massenverhältnis),
\texttt{pruef\_331} ($\xi$-Herleitung).
