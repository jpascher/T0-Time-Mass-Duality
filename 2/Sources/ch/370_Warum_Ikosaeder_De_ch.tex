...
Zwei Fragen, die dieses Dokument aufgeworfen hat, sind inzwischen beantwortet
-- eine negativ, eine durch Umkehrung. Was danach bleibt, ist eine einzige.

\paragraph{Die Galois-Faktoren: das Ikosaeder erklärt sie nicht.}
$\phi^8$ ist exakt, aber $m_\tau/m_e$ ist $74\,\phi^8$ mit $74 = 2\cdot37$, und
$37$ ist die Zahl, die in $K_\text{frak}$ und in $1/\alpha = 3700/27$ steht
(Dok.~338). Im Phasenkanal (Abschn.~5) ist $37$ die Einheitswurzel $\zeta_{37}$,
und sie lebt in GF$(3^{18})$: die Ordnung von $3$ modulo $37$ ist $18$. Die
ikosaedrische Phase $\zeta_5$ lebt in GF$(81)=$GF$(3^4)$. Und $4\nmid18$: GF$(81)$
ist \emph{kein} Teilkörper von GF$(3^{18})$ \BM. Die beiden Phasen sitzen in
unverträglichen Erweiterungen. Das ist ein negatives Ergebnis: $37$ kommt nicht
aus dem Ikosaeder, weder geometrisch noch über den Phasenkanal. Was GF$(3^{18})$
enthält, ist $\phi$ (aus GF$(9)$) und die Leptonenordnung $26$ (aus GF$(729)$);
seine ,,neuen`` Primzahlen, die in keinem Teilkörper vorkommen, sind $19$ und
$37$. Der Grad $18 = 2\cdot3^2$ ist der Grad von $\phi$ mal $|\mathbb{Z}_3|^2$ --
das Kompositum der $\phi$-Erweiterung mit der $\mathbb{Z}_3^2$-Erweiterung
GF$(3^9)$. Warum dieses Kompositum, und warum $37$ statt $19$, ist offen \SM.

\paragraph{Die fraktale Korrektur: in Lesart~B hergeleitet.}
Dok.~352 nimmt die T0-Leiter als fundamental; dann ist der Schritt von
Prozent- auf $10^{-3}$-Niveau eine gemessene Korrektur, und ihre Herkunft wird
der QED zugeschrieben (Lesart~A). Dieses Dokument setzt Koide als fundamental,
weil es aus dem Ikosaeder folgt. In dieser Lesart~B ist die T0-Leiter
$r_k\,\xi^{p_k}$ eine rationale Näherung an die transzendenten Amplituden
$a_k^2$, und die Korrektur ist der Abstand zwischen Kosinus und Potenzgesetz --
nicht gemessen, sondern ausgerechnet \KM. Sie fällt für die drei Leptonen
verschieden aus, weil die drei Kosinuswerte verschieden sind. Beide Lesarten
geben dieselben Zahlen; Dok.~369 §\,4 stellt sie nebeneinander, Register R121
hält den Statuswechsel fest. Offen bleibt in Lesart~B nicht die Korrektur,
sondern: warum sind die Näherungsparameter genau die GF$(3^n)$-Wicklungszahlen,
und warum ist die Näherung so gut?

\paragraph{Die eine Frage, die bleibt.}
Beide Punkte laufen auf dasselbe zu. Das Ikosaeder liefert Phasen in GF$(81)$:
$\zeta_5$, $\phi$, den Koide-Winkel. Die Zahlentheorie liefert Ordnungen in
GF$(3^n)$: Wicklungszahlen, $37$, $K_\text{frak}$. Beide Seiten treffen die
Leptonmassen -- die eine parameterfrei auf $10^{-3}\,\%$ als Verhältnis, die
andere mit Anker auf Prozentniveau als absolute Zahl. Aber sie treffen sich
nicht: GF$(81)$ und GF$(3^{18})$ haben nur GF$(9)$ gemeinsam, also $\phi$ und
sonst nichts. Ob es eine Struktur gibt, in der die ikosaedrische Phase und die
Galois-Ordnung dieselbe Wurzel haben -- oder ob sie zwei unabhängige Zugänge
sind, die sich in $\phi$ berühren und sonst getrennt bleiben -- ist die offene
Kante dieses Dokuments \SM.
