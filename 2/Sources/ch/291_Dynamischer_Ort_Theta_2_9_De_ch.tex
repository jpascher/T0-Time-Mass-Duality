...
Dieser Überschuss ist \emph{nicht} die fraktale Massenkorrektur $K_{\text{frak}}=1-100\xi$
(Dok.~133): deren präzise Form $(D_f^{\text{eff}}/3)^{D_f^{\text{eff}}/2}$ mit
$D_f^{\text{eff}}\approx2{,}973$ ergibt einen Faktor $\approx100$ ($1{,}333\,\%$), während
$\delta$ nur $\approx89\,\xi$ ($1{,}187\,\%$) benötigt -- weniger, also nicht $K_{\text{frak}}$.
Und die Rekursionstiefe ist in Dok.~275 ausdrücklich \emph{nicht} unabhängig fixiert: $1/\varphi^3$
ist dort ein \emph{Durchgangspunkt}, kein Fixpunkt (der einzige Fixpunkt der Dämpfung ist $0$), und
zu \emph{jedem} Ziel $c\in(0,1)$ existiert eine Tiefe $k^*(c)$ mit identischem Parameterstatus. Dass
$\delta^*$ nahe $1/\varphi^3$ liegt, ist daher \emph{kein} Schließen der Rekursion, und ein Fixieren
der Tiefe auf gerade diesen Wert wäre nach Dok.~275 keine Auszeichnung. Der $89$-Schritt-Überschuss
bleibt damit ein offener, nicht hergeleiteter Feinwert.
...
Überschuss von oben. Form, Ordnung, Exponent und Richtung stehen fest; der exakte $\delta$-Wert
-- der $89$-Schritt-Offset -- ist weder $K_{\text{frak}}$ noch eine fixierte Rekursionstiefe und
bleibt offen (\texttt{delta\_golden\_daempfung.py}).
