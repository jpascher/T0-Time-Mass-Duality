\section*{Abstract}
	...
	\end{equation}
	The T0 description uses the Hubble energy $E_H = E_0 \cdot \xi^{41/4}$, yielding $H_0^{\text{T0}} = E_H/\hbar \approx 66.2\,$km/s/Mpc ($-1.9\%$ deviation from Planck); the exponent $41/4$ is calibrated to the measured $H_0$, not parameter-free (see below and Doc.~190, R35).
	...
	\begin{center}
		...
		\begin{tabular}{lccc}
			...
			\midrule
			\textbf{T0 description} & \textbf{66.2} & \textbf{calibrated to $H_0$} & $H_0 = E_H/\hbar$ \\
