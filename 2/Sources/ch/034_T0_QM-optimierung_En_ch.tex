\section*{Abstract}
	...
	\begin{thebibliography}{9}
		...
		Analysis based on \texttt{2/tex/003\_T0\_Grundlagen\_v1\_En.pdf}.
		
		\bibitem{pascher:ml_addendum}
		J. Pascher, \textit{T0 Quantum Field Theory: ML-derived Extensions}, T0-Document Series, Nov. 2025.
		Analysis based on \texttt{2/tex/022\_T0-QFT-ML\_Addendum\_En.pdf}.
		...
		Analysis based on \texttt{2/tex/018\_T0\_Anomale-g2-10\_En.pdf}.
		
	\end{thebibliography}

\section{Structural Placement}

\textbf{[K]} The formalism described here is an alternative representation, not
an extension of computational power. An $n$-qubit register spans a compact,
finite state space; by Weyl's law its eigenvalue density grows by a power law
$N(\lambda) \sim \lambda^{d/2}$. Arithmetic counting densities of the form
$T \ln T$ (Riemann-von Mangoldt) are therefore not representable in principle
(Doc.~316, August 2026).

\textbf{[K]} The predicted $\xi$ corrections are of order $\sim 10^{-5}$.
Hardware measurements (IBM Heron r2, May 2026, Doc.~147) show a
device-to-device variation of $1.4\%$ -- some $1400\times$ larger than the
effect sought. The test requires error-corrected qubits with sub-ppm stable
precision.
