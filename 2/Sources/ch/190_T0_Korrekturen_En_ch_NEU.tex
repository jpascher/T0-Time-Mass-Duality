...
R114 & 351, 352 & Lepton residuals: Koide most precise ($0.046\cdot\xi$); Galois residual $-77.6\cdot\xi$ & Doc.~352 \textbf{[K]} \\
...
\section{Note on R92 (self-correction)}

R92(ii) concerned a statement in the \emph{old} Doc.~190 (edition 23~August 2026,
now Archive-2), Precision~4: the statement classified there as an internal
consistency check --- that an object with $M_0=\xi m_P/2$ would have
Schwarzschild radius exactly $L_0$ --- is an \emph{identity} (from
$r_s=2GM/c^2$ and $\ell_P=Gm_P/c^2$ it follows that $r_s(x\,m_P/2)=x\,\ell_P$
for every $x$). It tests neither $\xi$ nor the exponent in $L_0$.
The classification as ``probe'' was too strong.
This finding is resolved in the present new edition; the Archive-2 edition
documents the origin.

\section{Currently Open Bridges}

\begin{itemize}\itemsep2pt
  \item $\tau_\mu$ directly from FFGFT, without detour via $G_F$ and $v$ (R112, Doc.~351) \textbf{[S]}
  \item Why do $\varepsilon_i$ decrease with generation? Attribution to QED extraction + SI projection (R113-bridge, Doc.~352~\S10) \textbf{[S]}
  \item Koide amplitude $d/c=\sqrt{2}$ from FFGFT geometry: \emph{closed by Doc.~353 \textbf{[B]}} --- $Z_3$ equal-distribution condition and GF$(27)^*$ coupling matrix (Doc.~348 Theorem~C)
  \item K-041-a/b/c: candidate formulas to be clarified by author review
  \item Cosmic exponent $41/4$ to be derived forward (P20)
  \item CMB peak selection: $\{1,6,14,26\}$ to be generated forward (P29/P31)
  \item $C_\ell$ projection: physical source/window function (P30)
  \item $\Omega_{\mathrm{DM}}$ quantitatively model-neutral (P17)
  \item Spectral dimension $d_s=1.86$ vs.\ FFGFT $\approx2$
  \item Basis mass $M_\mathrm{basis}=m_P$ in the fundamental sector: independent justification needed (R90) \textbf{[S]}
  \item Adversarial $D_4$ operator specificity test against fitted comparison carriers (R95) \textbf{[S]}
  \item $\iota$-embedding HLV$\,\subset\,$FFGFT in the time sector (Doc.~297)
  \item R91: label correction $\lambda\to M_\mathrm{basis}$ in Docs.~019/201 pending \textbf{[S]}
  \item R46: intermediate-scale formulation (Doc.~296) cross-cuts corpus line; not yet addressed \textbf{[S]}
\end{itemize}
