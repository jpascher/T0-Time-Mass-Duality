...
\begin{remark}[Addendum (15~June 2026): Krüger's PHI5 confirms phi non-specificity]
Krüger's \emph{corrected} pairwise audit (PHI5, June 2026) is \emph{negative for phi
uniqueness}: phi is not the strongest outlier; other irrational controls reach equal or
stronger one-vs-rest / pairwise separability. From Krüger's \emph{own} pipeline -- and
consistent with the independent pairwise audit -- this confirms the reading taken here:
$\varphi$ is a \emph{generic ordering signature, not a $T^4$- or HLV-specific feature}.
Krüger has accordingly reset the HLV programme to a conservative hybrid carrier geometry
(native 6D backbone, dual boundary loops, ICE fibre, non-exact holonomy; pre-declared
kill tests HICE0--HICE7) and no longer makes a phi-specificity claim. An independent HICE
pilot implementation reproduces his DICE0 status (``precondition not satisfied''):
gauge-null, null-model separation and destructive tests pass, but benign-stability fails
(phason/window sensitivity of the cut-and-project). For FFGFT this changes nothing -- the
framework anchors in the Euler Tonnetz, not the golden ratio; the finding only supports
the statement above.
\end{remark}
...
\subsection{Epistemic Limit: Determinism and Resonance Density}

The open question about the independent fixing of $k^*$ is not
accidental but reflects a fundamental methodological limit documented
elsewhere in the corpus (Doc.~038, Markov determinism).

The recursion $r(k{+}1) = r(k)(1-\xi)$ is fully deterministic: every
passage value $c\in(0,1)$ exists at a precisely determined depth
$k^*(c)$. However, the recursion produces no distinguished discrete
levels --- it is continuous, and the passage points lie so densely
that without an independent external anchor, no single one can be
singled out. The resonance pattern is too dense for unambiguous
identification from the value alone.

This is structurally identical to two other cases in the corpus:
\begin{itemize}
  \item \textbf{Large semiprimes (Doc.~176):} The factors exist
        uniquely, but the search is practically infeasible because the
        candidates lie too densely.
  \item \textbf{Quantum mechanics (Doc.~038):} The substrate is
        deterministic; a statistical description becomes necessary when
        micro-states are no longer resolvable --- not because physics
        is stochastic, but because epistemic resolution is missing.
\end{itemize}

The conclusion for this document: the non-fixability of $k^*$ is not
a flaw of the theory but an honest limit of the methodology. The
theory is fully determined; the question of which passage point is
physically distinguished is practically unanswerable without an
external anchor. This is precisely why Doc.~190 P35 protects against
reading a finding from the resonance density when there is none.
The status of Doc.~275 --- observation, open --- is the correct
consequence of this limit.
