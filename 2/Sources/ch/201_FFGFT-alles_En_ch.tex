\begin{t0box}[Summary]
	...
	On cosmological scales, it explains the large-scale coherence of the universe, cosmic acceleration, and horizon-scale correlations without inflation or dark energy, supported by T0's infinite homogeneous geometry with the universal parameter $\xi = \frac{4}{3} \times 10^{-4}$. The universe is static and infinitely homogeneous, without expansion. The T0 description yields $H_0 \approx 66.2\,$km/s/Mpc (see Document~026; the value is calibrated to $H_0$, not derived from $\xi$).
...
\[
\begin{aligned}
	\text{matter} &\to \text{perturbs } \rho \\
	&\to \text{gradients in } \theta \\
	&\to T_{\mu\nu} \\
	&\to \text{curvature} \\
	&\to \text{motion of matter}.
\end{aligned}
\]
...
\end{table}
	...
	The effective vacuum amplitude on cosmological scales is governed by the homogeneous mode with the universal parameter $\xi$, the same value that determines particle masses and coupling constants.
	...
	\[
	\rho_{\text{DE}} \approx \rho_0^{\text{cosmo}} \cdot \xi^2 \sim \text{constant},
	\]
	...
	\begin{table}[htbp]
		\centering        \resizebox{\linewidth}{!}{ % Passt Breite an Zeilenbreite an, Höhe proportional
	...
	In T0 Theory, galaxies are embedded in the larger cosmological homogeneous background (characterized by $\xi$).
...
% ============================================================
% Addendum 7 September 2026 — R91: lambda in m_T = lambda/xi
% Source: Doc. 190-Archive-2 (R91)
% Source document remains unchanged (append-only, cf. R50).
% ============================================================

\section*{Addendum R91: Label correction $m_T = \lambda/\xi$ (7~September 2026)}

The label $\lambda$ as ``Higgs coupling parameter'' in $m_T = \lambda/\xi$
is misleading (R91). Correct: $\lambda = M_\text{basis}$ (basis mass of the
respective sector). Full justification: addendum in Doc.~019 and Doc.~180 (R89/R90).
