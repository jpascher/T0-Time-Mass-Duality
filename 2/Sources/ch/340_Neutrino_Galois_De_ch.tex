...
\medskip\noindent\textbf{Nachtrag (Dok.~372, 21.\,9.\,2026).} Die
$\cos^2(2\pi k/13)$-Lesungen für $\theta_{12}$ und $\theta_{23}$ in diesem
Abschnitt sind Größenordnungsangaben: die zwölf Werte
$\{\cos^2(2\pi k/13),\,1-\cos^2(2\pi k/13)\}$ liegen so dicht, dass bei
$6\,\%$ Toleranz fast jeder Parameter zwischen 0 und 1 einen Nachbarn findet.
Für $\theta_{23}$ liefert Dok.~372 die schärfere Aussage
$\sin^2\theta_{23}\in\{4/9,\,5/9\}$ aus dem $A_5$-Matrixelementspektrum
(Vorhersage $|\sin^2\theta_{23}-1/2| = 1/18$); $\cos^2(10\pi/13) = 0{,}560$
und $5/9 = 0{,}556$ sind algebraisch verschieden (Grad 6 gegen rational) und
nur numerisch nah \BM. Die Herleitung
$\sin^2\theta_{13} = (25\xi)^{2/3}$ oben bleibt bestehen; sie verwendet
$|A_5|$ und $\xi$, nicht die $\cos^2$-Lesung. Für $\theta_{12}$ gibt es
weiterhin nur die Größenordnung \SM.
...
Dok.~343 (Satz~D$'''$) zeigt, dass der quadratische Charakter das allgemeine
Kriterium ist: Orbit~1 ist ebenfalls selbstinvers (Majorana, $\nu_1$); Orbit~4
ist nicht selbstinvers (Dirac, $\nu_3$). Eine eigenständige Vorhersage steriler
Neutrinos aus der Galois-Algebra entfällt \BM.
...
\approx7.1\,\text{meV}\quad(\text{Majorana-Phasen null};
\nu_3\text{ Dirac}\Rightarrow\text{kein }m_3\text{-Beitrag, Dok.~343}~\BM).
\]
Die KamLAND-Zen-Grenze $m_{ee}<28$--$122\,\text{meV}$ (PRL 135 (2025) 262501; NME-abhängig) wird deutlich
eingehalten. Obergrenze ohne $\nu_3$: $m_{ee}\leq m_{\nu_1}+m_{\nu_2}\approx14.4\,\text{meV}$.
Bei destruktiver Interferenz: $m_{ee,\min}\approx1.2\,\text{meV}$.
...
\item \textbf{$\nu_3$ ist Dirac-Neutrino} \BM:
  Keine Majorana-Phase für $\nu_3$; $m_{ee}\leq m_{\nu_1}+m_{\nu_2}\approx14.4\,\text{meV}$.
  Die invertierte Hierarchie ist stärker ausgeschlossen.
  Testbar: nEXO ($m_{\beta\beta}\approx5$--$20$~meV, NME-abhängig); KamLAND-Zen-800: $<28$--$122$~meV.
