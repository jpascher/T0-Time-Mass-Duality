...
\paragraph{Topology frames, dynamics determines -- the reading that was already right.} The answer that
carries here was at heart always the dynamical one: a mass ratio is \emph{only} dynamically thinkable.
The spectrum $\sqrt{m_k}$ is the solution of an eigenvalue problem, and the fixed point
$\mathcal R(\Phi_*)=\Phi_*$ (Doc.~203) is \emph{both at once}: the topology (Z$_3$, $T^4$) fixes the
\emph{form}, self-consistency the \emph{value}; the two cannot be separated. A computed finding
(\texttt{kann\_topologie\_2\_9\_erzwingen.py}) \emph{supports} exactly this reading: $\theta=2/9$
\emph{rad} is not a static flux root of unity -- $\theta/2\pi=1/(9\pi)$ is irrational, and by
Lindemann-Weierstrass $\mathrm e^{\mathrm i\theta}$ is transcendental. A \emph{purely
combinatorial/counting} topology (flux quantization of a finite group) therefore yields only the
\emph{spacing} $2\pi/3$ (Z$_3$, the denominator; $Q=2/3$ is forced for \emph{every} $\theta$ by
$r=\sqrt2$ alone), not the offset. But this does \emph{not} pit topology against dynamics:
transcendence-in-$2\pi$ is here \emph{no prohibition} but the \emph{expected} signature of a
dynamically (rather than combinatorially) determined quantity -- a counted value would be a root of
unity, a dynamically set one typically is not. Properly posed, the open question is therefore: the
topology frames (Z$_3$ spacing, the form), a \emph{dynamical} condition on that stage fixes the offset
-- and \emph{which} condition yields exactly $\theta=2/9$ is the hard, still open core. Excluded is only
pure counting topology, not topologically framed dynamics (the fixed point itself). \emph{(Aside:} the
value $2/9$ also appears in the corpus as a dimensionless mode-locking rotation number, Doc.~286; this
is \emph{not} dimensionally the same as the radian angle $\theta$ -- $2\pi\cdot2/9=4\pi/9\neq2/9$ --, so
the identification remains open.)

\paragraph{Why $2/9$ cannot be had symmetrically (the $\cos3\theta$ theorem).} There is an exact reason
why no simple \emph{symmetric} principle yields $\theta=2/9$ (\texttt{einfachstes\_prinzip\_2\_9.py}).
The entire $\theta$-dependence of the mass configuration $a_k=1+\sqrt2\cos(\theta+2\pi k/3)$ runs
\emph{only} through $\cos3\theta$: the elementary symmetric functions are $e_1=3$ and $e_2=3/2$ (both
$\theta$-independent), only $e_3=\det=-\tfrac12+\tfrac{\sqrt2}{2}\cos3\theta$ carries $\theta$. Hence
\emph{every} Z$_3$-invariant quantity extremizes solely at $3\theta=n\pi$, i.e.\ $\theta=n\pi/3$ -- the
Z$_3$-symmetric points $\{0,\tfrac{\pi}{3},\tfrac{2\pi}{3},\dots\}$. Concretely, the smallest-denominator,
the widest-tongue and the spectral-determinant principles all select $1/3$ or $1/2$, never $2/9$. The
reason is a structural theorem, not a computational shortfall: \emph{symmetric functionals cannot have
asymmetric values as extrema}, and $2/9$ lies $2/3$ of the way between $0$ and $1/3$, irreducibly
Z$_3$-breaking. Thus the simplicity principle is not refuted but shifted: not the \emph{simplest
symmetric fraction} ($=1/3$), but the \emph{minimal symmetry-breaking value} -- and the holonomy test
(Doc.~286) shows that $2/9$ is exactly this: the only member of the $9$-family that splits off as a
singlet under the holonomy ($1/9,4/9$ stay paired). Simplicity sits in the \emph{breaking}, not in the
symmetric ground state; what remains precisely open is only the defined symmetry-breaking measure whose
minimum yields $2/9$ (rather than $1/9$ or $4/9$).

\paragraph{Test: the breaking reaches $2/9$ but does not fix it.} A direct test
(\texttt{symmetriebrechung\_2\_9.py}) couples the second triplet $3'$ of the doubling ($6=3\oplus3'$,
Doc.~285) with strength $\delta$ and relative phase $\chi$ and looks for the shifted extremum
$\theta^*$ of the mass configuration. Result: the breaking \emph{reaches} $\theta^*=2/9$ -- so it is not
only necessary but sufficient --, but at a strength $\delta$ that must be tuned ($\delta\approx0.52$ at
$\chi=0$, $0.24$ at $\chi=\pi/2$) and coincides with no independently fixed value ($\xi$, $1/\sqrt2$,
$1/\varphi$, $1/3$). Thus $2/9$ is \emph{reachable but not point-derived} through this functional: the
breaking strength remains a free modulus -- consistent with the transcendence and the free holonomy
phase. The open question is thereby precisely shifted -- no longer \enquote{why $2/9$} but \emph{what
fixes the strength of the Z$_3$ breaking}. \emph{(Caveat:} this is \emph{one} natural breaking
functional; it does not exclude that another fixes $\delta$ independently -- e.g.\ the self-consistency
of the fixed point with the fixed lepton exponents, Doc.~203.)

\paragraph{The common reason: everything determined is radial, $\theta$ is angular.} Precisely this
fixed-point route was tested (\texttt{fixpunkt\_radial\_vs\_winkel.py}) -- and it does not bind $\theta$.
The fixed point $\mathcal R(\Phi_*)=\Phi_*$ (Doc.~203) fixes $\xi_0$ via $\xi_0^{p_i-1}K_{\text{frak}}=1$
from the exponents $(3/2,1,2/3)$; $\theta$ does \emph{not} appear in it. The radial sector ($\xi_0$,
hierarchy) and the angular sector ($\theta$, $Q$) are decoupled: $\xi_0$ is $\theta$-free, $Q=2/3$ is
$\xi_0$-free. The same pattern runs through the whole investigation: \emph{everything FFGFT determines is
radial} -- $\xi$, the exponents, the hierarchy, $Q=2/3=r{=}\sqrt2$, the mean $\langle f\rangle$ -- and
\emph{everything open is angular} -- $\theta=2/9$, the holonomy phase, the harmonic phases of $f$, the
breaking strength $\delta$. Fixed point, duality, recursion and the torus counting apparatus all live in
the radial sector without exception; none touches the angle. This is the common structural reason why
$\theta=2/9$ resists every tool. To fix the offset one needs an \emph{angular} principle -- a dynamical
angle condition -- which FFGFT does not yet possess and which, by the transcendence finding, cannot be a
counting/topological one.

\paragraph{Not static-geometric either -- closing the elimination.} There remains the question whether the
angular principle is a \emph{fixed geometric angle} of the shared icosahedral stage on which FFGFT's
$3$-fold ($C_3$) and HLV's $5$-fold ($C_5$) coexist ($C_3<A_5$, Doc.~283). Tested
(\texttt{ikosaeder\_winkel\_2\_9.py}, strict, hit threshold $0.05^\circ$): the icosahedral axis angles are
$20.9^\circ$, $31.7^\circ$, $36^\circ$, $37.4^\circ$, $41.8^\circ$, $54.7^\circ$, $58.3^\circ$, $60^\circ$,
$63.4^\circ$, $69.1^\circ$, $70.5^\circ$, $72^\circ$, $79.2^\circ$, $90^\circ$.
$\theta=2/9\,\mathrm{rad}=12.73^\circ$ matches \emph{none} (nearest $20.9^\circ$, distance $8.2^\circ$),
nor do $2\theta$ or $\theta/2$. Control: $\arccos(1/\sqrt5)=63.43^\circ$ matches the $C_5$--$C_5$ angle
\emph{exactly} -- the method is correct, so the negative is real. (Koide's $45^\circ$ is not an axis angle
either: FFGFT's angular structure is not the icosahedral geometry.) This closes the elimination:
$\theta=2/9$ is \emph{not} symmetric ($\cos3\theta$ theorem), \emph{not} counting-topological
(transcendence), \emph{not} radial (fixed point), \emph{not} static-geometric (this computation). Exactly
\emph{one} category remains -- the \emph{dynamical skew-adjoint phase} $R$ (Doc.~283), an accumulated,
Berry-like angle. FFGFT's own recursion $\mathcal R$ (Doc.~203) is purely real and possesses no such
generator; only the HLV side carries it ($R=$ BD17A). The static-radial FFGFT side is thereby
\emph{demonstrably exhausted} for $\theta$; the angular principle is structurally HLV's dynamical $R$,
with a precise target: an accumulated angular phase yielding $\theta=2/9$ -- transcendental in $2\pi$,
Z$_3$-breaking, the singlet of the $9$-family.
...
lepton problem is thereby reduced to a single question -- and this document's
investigation has made it \emph{angular}: not \emph{whether} $T^4$/Z$_3$ forces
the angle (no counting topology can, $2/9$ being transcendental in $2\pi$), but
which \emph{dynamical} angle principle yields it. The elimination shows:
everything determined is radial ($\xi$, hierarchy, $Q=2/3=\sqrt2$), only the
offset $2/9$ is open -- structurally HLV's skew-adjoint dynamical phase $R$.
\end{conclusionBox}
