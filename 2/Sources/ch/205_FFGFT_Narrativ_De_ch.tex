...
% Status: 7. August 2026 (überarbeitet: Standortbestimmung erweitert, Brückenformel P40-konform, Schlussbemerkung aktualisiert)
% ============================================================
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item $v$ -- eine \emph{Brückenkonstante} von $v \approx 246$\,GeV,
        die den Übergang von der dimensionslosen Geometrie in SI-Einheiten
        herstellt. Sie ist nicht frei gewählt, sondern liegt im Bereich
        der Higgs-Skala und trägt zusammen mit $\xi_0$ und den rationalen
        Vorfaktoren $r_\ell$ die gesamte Masseninformation (Dok.~241,
        A040). In Verhältnissen zwischen Leptonen kürzt sie sich heraus;
        sie wird nur bei Absolutwerten benötigt.
\end{itemize}
...
\begin{itemize}\setlength\itemsep{0pt}
  ...
  \item \emph{Dunkle Materie} und \emph{Dunkle Energie}: Galaktische
        Rotationskurven folgen aus der Übergangsskala $a_0 = c^2\xi_0^{10}/(4\bar\lambda_e)$,
        die aus $\xi_0$ allein bestimmt ist --- dunkle Materie ist ein
        Trägheitsregime, kein Stoff (Dok.~308). Die kosmologische Konstante
        erhält mit $\Lambda^* = 1/R_H^2$ einen geometrischen Träger; das
        $10^{123}$-Problem reduziert sich vollständig auf eine einzige
        deklarierte Kalibrierung (Dok.~312).
...
\section{Der aktuelle Stand --- eine Standortbestimmung}

Seit der ersten Fassung dieser Übersetzung hat sich der Korpus an
mehreren Stellen geschärft. Drei Entwicklungen prägen den heutigen Stand
des Leptonsektors: Die Massen, die Feinstrukturkonstante und die
Koide-Phase sind heute genauer verortet als zuvor --- der Sektor ist im
Wesentlichen geschlossen. Was noch offen ist, ist klar benannt und eng
umrissen; eine Theorie, die ehrlich bleiben will, trennt beides sauber.

\paragraph{Der Leptonsektor, sauber getrennt in zwei Schichten.}
Die Massen der drei geladenen Leptonen --- Elektron, Myon, Tau ---
lassen sich auf zwei Wegen aus der Geometrie gewinnen, und die beiden
Wege tragen unterschiedliche Genauigkeit. Der \emph{Zirkulant} (die
Diagonalisierung eines Z$_3$-symmetrischen Massenoperators) trägt die
Präzision: Er trifft das Verhältnis Myon/Elektron auf wenige
Millionstel. Die gröbere \emph{$\xi$-Leiter} trägt die Größenordnung:
Sie liefert dieselben Verhältnisse auf etwa ein Prozent. Das ist kein
bloßer Grundsatz-Unterschied: Die genauere Rechnung steht heute bereits
zur Verfügung --- sie gewinnt die Leptonverhältnisse aus nur zwei
Strukturzahlen ($\sqrt2$ aus $Q=2/3$ und dem Winkel $2/9$) und trifft
Myon/Elektron einige hundert Mal schärfer als die grobe Leiter. Die
Verhältnisse selbst sind heute also erheblich genauer bestimmbar, als eine
Ein-Prozent-Rechnung vermuten lässt. Beide Schichten teilen dasselbe
$\xi$, sind aber unabhängig --- und man darf sie nicht vermischen. Insbesondere fungiert das Myon/Elektron-Verhältnis als
\emph{ein} deklarierter Referenzpunkt; einmal als Anker gesetzt, ist es
verbraucht und keine Prüfgröße mehr. Die eine unausweichliche Prüfung,
die bleibt, ist die Tau-Masse: FFGFT sagt $m_\tau = 1776{,}97$ MeV voraus,
und die kommende Belle-II-Präzision wird darüber ohne Ausweichmöglichkeit
entscheiden.

Bemerkenswert an diesem Sektor ist die Feinstrukturkonstante $\alpha$.
Sie ist \emph{keine} Kalibrierung, sondern läuft über zwei unabhängige
Wege zur selben charakteristischen Energie zusammen --- einen empirischen
(aus den gemessenen Massen) und einen rein geometrischen (aus $\xi$
allein, ohne die Elektronmasse zu benutzen). Dass beide Wege sich auf
wenige Hunderttausendstel treffen, ist Überbestimmung, kein Fit. Der
winzige verbleibende Rest ist selbst eine Aussage: Er deutet darauf hin,
dass die gemessene \enquote{angezogene} Polmasse nicht ganz die
geometrisch fundamentale Größe ist --- eine dünne Verkleidungsschicht
zwischen der nackten Geometrie und dem, was das Experiment sieht.

\paragraph{Der Koide-Winkel $\theta = 2/9$ --- die Zirkulant-Phase.} Alles, was FFGFT am Leptonspektrum bestimmt, ist der
\emph{Betrag}: die Hierarchie, die Koide-Amplitude $Q = 2/3$. Der Winkel $\theta = 2/9$ ist die \emph{Phase} desselben Objekts --- und er
ist \emph{kein} freier Parameter: er ist der Wert, den die Diagonalisierung
des Z$_3$-Zirkulanten ausgibt, gefunden, nicht gesucht. Eine explizite Kette charakterisiert ihn: er ist keine symmetrische
Invariante, nichts Abzählbares oder Topologisches (er ist transzendent in
$2\pi$), kein statischer Ikosaeder-Winkel --- sondern eine dynamische,
betragserhaltende Holonomie-Phase. Dass er transzendent ist, ist dabei kein
Mangel, sondern positives Wissen: es \emph{beweist}, dass der Wert aus keiner
flachen oder abzählenden Quelle stammen kann.

Dass dieses Prinzip \emph{dynamisch} ist, hat eine unbequeme Konsequenz:
Es lässt sich innerhalb der statischen FFGFT-Geometrie nicht erzwingen ---
$2/9$ ist transzendent in $2\pi$ und kann daher keine abzählbare oder
topologische Invariante sein. Ein blinder, vorab versiegelter Test gegen
einen externen Holonomie-Mechanismus bestätigte genau diese Erwartung: das
Ergebnis war negativ, wie vorhergesagt. Der Wert liegt darin, dass das
\enquote{Nein} blind erzeugt wurde --- ein ehrliches Resultat, kein
zurechtgebogenes. Für FFGFT ändert das nichts an der Sache: Der Wert steht
fest, seine Herkunft als Zirkulant-Phase ist geklärt, seine Natur ist
bestimmt. Die Koide-Phase ist damit erledigt --- so weit hergeleitet, wie es
jede andere aus $\xi$ gewonnene Größe der Theorie auch ist.

\paragraph{Was der Maßstab ist.} Über all dem steht ein methodisches
Prinzip, das sich in dieser Arbeit immer wieder als das entscheidende
erwiesen hat: Der Maßstab einer geometrischen Herleitung ist nicht die
punktgenaue Landung auf der letzten Nachkommastelle --- die ist
prinzipiell unerreichbar, weil die Realität reicher ist als jede
Messtechnik. Der Maßstab ist, ob die Vorhersagen \emph{innerhalb der
Toleranzen} bleiben, in denen die Größen überhaupt bestimmbar sind.
Vollständig wäre die Theorie dann, wenn kein Rest mehr über diese
Toleranzen hinausragt, der nur durch einen freien Parameter zu schließen
wäre. An dieser Messlatte --- Toleranz, nicht letzte Ziffer --- ist der
Fortschritt zu bemessen. An ihr steht im Leptonsektor genau ein Punkt noch
offen: die Grundeinheit der generationsabhängigen Korrektur (die Zahl
$N_0 \approx 38{,}6$). Er ist benannt, nicht wegerklärt --- und er ist der
einzige.

\paragraph{Drei neue Stützpunkte (August 2026).}
Seit der letzten Fassung sind drei Dokumente hinzugekommen, die den
Gesamtrahmen schärfen:

\emph{Die Abschlussskala} (Dok.~312): Die $\Lambda$-Funktion der
Feldgleichungen erhält einen geometrischen Träger --- $\Lambda^* = 1/R_H^2$,
kein neuer Parameter, dimensionslos $\Lambda^*\bar\lambda_e^2 = (\pi/2)^2\xi_0^{20}$.
Das lange offene $10^{123}$-Problem der kosmologischen Konstante zerfällt
vollständig in geometrische Faktoren; Feinabstimmung ist Buchhaltung,
kein Rätsel. Zugleich folgt die kompakte Zeitrichtung mit
$\tau_c = 2\pi/H_0 \approx 91{,}9$ Gyr; der Zyklus ist
vier Kammertöne mal die $\xi_0$-Leiterstufe, $L^* = 4\,\bar\lambda_e/\xi_0^{10}$,
ganzzahlig und ohne freien Vorfaktor (Dok.~190 R73).

\emph{Kein Anfang} (Dok.~313): Die gesamte beobachtbare Geschichte
füllt einen Halbzyklus --- der Partikelhorizont liegt auf $0{,}14\,\%$
am Antipoden. Der \enquote{Urknall} ist ein \emph{Ort} auf dem Zyklus
(Region kleinster Massen, langsamster Uhren), kein Ereignis und keine
Singularität. Aus $\xi_0$, $H_0$ und der Antipodenbedingung folgt
$\Omega_m^* = 0{,}3136$ --- ohne einen einzigen an $\Omega_m$
angepassten Parameter; der Planck-Wert beträgt $0{,}315 \pm 0{,}007$
($-0{,}19\,\sigma$). Das ist Überbestimmung, kein Fit.

\emph{Gitter im Hilbertraum} (Dok.~314): Der D4-Verband auf der $\mathbb{C}^3$-Faser
von $H = L^2(T^4) \otimes \mathbb{C}^3$ macht die Entartungsstruktur
der Leptonen explizit. Die Aufspaltung $24 = 12 + 12$ trennt räumliche
und Massenrichtung; die Zeitperiode folgt aus $T_{\rm osc}\cdot m = 2\pi/\gamma$,
Gleichheit nur bei der ruhenden, scharfen Mode.
Zugleich klärt Dok.~315, ob die fraktale Korrektur additiv
($1-100\xi_0$) oder multiplikativ ($(1-\xi_0)^{100}$) zu lesen ist:
die Hochpotenz $K_{\rm frak}^{-36} \approx 16/\pi^2$ --- der
Kehrwert der D4-Packungsdichte --- diskriminiert die additive Form
mit einem Faktor $31:1$. Wert $[K]$, Form $[B]$, bedingt bestätigt.
...
Wer die formalen Ableitungen prüfen will, findet sie in der A-Serie
(A010--A273): Die Grundlagen in Block~0 (A010--A095), den
Leptonsektor in A100--A130, die Methodik in Block~2 (A200--A250)
und die neuesten Erweiterungen --- Thermodynamik der Berechnung,
Abschlussskala, Zeitzyklus --- in Block~3 (A260--A273) sowie
in Dok.~312, 313, 314, 315. Das Korrekturregister Dok.~190 führt
alle Präzisierungen append-only; kein Quelldokument wird verändert.

Diese Behauptung kann richtig sein oder falsch. Sie ist auf
mehreren Wegen prüfbar. Innerhalb des Leptonsektors ist die
schärfste Vorhersage bereits gesetzt: $m_\tau = 1776{,}97$ MeV;
das Belle-II-Experiment wird mit $\sigma \approx 0{,}05$ MeV
ohne Ausweichmöglichkeit entscheiden. Im kosmologischen Sektor
hängt die gesamte $\Omega_m^*$-Kette an keinem angepassten
Parameter --- ein möglicher Widerspruch gegen künftige
CMB-Messungen wäre direkt falsifizierend. Ferner: kleine
$\xi_0$-Korrekturen in Bell-Tests ($\Delta\mathrm{CHSH} \sim \xi_0/(2\pi)$),
modifizierte Dispersionsrelationen bei hohen Energien,
geometrische Strukturen in Qubit-Phasenräumen. Welche dieser
Vorhersagen sich bewährt und welche nicht, wird die kommenden
Jahre entscheiden.
