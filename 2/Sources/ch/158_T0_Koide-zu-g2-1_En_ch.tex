...
This step of $1/3$ is not arbitrary. The lepton exponents form a geometric
sequence with ratio $q = 2/3$: $(p_e, p_\mu, p_\tau) = (3/2, 1, 2/3)$.
The step $\Delta p_{\mu\to\tau} = p_\mu \cdot (1-q) = 1 \cdot 1/3 = 1/3$
follows from this ratio, not directly from $1/D$.
The different step $1/2$ for the electron
($p_e \cdot (1-q) = 3/2 \cdot 1/3 = 1/2$) is a consequence
of the same geometric sequence.
...
This is the central result: \textbf{The step between the heavier lepton generations ($\mu\to\tau$) is $1/3$ in both the mass hierarchy and the $g$-$2$ hierarchy.} This is not a fit --- it follows from the geometric sequence $p_{n+1} = (2/3)\,p_n$, where $p_\mu \cdot (1-2/3) = 1/3$. The bridge formula uses precisely this $\mu$-$\tau$ transition.
...
\end{itemize}
Both are aspects of the same winding mode. The step $1/3$ between
the heavier generations follows from the geometric sequence
$p_{n+1} = (2/3)\,p_n$ — the ratio $q = 2/3$ is numerically identical
to the Koide value $Q = 2/3$.
...
\begin{enumerate}
	...
	\item T0 explains Koide through the geometric sequence $p = 3/2, 1, 2/3$ with ratio $q = 2/3$; the step $1/3$ in the $\mu$-$\tau$ transition follows from $p_\mu \cdot (1-q) = 1/3$.
...
See addendum P1 in Doc.~116: the values $\Delta Q < 0.00003\,\%$ and
$0.001\,\%$ are both correct and describe different contexts (internal
calculation vs.\ external presentation with measurement uncertainty of
the input masses).
