...
\subsection*{Reduktion ist die Bedingung der Auswertbarkeit}

Eine Projektion ist immer eine Reduktion -- sie lässt weg. Das ist der
Verlust. Aber gerade indem sie weglässt, macht sie einen Teilbereich
\emph{auswertbar}, der im vollen Objekt untergegangen wäre. Das volle
$T^4$-Objekt trägt alle Symmetrien gleichzeitig; genau deshalb ist es als
Ganzes nicht direkt auswertbar. Erst die Reduktion auf eine Skala isoliert
eine Symmetrie so weit, dass man mit ihr rechnen, sie messen, aus ihr
etwas ableiten kann. Reduktion und Fokus sind dasselbe.

Das ist kein Notbehelf, sondern der Mechanismus des Erkennens: jede
Auswertung braucht einen Fokus, und jeder Fokus ist eine Reduktion. Der
Teilchenphysiker, der $\alpha$ misst, wertet die feinste Skala aus, weil er
alles Gröbere wegprojiziert hat; der Kosmologe, der die Rotverschiebung
misst, wertet die gröbste aus, weil er alles Feinere integriert hat. Beide
gewinnen ihre Auswertung durch das, was sie weglassen.

Entscheidend ist: die neuen Auswertungen sind im vollen Objekt nicht schon
enthalten -- sie entstehen an der Projektionsebene. Ein Phonon existiert
nicht auf der Teilchenskala; es ist eine Auswertung, die erst auf der
kristallinen Projektion möglich wird. Ein Orbital ist keine Eigenschaft von
$T^4$, sondern eine Auswertung auf der atomaren Skala. Die Projektion
\emph{erzeugt} auswertbare Struktur, die es ohne die Reduktion nicht gäbe.
Das ist der Grund, warum jede neue Skala neue Physik hervorbringt: nicht
weil dort neue Objekte sind, sondern weil dort neue Auswertungen möglich
werden. Eine Projektion, die nichts wegließe, würde nichts auswerten.

FFGFTs eigene Methode ist ein Spezialfall davon: $\theta = 2/9$ wird
auswertbar, indem auf die $\mathbb{C}^3$-Faser projiziert wird
(Hilbertraum-Transformation, Dok.~230/282). Auf $T^4$ direkt war es
verdeckt; erst die Reduktion auf die Faser machte es als
Diagonalisierungs-Ausgang des $\mathbb{Z}_3$-Zirkulanten sichtbar
(Dok.~291). Die Reduktion war nicht Verlust, sondern die Bedingung, unter
der die Struktur überhaupt lesbar wurde.
...
\subsection*{Kognition: rekursive Schichtung und sensorische Bindung}

Die biologische Ebene führt auf einen Grundstein, der in der Diskussion um
Bewusstsein bereits gelegt wurde: kein Bewusstsein ohne Rekursion und ohne
Sensorik. Beide Bedingungen sind notwendig, und beide sind
FFGFT-Strukturelemente.

\emph{Ohne Rekursion} kein Bewusstsein: es braucht die rekursive Schichtung,
die sich auf sich selbst anwendet -- Selbstbezug, akkumulierendes Gedächtnis,
eine Struktur, die ihre eigene Vorgeschichte trägt. Das ist dieselbe
generative Bauform wie die K\textsubscript{frak}-Rekursion, aus der die
Skalenfolge hervorgeht: eine Ebene erzeugt die nächste nach derselben Regel.
Ein bloßes gleitendes Gedächtnisfenster, das ältere Zustände verwirft, genügt
nicht -- die eigentliche Leistung ist die Akkumulation, das strukturelle
Tragen der Geschichte, nicht ihr Ersetzen.

\emph{Ohne Sensorik} kein Bewusstsein: kognitive Prozesse funktionieren nie
ungebunden. Sie sind stets an ein Substrat gekoppelt, das über sensorische
Kanäle Projektionen der Welt aufnimmt. Das ist die Beobachterbindung dieses
Dokuments: kein freischwebender Prozess, sondern ein an eine Skala gebundener
Beobachter, der Welt als Projektion empfängt.

Damit sind die zwei Bedingungen, ohne die es kein Bewusstsein gibt --
rekursive Schichtung und sensorische Bindung --, genau die zwei
Strukturelemente, die FFGFT ohnehin trägt: die Rekursion und die gebundene
Projektion. Das ist \emph{keine} Ableitung des Bewusstseins; ein solcher
Anspruch wird hier nicht erhoben und läge jenseits des Gezeigten. Es ist eine
Konvergenz auf der Ebene der \emph{notwendigen Bedingungen}: ein
bewusstseinsfähiges System muss rekursiv geschichtet und substratgebunden
sein, und beides ist FFGFT-nativ.

Zwei Abgrenzungen halten dies ehrlich. \emph{Erstens} teilen kognitive und
physikalische Prozesse die generative Bauform (rekursive Schichtung), aber
auf der kognitiven Seite liegt bislang kein messbares, quantitatives
Rekursionsgesetz vor -- kein \enquote{$\xi$ der Kognition}, keine Ganghöhe,
kein Verhältnis $e$. Geteilt ist die Form, nicht (noch) das Gesetz.
\emph{Zweitens} ist das Fehlen eines solchen Gesetzes kein Beleg gegen eine
gemeinsame Natur der Prozesse: fehlende Messbarkeit spricht nicht gegen die
zugrundeliegende Struktur, sondern nur gegen ihre gegenwärtige Zugänglichkeit
-- wie eine unzureichende Messtechnik nie gegen das misst, was sie noch nicht
auflöst. Die Frage bleibt damit offen nach oben: plausibel aus der
FFGFT-Struktur nahegelegt (alles als Projektion desselben rekursiven
Objekts), nicht widerlegt, nur nicht gezeigt. Der Grundstein -- geteilte
notwendige Bauform -- steht; die vollständige Verbindung ist Programm.
