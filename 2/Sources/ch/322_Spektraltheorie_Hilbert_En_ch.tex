...
\end{equation}
is dense in $\mathcal{H}$. $\hat{F}$ is essentially self-adjoint on
$\mathcal{D}(\hat{F})$. \textbf{[S]} The complete proof (deficiency
index classification) is outstanding.
...
formulae from Doc.~320 are identified as eigenvalue equations.
The outstanding point is the full self-adjointness proof for $\hat{F}$.
