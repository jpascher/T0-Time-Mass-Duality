...
R115 & Dok.~341 (§6 neu), 355; pruef\_341\_primordnung, \_root7\_gf9, \_r26\_rein\_p27 & \textbf{Höhere Einheitswurzeln in GALG G$(6)$ und Koeffizientenreinheit (8.~September~2026).} (1)~Eintrittsskala $k=\mathrm{ord}_p(3)$: 13 ($k{=}3$), 5 (4), 11 (5), 7 (6); in keinem GF$(3^k)^*$ Elemente der Ordnung~3 ($3^k-1\equiv2$ mod 3) --- Tripotente sind Matrix-, nie Körperelemente \textbf{[K]}. (2)~Keine 7te Einheitswurzel mit Z3C-Koeffizienten ($7\nmid8$; $\cos(2\pi/7)$ Minimalpolynom $8x^3+4x^2-4x-1\notin$ GF$(3^k)$); Euler-Form $\cos\theta+B\sin\theta$ ist kein GALG-Element \textbf{[K]}. Ordnung-7-Element über GF$(9)$: $X_7=C_{\Phi_7}\oplus I_2$ (47 Blades, rein) \textbf{[K]}; kompakt $Y$ (6 reine Blades, ord $182=2\cdot7\cdot13$), $X=Y^{26}$ (10 Blades) \textbf{[B]}. (3)~$X^{27}=X\Leftrightarrow$ Spektrum $\subset$ GF$(27)$ (Matzkes ``p27''); Körperturm $X^3{=}X\subset X^9{=}X\subset X^{27}{=}X\subset X^{729}{=}X$ \textbf{[K]}. (4)~$1+j$ = Generator von GF$(9)^*$; Reinheitsforderung (Koeffizient $\in\{\pm1\}\cup\{\pm j\}$) ist physikalisch, nicht algebraisch, und erfüllbar: reines root26 mit 5 Blades \textbf{[B]}. (5)~GALG-Konvention $E_7=-(e_1\cdots e_6)$ erklärt Matzkes p27-Befund für $X_7$ (Pseudoskalar-Vorzeichen) \textbf{[K]}. Folge für Dok.~355: ``Phasen $k\cdot360°/7$'' von \textbf{[B]} auf \textbf{[S]} --- Charakteristik~3 kennt keine Winkel. \\
\end{longtable}
\endgroup
...
\begin{itemize}\itemsep2pt
  ...
  \item Warum fallen $\varepsilon_i$ mit der Generation? Die $\varepsilon_i$ sind berechnete Residuen \textbf{[K]}; ihre Attribution zu QED-Extraktion + SI-Projektion ist \textbf{[S]}; ob die QED-Selbstenergie aus der T$^4$/Z$_3$-Geometrie herleitbar ist, ist offen --- R113-Brücke (Dok.~352, §\,10) [S]
