...
R97 & Dok.~332 & Dritte Krüger-Präzisierung: punktweise Auswertung/Differenzierbarkeit ist keine Eigenschaft roher $L^2$-Elemente (Äquivalenzklassen fast-überall-gleicher Funktionen), sondern setzt einen hinreichend regulären Repräsentanten voraus ($C^0$ bzw.\ $C^1$/Sobolev $H^1$); Dok.~332 Abschnitt~2.1 entsprechend präzisiert \textbf{[K]} \\
R98 & Dok.~332, 190/R97 & Korrektur an R97: Krügers ursprünglicher Einwand (,,$H^1$ liefert nur schwache Ableitung, keine punktweise Differenzierbarkeit``) unterschätzte, was $H^1$ in 1D tatsächlich liefert: Die Sobolev-Einbettung $H^1(I)\hookrightarrow C^{0,1/2}(I)$ gibt einen absolut stetigen, Hölder-$1/2$-stetigen Repräsentanten, klassisch differenzierbar fast überall, mit Ableitung $=$ schwache Ableitung f.ü. -- weit mehr als nur eine schwache Ableitung. Zugleich gilt auch in 1D: $H^1$ garantiert weder klassische Differenzierbarkeit an jedem Punkt noch $C^1$-Regularität (Gegenbeispiel $f(t)=|t|\in H^1\setminus C^1$: nicht differenzierbar bei $t=0$). Der Unterschied zu $C^1$ ist zweifach: (a) $H^1$ gibt Differenzierbarkeit nur fast überall, nicht an jedem Punkt; (b) $C^1$ verlangt zusätzlich Stetigkeit der Ableitung. Dok.~332 Abschnitt~2.1 entsprechend präzisiert \textbf{[B]} \\
\end{longtable}
\endgroup
...
\textbf{R97 -- Präzisierung: Auswertung/Differenzierbarkeit sind Eigenschaften des Repräsentanten, nicht des $L^2$-Elements (August 2026):}
Dritte Rückmeldung (M.~Krüger, IPI, 26.~August 2026) zu Dok.~332,
Abschnitt~2.1: $L^2(S^1_m)$ (ebenso $\mathrm{Per}_{R_m}(\mathbb{R}_t)$)
besteht aus Äquivalenzklassen fast-überall-gleicher Funktionen; ein
Funktionswert an einem festen Punkt $t$ ist für ein allgemeines
$L^2$-Element ohne zusätzliche Regularität schlicht nicht wohldefiniert
-- unabhängig davon, ob man auf $S^1_m$ bleibt oder den Pullback nach
$\mathbb{R}_t$ vollzieht. Punktweise Auswertung und erst recht
Differenzierbarkeit setzen einen konkreten, hinreichend regulären
Repräsentanten voraus ($C^0$ für Auswertung, $C^1$ bzw.\ ein Sobolev-Raum
$H^1$ für Differenzierbarkeit). Der Pullback $\Phi=p^*$ ändert daran
nichts: Er liefert einen nichtkompakten Koordinatenbereich
$\mathbb{R}_t$, auf dem hinreichend reguläre periodische Repräsentanten
ausgewertet und abgeleitet werden können -- ein Gewinn an
Koordinatenstruktur, keine Eigenschaft, die aus der bloßen
$L^2$-Zugehörigkeit folgt. Status \textbf{[K]}.

\textbf{Einordnung:} Die vorherige Formulierung in Dok.~332
(,,$f$ ist an jedem Punkt $t\in\mathbb{R}$ ... auswertbar, ableitbar``)
hat diese Eigenschaft implizit dem $L^2$-Element selbst zugeschrieben.
Dok.~332 wurde entsprechend präzisiert: Die Eigenschaft wird jetzt
explizit einem hinreichend regulären Repräsentanten von $f$ zugeordnet.
Dies ist eine Formulierungskorrektur, keine inhaltliche Änderung an der
R96-Kernaussage (Isomorphie zu $L^2(S^1_m)$, kein neuer Sektor).

\emph{Deklariert 26.~August 2026}

\medskip

\textbf{R98 -- Korrektur an R97: Sobolev-Einbettung in 1D, $H^1$ liefert bereits fast-überall-Differenzierbarkeit (August 2026):}
Vierte Rückmeldung (M.~Krüger, IPI, 26.~August 2026) zu R97: Krüger merkt
an, ,,membership in $H^1$ guarantees the existence of a weak derivative
in $L^2$, but not classical pointwise differentiability or $C^1$
regularity`` und schlägt vor, R97 entsprechend abzuschwächen. Prüfung
ergibt: Diese Aussage unterschätzt, was $H^1$ in 1D tatsächlich liefert.
Der Sobolev-Einbettungssatz in einer Dimension liefert das stärkere
Resultat
\[
H^1(I) \;\hookrightarrow\; C^{0,1/2}(I)
\]
(Hölder-$1/2$-stetige Einbettung) auf jedem beschränkten Intervall $I$
bzw.\ auf $S^1_m$ selbst: Jedes $H^1$-Element besitzt einen stetigen,
absolut stetigen Repräsentanten, der nach dem Hauptsatz der
Differential- und Integralrechnung für Lebesgue-Integrale \emph{fast
überall} klassisch differenzierbar ist, mit Ableitung gleich der
schwachen Ableitung fast überall -- weit mehr als nur eine schwache
Ableitung in $L^2$. Gegenbeispiel zur Illustration:
$f(t)=|t|$ liegt in $H^1([-1,1])$ (schwache Ableitung $\mathrm{sign}(t)$,
beschränkt, also in $L^2$), ist nicht $C^1$ (Knick bei $t=0$), aber
überall stetig auswertbar und außer bei $t=0$ klassisch differenzierbar.
Zugleich zeigt dasselbe Gegenbeispiel: Auch in 1D garantiert $H^1$ weder
klassische Differenzierbarkeit an \emph{jedem} Punkt noch $C^1$-Regularität.
Der Unterschied zu $C^1$ ist zweifach: (a) $H^1$ gibt Differenzierbarkeit
nur fast überall, nicht an jedem Punkt; (b) $C^1$ verlangt zusätzlich
Stetigkeit der Ableitung. Status \textbf{[B]}.

\textbf{Einordnung:} Krügers ursprünglicher Einwand unterschätzte, was
$H^1$ in 1D liefert; seine nachfolgende Präzisierung (zweite Rückmeldung
zu R98, selber Tag) ist mathematisch korrekt: Auch in 1D gilt
$H^1\not\Rightarrow$ Differenzierbarkeit an jedem Punkt,
$H^1\not\Rightarrow C^1$. Dok.~332 Abschnitt~2.1 wurde entsprechend
präzisiert: Die $H^1$-Formulierung benennt den Sobolev-Gewinn (fast-überall-Differenzierbarkeit) und die verbleibende Lücke zu $C^1$ (fehlende
punktweise Differenzierbarkeit überall, fehlende Stetigkeit der Ableitung)
explizit. Dies korrigiert R97 inhaltlich, lässt R96 unberührt.

Prüfskript: \texttt{pruef\_marcel\_h1\_sobolev.py} (3/3 Prüfungen).\\
\emph{Deklariert 26.~August 2026}
