...
R98 & Dok.~332, 190/R97 & Korrektur an R97: Krügers Einwand gegen ,,$C^1$ oder $H^1$`` (,,$H^1$ liefert nur schwache Ableitung, keine punktweise Differenzierbarkeit``) ist korrekt für $n\geq2$, aber auf den hier vorliegenden 1D-Fall ($n=1$, Funktionen einer Variablen $t$) nicht anwendbar: Sobolev-Einbettung $H^1(I)\hookrightarrow C^{0,1/2}(I)$ liefert dort einen absolut stetigen Repräsentanten, fast überall klassisch differenzierbar mit Ableitung $=$ schwache Ableitung f.ü.; $C^1$ ist die strengere Bedingung stetiger (statt nur f.ü.\ definierter) Ableitung, kein qualitativ anderer Differenzierbarkeitsbegriff; Dok.~332 Abschnitt~2.1 entsprechend präzisiert (Gegenbeispiel $f(t)=|t|\in H^1\setminus C^1$, f.ü.\ differenzierbar) \textbf{[B]}
\end{longtable}
\endgroup
...
ergibt: Diese Aussage ist \textbf{korrekt für $n\geq2$} (Standardbeispiel:
$f(x)=\log\log(1/|x|)$ nahe dem Ursprung liegt in $H^1$ einer 2D-Scheibe,
ist aber unbeschränkt), \textbf{trifft jedoch nicht auf den hier
vorliegenden Fall $n=1$ zu} (Funktionen einer einzigen reellen Variablen
$t$ auf $S^1_m$ bzw.\ $\mathbb{R}_t$). Der Sobolev-Einbettungssatz in
einer Dimension liefert das stärkere Resultat
\[
...
schwachen Ableitung fast überall. Gegenbeispiel zur Illustration:
...
$C^1$-Regularität ist damit keine qualitativ andere, sondern nur eine
\emph{strengere} Bedingung (stetige statt nur fast überall definierte
Ableitung). Status \textbf{[B]}.

\textbf{Einordnung:} Krügers Einwand verallgemeinert einen für $n\geq2$
zutreffenden Fakt stillschweigend auf den hier vorliegenden 1D-Fall, in
dem die schwächere Aussage nicht mehr die volle Wahrheit ist. Dok.~332
Abschnitt~2.1 wurde entsprechend präzisiert: Die $H^1$-Formulierung
nennt jetzt explizit die 1D-Sobolev-Einbettung, das Gegenbeispiel
$f(t)=|t|$, und die Klarstellung, dass die Differenz zu $C^1$ in der
Gleichmäßigkeit, nicht in der punktweisen Existenz der Ableitung liegt.
Dies korrigiert R97 inhaltlich (nicht nur formulierungstechnisch), lässt
aber R96 (Isomorphie zu $L^2(S^1_m)$, kein neuer Sektor) unberührt.
