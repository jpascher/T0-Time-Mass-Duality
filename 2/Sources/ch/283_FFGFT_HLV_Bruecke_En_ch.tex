...
% ==============================================================================
\section{The bridge is containment, not contradiction}
% ==============================================================================
The fork above is a statement about the \emph{protected} invariant; it must not be read as
the two models being incompatible realities. At the level of groups and dimensions they are
reconcilable by \emph{containment}. The icosahedral group contains the $3$-fold: $C_3<A_5$ --
a $120^\circ$ rotation about a $(1,1,1)$ axis and a $72^\circ$ vertex rotation are \emph{both}
symmetries of the same icosahedron -- and $T^7=T^4\times T^3$ ($7=4+3$). Hence
$\mathrm{HLV}\supset\mathrm{FFGFT}$: HLV is FFGFT's $4$D core extended by the three perp
circles and the full $5$-fold. What the fork establishes is therefore not incompatibility but
the \emph{realised protected geometry}: FFGFT protects an independent $3$-fold column
($\sqrt2$, Koide $2/3$), HLV protects the $5$-fold ($2/\sqrt5$), and $\sqrt2$ is not among
HLV's protected directions (it appears only as an unprotected accidental angle, previous
section). The models share the ambient $3{+}1$ and are group/dimension-reconcilable while
realising different protected structure -- which is the empirically decidable difference, not
a coordinate artifact and not a logical incompatibility. Computed in Doc.~285
(\texttt{recompute\_reconcile.py}, \texttt{dim\_bridge\_1..3}).
...
the discrete spectral density $\sum_k g_k^2\,\delta(\omega-\omega_k)$ -- non-Markovian
with \emph{back-flow and revivals}, hence a \emph{discrete-spectrum, oscillatory}
kernel (exponential limit $\to$ internal variable). This is not quite the same shape
as HLV's $\chi$ in the fluid sector, which is a \emph{positive decaying} kernel
(Krüger's Navier-Stokes extension, Zenodo~20512650): both non-Markovian and
positive-type, but of different form. Where space forks (5- vs 3-fold), at the time
level two kernels of the same \emph{class} (non-Markovian positive memory) face each
other: \emph{a spatial fork, a possible $\chi$-memory bridge}.
...
a demonstrated ontology (Run~M1: the HLV kernel beats weak nulls but, on the
\emph{overdamped} observable sector, is statistically \emph{indistinguishable} from a
generic biexponential -- HLV favoured only by parsimony/BIC). The $\chi$ bridge is
thus \emph{structural kinship}, not a uniqueness claim -- consistent with
Doc.~276/281 ($\varphi$ is not specific either). This is sector-dependent: in the
\emph{coherence/revival sector} FFGFT's discrete-spectrum kernel carries a signature
(BLP back-flow $5.125$) that any \emph{decaying} kernel misses -- though uniqueness
against other \emph{structured} kernels stays open.
\end{enumerate}
...
HLV-unique (Run~M1: on the overdamped sector a generic biexponential is
\emph{indistinguishable}; HLV favoured only by parsimony). A bridge candidate, not a
uniqueness claim.
