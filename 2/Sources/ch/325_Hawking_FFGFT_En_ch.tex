...
\subsection{What the quantum carries}\label{sec:quant}
...
% ============================================================
\subsection{Quantitative information budget \B}
\label{sec:infobilanz}

Building blocks 1--5 supply all ingredients for a fully computable
information budget over the entire lifetime of a black hole.

\textbf{Endpoint of evaporation.}
From Doc.~329 \B: the collapse threshold $M_{\mathrm{coll}} = \mP/\sqrt{2}$
follows from pure geometry (Compton\,$=$\,Schwarzschild).
The Schwarzschild radius at the endpoint is
\begin{equation}
  \rs(M_{\mathrm{coll}}) = \sqrt{2}\,\lP,
\end{equation}
exactly one Planck length times $\sqrt{2}$ — the smallest horizon on
which a $T^4$ mode still fits.

\textbf{Residual entropy at the endpoint — exact identity \B:}
\begin{tcolorbox}[ffgftbox,title={Main result R93 \B}]
  \begin{equation}
    \SBH(M_{\mathrm{coll}})
  ...
  \end{equation}
  This equals exactly Matzke's stability threshold $n_{\mathrm{thr}}$
  (Doc.~329 \B).
  The evaporation endpoint carries precisely the threshold entropy —
  the same geometry (Compton\,$=$\,Schwarzschild) in two languages.
\end{tcolorbox}

\textbf{Total emission volume.} For a black hole of initial mass
$M_{\mathrm{init}} > M_{\mathrm{coll}}$:
\begin{equation}
  N_{\mathrm{emit}}
  = \SBH(M_{\mathrm{init}}) - 2\pi \;\text{nat}.
\end{equation}
Example $M_\odot$: $N_{\mathrm{emit}} \approx \num{1.05e77}\,\text{nat}$.

\textbf{Structural constant of the $\mathbb{Z}_3$ geometry \B.}
The ratio of sector information to thermal information is identical
for every mass:
\begin{equation}
  \frac{I_{\mathrm{sector}}}{I_{\mathrm{thermal}}}
  = \frac{\log_2 3}{1}
  = \log_2 3 \approx 1.585,
\end{equation}
since each emitted quantum carries exactly $\log_2 3$ bits of sector
information (§\ref{sec:quant}) and 1 nat of thermal area bookkeeping.
The ratio is a structural constant of the $T^4/\mathbb{Z}_3$ geometry,
independent of mass and scale. \textbf{[R94 \B]}

\textbf{Verification script:} \texttt{325\_informationsbilanz.py} —
9 assertions, all passed.
