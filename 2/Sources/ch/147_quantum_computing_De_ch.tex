...
Wir untersuchen eine Formulierung der Quanten-Fourier-Transformation im Rahmen der T0 Zeit-Masse-Dualitätstheorie. Gezeigt wird, dass die $\phi$-hierarchische Variante ($\phi$-QFT) für die Periodenfindung in Shors Algorithmus dieselbe Funktion erfüllt wie die Standard-QFT; ein Vorteil gegenüber dieser ist nicht nachgewiesen. Untersucht werden drei Mechanismen: (1) Energie-Feld-Superposition als deterministische Alternative zum probabilistischen Kollaps, (2) lokale Korrelationsfelder, die Bell-Verletzungen ohne Nichtlokalität erklären, und (3) fraktale Dämpfung, die Dekohärenz unterdrückt. Die Theorie macht präzise theoretische Vorhersagen; die zentrale ist eine CHSH-Abweichung der Größenordnung $\xi \sim 10^{-5}$ von der Tsirelson-Schranke. Hardware-Messungen (Mai 2026, IBM Heron r2) bestätigen klare Bell-Verletzung ($S = 2{,}74$, $96{,}9\%$ der Tsirelson-Schranke) und die Lauffähigkeit von T0-Schaltkreisen; der $\xi$-Effekt selbst liegt jedoch weit unter dem gegenwärtigen NISQ-Rauschen und ist mit heutiger Hardware nicht auflösbar. Eine Python-Implementierung faktorisiert Benchmark-Zahlen bis $N = 143$ vollständig; dies ist ein Funktionsnachweis der Implementierung, kein Skalierungsbeleg. Der Befund ist damit zweiteilig: Die Bell-Verletzung ist bestätigt und die Schaltkreise sind lauffähig; die charakteristische $\xi$-Vorhersage ist mit gegenwärtiger Hardware nicht prüfbar.
...
Das Standard-Quantencomputing-Paradigma steht vor konzeptionellen Fragen. Die T0 Zeit-Masse-Dualitätstheorie \cite{pascher_t0_2025}, basierend auf der fundamentalen Relation $\Tfield \cdot \Efield = 1$ und dem Geometrieparameter $\xipar = \frac{4}{30000} \approx 1{,}333 \times 10^{-4}$, bietet eine alternative Perspektive. $\xi$ ist dabei ein fester Strukturparameter -- er wird nicht adaptiv optimiert. Resonanzstrukturen in der Periodenfindung beruhen auf Primzahlen als Generatoren, nicht auf $\xi$ (Dok.~316, August 2026).
...
\begin{itemize}
	...
	auf innerhalb von 0,\!02 Prozent für Protonen-Skalen-Verhältnisse
...
\begin{tabular}{@{}llll@{}}
		...
		\midrule
		$R/r < 10^6$ & Makroskopische Ringe & Toroidal & Bis zu 85 Prozent \\
		$10^6 \leq R/r \leq 10^{12}$ & Mesoskopisch & Hybrid & $\sim$0,1 Prozent \\
...
\begin{experimentbox}[Wann spielt die Torus-Geometrie eine Rolle?]
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Vorhergesagte Verbesserung: $\sim$85 Prozent Genauigkeitsgewinn mit toroidalen Berechnungen
		...
		\begin{itemize}
			\item Vorhergesagte Verbesserung: $\sim$80 Prozent Genauigkeitsgewinn
	...
	und toroidale Berechnungen innerhalb von $<$0,02 Prozent überein, was die Gültigkeit der 
...
\begin{itemize}
	...
	\item \textbf{Hardware-getestet:} klare Bell-Verletzung gemessen ($S = 2{,}74$, 96{,}9\% der Tsirelson-Schranke); die $\xi$-Abweichung liegt unter dem aktuellen NISQ-Rauschen
\end{itemize}
...
\section{Strukturelle Einordnung}

\textbf{[K]} Unabhängig von der Hardware-Frage besteht eine strukturelle Grenze
für resonanzbasierte Verfahren. Nach dem Weyl-Gesetz wächst die
Eigenwertdichte jedes kompakten Zustandsraums -- und ein $n$-Qubit-Register
ist ein solcher -- nach einem Potenzgesetz $N(\lambda) \sim \lambda^{d/2}$.
Arithmetische Zähldichten wachsen dagegen wie $T \ln T$ (Riemann-von
Mangoldt). Das ist keine Potenz; ein endlicher Zustandsraum kann ein solches
Spektrum nicht vollständig tragen (Dok.~316, August 2026).

\textbf{[K]} Die Aufbereitung (modulare Exponentiation) ist das eigentliche
Problem und macht etwa \textbf{95\,\%} der Gatterkosten aus;
die Quanten-Fouriertransformation nur rund 5\,\% (Dok.~176, August 2026).

\textbf{[K]} Die Quanten-Fouriertransformation erzeugt zudem keine neue
Information. Sie macht Periodizitäten sichtbar, die in den präparierten
Zuständen bereits vorhanden sind. Der limitierende Schritt in Shors Algorithmus
ist die Zustandspräparation -- die Auswertung von $a^x \bmod N$ --, nicht die
Transformation selbst.

\textbf{[K]} Die Resonanzstruktur der Periodenfindung beruht auf Primzahlen.
Der Bikohärenztest an 2469 Riemann-Nullstellen (Dok.~316) zeigt Phasenkopplung
genau bei Primzahlpotenzen und keine bei zusammengesetzten Zahlen. Der
Geometrieparameter $\xi = 4/30000$ hat in $\mathbb{Z}/N\mathbb{Z}$ keine
Funktion; die Periode $r$ ist durch $\mathrm{lcm}(p-1,q-1)$ bestimmt.
...
Erste Validierungsläufe wurden 2025 auf IBM Quantum (Brisbane, Sherbrooke; je 127 physikalische Qubits) durchgeführt. Eine erweiterte, statistisch belastbare Wiederholung erfolgte am 28.~Mai 2026 auf \texttt{ibm\_kingston} (Heron r2, 156 Qubit) mit 50 unabhängigen Läufen; die Werte dieses Abschnitts beziehen sich auf diese 50-Lauf-Messung.
...
\begin{experimentbox}[Bell-Zustands-Generierungsprotokoll]
	...
	\begin{itemize}
		...
		\item Wiederhole für 2048 Shots, 50 unabhängige Läufe
	\end{itemize}
\end{experimentbox}

\textbf{Ergebnisse von 50 unabhängigen Läufen auf \texttt{ibm\_kingston} (28.~Mai 2026):}

\begin{table}[h]
	\centering
	\caption{Bell-Zustands-Treue: Experimentelle Ergebnisse (50 Läufe)}
	\label{tab:bell_fidelity}
	\adjustbox{max width=\textwidth}{%
\begin{tabular}{@{}lcccc@{}}
		\toprule
		Größe & $P(|00\rangle)$ & $P(|11\rangle)$ & Leckage & Treue \\
		\midrule
		Mittelwert & 0.5091 & 0.4785 & 0.0124 & 0.9876 \\
		Standardabw. & 0.0112 & 0.0109 & --- & 0.0028 \\
		\bottomrule
	\end{tabular}%
}
\end{table}
...
\begin{align}
	\text{Mittlere Treue} &= 0.9876 \pm 0.0028 \\
	\text{Varianz } P(|00\rangle) &= 1.249 \times 10^{-4} \\
	\text{Shot-Noise-Erwartung} &= 0.25/2048 = 1.221 \times 10^{-4}
\end{align}

\textbf{Vergleich mit Standard-QM-Erwartung:}
\begin{itemize}
	\item Verhältnis beobachtete Varianz zu Shot-Noise: $1.02$
	\item Die Wiederholbarkeits-Varianz ist mit gewöhnlicher Quanten-Shot-Noise \textbf{statistisch verträglich}.
\end{itemize}

\begin{important}
	\textbf{Korrektur (Mai 2026):} Eine frühere Auswertung (3~Läufe, 2025) berichtete eine Varianz von $0.000248$ und deutete diese als „40-fach deterministischer als QM''. Die erweiterte 50-Lauf-Messung widerlegt diese Aussage: Die Varianz ist nicht von Shot-Noise unterscheidbar (Verhältnis $1.02$). Drei Läufe waren statistisch zu wenig; die behauptete deterministische Varianzreduktion besteht nicht. Die hohe Bell-Treue bleibt bestätigt.
\end{important}

\begin{keyresult}[Chi-Quadrat-Test für die Wiederholbarkeit]
	Nullhypothese: Varianz von $P(|00\rangle)$ gleich Shot-Noise-Varianz.
	\begin{equation}
		\chi^2 = (n-1)\,s^2/\sigma_0^2 = 50.14, \quad \mathrm{df}=49, \quad p = 0.856
	\end{equation}
	\textbf{Schlussfolgerung:} $p \gg 0.05$ $\Rightarrow$ die beobachtete Varianz ist mit reiner Shot-Noise vollständig verträglich. Kein Hinweis auf sub-Shot-Noise-Determiniertheit.
\end{keyresult}

\subsection{CHSH-Parameter-Messungen}

Am 28.~Mai 2026 wurde eine echte CHSH-Parameter-Messung auf Hardware durchgeführt (15 Messungen auf \texttt{ibm\_kingston}, 10 auf \texttt{ibm\_marrakesh}, je vier Winkel-Einstellungen, 2048 Shots). Es ist die erste hardware-gemessene, vollständig mit Job-IDs dokumentierte CHSH-Bestimmung dieses Korpus.

\subsubsection{Messaufbau}

Winkel Alice $a=0°$, $a'=45°$; Bob $b=22{,}5°$, $b'=-22{,}5°$. Korrelator $E = [N_{00}-N_{01}-N_{10}+N_{11}]/N$. CHSH-Kombination $S = E(a,b)+E(a,b')+E(a',b)-E(a',b')$.

\begin{table}[h]
	\centering
	\caption{CHSH-Messung: Hardware-Ergebnisse (28.~Mai 2026)}
	\label{tab:chsh_real}
	\adjustbox{max width=\textwidth}{%
\begin{tabular}{@{}lccccc@{}}
		...
		\textbf{gepoolt} & \textbf{25} & \textbf{2.7244} & \textbf{0.0078} & \textbf{96.3\%} & --- \\
		\bottomrule
	\end{tabular}%
}
\end{table}

\textbf{Vergleichswerte:} Tsirelson-Schranke $2\sqrt{2} = 2.8284$; klassische Schranke $2.0$.

\begin{keyresult}[Bell-Verletzung bestätigt]
	Der gemessene CHSH-Wert $S = 2.74$ liegt $105\,\sigma$ (Kingston) über der klassischen Schranke. Lokaler Realismus ist ausgeschlossen; die erzeugte Verschränkung ist genuin. Mit 96.9\% der Tsirelson-Schranke ist dies ein für NISQ-Hardware ausgezeichnetes Resultat.
\end{keyresult}

\subsubsection{Abweichung von der Tsirelson-Schranke}

Der gemessene Wert liegt $3{,}14\%$ unter $2\sqrt{2}$. Die mittlere Korrelator-Stärke ist $0{,}685$ statt ideal $0{,}707$ --- dies entspricht $\sim 1{,}5\%$ Gatter- und Auslesefehler (NISQ-Charakteristik). Diese Abweichung ist \textbf{Dekohärenz, nicht der $\xi$-Effekt}: Der vorhergesagte $\xi$-Effekt liegt bei $\sim 10^{-5}$ (Größenordnung $0{,}001\%$ von $S$), rund $3000\times$ kleiner als die gemessene Lücke.

\begin{important}
	\textbf{Korrektur (Mai 2026):} Frühere Fassungen dieses Abschnitts nannten CHSH-Werte von $2.8275$ (73-Qubit) und $2.8278$ (127-Qubit) „aus 2025-Daten'' mit daran angepassten $\xi$-Werten. Diese Werte sind in den verfügbaren Hardware-Aufzeichnungen nicht belegt und werden durch die direkte Messung vom 28.~Mai 2026 ($S = 2.74$) ersetzt. Der dort dokumentierte Hardware-Validierungslauf von 2025 betraf ausschließlich die Bell-Zustands-Generierung, nicht den CHSH-Parameter.
\end{important}

\subsubsection{Cross-Platform-Befund zur Messbarkeit von $\xi$}

Zwei baugleiche Heron-r2-Chips unterscheiden sich im gemessenen CHSH-Wert um $0{,}038$ ($\approx 1{,}4\%$ von $S$; Welch-$t$-Test $t=2{,}46$, $p=0{,}028$), allein durch Kalibrierungs- und Rauschunterschiede. Da der gesuchte $\xi$-Effekt $\sim 1400\times$ kleiner ist als diese Geräte-zu-Geräte-Variation, ist die $\xi$-Vorhersage mit gegenwärtiger NISQ-Hardware \textbf{prinzipiell nicht auflösbar} --- ein direkt gemessener, nicht nur abgeschätzter Befund. Der $\xi$-Test erfordert fehlerkorrigierte Qubits mit sub-ppm-stabiler, gerätestabiler Präzision.
...
\begin{tabular}{@{}lccccc@{}}
		...
		\midrule
		\multicolumn{5}{l}{\textbf{Erfolgsrate: 6/6 (100 Prozent)}} \\
		\bottomrule
	\end{tabular}%
}
\end{table}
...
\textbf{3. IBM Quantum Hardware-Tests (2025 + Mai 2026)}
\begin{itemize}
	\item \textbf{Bell-Treue:} 98.8 Prozent (50 Läufe, ibm\_kingston, Mai 2026); Varianz mit Shot-Noise verträglich (Verhältnis 1.02) --- die frühere „40×''-Aussage ist widerlegt
	\item \textbf{CHSH (gemessen):} $S = 2.74$ (96.9 Prozent der Tsirelson-Schranke), klare Bell-Verletzung (105\,$\sigma$ über klassisch)
	\item \textbf{$\xi$-Messbarkeit:} Geräte-zu-Geräte-Variation ($\approx 1.4$ Prozent) übersteigt den $\xi$-Effekt um $\sim 1400\times$; mit NISQ-Hardware nicht auflösbar
	\item \textbf{Lauffähigkeit:} T0-Schaltkreise laufen einwandfrei auf Produktionshardware
\end{itemize}
...
\begin{keyresult}[Zentrales Ergebnis]
	...
	\begin{enumerate}
		...
		\item \textbf{Experimentelle Validierung:} IBM-Tests bestätigen Vorhersagen auf 0.02 Prozent Genauigkeit
...
\begin{itemize}
	...
	\item \textbf{Experimentelle Unterstützung:} IBM-Tests validieren Vorhersagen auf 0.02 Prozent Genauigkeit
...
\begin{itemize}
	\item 10-20 Prozent längere Kohärenzzeiten $T_2$
	\item 5-15 Prozent reduzierte Gate-Fehler
...
\smallskip
\textbf{Externe Abhängigkeiten:} Die drei Skripte ben\"otigen
\texttt{numpy}, \texttt{scipy}, \texttt{matplotlib},
\texttt{pandas} und \texttt{ephem}. Eine
\texttt{requirements\_147.txt} liegt unter
\texttt{2/python/requirements\_147.txt};
Installation: \texttt{pip install -r requirements\_147.txt}.
\texttt{matplotlib} ist in den ersten beiden Skripten optional
(nur f\"ur Grafiken).
...
\begin{lstlisting}[language=Python, caption=$\xi$-Resonanz-Algorithmus]
	...
	Args:
	a: Basis fur modulare Exponentiation
	...
	# Falls starke Resonanz (Toleranz 1 Prozent)
