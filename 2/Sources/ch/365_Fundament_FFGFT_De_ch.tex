...
$\theta=2/9$ & Ausgabe der $\mathbb{Z}_3$-Circulant-Diagonalisierung; tritt in §~\ref{sec:schicht1} auf \\
\midrule
...
Typ~I & Dualitäts-Dekompaktifizierung; informationserhaltend (Grenzfall: $R\to\infty$) \\
Typ~II & geometrische Projektion; verlustbehaftet (z.\,B.\ Projektion auf Teilräume) \\
...
Typ~III, bijektiv. Typ~I bezeichnet Dualitäts-Dekompaktifizierungen ($R\to\infty$);
Typ~II geometrische Projektionen mit Informationsverlust. Ermöglicht Anschluss an Schrödinger, Dirac, Produktzustände
...
Der Mischungswinkel $\theta=2/9$ ist die Ausgabe der
$\mathbb{Z}_3$-Circulant-Diagonalisierung, nicht eingegeben
(Dok.~230/231, \enquote{gefunden, nicht gesucht}); er tritt als Phase
in der Berechnung der Massenschichten auf.\label{sec:schicht1}

\paragraph{Gravitation als abgeleitete Größe.}
Gravitationseffekte folgen aus $\tilde T\cdot m=1$, lokal angewendet: Variiert
die Masse $m(x)$ im Gravitationsfeld, variiert $\tilde T(x)=1/m(x)$ zwingend
mit. Es gibt kein eigenständiges Gravitationsfeld und kein Graviton als
unabhängiges Feld; Gravitation ist eine Konsequenz des ortsabhängigen Zeitfelds,
nicht ein separates Fundament (Dok.~078 \KM). Die Wahl zwischen „Masse konstant,
Raum gekrümmt" (Einstein) und „Raum flach, Masse variiert" (FFGFT) ist eine
\emph{Darstellungswahl} --- exakt äquivalent und durch Koordinatentransformation
ineinander überführbar, wie das Schrödinger- und das Heisenberg-Bild in der QM.
Die physikalisch natürlichere Lesart im Rahmen von $\tilde T\cdot m=1$ ist,
dass \emph{beides} variiert und die Konstanthaltung des einen oder anderen eine
Vereinfachungsannahme ist (Dok.~078).

\paragraph{Keine Kluft zwischen Relativitäts- und Quantentheorie.}
Das scheinbare Spannungsverhältnis zwischen Relativitätstheorie und
Quantenmechanik ist ein Artefakt der Beschreibungsebene. Auf Ebene~1
(Torusgeometrie) gibt es diesen Gegensatz nicht: RT und QT sind zwei
Typ~III-Übersetzungen derselben statischen Momentaufnahme ---
RT in die Sprache der Raumkrümmung, QT in die Sprache der Operatoren
auf $\mathcal{H}$. Die FFGFT ist keine neue Theorie, die beide
\emph{vereint}; sie zeigt, dass der Gegensatz auf Ebene~1 nicht existiert.

Die FFGFT leistet darüber hinaus mehr, als die Standard-QM beschreibt.
In der Standard-QM ist die Zeit $t$ ein externer Parameter außerhalb des
Systems --- die Wellenfunktion entwickelt sich in einer vorgegebenen
Hintergrundzeit, die selbst nicht quantisiert ist. In der FFGFT gehört
die Zeit zum System: $\tilde T=1/m$ ist eine Eigenschaft des physikalischen
Objekts, keine Hintergrundstruktur. Damit ändert sich der Status des
Quantenzufalls: Was in der Standard-QM als fundamentaler, ontologischer
Zufall erscheint, ist in der FFGFT deterministische Zeitfeld-Dynamik ---
prinzipiell bestimmt, praktisch unvorhersagbar (Dok.~020 \KM). Es gibt
keinen Kollaps: Wellenfunktionen entwickeln sich kontinuierlich; die
Born-Regel emergiert aus der Zeitfeld-Wechselwirkung, statt als Axiom
gesetzt zu werden. Zufall ist epistemisch, nicht ontologisch.
