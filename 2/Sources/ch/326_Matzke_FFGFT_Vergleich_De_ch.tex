...
\begin{tcolorbox}[divbox, title={Entschieden: die Schwelle ist skalenabhängig \B\ (Dok.~329)}]
  Matzkes $n_{\mathrm{thresh}} = 6{,}41$ ist elegant, hängt aber an der Setzung
  $m_{\mathrm{bit}} = m_P\ln 2/(2\pi)$ -- Landauer, ausgewertet bei der
  KMS-Temperatur der Planckskala $T_P/(2\pi)$.
  Dok.~329 entscheidet die Frage numerisch.

  Die Kollapsbedingung selbst ist \emph{bitwertfrei}: Compton~$=$~Schwarzschild
  liefert $M_{\mathrm{coll}} = m_P/\sqrt{2}$ rein geometrisch \B.
  Alles Nichtgeometrische an $n_{\mathrm{thresh}} = M_{\mathrm{coll}}/m_{\mathrm{bit}}$
  steckt im Nenner. Setzt man dort die FFGFT-Bit-Energie
  $E_{\mathrm{bit}}(L) = \hbar c/L$ ein, folgt exakt
  \begin{equation}
    n_{\mathrm{thresh}}(L) = \frac{L}{\sqrt{2}\,\lP},
  \end{equation}
  also eine \textbf{lineare Funktion der Skala}, kein invarianter Zahlenwert.
  Über die Skalen des Korpus ($L_0 = \xi\lP$ bis $1$\,nm) variiert sie um
  Faktor $4{,}6\times10^{29}$.\\[4pt]
  \textbf{Matzkes Skala ist rekonstruierbar}: Aus
  $m_{\mathrm{bit}} = \hbar/(cL)$ folgt
  $L_{\mathrm{Matzke}} = 2\pi\lP/\ln 2 = 9{,}0647\,\lP$, und dort gibt die
  Formel exakt $6{,}4097$ zurück \B. Der \enquote{universelle} Bitwert
  \emph{ist} die FFGFT-Bit-Energie bei rund neun Plancklängen.\\[4pt]
  $n_{\mathrm{thresh}} = 6{,}41$ als universelle Zahl: \X\ (Dok.~329, R88).
  Unberührt bleiben Compton~$=$~Schwarzschild, $r_s$ ohne Feldgleichungen
  und die $\mathrm{Cl}(6)$-Struktur \B.
\end{tcolorbox}
