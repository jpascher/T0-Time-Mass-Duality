...
\begin{tcolorbox}[ffgftbox, title={FFGFT}]
  Stabile Materie folgt aus $\widetilde{T}\cdot m = 1$:
...
\begin{tcolorbox}[ffgftbox, title={FFGFT (Dok.\ 325)}]
  ...
  $\tau = 8\pi GM/c^3$ aus $\widetilde{T}\cdot m = 1$ \K\ (Dok.\ 313 G, 325).\\
...
\begin{tcolorbox}[divbox, title={Divergenz — die Bitwert-Grundlegung}]
  ...
  $\widetilde{T}\cdot m=1$ plus systemabhängige Bit-Energie) kommt ohne
...
$\widetilde{T}\cdot m = 1$, der $\mathbb{Z}_3$-Struktur und der
