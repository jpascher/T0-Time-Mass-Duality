...
\begin{equation}
  \resizebox{\textwidth}{!}{$\displaystyle \begin{aligned}
...
\end{equation}
	...
	\begin{itemize}
		...
		\item Scaling to mass range: $321.87/1.314 \approx 245$ (factor $1.314$ is an empirical anchor; it follows from $K = R_{pe}/(4/3)^7$, not from an independent geometric derivation)
	\end{itemize}
...
%}
	...
	The mass-scaling exponent $\kappa = 7$ is \textbf{not} determined by backward fitting, but emerges as the \textbf{unique self-consistent solution} for the complete e-p-$\mu$ system.
