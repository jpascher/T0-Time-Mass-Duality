	...
	\textbf{Vorbemerkung (Nachtrag 3.~September 2026, Prüfskript \texttt{pruef\_343e\_dok186\_check.py})}\\[3pt]
	...
	\begin{itemize}\setlength{\itemsep}{1pt}
	  \item \textbf{Bestätigt [B]:} TFLN-Güte $Q>10^6$ löst die Skala $\xi=1{,}33\times10^{-4}$ auf (nötig $Q>1/\xi=7500$), nicht aber $\xi^2$ (nötig $Q>5{,}6\times10^{7}$).
	  \item \textbf{Korrektur (Abschn.~4.2, ,,Einrasten``) [B]:} Bei $K_\mathrm{eff}=2\pi\xi$ liegt $1/\varphi$ außerhalb jeder Arnold-Zunge (Halbbreiten $10^{-32}\ldots10^{-83}$ an den Fibonacci-Näherungen). Die $\varphi^{-k}$-Verhältnisse sind stabil, \emph{weil} sie nicht einrasten (KAM, Dok.~328 [K]). Die Diskretheit $\oint\nabla\phi=2\pi\mathcal N$ ist topologisch; ,,Phase-Locking`` ist die falsche Vokabel (Dok.~343, Satz~G$''$).
	  \item \textbf{Korrektur (Abschn.~4.1, $\mathcal N\in\{F_k\}$) [B]:} Die Wicklungszahlen $r_e=4/3$, $r_\mu=16/5$ (Dok.~338) erfüllen die Fibonacci-Bedingung nicht ($4,16\notin F$). Zähler sind Oktavpotenzen $2^2,2^4$, Nenner die Primzahlen $3,5$ der GF$(3^k)$-Hierarchie (Dok.~342).
	  \item \textbf{Korrektur (Abschn.~8.2, B-Meson) [X]:} Eine Ratenmodifikation $\alpha=\xi$ (0,013\,\%) benötigt für $4\sigma$ etwa $9\times10^8$ Ereignisse; LHCb hat im Kanal $O(10^3$--$10^4)$. Die Gleichsetzung einer Phase $\Delta\phi=\xi$ mit einer Rate ist ein Kategorienfehler; als Erklärung der Anomalie ausgeschlossen.
	  \item \textbf{Einordnung (Abschn.~6/7, photonische Operationen):} Analoge Matrixfaltung und Resonator-Einrasten sind Resonanzoperationen ohne Zurückschwellen; sie unterliegen dem Auflösungsboden $\varepsilon=1/Q\approx10^{-6}$ und damit $N_\mathrm{max}\approx10^{6}$ (Dok.~343, Satz~G$'$). Ein Gattermodell auf TFLN ist davon unabhängig, aber durch Photonenverlust ($<1$~dB/Facette $=$ bis $20\,\%$) begrenzt.
	  \item \textbf{Präzisierung (Abschn.~6.2, 50~GHz):} Die 50-GHz-Bandbreite liegt nicht auf der Sweet-Spot-Leiter aus Dok.~034/162 (nächste Sprosse 42,8~GHz, 14\,\% daneben) und ist die $\sim$174.\ goldene Subharmonische von $f_0$ --- keine ausgezeichnete Stufe.
	\end{itemize}
	\end{minipage}}
	\end{center}
	...
	\section{Der Geometrieparameter $\xi_{\text{par}}$ und die sub-plancksche Zeitskala $t_0$}
	...
	\subsection{Grundfrequenz $f_0$ als geometrischer Operator}
	...
	\begin{equation}
		f_n = n \cdot f_0 \quad \text{oder} \quad f_n = \phi^{-k} \cdot f_0
	\end{equation}
	...
	Dabei gilt:
	\begin{itemize}
		\item $\alpha_\phi = \phi^{-1} \approx 0{,}618$ ist der Fibonacci-Skalierungsfaktor zwischen den fraktalen Ebenen (nicht die Feinstrukturkonstante)
	...
	Dies entspricht dem fundamentalen Geometrieparameter $\xi_{\text{par}} = 4/30000 \approx 1{,}333 \times 10^{-4}$. (Hinweis: $\phi^{-4} \approx 0{,}146$ -- das wäre zwei Größenordnungen zu groß. Die korrekte Zuordnung ist $\xi_{\text{par}}$, nicht $\phi^{-4}$.)
	...
	\begin{equation}
		 f_{\text{Myon}} = \phi^{-3} \cdot f_0, \quad f_{\text{Tau}} = \phi^{-5} \cdot f_0
	\end{equation}

	\begin{table}[h]
		\centering
		\caption{Leptonen als Resonanzen -- qualitative Analogie und T0-FFGFT-Werte (Dok.~189)}
		...
		\begin{tabular}{llll}
			...
			Myon & $\phi^{-3} \cdot f_0$ (qualitativ) & $\frac{16}{5}\cdot\xi^{1}$ & $105{,}1$ \\
			Tau & $\phi^{-5} \cdot f_0$ (qualitativ) & $\frac{25}{9}\cdot\xi^{2/3}$ & $1785{,}0$ \\
			\bottomrule
		\end{tabular}
		}} % resizebox
		\begin{center}{\small\textit{Die $\phi^{-k}$-Spalte ist eine qualitative Resonanzanalogie.
			Die quantitative T0-FFGFT-Ableitung erfolgt über $r_i\cdot\xi_{\text{par}}^{p_i}$ (Dok.~189).}}
		\end{center}
	\end{table}
