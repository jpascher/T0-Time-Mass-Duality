...
\subsection{Flächenbilanz}\label{sec:flaeche}
...
\subsection{Was das Quant trägt}\label{sec:quant}
...
% ============================================================
\subsection{Quantitative Informationsbilanz \B}
\label{sec:infobilanz}

Die Bausteine 1--5 liefern eine vollständig berechenbare
Informationsbilanz über die gesamte Lebensdauer eines Schwarzen Lochs.

\textbf{Endpunkt der Verdampfung.}
Aus Dok.~329 \B: die Kollapsgrenze $M_{\mathrm{coll}} = \mP/\sqrt{2}$
folgt rein geometrisch (Compton\,$=$\,Schwarzschild).
Der Schwarzschild-Radius am Endpunkt ist
\begin{equation}
  \rs(M_{\mathrm{coll}}) = \sqrt{2}\,\lP,
\end{equation}
also genau eine Plancklänge$\cdot\sqrt{2}$ — der kleinstmögliche
Horizont, auf den noch eine $T^4$-Mode passt.

\textbf{Restentropie am Endpunkt — exakte Identität \B:}
\begin{tcolorbox}[ffgftbox,title={Hauptresultat R87 \B}]
  \begin{equation}
    \SBH(M_{\mathrm{coll}})
    = \frac{4\pi\,\rs^2}{4\,\lP^2}\bigg|_{\rs=\sqrt{2}\lP}
    = 2\pi \;\text{nat}
    = \frac{2\pi}{\ln 2} \;\text{bit}
    \approx 9{,}065\;\text{bit}.
  \end{equation}
  Dies ist exakt Matzkes Stabilitätsschwelle $n_{\mathrm{thr}}$
  (Dok.~329 \B).
  Der Verdampfungsendpunkt trägt gerade die Schwellen-Entropie —
  dieselbe Geometrie (Compton\,$=$\,Schwarzschild) in zwei Sprachen.
\end{tcolorbox}

\textbf{Gesamtemissionsvolumen.} Für ein Schwarzes Loch der
Anfangsmasse $M_{\mathrm{init}} > M_{\mathrm{coll}}$:
\begin{equation}
  N_{\mathrm{emit}}
  = \SBH(M_{\mathrm{init}}) - 2\pi \;\text{nat}.
\end{equation}
Beispiel $M_\odot$: $N_{\mathrm{emit}} \approx \num{1.05e77}\,\text{nat}$.

\textbf{Strukturkonstante der $\mathbb{Z}_3$-Geometrie \B.}
Das Verhältnis von Sektorinformation zu thermischer Information ist
für jede Masse identisch:
\begin{equation}
  \frac{I_{\mathrm{Sektor}}}{I_{\mathrm{thermisch}}}
  = \frac{\log_2 3}{1}
  = \log_2 3 \approx 1{,}585,
\end{equation}
da jedes emittierte Quant genau $\log_2 3$~Bit Sektorinformation
(§\ref{sec:quant}) und 1~nat thermische Flächenbuchung
(§\ref{sec:flaeche}) trägt.
Das Verhältnis ist eine Strukturkonstante der $T^4/\mathbb{Z}_3$-Geometrie,
unabhängig von Masse und Skala. \textbf{[R88 \B]}

\textbf{Prüfskript:} \texttt{325\_informationsbilanz.py} —
9 Assertionen, alle bestanden.
