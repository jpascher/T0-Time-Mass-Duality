...
\title{Doc.~308 -- $\Lambda$ as a Reading Artefact:\\
...
$\xi$                    & Dimensionless fundamental parameter, $\xi = 4/30000$;
                           geometric consequence of the fractal Hausdorff dimension
                           $D_f = 3 - \xi$ (posit, P33). \\[4pt]
$\tilde T \cdot m = 1$   & Foundational relation of FFGFT: intrinsic time $\tilde T$
                           and mass $m$ are inversely coupled —
                           compactification relation on the torus $T^4$. \\[4pt]
$T^4$                    & Compact four-dimensional torus; carrier of the FFGFT geometry. \\[4pt]
$\Lambda$                & Cosmological constant ($\Lambda$CDM); analysed here
                           as a reading artefact. \\[4pt]
$a_0$                    & Characteristic acceleration scale of the MOND regime,
                           empirically $\approx 1.2\times10^{-10}$~m/s$^2$;
                           in FFGFT: $a_0 = cH_0/(2\pi)$ from the $\xi^{10}$ chain. \\[4pt]
$H_0$                    & Hubble constant; from FFGFT: $H_0 = (\pi/2)\,
...
$\mu(x)$                 & Interpolation function of the MOND equation
                           $a_\text{tot}\cdot\mu(a_\text{tot}/a_0) = a_\text{Newton}$;
                           here: Simple form $\mu(x) = x/\sqrt{1+x^2}$
                           (limits from FFGFT, form a posit). \\[4pt]
$\gamma_\text{PPN}$      & Parametrised post-Newtonian parameter;
                           K3 yields $\gamma_\text{PPN} = 1$ exactly
                           (Cassini: $|\gamma-1| < 2.3\times10^{-5}$). \\[4pt]
$K_\text{frak}$          & Bridge factor $K_\text{frak} = 1 - 100\xi = 74/75$;
                           absorbs fractal corrections in SI absolute values. \\[4pt]
$\xi$ chain              & Shorthand for derivation chains of the form
                           $H_0 \propto \xi^{10}$, $a_0 \propto \xi^{10}$ etc. —
                           all from the same $\xi$, without fits. \\[4pt]
[K3]                     & Area-invariance rule: time-space 2-cell invariant,
                           $L_t \cdot l = \mathrm{const}$; sole survivor of the
                           T$^4$ selection computation (Stage B). \\
\end{tabular}
