...
\textbf{Die Zyklenlänge ist die primäre Größe -- und sie ist
vorfaktorfrei.} Schreibt man die Kette nicht als Rate, sondern als
Länge, verschwindet das $\pi/2$ aus $H_0 = (\pi/2)c\,\xi^{10}/
\bar\lambda_e$ vollständig:
\begin{equation}
  L^{*} \;=\; \frac{2\pi c}{H_0}
  \;=\; 4\,\frac{\bar\lambda_e}{\xi^{10}},
  \label{eq:Lstar4}
\end{equation}
exakt (numerisch auf zehn Stellen: $L^{*}/\bar\lambda_e =
2{,}252541\times10^{39} = 4/\xi^{10}$). In Worten: \emph{Der Zyklus
ist vier Kammertöne mal die Leiterstufe.} Das $\pi/2$ der
$H_0$-Form ist damit kein Vorfaktor, sondern die Umrechnung
$H_0 = 2\pi c/L^{*}$ -- reine Schreibweise. Der Einwand, ein frei
wählbarer Ordnung-1-Vorfaktor entwerte den Leiter-Treffer, greift
deshalb nicht: An seiner Stelle steht eine ganze Zahl.

Offen bleibt die \emph{Herkunft der Vier}. Naheliegende
Kandidaten -- die vier Torusrichtungen, die Verdopplung
Weg/Rückweg ($2^2$), das D4-Gitter -- sind sämtlich post-hoc und
werden nach R61-Disziplin \emph{nicht} gebucht, solange keine
Vorwärtsherleitung vorliegt.
