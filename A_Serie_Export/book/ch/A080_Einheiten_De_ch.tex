...
\approx7{,}398\,\text{MeV}$ \emph{[Vermerk 3.~August 2026: $E_0$ ist die Kalibrierung des Einheitensystems -- nach A085 gibt es genau eine --, und geführt wird der Wert, der die Verankerung im SI leistet: $7{,}398$~MeV trifft über $\alpha = \xi E_0^2$ den Messwert auf $5\times10^{-6}$. Das geometrische Mittel $\sqrt{m_em_\mu} = 7{,}348$~MeV (A130, Weg~1) benennt die Herkunft der Größenordnung, taugt aber nicht als Anker: eingesetzt ergäbe es $1/138{,}9$, also $-1{,}35\,\%$ in $\alpha$, und diese Abweichung pflanzte sich in jede abgeleitete absolute Größe fort und summierte sich bei Vergleichen. Die Formelschreibweise ist hier verkürzt; maßgeblich ist der Ankerwert (A130, Weg~2: $E_0^2 = 4\sqrt2\,m_\mu/\xi^{p}$, ohne $m_e$).]} folgt aus gemessenen Leptonmassen, nicht aus $\xi$
...
\begin{itemize}
  \item Exaktheit der Vier-Kleidungen-Identität $m_0c^2=E_0=k_BT_0=\hbar/\tilde T$
    ...
    Rundung): \path{python/A_Serie_Skripte/a080_vier_kleidungen.py}
