...
% - scrbook (KOMA-Script) for print versions → BCOR + DIV in documentclass
% - book (Standard) for eBook versions → geometry loaded below
%
...
% === CHAPTER 0: COUNTER FIX (must be FIRST) ===
% FIX: Prevent "No room for a new \write" error with many \input commands
% morewrites extends the number of available write streams from 16 to 256
...
% THEN unicode-math (MUST come BEFORE amsmath!)
\RequirePackage{unicode-math}

% Math Font (MUST come AFTER unicode-math!)
% IMPORTANT: Libertinus Math for correct \underbrace display!
\setmathfont{Libertinus Math}[Scale=1.0]

% NOW load AMS packages (AFTER unicode-math!)
% amssymb removed - conflicts with unicode-math (symbols already defined)
...
% IMPORTANT: Geometry is NOT defined here, but in the master documents!
% Each format (Standard, A4, eBook) has its own geometry in the master document.
% Reason: Easier maintenance - only 2 preambles instead of 6!

\setlength{\headheight}{15pt}

% A4 page geometry (210 x 297 mm)
\usepackage[
  paperwidth=6in,    % = 432bp in PDF (exactly 6.000")
  paperheight=9in,   % = 648bp in PDF (exactly 9.000")
  top=0.75in,
  bottom=0.75in,
  inner=0.875in,     % gutter (KDP min 0.625" for 414+ pages)
  outer=0.625in,
  twoside,
  headheight=14pt,
  headsep=8pt,
...
% Additional common environments - synchronisiert mit Deutscher Version
...
\newenvironment{proofenv}{\begin{quote}}{\end{quote}}
\newenvironment{remarkenv}{\begin{quote}}{\end{quote}}

% German environment names (for bilingual compatibility)
...
% === FIX: Support >26 appendices ===
% Standard LaTeX uses \Alph (A-Z, max 26) for appendices
% This changes to Arabic numerals to support unlimited appendices
\makeatletter
\renewcommand\appendix{\par
  \setcounter{chapter}{0}%
  \setcounter{section}{0}%
  \gdef\@chapapp{\appendixname}%
  \gdef\thechapter{\@arabic\c@chapter}}  % Changed from \@Alph to \@arabic
\makeatother
...
% === GEOMETRY REMOVED ===
% Geometry is now defined in master documents (Teil*.tex)
% See UMSTELLUNG_2_PREAMBLES_COMPLETE.txt for details
...
% =============================================================================
% cleardoublepage: blank page between chapters, no header/page number
% =============================================================================
\makeatletter
\def\cleardoublepage{%
  \clearpage
