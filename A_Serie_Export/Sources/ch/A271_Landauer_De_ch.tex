...
% Kapitelquelle zur A-Serie; Wrapper: wr_standalone_A4/A271_Landauer_De.tex
...
\begingroup\small\noindent
\textbf{Prüfskript.} \texttt{a271\_landauer.py} (Checks 1--10). Alle
...
\textbf{[SETZUNG]} \textbf{Ein physikalisches System belegt ein Gebiet im Phasenraum.} Dieses
...
\textbf{[B]} \textbf{Beschreibungen kommen von außen.} Wenn man entscheidet, das Gebiet in
...
\textbf{[B]} Das wird regelmäßig verwechselt. Klarstellung in einem Absatz: Ein
...
\textbf{[B]} \textbf{Ebene 1 --- Konstruktion.} Der Ingenieur entscheidet beim Chip-Design,
...
\textbf{[B]} \textbf{Ebene 2 --- Zustandsübergang im laufenden Betrieb.} Sobald der Rechner
...
\textbf{[B]} \textbf{Ebene 3 --- Beschreibung.} Das Programm, die Rechnung, eine
...
\textbf{[B]} Ein Kolloid-Teilchen in einer 1-\textmu m-Falle enthält physikalisch rund
$\mathbf{7{,}9\cdot10^{9}}$ \textbf{orthogonale Quantenzustände}
\emph{(Prüfskript, Check 7)}. Nennt man grob zwei Zonen „links“ und „rechts“, beschreibt man
...
\textbf{[B]} \textbf{Die Kosten des Löschens hängen am Gebiet, nicht an der Beschreibung.}
Die Formel lautet
\[
...
\textbf{[Q]} \textbf{Der analoge Fall zeigt das besonders klar --- und zugleich seine
...
und bleibt eine Beschreibungsentscheidung \emph{(Prüfskript, Check 8: Illustrationsbeispiel
mit deklariertem $\delta x$)}.

\subsection{Shannon und Boltzmann sind verschiedene Buchführungen}

\textbf{[B]} [B18][B19] Shannon beschreibt, wie viel Unsicherheit eine \emph{Beschreibung}
...
\textbf{[Q]} \textbf{Was Landauer 1961 wirklich sagte} [B1]: Er schrieb ausdrücklich
...
\textbf{[B]} \textbf{Und deshalb ist sie für sich genommen keine physikalische Bedingung.}
...
\textbf{[B]} \textbf{Der Fehler der Standarddarstellung} ist damit: Sie behandelt die
...
\begin{quote}
\textbf{[B]} Die Zeitentwicklung eines abgeschlossenen Systems \textbf{erhält das
...
\textbf{[B]} \textbf{Folgerung: Ein abgeschlossenes System kann nicht löschen.} Kein
...
\[
  \textbf{[B]}\quad Q \;\ge\; T\,\Delta S_{\text{Bad}} \;=\; k_B\,T \ln 2
\]
...
\emph{(Prüfskript, Check 4)}. Die $\ln 2$ ist der Informatik-Spezialfall.

\textbf{[B]} \textbf{Was zählt als Stufe? Nicht die Energie.} Energieniveaus taugen nicht
...
besetzte Systeme tragen automatisch weniger Entropie \emph{(Prüfskript, Checks 5--6)}.

\textbf{[B]} \textbf{Die Grund-Diskretisierung existiert --- und ist $h^f$.} Klassisch hat
...
\]
\emph{(Prüfskript, Check 7: Kolloid in 1-\textmu m-Falle, rund $7{,)}9\cdot10^{9}$ orthogonale
...
Gebietsreduktion kostet $k_B T \ln(L/\delta x)$ \emph{(Prüfskript, Check 8)}. $\delta x$ ist
...
\begin{quote}
\textbf{[B]} \textbf{Gebietsreduktion kostet für jede Belegung identisch --- unabhängig
...
\textbf{[B]} \textbf{Die Hardware ist nicht die Rechenoperation.} Ein 64-Bit-Prozessor ist
...
\textbf{[B]} \textbf{Die Rechenoperation ist eine Abstraktion über der Hardware.} Sie
...
\textbf{[B]} \textbf{Der Rechner schrumpft nicht.} Ein 64-Bit-Prozessor hat vor und nach
...
\textbf{[B]} \textbf{Was wirklich ins Bad geht, ist die Korrelation.} Die logische
...
\textbf{[B]} \textbf{Der Vergessensvorgang ist inhaltsneutral.} Ein Prozessor, der
...
\textbf{[B]} Eine DRAM-Zelle, die eine Stunde lang „nichts tut“, hat in dieser Stunde
...
\textbf{[Q]} \textbf{Dekohärenzzeit als DRAM-Analogon.} Ein supraleitendes Qubit hat
...
\textbf{[H]} \textbf{Offene Frage.} Beim klassischen Rechner ist der kontinuierliche
...
\textbf{[B]} \textbf{Volumenerhaltende Abbildungen} --- das Gebiet wird umgeformt, nicht
...
\textbf{[B]} \textbf{Volumenverkleinernde Abbildungen} --- mehrere Eingangsgebiete landen
...
\textbf{[B]} \textbf{Bennett} [B3] hat gezeigt, dass jede Folge von Gebietsabbildungen im
...
\begin{quote}
\textbf{[B]} \textbf{Die Untergrenze klebt nicht an der Abbildung, sondern am Verzicht auf
...
\textbf{[Q]} \emph{(Prüfskript, Checks 2--3)} Ein realer CMOS-Schaltvorgang liegt bei
...
\textbf{[S]} \textbf{Das Weltmeer-Bild.} Das Bit-Volumen, das bei der Gebietsreduktion ins
...
\textbf{[B]} Nicht messbar heißt nicht nicht vorhanden. Die Mikrozustände des Bades nach
...
\textbf{[B]} \textbf{Was aber ontologisch wahr ist:} Ein kleineres Gebiet kann unter
...
\textbf{[Q]} \textbf{Maxwells Dämon [B12] und Bennetts Auflösung [B3].} Ein Wesen, das alle
...
Zusatzdissipation \emph{(Prüfskript, Check 9)}. Sie arbeiten im quasistatischen Grenzfall ---
...
\textbf{[Q]} \textbf{Feedback-Erweiterung.} Mit Messung und Rückkopplung gilt der
...
\midrule
\endhead
Bit $\ne$ physikalisches Gebiet (Shannon vs.\ Boltzmann) & \textbf{[B]} \\
$S = k_B \ln W$ als Ausgangspunkt & \textbf{[SETZUNG]} \\
Gebietsreduktion $\{A,B\}\to Z$ (Physik, keine Beschreibung) & \textbf{[SETZUNG]} \\
Gebietsreduktion $\Rightarrow \Delta S = -k_B \ln(W_{A+B}/W_Z)$ & \textbf{[B]} \\
Liouville-Theorem & \textbf{[B]} \\
Hinreichend ist die Nicht-Bijektivität der Gebietsabbildung, nicht die logische Irreversibilität & \textbf{[B]} \\
Abgeschlossenes System kann nicht löschen & \textbf{[B]} \\
Landauer-Grenze $Q \ge k_B T \ln 2$ & \textbf{[B]} \\
Numerischer Wert $2{,}87\cdot10^{-21}$\,J bei 300\,K & \textbf{[B]} \\
Zählbasis $=$ Orthogonalität, nicht Energie & \textbf{[B]} \\
$h^f$ als Grund-Diskretisierung & \textbf{[B]} \\
Gibbs-Form als allgemeine Entropie & \textbf{[SETZUNG]} \\
Inhalt (wahr/falsch) geht nicht in die Rechnung ein & \textbf{[B]} \\
Hardware $\ne$ Rechenoperation & \textbf{[B]} \\
Rechner schrumpft nicht & \textbf{[B]} \\
Korrelation ins Bad (nicht Silizium) & \textbf{[B]} \\
Vergessensvorgang inhaltsneutral & \textbf{[B]} \\
DRAM ${\sim}1{,}12\cdot10^{5}$ Refresh-Zyklen pro Stunde (32\,ms) & \textbf{[B]} \\
No-Cloning-Theorem & \textbf{[B]} \\
$T_2 \sim 100\,\text{\textmu s}$ supraleitende Qubits & \textbf{[Q]} \\
Minimalpreis Kohärenzerhaltung & \textbf{[H]} \\
Volumenerhaltende Abbildungen: keine Untergrenze & \textbf{[B]} \\
Volumenverkleinernde Abbildungen: das Bad zahlt & \textbf{[B]} \\
Epistemisch vs.\ ontologisch: Korrelationen bleiben & \textbf{[B]} \\
Sagawa-Ueda-Term $k_B T\,I$ & \textbf{[Q]} \\
\bottomrule
\end{longtable}
\end{center}

\textbf{Bilanz der Tabelle:} 19$\times$ \textbf{[B]} (algebraisch/mathematisch aus
deklarierten Grundlagen) $\cdot$ 2$\times$ \textbf{[Q]} (korpusexterne Primärquelle oder
Messwert) $\cdot$ 3$\times$ \textbf{[SETZUNG]} (deklarierte Ausgangspunkte) $\cdot$
1$\times$ \textbf{[H]} (offene Forschungsfrage) $=$ 25 Einträge.
...
\textbf{Erstens.} Ob es einen prinzipiellen Minimalpreis für das Aufrechterhalten
von Quantenkohärenz gibt --- unabhängig von der Güte der Fehlerkorrektur --- ist
eine offene Frage der Quantenthermodynamik [B17].

\textbf{Zweitens.} Der kontinuierliche Entropiestrom eines physikalischen Rechners
als Funktion von Taktfrequenz, Bitbreite und logischer Irreversibilität ist in der
Literatur nicht abschließend behandelt.

\section{Supersedes / Ersetzt}

Dieses Dokument enthält kein direktes Altbestand-Pendant; es ist ein
eigenständiges Dokument zu Landauer-Buchhaltung und Phasenraum-Analyse.
