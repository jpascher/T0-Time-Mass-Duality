...
% Kapitelquelle zur A-Serie; Wrapper: wr_standalone_A4/A272_Traeger_und_Information_En.tex
...
of an abstraction --- that carried it. \emph{Relation to A271:} A272 adds nothing
to the physics. A271 does the accounting; A272 does the textual criticism. The
...
\textbf{[B]} \textbf{First: this is not a refutation of Landauer's principle.} What is
...
\textbf{[B]} \textbf{Second: the physical substance is in A271, not here.} That the
thermodynamic lower bound attaches to the physical region and not to its
description is derived there from Liouville. A272 adds nothing to that. What A272
...
\subsection{Dictionary: A271 and A272}
\label{sec:dictionary}

\textbf{[SETZUNG]} The two documents draw the same distinction in different words. So that the
...
\toprule
\textbf{A271 (accounting)} & \textbf{A272 (textual criticism)} & \textbf{Level} \\
\midrule
...
\textbf{[B]} The mapping is a translation, not an extension. Whoever reads “carrier” may
...
\textbf{[SETZUNG]} Both definitions are stipulations. They are not proved; they fix what is being
...
\textbf{[B]} \textbf{Proof.} Suppose a bit of information had an intrinsic energy. Then that
...
\textbf{[B]} An instructive example is the bit width of a data bus:
...
\textbf{[Q]} Landauer's central model (Fig.~1 in [B1]) is a classical particle in a
...
\textbf{[Q]} Landauer computes the entropy change on resetting as
\begin{equation}
...
\textbf{[B]} The entropy change in equation~\eqref{eq:landauer} is a property of the
...
\textbf{[B]} \textbf{Proof.} Let $W$ be the number of physical microstates of the carrier.
...
\textbf{[B]} Precision is needed here, otherwise the objection is aimed at an opponent who
...
\begin{quote}
\textbf{[B]} Landauer's bound has jurisdiction over carrier operations that shrink the
...
\textbf{[B]} The real question thereby shifts, and it is harder than the original one:
...
\textbf{[Q]} Landauer formulated a restriction that the later reception mostly passes over:
...
\textbf{[Q]} Landauer resolves the question himself immediately afterwards: for a random
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.3em]
  \item \textbf{[Q]} Landauer is aware of the limits of his model and works them out
        himself. The charge of over-generalisation hits the reception, not him.
  \item \textbf{[B]} His result $kT\ln 2$ holds strictly for the equilibrium ensemble. For
  ...
  \item \textbf{[B]} Above all: this extension too remains a statement about
...
\textbf{[B]} This claim goes beyond Landauer's proof. What is proved is:
...
\textbf{[Q]} The step Landauer's proof does not contain is the bridge from the carrier
...
\textbf{[B]} That every information-theoretic erasure requires a state-space-shrinking
...
\textbf{[Q]} The thermodynamics of information with feedback [B5][B6] gives, for the
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.25em]
  \item \textbf{[B]} If the experimenter knows whether the carrier is at $0$ or $1$
        ($I = \ln 2$), the carrier can be reset reversibly --- $Q_{\min} = 0$.
  \item \textbf{[B]} If not ($I = 0$), the reset must be irreversible ---
        $Q_{\min} = kT\ln 2$.
\end{itemize}

\textbf{[B]} The heat therefore arises not \emph{because} information is lost, but because
...
\textbf{[B]} And the symmetry must be seen at the same time: the knowledge $I$ is itself
...
\textbf{[Q]} The conceptual imprecision would have remained inconsequential had the
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em]
  \item \textbf{[Q]} \textbf{Lairez} [B9] notes that criticism of the generalisation has
        been available for decades but has had no broad effect.
  \item \textbf{[Q]} \textbf{Norton} [B4] shows that the standard argument rests on an
  ...
  \item \textbf{[Q]} \textbf{Hemmo and Shenker} [B10] show that in the light of the
        multiple-computations theorem the principle cannot be universally valid.
\end{itemize}

\textbf{[S]} The reason for the persistence lies not in a refutation of this criticism but
...
\textbf{[B]} This diagnosis is a plausibility sketch and is marked as such. It names
...
\textbf{[B]} The problem is not confined to Landauer. The whole of digital technology uses a
...
\textbf{[B]} A more precise usage would distinguish:
...
\textbf{[S]} The mechanism described --- an abstraction is treated as a physical quantity
...
\textbf{[Q]} The bit is an abstraction. Shannon [B11] defined it as a statistical property
...
\textbf{[S]} The mechanism is not new in the history of science. Entropy, the electric
...
\textbf{[B]} This section is marked as a sketch because it offers an interpretation and
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.35em]
  \item \textbf{[B]} Landauer's proof for the physical operation \textup{RESTORE TO ONE} on
        a thermal equilibrium ensemble is correct and precise.
  \item \textbf{[B]} It shows a lower bound for \emph{carrier operations} --- not a
        universal statement about abstract information.
  \item \textbf{[B]} The transfer to the information-theoretic notion of the bit
  ...
  \item \textbf{[Q]} The double sense of “bit” is present in the original paper and
        encouraged the generalisation in the reception.
  \item \textbf{[Q]} Landauer himself discusses the limits of his ensemble argument and
        concedes that non-random input data lower the dissipation below
        $kT\ln 2$.
  \item \textbf{[B]} What is not shown here: that Landauer's principle is false. What is
...
\textbf{[B]} A formulation that reflects the actual proof reads:
...
\textbf{[H]} \textbf{Open question.} Under what conditions does a given computational
...
open question as the continuous entropy current in A271 --- posed from the other
side.
...
\midrule
\endhead
Definition of information (abstract, without energy) & \textbf{[SETZUNG]} \\
Definition of carrier (physical, with energy) & \textbf{[SETZUNG]} \\
Dictionary mapping A271 $\leftrightarrow$ A272 & \textbf{[SETZUNG]} \\
Proposition 1: energy attaches to the carrier, not to the bit & \textbf{[B]} \\
Bus example: bus width is hardware, not information & \textbf{[B]} \\
Landauer's double-well model and \textup{RESTORE TO ONE} & \textbf{[Q]} \\
$\Delta S = k\ln 2$ presupposes $W{=}2 \to W{=}1$ at the carrier & \textbf{[Q]} \\
Proposition 2: interpretive erasure does not shrink $W$ & \textbf{[B]} \\
The objection is a finding about jurisdiction, not a refutation & \textbf{[B]} \\
Bennett: logically irreversible functions executable reversibly & \textbf{[B]} \\
Landauer's ensemble restriction (original quotation) & \textbf{[Q]} \\
Landauer's own resolution for random data & \textbf{[Q]} \\
For correlated data the bound is $kT\,H$, not $kT\ln 2$ & \textbf{[B]} \\
Landauer uses “bit” in a double sense & \textbf{[Q]} \\
Feedback bound $Q_{\min} = kT(\ln 2 - I)$, $I$ in nats & \textbf{[Q]} \\
The knowledge $I$ is itself carrier-bound (Bennett) & \textbf{[B]} \\
Reception findings: Lairez / Norton / Hemmo--Shenker & \textbf{[Q]} \\
Structural reasons for the persistence & \textbf{[S]} \\
Language table: carrier versus information & \textbf{[B]} \\
Hypostatisation as an interpretive frame & \textbf{[S]} \\
Historical parallels (entropy, field, gene) & \textbf{[S]} \\
No demonstration that Landauer's principle is false & \textbf{[B]} \\
Quantitative form of the unavoidable state-space relinquishment & \textbf{[H]} \\
\bottomrule
\end{longtable}
\end{center}

\textbf{Balance:} 10$\times$ \textbf{[B]} $\cdot$ 8$\times$ \textbf{[Q]} $\cdot$ 3$\times$ \textbf{[SETZUNG]}
$\cdot$ 3$\times$ \textbf{[S]} $\cdot$ 1$\times$ \textbf{[H]} $=$ 25 entries.
...
This document carries no figures of its own; the numerical statements are held in
A271 (\texttt{a271\_landauer.py}) and A273 (\texttt{a273\_rechenkugel.py}).

\section{What remains open}

Under what conditions a given computational process enforces a
state-space-shrinking carrier operation is quantitatively open; Bennett answers the
limiting case, the practically interesting question remains.

\section{Supersedes}

This document has no direct counterpart in the older corpus; it supplements A271
with the textual criticism and the reception history.
