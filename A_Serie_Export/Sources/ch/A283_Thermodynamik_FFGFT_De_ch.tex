...
% Stand: August 2026 -- Überarbeitet
% =============================================================================
...
Der thermodynamische Inhalt der FFGFT ist auf mehrere Dokumente verteilt:
Dok.~157 behandelt den Zeitpfeil, A271--A282 die Landauer-Buchhaltung,
Dok.~312/313 die kosmologische Thermodynamik.
...
$1{,}4 \times 10^9$\,km$^3$. Der Preis: das Bad muss das Volumen
aufnehmen, Wärme fließt. Die Zählbasis ist Unterscheidbarkeit
(Orthogonalität), nicht Energie; absolute Grunddiskretisierung: $h^f$.

\textbf{[B]} A281: Landauers eigene Einschränkung – logische Irreversibilität
...
Der Grenzübergang zur Brownschen Rechenkugel verbindet stetig mit
Landauers Geltungsbereich.

\textbf{[H]} Offene Frage: Minimaler Kohärenzpreis für Quantenkohärenz
unabhängig von der Fehlerkorrektur (A271, \S\,7.4).
...
\begin{center}
	...
	\begin{tabular}{llll}
		...
		$m \to m_\text{min}$, Uhren laufen am langsamsten \\
...
Kein Schritt enthält einen an $\Omega_m$ angepassten Parameter.
...
Rückkehr-Hälfte. Poincar\'e-Diskrepanz: $\sim 10^{10^{104}}$ dynamische
...
Die Familienleiter trifft den Kosmos bei $r_s = R_H/2$ (exakt $0{,}500$).
...
feinere Strukturen, kein Wachstum nach außen. Antipode = ungezoomte
...
% ===================== 11 =====================
\section{Minimale Länge und fraktale Verfeinerung: ein Scheinwiderspruch}

\textbf{[S]} Auf den ersten Blick scheint ein Spannungsverhältnis zu bestehen:
Einerseits spricht die FFGFT von einer \emph{minimalen Länge} am Antipoden
($\bar\lambda = \hbar/(m_\text{min} c)$), andererseits von einer
\emph{fraktalen Verfeinerung nach innen} im Verlauf des Zeitzyklus.
Handelt es sich um einen Widerspruch?

Die Antwort ist \textbf{nein} – der Schlüssel liegt in der
\emph{Bezugssystem-Logik} der Theorie.

\subsection{Was bedeutet „minimale Länge"?}

Die „minimale Länge" in der FFGFT ist \emph{keine} absolute Planck-Länge,
sondern eine \textbf{masseabhängige Größe}:
...
Am Antipoden ($m \to m_\text{min}$) werden diese Längen \emph{maximal}.
Dort sind massebehaftete Strukturen am wenigsten scharf ausgeprägt –
es herrscht ein Regime \emph{minimaler metrischer Schärfe}, dominiert
von Strahlung. Die „minimale Länge" ist also tatsächlich die
\emph{größte charakteristische Länge} des Systems.

\subsection{Was bedeutet „fraktale Verfeinerung nach innen"?}

Mit wachsender Masse $m(\theta)$ entlang des Zeitzyklus werden alle
charakteristischen Längen \emph{kürzer}:
...
Das bedeutet: Materie wird \emph{differenzierter}, \emph{strukturierter},
\emph{feiner aufgelöst} – wie ein \textbf{Zoom nach innen}.
Der Raum selbst dehnt sich nicht aus; die \emph{Skala der Objekte}
ändert sich.

\subsection{Warum ist das kein Widerspruch?}

Die scheinbare Spannung löst sich auf, sobald man zwei
\emph{unterschiedliche Perspektiven} unterscheidet:

\begin{center}
	\small
	\begin{tabular}{p{0.42\textwidth}p{0.48\textwidth}}
		\toprule
		\textbf{Perspektive} & \textbf{Bedeutung in der FFGFT} \\
		\midrule
		Minimale Länge (am Antipoden) & 
		Maximale Compton-Wellenlänge, minimale Strukturierung,
		strahlungsdominiertes Regime \\
		Fraktale Verfeinerung nach innen & 
		Zunehmende Differenzierung, wachsende Massen,
		kürzere Wellenlängen, heutige Materiedominanz \\
		\bottomrule
	\end{tabular}
\end{center}

Die \textbf{Richtung} ist eindeutig:

\[
\text{Antipode} \;(m_\text{min},\; \bar\lambda_\text{max})
\;\longrightarrow\;
\text{heutige Position} \;(m_0,\; \bar\lambda_0)
\]

Dabei \emph{nimmt die strukturelle Auflösung zu} (fraktal nach innen),
während die \emph{Compton-Längen abnehmen}. Das ist kein Gegensatz,
sondern eine \textbf{komplementäre Beschreibung} desselben Prozesses.

\subsection{Vergleich mit dem Standardmodell}

Das Standardmodell sagt:  
„Der Raum expandiert, Wellenlängen werden gedehnt."

Die FFGFT sagt:  
„Die Massen wachsen, Wellenlängen schrumpfen, Struktur wird feiner."

Beide Beschreibungen sind \textbf{beobachtungsäquivalent} (A265),
weil sich \emph{Uhren und Maßstäbe gemeinsam ändern} – kein lokaler
Beobachter kann die Skalierung messen.

\subsection{Fazit}

Die Begriffe \enquote{minimale Länge} und \enquote{fraktale Verfeinerung
	nach innen} sind \emph{kein} Widerspruch, sondern zwei Seiten derselben
geometrischen Medaille:

\begin{itemize}[leftmargin=1.4em,itemsep=0.2em]
	\item \textbf{Längenperspektive:} von groß (Antipode) nach klein (heute)
	\item \textbf{Strukturperspektive:} von grob (Antipode) nach fein (heute)
\end{itemize}

Beide folgen aus demselben Massenprofil $m(\theta)$ und sind in der
FFGFT konsistent miteinander verbunden.

\textbf{[S]} Diese Klarstellung macht die innere Konsistenz der
FFGFT-Geometrie sichtbar und behebt einen häufigen Missverständnispunkt.
...
Fehlerkorrektur (A271, \S\,7.4).
(2) Kontinuierlicher Entropiestrom als Funktion von Takt, Bitbreite
und logischer Irreversibilität (A271, \S\,8).
...
\begin{center}
	...
	\begin{longtable}{@{}p{0.70\textwidth}l@{}}
		...
		Minimale Länge = maximale Compton-Wellenlänge & \textbf{[S]} \\
		Fraktale Verfeinerung = zunehmende Strukturierung & \textbf{[S]} \\
		Kein Widerspruch – zwei Seiten derselben Geometrie & \textbf{[S]} \\
