...
\textbf{[K]} \emph{Vorangestellt: Was tatsächlich bewiesen ist und was nicht.}
Die vier Generationen von Bell-Experimenten haben eines bewiesen: Die Korrelationsstatistik verletzt die klassische Schranke ($S > 2$). Was sie \emph{nicht} bewiesen haben und nicht beweisen konnten: dass die Korrelation \emph{instantan} entsteht. Die Instantanität ist keine experimentelle Beobachtung, sondern eine \emph{Interpretation} aus dem Standard-QM-Formalismus, in dem der Kollaps per Konstruktion zeitlos ist. Keine der vier Generationen hat die Timing-Frage gestellt --- die Zeitauflösung war entweder zu grob (Aspect: $\mu$s), oder das Experiment hatte keinen gemeinsamen Quellpunkt-Zeitstempel (Micius). Der Satz \enquote{Bell-Korrelationen sind instantan} ist korrekt als \emph{Formulierung des QM-Formalismus}, aber er ist \emph{nicht experimentell nachgeprüft}.

\textbf{[K]} \emph{Vergleich mit den Korpus-Bell-Dokumenten:}
A160 (topologische Verbindung, CHSH, $\xi$-Rest) und A165 (Hardware-Validierung, 445-ns-Vorhersage) behandeln Bell-Tests ausführlich. Dok.~131 (scheinbare Instantanität) zeigt, dass $T \cdot E = 1$ eine \emph{lokale Zwangsbedingung} ist, keine dynamische Fernwirkung, und dass die Feldgleichungen endliche Ausbreitungsgeschwindigkeit $v \le c$ haben. Keines dieser Dokumente stellt aber explizit fest: \emph{Die Instantanität der Korrelation wurde nie direkt gemessen, nur aus der Standard-QM-Interpretation erschlossen.} Das ist die Lücke, die dieser Abschnitt schließt.

\textbf{[K]} \emph{Einordnung von Dok.~023 (Bell-Tests im T0-Kontext, Altkorpus):}
Dok.~023 --- inzwischen durch A160 ersetzt --- beschreibt eine $\xi$-Modifikation der Bell-Korrelation:
\[
  E^{\mathrm{T0}}(a,b) = -\cos(a-b)\cdot(1-\xi\cdot f(n,l,j))
\]
Das ergibt $\mathrm{CHSH}^{\mathrm{T0}} \approx 2{,}827$ statt $2\sqrt{2} = 2{,}828$ --- eine Reduktion um $\Delta \approx 10^{-4}$, konsistent mit A165 ($\Delta\mathrm{CHSH} \approx \xi/(2\pi) \approx 2{,}1\times10^{-5}$; die Zahlendifferenz erklärt sich aus dem unterschiedlichen Ansatz über $f(n,l,j)$-Faktoren). Das Prinzip ist in beiden dasselbe: Die FFGFT modifiziert die Korrelation um $O(\xi)$. Zusätzlich leitet Dok.~023 aus ML-Divergenz bei $\Delta\theta > \pi$ eine erweiterte Formel ab --- das ist eine simulationsartefaktische Schlussfolgerung die nie in den Hauptkorpus aufgenommen wurde und [S] bleibt.

\textbf{Entscheidend:} Dok.~023 behandelt ausschließlich die \emph{Winkelabhängigkeit} der Korrelation (ob $S > 2$). Die Timing-Frage --- \emph{wann} entsteht die Korrelation? --- wird auch dort nicht gestellt. Das bestätigt die Aussage dieses Abschnitts: Alle Bell-Dokumente des Korpus, einschließlich Dok.~023, haben die Instantanitätsfrage nicht gestellt. A284 ergänzt diese Dimension --- ohne Widerspruch zu den CHSH-Korrekturen.


\textbf{Die vollständige Bestandsaufnahme der Altdokumente ergibt fünf inhaltliche Gruppen:}

\textbf{[K] Gruppe~1 --- Konsistent mit Hauptkorpus (Dok.~023, A160, A165):}
CHSH-Korrektur $O(\xi)$ \emph{unterhalb} der QM-Grenze:
$E^{\mathrm{T0}}(a,b) = -\cos(a-b)\cdot(1-\xi\cdot f(n,l,j))$,
ergibt $S^{\mathrm{T0}} \approx 2{,}827 < 2\sqrt{2}$.
Vorhersage: $\Delta\mathrm{CHSH} \approx 10^{-4}$, messbar.
Im Hauptkorpus (A160, A165) verankert und gültig.

\textbf{[S$\to$?] Gruppe~2 --- Widerspruch zum Hauptkorpus (Dok.~074):}
Dok.~074 behauptet, T0 \emph{überschreite} die Tsirelson-Schranke $2\sqrt{2}$
um 133\,ppm: $|S_{\mathrm{T0}}| = 2{,}389133$.
Zugleich benutzt es Superdeterminismus (Verletzung der Messfreiheits-Annahme)
als Erklärungsstrategie: Apparatur und Quantensystem korrelieren durch
räumlich ausgedehnte Energiefelder.
Das widerspricht direkt A160: \emph{„Was sich nicht ändert: CHSH und
Tsirelson-Schranke."} Zwei Alternativen:
(a) Die 133-ppm-Vorhersage ist ein Rechenartefakt (Dok.~074 setzt
$\varepsilon_{T0,\text{direkt}} = \xi$ ohne Gravitationsunterdrückung) und
zu verwerfen.
(b) Sie ist eine echte Alternative zu A160 --- dann sind beide in denselben
Experimenten gleichzeitig falsifizierbar.
Dok.~074 ist im Hauptkorpus nicht aufgenommen; der Superdeterminismus-Rahmen
ist nicht mit der topologischen These von A160 deckungsgleich.

\textbf{[S$\to$Widerspruch] Gruppe~3 --- Distanzabhängige Dämpfung (Dok.~023a):}
$\rho^{\mathrm{T0}} = \rho_0\cdot\exp(-\xi\cdot\Delta d/D_f)$ ---
Verschränkung nimmt mit Abstand ab.
Das steht im \emph{direkten Widerspruch} zu A160 (Windungsklassen haben
keinen Ort, keine Distanz; Verschränkung ist Identität, keine Übertragung).
Zwei strukturell verschiedene Rahmen: Dämpfungsmodell (Dok.~023a) vs.topologisches Identitätsmodell (A160).
Der Hauptkorpus (A160) ist maßgeblich; Dok.~023a ist ersetzt.

\textbf{[S] Gruppe~4 --- Energieabhängige Timing-Differenz (Dok.~055):}
Für verschränkte Photonen mit $\omega_1 \ne \omega_2$:
\[
  \Delta t_{\mathrm{corr}} = \frac{G\langle m\rangle}{r}
  \left|\frac{1}{\omega_1} - \frac{1}{\omega_2}\right|
\]
Timing-Aussage über \emph{Differenzen im Paar}, nicht über Instantanität
zwischen A und B. Mechanismus ($Gm/r$, gravitativ) verschieden von
$\xi\cdot d/c$ (A165, geometrisch-fraktal).
Mögliche Deckung: Wenn $G\langle m\rangle/r$ über $\xi = 2\sqrt{G}\cdot m$
auf das $\xi$-Schema reduzierbar ist, wären beide kompatibel.
Diese Reduktion ist im Korpus nicht ausgearbeitet --- offene Brücke [H].

\textbf{[Q] Gruppe~5 --- Externer Beleg für endliche Entstehungszeit
(Dok.~142, Jiang et al., PRL 2024):}
...
Quantenverschränkung entsteht nicht instantan, sondern über
$\sim 232$\,Attosekunden (theoretische Vorhersage, kein Laborexperiment).
Einzige peer-reviewed Quelle im Korpus, die direkt belegt:
Verschränkungsentstehung hat eine endliche Zeitstruktur.
Stärkt A284 direkt --- in einem anderen System (Helium, EUV-Laser),
aber strukturell identisch.

\textbf{[K] Gesamtbewertung:}
\begin{enumerate}[leftmargin=1.6em,itemsep=0.2em]
  \item Gruppe~1 (Dok.~023/A160/A165): gültig, konsistent, Hauptkorpus.
  \item Gruppe~2 (Dok.~074): Tsirelson-Überschreitung und Superdeterminismus
    --- Widerspruch zu A160; zu klären ob Artefakt oder Alternative.
  \item Gruppe~3 (Dok.~023a): Distanzabhängige Dämpfung --- Widerspruch
    zu A160; Hauptkorpus ist maßgeblich.
  \item Gruppe~4 (Dok.~055): Energieabhängige Timing-Differenz ---
    kompatibel, Brücke zu $\xi$ noch offen [H].
  \item Gruppe~5 (Dok.~142): Externer Beleg endliche Entstehungszeit ---
    stärkt A284.
\end{enumerate}
...
\section{Offene Frage: Sind A und B im Feld derselbe Punkt?}
\label{sec:identischer_punkt}

\subsection{Die Frage}

\textbf{[H]} Im projizierten $\mathbb{R}^3$ liegen zwei Detektoren
A und B bei $d = 1000$\,km Abstand. Im Torusbild gilt:
\[
  \mathbf{x}_A \equiv \mathbf{x}_B \pmod{L^*}
\]
Da $L^* = 4\bar\lambda_e/\xi_0^{10} \approx 1{,}3 \times 10^{26}$\,m,
ist $d/L^* \approx 10^{-20}$. Im Torusbild liegen A und B in derselben
Fundamentaldomäne --- praktisch am selben Ort.

\textbf{[H]} Die tiefere Frage ist feldontologisch, nicht metrisch:
Sind A und B für das verschränkte Feld zwei verschiedene Punkte ---
oder \emph{derselbe} Feldpunkt, der an zwei Projektionsstellen
abgelesen wird?

Der Korpus hat die Elemente der Frage (A030: Windungsklassen haben
keinen Ort; A090: Identifikation $t \sim t + R_m$; A160: verschränktes
Paar ist ein Objekt mit zwei Projektionsbildern), aber formuliert sie
nicht explizit. Das ist eine echte Lücke.

\subsection{Drei strukturell verschiedene Möglichkeiten}

\textbf{[H]} Drei Antworten, die nicht äquivalent sind:

\textbf{Möglichkeit~1 --- Zwei Punkte, topologisch verbunden [S]:}
A und B sind im Torusbild verschiedene Punkte, aber durch dieselbe
Windungsklasse verbunden. Die Feldmodifikation nach der A-Messung
propagiert als Wellenfront zu B. Die $\xi$-Verzögerung
$\Delta t = \xi \cdot d/c \approx 445$\,ns ist messbar.

\textbf{Möglichkeit~2 --- Identischer Punkt in der
Fundamentaldomäne [H]:}
Da $d \ll L^*$, sind A und B in der Fundamentaldomäne
$\mathbf{x}_A \approx \mathbf{x}_B$. Die verschränkte Mode ist
vollständig lokal --- es gibt keinen Abstand zwischen ihnen im
Torusbild. Die 445\,ns sind ein Artefakt der Projektion.

\textbf{Möglichkeit~3 --- Identischer Punkt im Modenraum [H]:}
Die relevante Variable ist nicht $\mathbf{x} \in \mathbb{R}^3$,
sondern die Windungsklasse $(n_1, n_2, n_3, n_4) \in \mathbb{Z}^4$.
Zwei verschränkte Photonen teilen dieselbe Windungsklasse --- sie sind
im Modenraum \emph{identisch}. Räumliche Trennung ist vollständig ein
Projektionsartefakt. Auch Feldlaufzeit ist dann ein Konzept des
projizierten Raums.
...
\toprule
\textbf{Möglichkeit} & \textbf{Status} & \textbf{Vorhersage} \\
\midrule
Zwei Punkte, topologisch verbunden & [S] &
  $\Delta t = \xi \cdot d/c$ messbar \\
Identisch in Fundamentaldomäne & [H] &
  445\,ns ist Projektionsartefakt \\
Identisch im Modenraum & [H] &
  Feldlaufzeit ist Projektionskonzept \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Was der Korpus impliziert}

\textbf{[B]} A030 (Windungsklassen ohne Ort) und A090 (Identifikation
$t \sim t + R_m$) stellen eine Struktur bereit, in der ``Ort'' für
Feldmoden kein primäres Konzept ist. Das spricht eher für
Möglichkeit~2 oder~3.

\textbf{[B]} A160: Das verschränkte Paar ist ``ein Objekt mit zwei
Projektionsbildern''. Wenn es \emph{ein} Objekt ist, hat es im
Torusbild \emph{eine} Feldkonfiguration, nicht zwei an verschiedenen
Orten. Die beiden Projektionsbilder entstehen erst durch die Typ-II-Projektion
auf den $\mathbb{R}^3$. Das spricht für Möglichkeit~3.

\textbf{[S]} Wenn Möglichkeit~3 zutrifft: Die Frage ``Warum ist die
Korrelation instantan?'' ist keine Frage nach Ausbreitungsgeschwindigkeit
--- sie ist eine Frage danach, warum wir die Projektion eines Objekts
an zwei Stellen für zwei Messungen halten. Die Antwort ist nicht Physik,
sondern Erkenntnistheorie: Das Projektionsbild täuscht uns über die
Anzahl der Objekte.

\subsection{Konsequenz für die 445-ns-Vorhersage}

\textbf{[H]} Die Vorhersage $\Delta t = \xi \cdot d/c$ aus A165
setzt Möglichkeit~1 voraus. Sie ist korrekt, wenn A und B im
Torusbild verschiedene Feldpunkte sind.

Wenn stattdessen Möglichkeit~2 oder~3 zutrifft, ist die Vorhersage
konzeptuell verschoben: Der Effekt existiert dann nicht als Feldlaufzeit
zwischen A und B, sondern als Phasenversatz in der Projektion ---
messbar als Timing-Asymmetrie, aber mit anderem Ursprung.

\textbf{[H]} Diese Frage kann im FFGFT-Korpus gegenwärtig nicht
entschieden werden. Sie erfordert eine präzise Formulierung dessen,
was ``Ort im Feld'' für eine Modenstruktur auf $T^4$ bedeutet ---
eine Brücke zwischen A030 und A090, die noch nicht geschlossen ist.

Bis zur Klärung bleibt A165 gültig --- unter der expliziten Annahme
von Möglichkeit~1. Die Annahme ist zu deklarieren, nicht zu
verschweigen.


\section{Die plausibelste Erklärung --- ein Urteil}
\label{sec:urteil}

\textbf{[S]} Dieser Abschnitt enthält ein Urteil des Autors. Es ist keine
bewiesene Aussage des Korpus, sondern eine begründete Einschätzung auf
Basis der in §\,\ref{sec:lichtstrahlen} bis §\,\ref{sec:identischer_punkt}
dargelegten Analyse.

\subsection{Gruppe~2 (Dok.~074): Artefakt, nicht Alternative}

A160 formuliert explizit als [SETZUNG]-Falsifikationsbedingung:
\emph{„Wird ein CHSH-Wert gemessen, der die Tsirelson-Schranke
überschreitet, ist die Theorie an dieser Stelle widerlegt."}
Die Tsirelson-Schranke $2\sqrt{2}$ ist ein algebraischer Beweis,
kein experimenteller Befund. Sie zu überschreiten würde bedeuten,
die Standard-Quantenmechanik selbst zu widerlegen.

Dok.~074 erreicht die 133-ppm-Überschreitung durch Weglassen des
Gravitationsunterdrückungsterms ($G\langle m\rangle/r \approx 10^{-30}$)
und direktes Einsetzen von $\xi$. Das ist eine nicht begründete
Parameterwahl, kein physikalisches Argument --- Dok.~074 nennt es
selbst \enquote{Alternative Interpretation}.

\textbf{[S] Urteil:} Artefakt der Parameteridentifikation. Verworfen.

\subsection{Gruppe~3 (Dok.~023a): Experimentell widerlegt}

A160 begründet \textbf{[B]} algebraisch: Der $\mathbb{C}^3$-Tensorfaktor
ist für Korrelationsgrößen unsichtbar --- die Observable wirkt dort
als Identität (A070). Distanzabhängige Dämpfung würde messbare
Konsequenzen haben: Verschränkung nähme mit Abstand ab.
Micius~2017 zeigt Bell-Verletzung über $1200$\,km auf demselben
Niveau wie Laborexperimente. Das ist experimentell ausgeschlossen.

\textbf{[S] Urteil:} Experimentell widerlegt. Verworfen.

\subsection{Gruppe~4 (Dok.~055): Kompatibel, aber zweite Ordnung}

In natürlichen Einheiten gilt $\xi = 2\sqrt{G}\cdot m$, also
$G = \xi^2/(4m^2)$. Der Gravitationsterm von Dok.~055 ist damit
$\propto \xi^2$ --- zweite Ordnung. Die vorhergesagte Zeitdifferenz
liegt bei $\sim 10^{-30}$\,s, experimentell nicht zugänglich.

\textbf{[S] Urteil:} Konsistent mit dem $\xi$-Schema in zweiter Ordnung,
viele Größenordnungen kleiner als A165. Kompatibel, aber im Rauschen.

\subsection{Gruppe~5 (Dok.~142, Jiang PRL 2024): Strukturell stärkstes Argument}

Die Jiang-Arbeit ist unabhängig vom FFGFT-Rahmen. Sie zeigt aus der
zeitabhängigen Schrödinger-Gleichung für Helium, dass Verschränkung
eine endliche Aufbauzeit hat. Das ist kein FFGFT-spezifisches Resultat,
sondern ein allgemeines Ergebnis der zeitabhängigen Quantenmechanik.

\textbf{[S] Urteil:} Quantenmechanik selbst schließt strenge Instantanität
aus, wenn man die zeitabhängige Entwicklung ernst nimmt. Die Standard-QM
setzt Instantanität durch die stationäre Eigenzustands-Basis, nicht durch
die zeitabhängige Dynamik. Das ist strukturell identisch mit der
Feldlaufzeit-These von A284 --- in einem anderen System, aber mit
demselben Mechanismus.

\subsection{Das Gesamturteil}

\textbf{[S]} Die plausibelste Erklärung für Bell-Korrelationen in der
FFGFT ist \textbf{Gruppe~1 und Gruppe~5 kombiniert}:

\begin{itemize}[leftmargin=1.4em,itemsep=0.3em]
  \item Die Korrelation entsteht \emph{nicht} instantan (Jiang, [Q]) ---
        sie hat eine endliche Aufbauzeit in der Feldgeometrie.
  \item Im kompakten Torusbild ist das keine Fernwirkung, weil A und B
        Projektionsstellen desselben globalen Feldes sind
        (A160, [B]).
  \item Die CHSH-Korrektur liegt bei $O(\xi) \approx 10^{-4}$
        unterhalb der Tsirelson-Schranke (A165, [K]).
  \item Distanzabhängige Dämpfung (Gruppe~3) ist experimentell
        ausgeschlossen; Tsirelson-Überschreitung (Gruppe~2) ist
        ein Rechnenartefakt.
\end{itemize}

\textbf{[H]} Was offen bleibt: Ob A und B im Torusbild derselbe
Feldpunkt sind (§\,\ref{sec:identischer_punkt}). Diese Frage entscheidet,
ob die 445-ns-Verzögerung (A165) eine messbare Konsequenz hat --- oder
selbst ein Projektionsartefakt ist, weil es keinen Feldabstand zwischen
A und B gibt.

\textbf{[S]} Das ist der ehrliche Restbestand. Die qualitative Antwort
ist klar: keine Fernwirkung, keine Instantanität, keine Distanzabhängigkeit.
Die quantitative Vorhersage (445\,ns) hängt an der ungeklärten Frage,
ob Feldpunkte auf $T^4$ überhaupt einen wohldefinierten Abstand tragen.


\section{Offene Punkte}

\textbf{[H]} Drei echte offene Punkte:

\begin{enumerate}[leftmargin=1.6em,itemsep=0.3em]
  \item \textbf{Born-Regel aus Typ-II-Verlust:} Die quantitative
    Herleitung der quadratischen Form $P = |\psi|^2$ aus dem
    Projektionsverlust ist nicht ausgearbeitet. Warum Quadrat und
    nicht Betrag oder höhere Potenz? Die isometrische Eigenschaft
    der Hilbertraum-Projektion ist der Kandidat (A090), aber der
    Beweis fehlt.
  \item \textbf{Wegzuordnung bei Verschränkung:} Welche geschlossenen
    Wege auf $T^4$ welchen konkreten Verschränkungszuständen entsprechen
    (A160, [S]). Ohne diese Zuordnung bleibt die topologische These
    eine Strukturaussage, kein operationales Kriterium.
  \item \textbf{Ort im Feld:} Ob A und B im Feld derselbe Punkt
    sind (§\,\ref{sec:identischer_punkt}) --- drei Möglichkeiten,
    alle offen, Konsequenz für die 445-ns-Vorhersage unentschieden.
\end{enumerate}
...
\midrule
\endhead
Messung ist Typ-II-Projektion (A090) & \textbf{[B]} \\
$\partial T^4 = \emptyset$: randlos, globale Eigenmoden ohne Ort & \textbf{[B]} \\
Messapparat wird Teil des Gesamtsystems & \textbf{[B]} \\
Windungsklassen haben keinen Ort (A030) & \textbf{[B]} \\
Born-Regel: $P = (1+z)/2 = |\alpha|^2$ aus Phase-Mittelung & \textbf{[B]} \\
Bell: keine lokalen verborgenen Variablen auf $\mathbb{R}^3$ & \textbf{[B]} \\
FFGFT: $\theta$ ist globales Merkmal, kein lokaler Parameter & \textbf{[B]} \\
Verschränktes Paar: ein Objekt, zwei Projektionsstellen (A160) & \textbf{[B]} \\
Signalfreiheit: Folge der Konstruktion, nicht Zusatzbedingung & \textbf{[B]} \\
Räumliche Verzögerung $\Delta t = \xi \cdot d/c$ (A165) & \textbf{[K]} \\
Messung an A modifiziert globalen Modeninhalt & \textbf{[S]} \\
Born-Regel als Statistik des Typ-II-Verlusts & \textbf{[S]} \\
$\xi$-Verzögerung als Feldphasenversatz, nicht Signal & \textbf{[S]} \\
Lichtstrahlen als Beobachterabstraktion (Strahlenoptik = Grenzfall) & \textbf{[S]} \\
Born-Regel quadratische Form aus Projektion & \textbf{[H]} \\
Wegzuordnung Verschränkungszustand $\leftrightarrow$ $T^4$-Weg & \textbf{[H]} \\
Sind A und B im Feld derselbe Punkt? & \textbf{[H]} \\
\bottomrule
\end{longtable}
\end{center}

\textbf{Bilanz:} 9$\times$ \textbf{[B]} $\cdot$
1$\times$ \textbf{[K]} $\cdot$
4$\times$ \textbf{[S]} $\cdot$
3$\times$ \textbf{[H]}.
...
\small
\item[{A030}] Pascher, J., \emph{Kompakte Geometrie}. A-Serie, 2026.
  Windungsklassen ohne Ort, $\partial T^4 = \emptyset$.
\item[{A070}] Pascher, J., \emph{Hilbertraum-Übersetzung}. A-Serie, 2026.
  Messung als Polübergang, Born-Regel aus Phasenunkenntnis.
\item[{A090}] Pascher, J., \emph{Projektionskette}. A-Serie, 2026.
  Typ-I/II/III-Operationen, Reversibilität, Informationsverlust.
\item[{A160}] Pascher, J., \emph{Quantenmechanik und Bell-Korrelationen}.
  A-Serie, 2026. Topologische Verbindung, CHSH, $\xi$-Rest.
\item[{A165}] Pascher, J., \emph{Bell-Hardware und Quantencomputing}.
  A-Serie, 2026. $\Delta t = \xi \cdot d/c$, IBM-Heron-Messungen.
\item[{Dok.~131}] Pascher, J., \emph{Scheinbare Instantanität}.
  Altkorpus, 2025. Green-Funktion, Kausalität.
\end{description}
