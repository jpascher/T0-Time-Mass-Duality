...
## 5. Juli 2026 — v1.2.0: Spiral-Zeit-Abgleich abgeschlossen (294/295)

v1.2.0 schließt den in v1.1.9 (Dok. 295) geöffneten Faden: die dort pränotierte Testfrage „trägt HLVs Spiral-Zeit das Verhältnis e?" ist beantwortet — **klares Negativ**. FFGFT-interner Abschluss, kein neuer externer Anspruch; HLV bleibt nachrangig (vgl. negatives BD17A-Verdikt v1.1.7). Inhaltlich in dieser Session gebaut (Detail-Nachträge unten): (1) Dok. 294 Abgrenzung „welches HLV-Objekt geteilt ist" — geteilt ist die geometrische φ-ikosaedrische Schicht, NICHT HLVs temporal-dynamische Spiral-Zeit; (2) Dok. 295 Einordnung ersetzt + Abgleich-Block — zwei disjunkte HLV-„Spiral-Zeiten" (geometrischer A₅-Singlet vs. temporal-dynamisches Memory-Objekt), e-Kriterium (FFGFT trägt e = 2,71947, Marcels temporal-dynamisches Objekt nicht = konstanter Defekt → 1,0011, φ-Geometrie trägt φ⁴ = 6,854; kein HLV-Strang trägt e), Basis+Dimensionen (3/6 gleiche Zahl, andere Bedeutung), Open-System-Erdung (Marcels Objekt nach eigenem Spiral-Time-Papier offene Quantendynamik — unitäre Dilatation H_S⊗H_M, Prozesstensor/CP-Divisibilität/Hard-Reset/Finite-Bath —, dynamische statt geometrische Kategorie); (3) zwei neue numpy-only-Skripte (spiralzeit_verhaeltnis_probe.py, basis_6d_achsen_probe.py). Durchgehend ehrlich gehalten: Kandidat/Brücken-Bedingung, kein Verdikt über Marcels Rahmen (P35). **NEU (5. Juli, Zwei-Darstellungen):** in 295 (DE+EN) + Release Notes v1.2.0 festgehalten, dass FFGFTs Nichtschließung gleichwertig in zwei Darstellungen vorliegt — geometrisch als log-Spirale (295) und im Hilbertraum als Gedächtniskern (FT der diskreten Spektraldichte des T⁴-Connection-Laplace, Dok. 283, verlustfreie Bijektion H = L²(T⁴)⊗ℂ³); ausdrücklich Darstellungs-Robustheit, KEINE zusätzliche Evidenz (Übersetzung ist nicht Begründung); erlaubt den Abgleich in Kern-Sprache (FFGFTs Kern oszillierend/diskret-spektral, Revivals/BLP-Rückfluss 5,125, vs. Marcels abklingend — gleiche Klasse, andere Gestalt, Dok. 283). **Release-Satz:** Release Notes v1.2.0 DE+EN erstellt; README DE+EN Zenodo-Zeile + Versionshistorie auf v1.2.0. DOI wird beim Zenodo-Upload vergeben (löst v1.1.9/21193007 ab); drei DOI-Platzhalter in README (Badge, Zenodo-Zeile, Versionshistorie) nach Upload nachzuziehen — der Badge zeigt bis dahin die v1.1.9-DOI. Gesamtzip als FFGFT_v1_2_0_Release_2026-07-05.zip.
