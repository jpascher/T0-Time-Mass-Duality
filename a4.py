...
def berechne_level_2(self):
    """
    Level 2: Gravitationskonstante über T0-Theorie (HARMONISIERT MIT LATEX)
    
    HARMONISIERTE FORMEL (aus LaTeX):
    - G_SI = (ξ² / (4 m_e)) × C_conv × K_frak
    - m_e = 0.5109989461 MeV (physische Elektronmasse)
    - C_conv = 7.783 × 10^{-3} (SI-Conversion-Faktor, abgeleitet aus (ℏc)^2 / m_e etc.)
    - K_frak = 0.986 (fraktale Korrektur aus D_f = 2.94)
    - Dimension: [MeV^{-1}] → [m³ kg^{-1} s^{-2}]
    - T0-Fundamentalformel: ξ = 2√(G · m) → G = ξ² / (4 m)
    """
    if hasattr(self, '_level2_erledigt'):
        return
    
    # Physische Parameter (harmonisiert mit LaTeX)
    xi = float(self.xi)
    m_e = 0.5109989461  # MeV (Elektronmasse)
    C_conv = 7.783e-3   # Conversion-Faktor zu SI [m³ kg^{-1} s^{-2} MeV]
    K_frak = 0.986     # Fraktale Korrektur (1 - (D_f - 2)/68 mit D_f=2.94)
    
    # T0-Grundformel: G_base in natürlichen Einheiten [MeV^{-1}]
    G_base = (xi ** 2) / (4 * m_e)
    
    # Vollständige SI-Berechnung (einstufig: G_base × C_conv × K_frak)
    G_SI = G_base * C_conv * K_frak
    
    # Validierungen (wie im Original)
    G_SI_referenz = 6.674e-11  # Bekannter SI-Wert
    hbar_ref = self.experimentelle_werte['hbar']
    c_ref = self.experimentelle_werte['c']
    l_P = self.l_P
    planck_umrechnungsfaktor = (l_P ** 2 * c_ref ** 3) / hbar_ref  # Planck-basierte G-Äquivalent
    
    # Speichere (angepasst: explizite Komponenten für Transparenz)
    self.berechnete_konstanten['G'] = G_SI
    self.berechnete_konstanten['G_base'] = G_base  # Neu: Basis in [MeV^{-1}]
    self.berechnete_konstanten['C_conv'] = C_conv
    self.berechnete_konstanten['K_frak'] = K_frak
    self.berechnete_konstanten['G_SI_referenz'] = G_SI_referenz
