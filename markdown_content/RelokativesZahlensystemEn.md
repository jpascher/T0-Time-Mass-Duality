			...
			\begin{adjustbox}{width=0.98\textwidth}
				\begin{tabular}{lll}
					...
					**Symbol** & **Meaning** & **Notes** \\
					\midrule
					\multicolumn{3}{c}{**Relational Basic Operations**} \\
					...
					\midrule
					\multicolumn{3}{c}{**Interval Representation**} \\
					...
					\midrule
					\multicolumn{3}{c}{**Transformations**} \\
...
# Introduction: Shifting the Reference Point
	...
	The idea of shifting the reference point to construct a number system based on ratios while reinterpreting the role of prime numbers is the key to a more fundamental understanding of mathematics. **Prime numbers correspond to ratios in an alternative number system that is fundamentally more basic** than our familiar set-based system.
	
	## What does shifting the reference point mean?
	
	Previously, we have thought of the reference point (the denominator in a fraction like $P/X$) often as 1, representing a fixed, absolute unit. However, when we shift the reference point, we no longer think of absolute numerical values, but of **relational steps or transformations**.
	
	Imagine we define numbers not as three apples, but as the **relationship or operation** that transforms one quantity into another.
	
	# Music as a Model: Intervals as Operations
	
	In music, an interval (e.g., a fifth, $3/2$) is not just a static ratio, but an **operation** that transforms one tone into another. When you shift a tone up by a fifth, you multiply its frequency by $3/2$.
	...
	\begin{table}[htbp]
		...
		\begin{adjustbox}{width=0.85\textwidth}
			\begin{tabular}{lccc}
				\toprule
				**Interval** & **Ratio** & **Prime Factor** & **Vector** \\
				\midrule
	...
	These ratios can be written as **products of prime numbers with integer exponents**:
	
	\begin{equation}
		\text{Interval} = 2^a \cdot 3^b \cdot 5^c \cdot 7^d \cdot \ldots
	\end{equation}
	
	Depending on how many prime numbers one allows (2, 3, 5 – or also 7, 11, 13 \ldots), one speaks of a **5-limit**, **7-limit** or **13-limit** system.
	...
	This allows a clear **algebraic structure** for intervals, including addition, inversion, etc. over the exponent vectors.
	...
	\begin{itemize}
		\item **Pythagorean harmony doctrine**: The Pythagoreans already recognized that *Everything is number* -- understood as ratio, not as quantity
		\item **Euler's Tonnetz** (1739): Prime number-based representation of musical intervals in a two-dimensional lattice
		\item **Grassmann's Ausdehnungslehre** (1844): Multiplication as fundamental operation that creates new geometric objects
		\item **Dedekind cuts** (1872): Numbers as relations between rational sets
	\end{itemize}
	...
	\begin{category}
		...
		\begin{itemize}
			\item **Objects** = ratio vectors $\vect{v} = (a_1, a_2, a_3, \ldots)$
			\item **Morphisms** = proportional transformations between relations
			\item **Tensor product** $\otimes$ = composition $\circ$ of relations
			\item **Unit object** = identity relation $\primrel{1}$
		\end{itemize}
	...
	If we transfer this musical approach to numbers, we can interpret prime numbers not as independent numbers, but as **fundamental, irreducible proportional steps or transformations**:
	...
	In a relational system, numbers would not be static quantities, but **compositions of ratios**:
	
	\begin{itemize}
		\item **Starting point**: Base unit $(1:1)$
		\item **Numbers as paths**: Each number is a path of operations
		\begin{itemize}
	...
	\begin{axiom}[Relational Arithmetic]
		...
		\begin{enumerate}
			\item **Associativity**: $(\primrel{a} \circ \primrel{b}) \circ \primrel{c} = \primrel{a} \circ (\primrel{b} \circ \primrel{c})$
			\item **Neutral element**: $\exists \primrel{1} \forall \primrel{a}: \primrel{a} \circ \primrel{1} = \primrel{a}$
			\item **Invertibility**: $\forall \primrel{a} \exists \primrel{a}^{-1}: \primrel{a} \circ \primrel{a}^{-1} = \primrel{1}$
			\item **Commutativity**: $\primrel{a} \circ \primrel{b} = \primrel{b} \circ \primrel{a}$
		\end{enumerate}
	\end{axiom}
	...
	\begin{itemize}
		\item **Sets**: $2 + 3 = 5$ apples (original parts recognizable as subsets)
		\item **Wave superposition**: Frequencies $f_1$ and $f_2$ are still detectable in the spectrum
		\item **Forces**: Vector addition - both original forces are present
	\end{itemize}
	...
	\begin{itemize}
		\item **Area calculation**: $2m \times 3m = 6m^2$ (new dimension)
		\item **Proportional change**: Doubling $\circ$ tripling = sixfolding
		\item **Musical intervals**: Fifth $\times$ octave = new harmonic position
	\end{itemize}
	...
	\begin{enumerate}
		\item **Scale transformation**: From proportional to linear scale
		\item **Nature of perception**: Many sensory perceptions are logarithmic
		\begin{itemize}
			\item **Hearing**: Frequency ratios as equal steps
			\item **Light**: Logarithmic brightness perception
			\item **Sound**: Decibel scale
		\end{itemize}
		\item **Physical systems**: Exponential growth becomes linear
		\item **Unification**: Addition and multiplication are connected by transformation
	\end{enumerate}
	...
	\begin{table}[htbp]
		...
		\begin{adjustbox}{width=0.85\textwidth}
			\begin{tabular}{ll}
				\toprule
				**Relational System** & **Quantum Mechanics** \\
				\midrule
	...
	\begin{table}[htbp]
		...
		\begin{adjustbox}{width=0.9\textwidth}
			\begin{tabular}{lll}
				\toprule
				**Modulation** & **Description** & **Examples** \\
				\midrule
	...
	\begin{itemize}
		\item **Timbre**: Additive superposition of harmonic overtones with multiplicative frequency ratios
		\item **Harmony**: Consonance through simple multiplicative ratios ($3:2$, $5:4$)
		\item **Melody**: Multiplicative frequency steps in additive time sequence
	\end{itemize}
	
	# The Elimination of Absolute Quantities
	
	A central feature of this system is that the concrete assignment to a quantity is not necessary in the fundamental definitions. **The assignment to a specific quantity can be omitted and only becomes important when these relational numbers are applied to real things.**
	
	\begin{definition}[Relational vs. Absolute Numbers]
		\begin{itemize}
			\item **Fundamental level**: Numbers are abstract relationships
			\item **Application level**: Measurement in concrete units (meters, kilograms, hertz)
			\item **Natural units**: $E = m$ (energy-mass identity as pure relation)
		\end{itemize}
	\end{definition}
	...
	\begin{table}[htbp]
		...
		\begin{adjustbox}{width=0.85\textwidth}
			\begin{tabular}{llll}
				\toprule
				**Algorithm** & **Property** & **Complexity** & **Application** \\
				\midrule
	...
	\begin{itemize}
		\item **Closure**: $\primrel{a} \circ \primrel{b} \in \mathcal{R}$
		\item **Associativity**: $(\primrel{a} \circ \primrel{b}) \circ \primrel{c} = \primrel{a} \circ (\primrel{b} \circ \primrel{c})$
		\item **Identity**: $\primrel{1}$ is neutral element
		\item **Inverses**: Each relation $\primrel{a}$ has inverse $\primrel{a}^{-1}$
	\end{itemize}
	...
	\begin{enumerate}
		\item **Fundamental nature**: Captures the essence of relationships
		\item **Logarithmic harmony**: Compatible with natural laws
		\item **Multiplicative primary operation**: Natural connection
		\item **Practical application**: Already implemented in FFT/QFT/Shor
	\end{enumerate}
	
	## Challenges
	
	\begin{enumerate}
		\item **Addition**: Complex definition in purely relational spaces
		\item **Intuition**: Unfamiliar for set-based thinking
		\item **Practical implementation**: Requires new mathematical tools
	\end{enumerate}
	...
	\begin{itemize}
		\item **Operationalism**: Numbers are defined by their transformative effects, not by static properties
		\item **Process ontology**: Being is understood as a dynamic network of transformations
		\item **Neo-Pythagoreanism**: Mathematical relations as fundamental substrate of reality
		\item **Structuralism**: The structure of relationships is primary over *objects*
	\end{itemize}
	...
	\begin{enumerate}
		\item **Canonical addition**: How can addition be naturally defined in the relational system without transitioning to logarithmic space?
		\item **Topological structure**: Is there a natural topology on the space of prime relations?
		\item **Non-commutative generalizations**: Can the system capture quantum groups and non-commutative structures?
		\item **Algorithmic complexity**: Which computational problems become easier or harder in the relational system?
		\item **Cognitive modeling**: How is relational thinking reflected in neural structures?
	\end{enumerate}
	...
	**Core insights**:
	...
	This scaling demonstrates the **relational principle**: The parameter $\xi$ is not set absolutely, but **relative to the problem size**.
	...
	The wave velocity is defined **not as an absolute constant**, but as a **relation to $\xi$**.
	...
	\begin{example}[T0-Hadamard Gate]
		...
		The Hadamard gate uses **relational corrections** instead of fixed transformations.
	\end{example}
	...
	\begin{example}[T0-CNOT Gate]
		...
		The CNOT operation is based on **ratios and thresholds**, not on discrete states.
	\end{example}
	...
	This implementation shows how **Shor's period finding** is replaced by **relational energy field correlations**.
	...
	\begin{table}[htbp]
		...
		\begin{adjustbox}{width=0.9\textwidth}
			\begin{tabular}{lccc}
				\toprule
				**$\xi$-Parameter** & **Success Rate** & **Average Time** & **Stability** \\
				\midrule
	...
	The results show: **Relational parameters** (that adapt to problem size) are **significantly more effective** than absolute constants.
	...
	\begin{enumerate}
		\item **Adaptive parameters**: No universal constants, but context-sensitive relations
		\item **Ratio-based operations**: All calculations use correction factors like $(1 + \xi)$
		\item **Logarithmic scaling**: Parameters change exponentially with problem size
		\item **Composition of relations**: Complex operations as concatenation of simple ratios
		\item **Empirical validation**: Relational approaches measurably outperform absolute constants
	\end{enumerate}
	
	This implementation shows that the **relational number system is not only theoretically elegant**, but also **practically superior** for complex calculations like prime factorization.
