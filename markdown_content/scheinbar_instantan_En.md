...
# Introduction: The Instantaneity Problem
	...
	**1. The complete field equation:**
	\begin{equation}
		\nabla^2 m = 4\pi G \rho(\mathbf{x},t) \cdot m \label{eq:field_equation}
	\end{equation}
	where $\rho(\mathbf{x},t)$ is the mass density. This equation is *not* instantaneous but rather a wave equation with finite propagation speed $v \leq c$.
	...
	**2. The modified Schrödinger equation:**
	...
	\begin{center}
		\begin{tikzpicture}[scale=1.2]
			% Title
			\node at (6, 7) {\Large **Local Constraint vs. Global Propagation**};
			
			% Local constraint (left)
			\draw[thick, fill=t0blue!20] (0,0) circle (2);
			\node at (0, 3) {**Local Level**};
	...
	\begin{tcolorbox}[colback=t0blue!5!white, colframe=t0blue!75!black, title=T0 Prediction]
		...
		\end{itemize}
		This is *not* an empirical adjustment but a geometric prediction.
	\end{tcolorbox}
	...
	\begin{center}
		\begin{tikzpicture}[scale=1.3]
			% Title
			\node at (0, 6) {\Large **The Fundamental Energy-Time Duality**};
			...
			\node at (0, -2.5) {**Complementarity Principle:**};
			...
			\node at (0, -0.8) {\small **Planck Scale:** All equal};
			
			% Scale dependence
			\node[right] at (-1, 2.2) {\small **Dominant at:**};
	...
	\begin{itemize}
		\item **Planck scale:** All aspects are equivalent, no approximation valid
		\item **Atomic scale:** Energy-time duality dominates, gravity negligible
		\item **Macroscopic scale:** Mass aspect dominant, quantum effects suppressed
		\item **Cosmological scale:** Space-time structure dominant, local quantum effects irrelevant
	\end{itemize}
	...
	\begin{tcolorbox}[colback=t0yellow!10!white, colframe=t0yellow!75!black, title=Important Insight]
		Singularities are **not** the goal of T0 theory. They rather represent limits of applicability:
		\begin{itemize}
	...
	\begin{enumerate}
		\item **Scale awareness:** Always check which scale is dominant
		\item **Take small effects seriously:** Don't ignore $\xi$ corrections and gravitational effects
		\item **Avoid singularities:** Understand them as hints at theoretical limits
		\item **Respect complementarity:** Not all aspects can be sharp simultaneously
		\item **Experimental verifiability:** Only make predictions that are measurable in principle
	\end{enumerate}
	...
	\begin{itemize}
		\item **Local condition:** $T \cdot E = 1$ at both measurement locations
		\item **Shared field:** Entangled particles share field configuration
		\item **Causal propagation:** Field changes propagate with $c$
		\item **Correlation without communication:** Pre-structured field, no signal transmission
	\end{itemize}
	...
	**Numerical example:**
	...
	\begin{enumerate}
		\item **Satellite Bell test:** Entangled photons between ground station and satellite
		\item **Lunar laser ranging:** Precision measurement of quantum correlations Earth-Moon
		\item **Deep space quantum network:** Test at interplanetary distances
	\end{enumerate}
	...
	\begin{enumerate}
		\item **Local level:** $T \cdot E = 1$ as constraint condition (no dynamics)
		\item **Field level:** Wave equation with propagation $v \leq c$ (causal dynamics)
		\item **Measurable level:** Appears instantaneous because $\Delta t < $ resolution
	\end{enumerate}
	...
	\begin{thebibliography}{99}
		...
		T0 Theory Foundations (2024). *Time-Mass Duality and Geometric Field Theory*. Internal Research Document.
