...
# Methodik
	...
	\begin{enumerate}
		\item **Trial Division**: Systematische Divisionsversuche bis $\sqrt{n}$
		\item **Fermats Methode**: Suche nach Darstellung als Differenz von Quadraten
		\item **Pollard Rho**: Probabilistische Periodenfindung in pseudozufälligen Sequenzen
		\item **Pollard $p-1$**: Methode für Zahlen mit glatten Faktoren
		\item **T0-Framework**: Deterministische Periodenfindung in modularer Exponentiation (klassisch Shor-inspiriert)
	\end{enumerate}
	...
	\begin{itemize}
		\item **Erfolg/Misserfolg**: Binäres Ergebnis
		\item **Ausführungszeit**: Millisekundengenauigkeit
		\item **Gefundene Faktoren**: Für erfolgreiche Tests
		\item **Algorithmusspezifische Parameter**: Je nach Methode
	\end{itemize}
	...
	\begin{itemize}
		\item **Kernidee**: Finde Periode $r$ wobei $a^r \equiv 1 \pmod{n}$
		\item **Faktorextraktion**: Nutze Periode um $\gcd(a^{r/2} \pm 1, n)$ zu berechnen
		\item **Mathematische Basis**: Eulers Theorem und Ordnung von Elementen in $\mathbb{Z}_n^*$
	\end{itemize}
	...
	\begin{itemize}
		\item **Periodensuchraum**: Beide suchen nach Perioden $r$ wobei $a^r \equiv 1 \pmod{n}$
		\item **Maximale Periode**: Die Ordnung jedes Elements ist höchstens $n-1$, aber typischerweise viel kleiner
		\item **Erwartete Periodenlänge**: $O(\log n)$ für die meisten Elemente aufgrund Eulers Theorem
		\item **Periodentest**: Jeder Periodentest benötigt $O((\log n)^2)$ Operationen für modulare Exponentiation
		\item **Gesamtkomplexität**: $O(\log n) \times O((\log n)^2) = O((\log n)^3)$
	\end{itemize}
	...
	**Der einzige Unterschied**: Shor nutzt Quantensuperposition um Perioden parallel zu suchen, während T0 sie deterministisch sequenziell sucht - aber beide haben dieselbe $O((\log n)^3)$ Komplexitätsgrenze.
	...
	\begin{itemize}
		\item **Maximale Periode**: $r_{\max} = \lambda(n) \leq n-1$ (Carmichael-Funktion)
		\item **Typischer Periodenbereich**: $r_{typical} \in [1, O(\sqrt{n})]$ für die meisten Basen
		\item **Resonanzfrequenzen**: $\omega = 2\pi/r$ beschränkt auf diskrete Werte
		\item **Vibrationsmodi**: Nur $O(\sqrt{n})$ unterschiedliche Schwingungsmuster möglich
	\end{itemize}
	...
	\begin{itemize}
		\item **Endlich**: Durch Zahlengröße beschränkt
		\item **Diskret**: Nur ganzzahlige Perioden erlaubt
		\item **Strukturiert**: Folgt mathematischen Mustern basierend auf $n$s Primstruktur
		\item **Vorhersagbar**: Resonanzspitzen clustern in mathematisch bestimmten Bereichen
	\end{itemize}
	...
	\begin{itemize}
		\item **Eingabemerkmale**: Zahlengröße, Bitlänge, mathematischer Typ (Zwillingsprim, glatt, etc.)
		\item **Zielvorhersagen**: Optimale $\xi$-Strategie, erwartete Resonanzperioden, Erfolgswahrscheinlichkeit
		\item **Musterbeispiele**: 37 Testfälle mit dokumentierten Erfolgs-/Misserfolgsmuster
		\item **Merkmalstechnik**: Extraktion mathematischer Invarianten (Primlücken, Glätte, etc.)
	\end{itemize}
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/factorization_benchmark_library.py}
	...
	\begin{itemize}
		\item **UniversalT0Algorithm**: Kernimplementierung mit optimierten $\xi$-Profilen
		\item **FactorizationLibrary**: Zentrale API für alle Algorithmen
		\item **FactorizationResult**: Erweiterte Datenstruktur mit T0-Metriken
		\item **TestCase**: Strukturierte Testfalldefinition
	\end{itemize}
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/easy_test_cases.py}\\
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/borderline_test_cases.py}\\
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/impossible_test_cases.py}\\
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/automatic_xi_optimizer.py}\\
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/focused_xi_tester.py}\\
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/t0_uniqueness_test.py}\\
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/xi_strategy_debug.py}\\
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/updated_impossible_tests.py}\\
	...
	**Quelle**: \url{https://github.com/jpascher/T0-Time-Mass-Duality/blob/main/rsa/xi_explorer_tool.html}\\
	...
	\begin{itemize}
		\item **Erfolg/Misserfolg**: Binäres algorithmisches Ergebnis
		\item **Ausführungszeit**: Hochpräzise Zeitmessungen
		\item **Faktorkorrektheit**: Produktverifikation gegen Eingabe
		\item **T0-spezifische Daten**: $\xi$-Strategie, Resonanzbewertung, getestete Perioden
		\item **Speichernutzung**: Ressourcenverbrauchsüberwachung
		\item **Methodenspezifische Parameter**: Algorithmusabhängige Metadaten
	\end{itemize}
	...
	\begin{itemize}
		\item **Klassische Methoden**: Versagen jenseits 20-25 Bits
		\item **T0-Framework**: Erweitert Erfolg auf 25-30 Bits
		\item **Hardware-Grenzen**: Betreffen alle Methoden jenseits 30 Bits
		\item **RSA-Sicherheit**: Beruht auf diesen mathematischen Grenzen
	\end{itemize}
	...
	\begin{thebibliography}{99}
		...
		Python Software Foundation. (2023). *fractions --- Rationale Zahlen*. Python 3.9 Dokumentation.
		
		\bibitem{pollard1975}
		Pollard, J. M. (1975). Eine Monte-Carlo-Methode zur Faktorisierung. *BIT Numerical Mathematics*, 15(3), 331--334.
		
		\bibitem{fermat1643}
		Fermat, P. de (1643). *Methodus ad disquirendam maximam et minimam*. Historische Quelle.
		
		\bibitem{knuth1997}
		Knuth, D. E. (1997). *Die Kunst der Computerprogrammierung, Band 2: Seminumerische Algorithmen*. Addison-Wesley.
		
		\bibitem{cohen2007}
		Cohen, H. (2007). *Zahlentheorie Band I: Werkzeuge und diophantische Gleichungen*. Springer Science \& Business Media.
	\end{thebibliography}