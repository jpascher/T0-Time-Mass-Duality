...
# Methodology
	...
	\begin{enumerate}
		\item **Trial Division**: Systematic division attempts up to $\sqrt{n}$
		\item **Fermat's Method**: Search for representation as difference of squares
		\item **Pollard Rho**: Probabilistic period finding in pseudorandom sequences
		\item **Pollard $p-1$**: Method for numbers with smooth factors
		\item **T0-Framework**: Deterministic period finding in modular exponentiation (classical Shor-inspired)
	\end{enumerate}
	...
	\begin{itemize}
		\item **Success/Failure**: Binary result
		\item **Execution time**: Millisecond precision
		\item **Found factors**: For successful tests
		\item **Algorithm-specific parameters**: Depending on method
	\end{itemize}
	...
	\begin{itemize}
		\item **Core idea**: Find period $r$ where $a^r \equiv 1 \pmod{n}$
		\item **Factor extraction**: Use period to compute $\gcd(a^{r/2} \pm 1, n)$
		\item **Mathematical basis**: Euler's theorem and order of elements in $\mathbb{Z}_n^*$
	\end{itemize}
	...
	\begin{itemize}
		\item **Period search space**: Both search for periods $r$ where $a^r \equiv 1 \pmod{n}$
		\item **Maximum period**: The order of any element is at most $n-1$, but typically much smaller
		\item **Expected period length**: $O(\log n)$ for most elements due to Euler's theorem
		\item **Period testing**: Each period test requires $O((\log n)^2)$ operations for modular exponentiation
		\item **Total complexity**: $O(\log n) \times O((\log n)^2) = O((\log n)^3)$
	\end{itemize}
	...
	**The only difference**: Shor uses quantum superposition to search periods in parallel, while T0 searches them deterministically in sequence - but both have the same $O((\log n)^3)$ complexity bound.
	...
	\begin{itemize}
		\item **Maximum period**: $r_{\max} = \lambda(n) \leq n-1$ (Carmichael function)
		\item **Typical period range**: $r_{typical} \in [1, O(\sqrt{n})]$ for most bases
		\item **Resonance frequencies**: $\omega = 2\pi/r$ constrained to discrete values
		\item **Vibrational modes**: Only $O(\sqrt{n})$ distinct oscillation patterns possible
	\end{itemize}
	...
	\begin{itemize}
		\item **Finite**: Constrained by number size
		\item **Discrete**: Only integer periods allowed
		\item **Structured**: Follows mathematical patterns based on $n$'s prime structure
	...
	\begin{itemize}
		\item **Input features**: Number size, bit length, mathematical type (twin prime, smooth, etc.)
		\item **Target predictions**: Optimal $\xi$-strategy, expected resonance periods, success probability
		\item **Pattern examples**: 37 test cases with documented success/failure patterns
		\item **Feature engineering**: Extract mathematical invariants (prime gaps, smoothness, etc.)
	\end{itemize}
	...
	\begin{thebibliography}{99}
		...
		Python Software Foundation. (2023). *fractions --- Rational numbers*. Python 3.9 Documentation.
		
		\bibitem{pollard1975}
		Pollard, J. M. (1975). A Monte Carlo method for factorization. *BIT Numerical Mathematics*, 15(3), 331--334.
		
		\bibitem{fermat1643}
		Fermat, P. de (1643). *Methodus ad disquirendam maximam et minimam*. Historical source.
