...
`T0\_SI\_De`, `T0\_xi\_ursprung\_De` und `T0\_xi-und-e\_De`
...
Der Artikel *A single-clock approach to fundamental metrology*
...
In der T0-Theorie wird in `T0\_SI\_De` \cite{pascher_T0_SI_2024} gezeigt,
...
`T0\_xi\_ursprung\_De` \cite{pascher_xi_ursprung_2025} und
`T0\_xi-und-e\_De` \cite{pascher_xi_und_e_2025} wird die
...
(`T0\_SI\_De`). Die T0-Theorie geht sogar einen Schritt weiter, indem
...
Dieser Ansatz ist mit der in `T0\_SI\_De` gegebenen Herleitung
...
# Massenbestimmung aus Frequenzen und Zeit
...
In der T0-Theorie wird diese Beobachtung in `T0\_xi-und-e\_De` explizit
in der Form
\begin{equation}
  T \cdot m = 1
\end{equation}
...
In T0 werden in `T0\_SI\_De` entsprechende Beziehungen für $e$, $\alpha$,
...
# Zusammenhang mit den T0-Dokumenten
\label{sec:t0_zusammenhang}

## T0\_SI\_De: Von $\xi$ zu SI-Konstanten

In `T0\_SI\_De` wird ausführlich dargelegt, wie aus dem einzelnen
...
## T0\_xi\_ursprung\_De: Massenskalierung und $\xi$

`T0\_xi\_ursprung\_De` behandelt die Frage, wie die konkrete numerische
...
## T0\_xi-und-e\_De: Zeit-Masse-Dualität

In `T0\_xi-und-e\_De` wird die Beziehung $T\cdot m = 1$ als Ausdruck
...
# Quantengravitation und Gültigkeitsbereich
...
`T0\_SI\_De` wird $L_0 = \xi\,l_P$ als absolute Untergrenze der
...
Die T0-Theorie liefert mit ihren Dokumenten `T0\_SI\_De`,
`T0\_xi\_ursprung\_De` und `T0\_xi-und-e\_De` eine ergänzende
