...
# -*- coding: utf-8 -*-
"""
Escape all underscores in PDF filenames for English documents.
Example: 040_Hdokument_En.pdf -> 040\_Hdokument\_En.pdf
"""
...
def escape_pdf_underscores(file_path):
    ...
    for line_num, line in enumerate(content, 1):
        ...
        def replace_underscores(match):
            ...
            # Replace all underscores NOT already escaped
            escaped = re.sub(r'(?<!\\)_', r'\\_', pdf_name)
            return escaped
        
        # Match PDF filenames ending in _En.pdf
        line = re.sub(r'\b[\w\-]+(?<!\\)_(?:[\w\-]+(?<!\\)_)*(?:En)\.pdf\b', 
    ...
    if modified:
        # Backup original
        backup_path = str(file_path) + '.bak_en'
...
def main():
    ...
    print(f"Backups saved as *.tex.bak_en")
    print(f"{'='*80}")
